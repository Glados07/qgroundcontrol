...
<context>
    ...
    <message>
        ...
        <location filename="../src/Gimbal/GimbalControl.SettingsGroup.json"/>
        <source>Each tap moves to the next minimum-anchored legal target within the documented 1.0x to 30.0x absolute-command range; when less than one configured step remains, the exact 30.0x boundary is the final target. Where tap and hold are both available, holding for 420 ms starts native direction-only 0x05 zoom. Above 30.0x, where tap is unavailable, held zoom starts on press. Command 0x05 has no speed field, so the configured step does not change native held speed. A hold directly supersedes a pending 0x0f target or a direction latched at a confirmed physical endpoint, without a preceding 0x05(0) stop or artificial delay. Ordinary protected takeovers use a complete bounded series of direction-only copies because residual 0x18 motion from the previous controller cannot prove takeover. A reversal which starts from the opposite physical endpoint can keep direction copies active for about 36 seconds. Its first retry copy resets the proof baseline, after which two directed 0x18 changes retire the remaining copies; release or endpoint completion also cancels every remaining copy. During a hold, authoritative 0x18 feedback is requested every 100 ms. After this generation has reported non-endpoint travel, two consecutive endpoint samples finish the UI gesture without sending 0x05(0); the next reverse hold immediately overrides the latched endpoint direction. A disputed endpoint start retains the safety-stop path. Normal release and cancellation still send an immediate stop and one bounded 150 ms safety copy. Above 30.0x, tap control is disabled in both directions. The default step is 1.0x.</source>
        <translation>在协议规定的1.0x至30.0x绝对命令范围内，每次短按都会前进到以最小倍率为锚点的下一个合法目标；剩余区间不足一个配置步长时，以精确30.0x作为最后目标。当短按和长按都可用时，按住420毫秒启动原生0x05方向变焦；30.0x以上没有短按动作，因此按下即启动长按变焦。0x05没有速度字段，配置步长不会改变原生长按速度。长按会直接接管仍在pending的0x0f目标或已确认物理端点处锁存的方向，前面不再发送0x05(0)，也没有人为等待。由于前一控制器残留的0x18运动不能证明新方向已接管，普通受保护接管会完整发送一组有界的纯方向副本。从相反物理端点起步的反向接管可保持方向补发约36秒；第一份重试副本会重置运动证明基线，之后两次沿请求方向的0x18变化才会停止剩余补发，松手或端点结束也会取消全部剩余副本。长按期间每100毫秒请求一次权威0x18反馈。本代控制已出现非端点运动后，连续两份端点样本只结束界面手势而不发送0x05(0)；下一次反向长按立即覆盖端点锁存方向。无法证明本代运动的争议端点仍保留安全停止路径。正常松手和取消仍会立即发送停止，并在150毫秒后最多发送一份有界安全停止副本。30.0x以上双向禁用短按。默认步长为1.0x。</translation>
    </message>
</context>
<context>
