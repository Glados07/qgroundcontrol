...
View3D {
    ...
    property bool _useExternal3DMapSource:      _viewer3DSettings.useExternal3DMapSource.rawValue && !_viewer3DSettings.useGoogle3DMapSource.rawValue
    readonly property string _externalMapStatusText: (_useExternal3DMapSource && mapGeometryLoader.status === Loader.Ready && mapGeometryLoader.item && mapGeometryLoader.item.statusText !== undefined) ? mapGeometryLoader.item.statusText : ""
    ...
    on_UseExternal3DMapSourceChanged: {
        if (_viewer3DEnabled && mapGeometryLoader.active) {
            // 外部模型和 OSM 地图是互斥地图源，切换时强制重载地图层，避免旧几何残留。
    ...
    Viewer3DProgressBar{
        ...
        visible: !_useExternal3DMapSource
    ...
    Binding{
        ...
        value: (!_useExternal3DMapSource && mapGeometryLoader.active && mapGeometryLoader.item && mapGeometryLoader.item.textureDownloadProgress !== undefined) ? mapGeometryLoader.item.textureDownloadProgress : 100
        when: mapGeometryLoader.status == Loader.Ready
    }

    Rectangle {
        id: externalMapStatusPanel
        anchors {
            top: parent.top
            horizontalCenter: parent.horizontalCenter
            margins: ScreenTools.defaultFontPixelWidth
        }
        width: Math.min(parent.width * 0.86, ScreenTools.defaultFontPixelWidth * 86)
        height: externalMapStatusText.implicitHeight + ScreenTools.defaultFontPixelHeight * 1.4
        radius: ScreenTools.defaultFontPixelWidth * 0.4
        color: "#D9232323"
        border.color: "#66FFFFFF"
        visible: _externalMapStatusText.length > 0
        z: 10

        QGCLabel {
            id: externalMapStatusText
            anchors {
                fill: parent
                margins: ScreenTools.defaultFontPixelWidth * 0.8
            }
            color: "white"
            wrapMode: Text.WordWrap
            text: _externalMapStatusText
        }
    }
    ...
    Loader3D{
        ...
        active: false
        sourceComponent: _useExternal3DMapSource ? externalMapComponent : buildingsGeometryComponent
    }
