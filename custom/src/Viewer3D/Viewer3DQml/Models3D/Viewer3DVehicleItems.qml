...
Node {
    ...
    property var _viewer3DSetting:              QGroundControl.corePlugin.viewer3DSettings
    property var _altitudeBias:                 _viewer3DSetting.altitudeBias.rawValue
    property bool _useExternal3DMapSource:      _viewer3DSetting.useExternal3DMapSource.rawValue && !_viewer3DSetting.useGoogle3DMapSource.rawValue
    property double _externalOriginAltitude:    Number(_viewer3DSetting.external3DMapOriginAltitude.rawValue)
    ...
    function displayAltitudeForMissionItem(missionItem) {
        if (!missionItem) {
            return 0
        }

        if (_useExternal3DMapSource && missionItem.amslEntryAlt !== undefined) {
            // 外部 3D 模型地图用真实海拔配准：任务点高度 = 任务点 AMSL 海拔 - 模型原点海拔。
            var amslAltitude = Number(missionItem.amslEntryAlt)
            if (!isNaN(amslAltitude)) {
                return amslAltitude - _externalOriginAltitude
            }
        }

        var altitude = Number(missionItem.altitude.value)
        ...
        for (var i = 0; i < _missionController.visualItems.count; i++) {
            ...
            if(isItemAcceptable(_missionItem)){
                if(isReturnToLaunchItem(_missionItem)){
                    ...
                    _geo2EnuCopy.coordinate.altitude = displayAltitudeForMissionItem(_missionItemPrevious);
                }else{
                    _geo2EnuCopy.coordinate = _missionItem.coordinate;
                    _geo2EnuCopy.coordinate.altitude = displayAltitudeForMissionItem(_missionItem);
                }
    ...
    DroneModelDjiF450{
        ...
        gpsRef: _backendQml.gpsRef
        useAmslAltitude: _useExternal3DMapSource
        originAltitude: _externalOriginAltitude
    }
