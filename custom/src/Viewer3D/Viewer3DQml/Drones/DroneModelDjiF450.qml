...
Node{
    ...
    property bool useAmslAltitude: false
    property double originAltitude: 0
    ...
    function displayAltitudeMeters() {
        if (!vehicle) {
            return 0
        }

        var relativeAltitude = Number(vehicle.altitudeRelative.value)
        if (useAmslAltitude) {
            // 外部 3D 模型地图用真实海拔配准：车辆高度 = 车辆 AMSL 海拔 - 模型原点海拔。
            var amslAltitude = Number(vehicle.altitudeAMSL.value)
            if (!isNaN(amslAltitude)) {
                return amslAltitude - originAltitude
            }
        }

        return isNaN(relativeAltitude) ? 0 : relativeAltitude
    }

    property double pose_y: (geo2Enu.localCoordinate.y)?(geo2Enu.localCoordinate.y * 10) : (0)
    property double pose_z: (displayAltitudeMeters() + altitudeBias) * 10
