...
DECLARE_SETTINGGROUP(RemoteID, "RemoteID")
{
    ...
    // Basic ID identifies the current aircraft and must be entered again for each QGC session.
    basicIDType()->setRawValue(0);
    basicIDTypeChina()->setRawValue(0);
    basicIDUaType()->setRawValue(0);
    basicID()->setRawValue(QString());
    sendBasicID()->setRawValue(false);
...
DECLARE_SETTINGSFACT(RemoteIDSettings,  basicIDTypeChina)
...
DECLARE_SETTINGSFACT(RemoteIDSettings,  categoryChina)
DECLARE_SETTINGSFACT(RemoteIDSettings,  classEU)
DECLARE_SETTINGSFACT(RemoteIDSettings,  classChina)
