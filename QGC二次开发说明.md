...
- MT11的 `currentZoom`发布从1.0x锚定且不超过物理上限的合法步长目标；`actualZoom/actualZoomKnown`继续在Manager内部保存0x18权威实测值，供确认、端点判断和诊断使用，但相机控制栏与A8一致，只显示一个目标倍率数值。默认步长1.0x且产品物理上限为165.1x时，目标最高165.0x，内部实测仍可到165.1x；步长0.1x时165.1x本身也是合法目标。短按按独立 `mt11ZoomStep`从上一目标计算1.0～30.0x的0x0F；不足完整步长时把精确30.0x追加为最后目标，例如step 2.0使用1、3、…、29、30。SDK的0x05只有 `-1/0/+1`方向/停止字段，没有速度字段，不再用周期stop/start或逐档0x0F伪造长按速度。1～30x内同时存在tap与hold时仍以420 ms区分；30x以上两方向tap均关闭，控件在按下时立即启动hold。长按接管仍pending的0x0F、上一hold settled状态或已确认端点锁存方向时，第一份新方向在同一次调用中立即发送，前面不插入0x05(0)，也没有300 ms/3秒人为等待。3秒takeover-protection Timer只保证0x0F恰在QML判定期间结算时仍进入受保护路径，从不延迟首包。普通受保护接管在首包后按450 ms间隔额外发送最多六份纯方向副本；前一控制器的残留0x18不能证明新方向已接管，所以不会提前取消这六份。若新方向从其相反物理端点起步，则进入端点反向恢复通道：首包仍立即发送，随后最多补80份同方向命令、覆盖约36秒；第一份补发成功后才重置运动证明基线，随后两次沿请求方向的权威0x18变化证明镜头已反向运动并提前停止补发，release或端点结束也会同步取消余下副本。运动期间每100 ms查询0x18；本代已经看到非端点运动后，连续两份同方向端点反馈只结束QGC手势并锁存端点方向，不发送0x05(0)，下一次反向hold直接覆盖该方向。无法证明本代运动的争议端点仍走安全停止。普通release/cancel仍取消全部待发方向、立即发送停止并安排一份150 ms有界安全副本；生命周期关闭、SDK endpoint变化和主镜头切换会显式中和端点锁存状态。60秒watchdog按“无进展”重置，165.1x端点允许立即反向退出。
...
- 2026-08-25真机在165x端点出现20～30秒无法反向，证明此前300 ms等待和约3秒方向取得窗口并未解决根因。SDK只明确0x05属于“手动变倍自动对焦”且0值为“停止缩放（松手后发送）”，没有规定0值会产生20～30秒正常对焦等待，也没有要求客户端在物理端点主动发送0。旧程序却在连续两份0x18已经确认165x端点后调用 `stopZoom()`，立即发送0并在150 ms后再发一份；tap转hold时仍pending的0x0F也会先收到0再等待300 ms，这两条交互路径都把额外停止放在用户反向命令之前。当前修正将“用户松手”和“镜头已到物理端点”拆开：可靠端点只逻辑结束并锁存方向，tap转hold直接由新0x05方向覆盖0x0F确认代次，均不先发0、不等待；30x以上hold-only区域按下即发方向。普通松手及生命周期安全停止保持不变。考虑到用户可能在第二份端点反馈到达前主动松手，下一次从相反物理端点开始的反向hold也会立即发首方向，并以最长约36秒的纯方向补发覆盖实测固件忙期；第一份补发重置基线后，两次0x18反向进展会立即结束余下补发，避免迟到旧反馈在450 ms前误取消恢复。因文档没有定义方向覆盖语义，仍须以真机抓包和倍率反馈验证，但程序侧已不存在20～30秒的软件等待Timer，也不再要求用户松手后重新按第二次。
...
- 当前代码已完成扩展后的两套独立QtTest编译运行：`Mt11ProtocolTest`为11 passed、0 failed（9个业务槽加init/cleanup），`SiyiProtocolTest`为42 passed、0 failed（40个业务槽加init/cleanup）；`Mt11Sdk.cc`也曾独立编译通过。MT11测试覆盖1～30x短按网格、step 2.0的1/3/…/29/30精确端点及对称返回、30.0/30.1协议门禁、0x18实测对齐、165.1物理端点显示边界、三种0x11官方payload和0x10模式域。该结果只证明Protocol/Policy与接口静态范围，不覆盖Manager的450/150/100 ms Timer、端点方向锁存、tap→hold直接覆盖、QML hold-only按下启动、完整QGC Qt 6构建或MT11真机原生连续运动/模式切换。
...
| `custom/src/FlightDisplay/GimbalZoomControl.qml` | A8/MT11共享手势壳和视觉参数：加/减按钮与面板其他操作一致，使用深色圆角方形、青色悬停描边及浅色按压缩放；倍率读数为胶囊内的单个青色目标数值。视觉参数由共享面板注入；tap与hold同时可用时以420 ms区分，tap不可用而hold可用时在pressed阶段立即调用Manager，因此MT11 30x以上及165x端点反向不再等待长按阈值。两条路径都先消费手势，release/cancel/移出/隐藏/后台/销毁可靠停止且不补tap。`showActualZoom`只保留为可选诊断能力，当前A8与MT11 wrapper均关闭；QML不复制设备协议、节奏Timer或上限策略。 |
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义11个Gimbal Fact；独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，只决定tap目标和长按期间的目标参考网格，不参与原生长按物理速度。longDesc明确短按精确30x端点、0x05没有rate字段、30x以上hold-only按下即启动、tap/端点接管不先发停止且没有人为等待、100 ms的0x18确认、可靠端点只逻辑结束、普通松手仍有150 ms安全停止副本；不改变设备上限或RTSP URL。 |
...
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11 QML业务门面；合法目标由 `currentZoom`发布，0x18实测仍由 `actualZoom/actualZoomKnown`保留作内部确认与可选诊断；继续暴露设备上限、独立步长、tap/hold availability、媒体与错误状态。长按状态保留原生方向、3秒takeover-protection标记、普通接管六份及端点反向80份的450 ms方向副本上限、两次反向进展确认、150 ms安全停止、100 ms反馈、端点方向锁存、松手端点候选以及端点/watchdog Timer；已删除300 ms handoff Timer，protection只决定副本资格而不延迟首方向，仍不持有pace Timer或drive-running PWM状态。三模式QML契约为 `videoModeKnown`、`videoMode`、`videoModePending` 和 `setVideoMode(int)`，枚举值0/2/3与协议 `main_stream` 一致；旧 `thermalModeKnown/thermalModeEnabled/thermalCommandPending/toggleThermalMode()` 只为已有调用者保留兼容，新UI不以它们表达三态。常量区分0x0F上限30.0x、产品物理上限165.1x和线反馈上限255.9x。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 短按维持1～30x相邻0x0F，并在最后不足step时直接到精确30x。0x05协议没有速度字段；普通hold在全倍率范围使用原生方向并保持到release/cancel/端点，不用周期stop/start或逐档0x0F。tap转hold直接退休本地0x0F确认代次并立即发送新方向，不先发0、不等待；旧hold待发安全stop在新方向前取消。3秒takeover protection只跨越QML手势判定并决定副本资格，不延迟首包。普通受保护接管在首包后完整补发最多六份450 ms纯方向，残留0x18不能提前退休该序列；从请求方向的相反物理端点起步时改用最多80份、约36秒的端点反向恢复，第一份补发重置证明基线后才允许两次沿请求方向的0x18进展提前停止补发。100 ms查询0x18并把控制栏目标沿方向单调对齐到合法参考档；本代已观察到非端点运动后，连续两次端点反馈只结束QGC状态并锁存端点方向，不发送停止，下一反向方向直接覆盖。无法证明本代运动的端点走安全stop；普通release/cancel先取消全部方向副本，再发送stop和一份150 ms安全停止副本。模式切换、disable、endpoint变化及析构会中和端点锁存。60秒watchdog按0x18进展刷新。 |
...
| `custom/test/Gimbal/Mt11ProtocolTest.cc` | MT11纯协议与策略QtTest，共9个业务slot；覆盖帧、CRC、0x05/0x0F/0x16/0x18倍率格式、1～30x tap门禁、step 2.0的1/3/…/29/30精确边界及对称返回、Target对齐和165.1物理端点规则；同时精确比对0x11的 `[00 02]`/`[02 00]`/`[03 02]`完整帧，验证0x10对main 0～5、sub 0/1/2/6的兼容解析以及只将main 0/2/3映射到三个UI工作模式。当前11 passed、0 failed（含init/cleanup）；仍不覆盖Manager的450/150/100 ms Timer、端点方向锁存、直接控制器覆盖、模式2.5秒Timer、真实UDP或真机原生连续运动/视频模式切换。 |
...
MT11完整缩放调用链为：合法SDK包维持6秒在线 -> 0x16能力与0x18实际位置各维持6.5秒新鲜度 -> Policy对齐合法步长Target -> tap在1～30x发送0x0F，最后不足一步直接到精确30x -> tap/hold并存区在420 ms后启动hold，30x以上hold-only区在pressed阶段启动 -> 新hold先取消旧stop安全副本、在QGC本地退休pending的0x0F确认代次，并在同一次Manager调用中立即发送0x05方向，不插入stop或handoff等待 -> 每100 ms查询0x18；3秒protection标记只保留普通受保护资格，普通路径在首方向后按450 ms完整补最多六份同方向副本，前驱残留0x18不作为新0x05的确认；若从请求方向的相反物理端点起步，则首方向后最多补80份、覆盖约36秒，第一份补发成功后重置基线，再由两次沿请求方向的0x18进展提前停止该序列 -> 内部保存实测值并沿方向单调发布合法目标 -> 本代已观察到非端点运动后，连续两份端点反馈只结束QGC手势并锁存端点方向，不发送0；下一反向命令直接覆盖 -> release/cancel取消全部方向副本、立即发送0x05停止及一份150 ms安全副本。争议端点仍走安全stop，生命周期边界显式中和锁存。0x05 ACK只维持可达性，0x18是唯一位置真值；UI只显示目标倍率。默认step 1.0时目标上限165.0而内部实测可到165.1，step 0.1时165.1亦为目标。step不参与原生物理速度，1.0与2.0使用相同物理命令策略。
...
4. 右栏在A8与MT11同时启用时显示带独立在线点的分段胶囊选择器；切换只更换Manager、会先关闭当前弹层，不切换中心视频。两个相机面板共享高透明蓝灰背景、青色强调、统一浅蓝外边缘、圆角按钮和悬停/按压动效；通用错误不再形成整栏红色外圈。MT11倍率与A8一致，只显示目标数值：tap发送1～30x绝对目标且不足一步直接到30x；tap/hold并存区按住420 ms启动，30x以上hold-only区按下即启动。紧接tap或可靠端点时，新方向必须立即发送且前面没有stop/等待；普通受保护接管补发最多六份同方向副本，从相反物理端点起步的反向接管则最多补80份/约36秒并由两次反向0x18进展提前结束，中间绝不发stop，release或端点结束取消余下副本。可靠端点结束及随后的release不发stop，普通release/cancel仍取消未来方向并发送正常停止。MT11拍照上方的模式按钮打开左侧优先的紧凑Overlay Popup，三个模式项与主栏按钮同为 `actionSize`正方形，以0x11选择变焦、热成像或变焦+热成像拼接，仅回包确认后更新当前模式。拍照、录像与媒体徽标语义不变。
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> 30x以上两方向tap availability关闭且hold-only在pressed阶段立即启动，不再等待420 ms；Manager直接调用仍防御性提示仅长按
         -> 0x0F确认期间250 ms轮询0x18、最长10秒；固定sequence/command-only ACK不直接回滚，只由精确0x18或超时收敛
         -> 长按：普通路径启动原生0x05方向并保持连续运动，step不参与物理速度；普通受保护覆盖在首方向后每450 ms完整补发最多六份纯方向，前驱0x18不提前取消
         -> 100 ms轮询0x18并单调更新合法目标；本代先见非端点运动、再连续收到两份端点时，只结束QGC手势并锁存方向，不发送0x05(0)
         -> tap pending后长按：取消旧安全stop副本 -> 本地退休0x0F确认代次 -> 同一次调用立即发送所按方向 -> 普通路径完整补发最多六份纯方向，release/端点结束取消余下副本
         -> 端点后立即反向：QML按下即调用 -> 新0x05方向直接覆盖端点锁存方向；即使松手抢在第二份端点反馈之前，也由相反物理端点或短期松手候选识别恢复路径；无前置stop、无300 ms/3秒软件等待 -> 首方向后最多补80份/约36秒，第一份补发重置基线后两次反向0x18进展提前结束；无法证明本代运动的争议端点仍走安全停止
     ...
     -> A8或MT11任一enabled
        ...
        -> 合并栏始终显示，sdkResponding不决定可见性；SDK超时会失效卡录能力并锁缩放，待新0x20恢复
         -> 顶部 GimbalZoomControl.qml（统一深色圆角按钮、青色强调的 + / 倍率 / -，调用当前选择的Manager）
            ...
            -> MT11：tap在1～30x按step发0x0F并追加30x精确端点；hold全域单次启动原生0x05，100 ms轮询0x18并只显示单调合法目标
        -> 下部双目标相机控制
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - MT11 tap↔hold与首按接管矩阵：任意0x0F tap仍pending时开始同向或反向hold，都必须取消上一轮待发stop副本、只在QGC本地退休0x0F确认代次，并在本次 `startZoomWithPressDuration()` 返回前发送所按方向；前面不得出现0x05 `0`或300 ms Timer。1～30x仍由QML在420 ms识别hold后调用，30x以上tap不可用时必须在pressed阶段立即调用。若0x0F恰在420 ms内确认，3秒protection标记只让本次进入普通受保护副本路径，不得延迟首方向。普通受保护hold持续按住时，首方向后每450 ms额外发送一份纯方向副本、最多六份；残留0x18无论正向、静止或逆向都不得提前退休该序列，副本之间不得出现0。pending 0x0F处于1x或30x时也必须保持该普通六份路径，不得把旧绝对控制器的同向0x18误作端点反向接管证明。分别在任一副本前验证release/cancel/移出/后台：不得补tap或迟发方向，且必须取消余下副本。普通release立即停止并仅保留一份150 ms安全副本；settled门禁只阻止tap，不阻止可行方向的新hold。
   - MT11原生连续长按矩阵：tap/hold并存区在420 ms成立后启动，hold-only区按下即启动。远离控制器覆盖的普通hold抓包只能出现一份方向start，整个按住期间不得周期stop/start或发送逐档0x0F。接管pending tap或旧hold settled时，第一份方向也必须立即发送，随后完整运行最多六份450 ms方向副本；从请求方向的相反物理端点起步或反向覆盖端点锁存方向时，第一份方向同样立即发送，随后最多运行80份450 ms方向副本（约36秒）。首包到第一份补发之间的0x18不得取消80份恢复；第一份补发成功时重置证明基线，只有其后两次沿请求方向的权威0x18进展才可提前取消余下副本。所有副本之间不得发送stop，release或端点结束必须取消余下副本。分别把step设为1.0和2.0，取得控制权后的0x05序列和真实倍率—时间斜率应基本一致，只有控制栏目标网格粗细不同。每100 ms查询权威0x18，UI只显示沿运动方向单调对齐的合法目标。普通release/cancel立即取消方向Timer、stop并仅有一份150 ms安全副本；本代已观察到非端点运动后到达1x或0x16物理上限时只逻辑结束并锁存方向，不发stop，随后的QML release也不得补stop。60秒watchdog、发送失败、模式切换及SDK离线仍必须可靠收尾。约11x前后需单独记录设备固件曲线；SDK没有rate字段，不能重新用step或周期停止伪造平滑速度。
   - MT11争议端点矩阵：在raw恰位于所按方向端点、但pending/settled保留目标表明仍可运动时启动hold。900 ms前连续注入旧端点0x18不得把它当成本代可靠运动；900 ms失效保护后连续两份端点走安全stop。另从非端点开始真实运动，至少注入一份本代非端点0x18，再连续注入两份同方向端点：必须结束QGC手势、锁存旧方向且抓包中没有0x05 `0`；按住状态被同步消费，release不补stop。随后在165x按下缩小（或1x按下放大），第一份反向0x05必须立即出现且前面没有0，端点锁存由该成功发送或0x0F绝对目标覆盖。再覆盖“仅收到第一份端点0x18便松手”：release仍须发0和150 ms安全副本，但要保存短期端点候选；即使下一次press前迟到0x18把实测覆盖为非端点，立即反向仍须进入80份上限恢复路径；首包后的旧0x18不得取消，第一份补发重置基线后两次反向进展才可取消。模式切换、disable、SDK endpoint变化与析构仍须显式安全中和锁存及候选。
...
选择MT11时视频有画面但缩放按钮仍灰色，应检查SDK在线、0x16/0x18新鲜度和模式pending；RTSP、A8的0x20及分辨率不能解锁MT11。settled等待只应锁定tap，物理方向可行的hold必须保持可用。>30x没有tap动作，当前控件在按下时就直接启动hold。普通长按抓包应只看到一份方向start，按住期间不能出现周期stop/start或逐档0x0F；紧接pending tap、release settled或可靠端点反向时，第一份方向也必须立即出现，前面没有额外0或300 ms等待，随后才允许450 ms有界纯方向取得副本。普通接管最多额外六份且残留0x18不提前取消；从相反物理端点起步的反向接管最多额外80份/约36秒，第一份补发重置基线后两次请求方向的0x18进展才会提前取消。step 1.0与2.0只改变目标网格，不改变物理策略。165.1端点且step 1.0时UI显示165.0、内部0x18可为165.1；端点确认前应已出现本代非端点反馈，第二份端点反馈后抓包不得出现0x05(0)，按下减号应立即出现0x05(-1)。若只收到第一份端点反馈便松手，正常05 00仍是安全行为，但下一次反向首包同样必须即时，且迟到反馈不能取消端点恢复。若仍要等数十秒，先确认实际运行的是本修正构建，再抓包核对端点后是否仍有不属于手动release的05 00、反向首包是否在pressed时发出、未检测到反向进展时是否每450 ms续发05 FF、来源IP/端口及0x18是否持续；日志不再逐包显示unmatched ACK，抓包才是方向、停止与时间线证据。
...
A8 hold仍按原有单次0x05与按压时长目标策略。MT11成功取得控制权后使用原生0x05连续运动，但不使用A8的按压时长目标估算，而以100 ms的0x18实测更新单值目标。tap与hold都可用时仍在420 ms后启动；30x以上hold-only在pressed阶段立即启动。仍pending的0x0F只在QGC本地退休确认代次，可靠端点只结束QGC手势并锁存旧方向；两者都由新方向直接接管，不先发stop且没有300 ms/3秒等待。3秒protection仅跨过420 ms判定并选择普通受保护路径；普通接管在首包后完整发送最多六份450 ms纯方向副本，残留0x18不能提前结束。相反物理端点起步的反向接管在首包后最多发送80份450 ms纯方向副本，约覆盖36秒；第一份补发成功前不接受0x18运动证明，随后两次沿请求方向的0x18进展才会提前结束，release或端点结束均取消余下副本。普通release/cancel发送stop，并在150 ms后发送一份有界安全副本；可靠端点自动结束后的release不会再补stop，抢在第二份端点反馈前的release仍安全stop，但短期端点候选保证紧随其后的反向hold进入长恢复路径。
...
截至2026-08-25，本轮MT11端点反向即时响应修正的静态验证为：Gimbal JSON和两份TS XML均可解析且longDesc source完全一致，Manager头文件Qt moc exit 0，`GimbalZoomControl.qml` Qt 5.14 qmllint exit 0，英文/中文lrelease均exit 0，`git diff --check` exit 0且只有行尾转换提示；首方向同步发送、release/cancel、端点锁存、松手端点候选、普通六份副本、端点反向80份/约36秒副本、第一份补发后才启用的两次反向进展确认、析构、状态失效及错误退出路径已逐项静态审查，旧handoff Timer/300 ms等待无残留。当前机器没有Qt 6构建目录或 `compile_commands.json`，现有 `Mt11ProtocolTest`也不覆盖Manager的QTimer状态机，因此仍需Qt 6完整构建和MT11真机验证。现行算法以本段和2026-08-25根因条目为准；下面“截至2026-08-24”段落仅保留为历史记录，已被“首方向即时、普通接管六份固定副本、端点反向最长约36秒且运动确认后提前结束”的方案取代。
