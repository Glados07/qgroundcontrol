...
- `GimbalControlManager` 直接接收上述主显示sink的协商尺寸；非GStreamer构建才使用 `VideoManager::videoSizeChanged/decodingChanged`，2秒轮询入口只负责补查尚未完成的首次锁存。能力锁存后立即启动0x18稳定同步，后台每2秒在空闲时继续查询0x18并查询0x0a相机状态；绝对目标或稳定确认期间由专用串行查询状态机接管。每次先等待350 ms隔离旧包，同一时刻只允许一个0x18查询在途；单次查询1秒无响应才重试，整次缩放操作10秒未确认才置unknown。该探测不依赖控件可见或活动飞行器。接收端以 `QHostAddress::isEqual()`比较来源逻辑IP，并要求精确帧长、CRC及ACK control严格等于0x02；只有业务payload也合法的回包才置 `sdkResponding=true`。
...
| 文件 | 详细作用 |
|---|---|
| `custom/src/Gimbal/A8MiniZoomPolicy.h` | A8 Mini缩放策略的纯静态接口，将视频尺寸到最大倍率映射、固定网格和真实边界吸附计算从Manager状态机中分离，便于独立测试；不访问网络、QSettings或UI。实际拉流唯一来源由Manager根据 `VideoManager::decoding/videoSize` 强制执行。 |
...
| `custom/src/Gimbal/GimbalControlManager.h` | 思翼云台相机业务的QML门面和运行态声明。`currentZoom`只表示设备确认值，另向QML暴露按固定网格及真实边界转换计算的 `zoomInAvailable/zoomOutAvailable`。能力状态只保存首次拉流尺寸是否已锁存及对应上限；倍率状态分别保存唯一绝对目标、目标命中次数、稳定值候选、查询在途门禁、350 ms隔离Timer、1秒单次查询Timer和10秒操作Timer。默认5.5x仅是能力未知时的界面占位，不能用于发送缩放命令。 |
| `custom/src/Gimbal/GimbalControlManager.cc` | 由 `CustomPlugin`创建一次并持有 `SiyiSdk`，没有修改或复制 `src/VideoManager`。GStreamer构建只接受主显示sink直接报告的最终协商尺寸，避免原生 `query_caps` 早期值抢先锁错；非GStreamer构建继续只读复用 `VideoManager::decoding/videoSize`。首次观察到1920×1080、1920×1088或2560×1440即锁存上限，后续断流、重连和尺寸变化均忽略，0x16完全不参与；锁存日志包含尺寸、来源和上限，倍率双确认完成时另有“zoom controls are ready”日志。单击和长按共用串行0x0f目标路径；中间移动使用完整步长，最后一个网格点与真实小数上限之间允许显式边界吸附及反向返回。一个目标在途时拒绝追加操作，只有两个独立0x18查询都命中目标才更新 `currentZoom`。状态机保证同一时刻最多一个0x18查询，超时经隔离后重试；启动或外部控制留下的其他非网格稳定值会在显示解锁前吸附到最近合法停点（固定网格点或真实上限）。功能关闭后忽略关闭前请求的迟到回包，不能重新点亮在线状态或触发自动倍率修正命令。 |
...
Gimbal启用后，Manager即使不在Fly View也会持续运行，并每2秒向 `sdkHost:sdkPort`探测：GStreamer构建由主显示sink直接报告最终协商尺寸，非GStreamer构建以 `VideoManager::videoSizeChanged/decodingChanged` 补查；每轮发送0x0a相机状态，只有缩放状态空闲且没有0x18查询在途时才查询当前倍率，Manager不发送0x16。绝对目标和稳定确认由专用状态机管理：先留350 ms隔离窗口，再发送唯一在途的0x18；1秒未收到合法倍率才安排下一次隔离重试，10秒整体deadline不会被重试刷新。Fly View右侧纵向合并栏不等待探测结果，无论是否连接飞控或云台都立即显示。只有来源逻辑IP匹配配置endpoint、通过帧头、精确长度、CRC、control=0x02及对应payload校验的SDK响应，`sdkResponding`才变为true。

有效上限只取QGC实际解码的拉流尺寸，且当前支持1920×1080→5.5x、设备解码链coded-height对齐形式1920×1088→5.5x、2560×1440→3.5x。这里不是相机机内卡录分辨率，也不读取0x16；GStreamer主视频在真实首帧到达时从sink、解码器peer或ghost target的current CAPS读取宽高，同时以主 `GstGLQt6VideoItem` 根据最终 `GstVideoInfo` 设置的隐式尺寸作为独立备援。两条路径任一取得受支持尺寸就直接交给Manager，并由pad路径同步校正 `VideoManager::videoSize`。该尺寸在Manager/QGC应用会话内只锁存一次，之后断流、重连或尺寸变化不重算；锁存前视频未解码或尺寸不受支持时保持unknown，单击和长按均锁定。能力确认后立即启动同步，在350 ms旧包隔离后发出0x18；需两次串行稳定确认并在必要时对齐到固定网格点或真实上限才解锁。普通SDK在线探测使用1.5秒响应窗口；正在进行的倍率确认可在10秒整体deadline内重试。拍照仍可重试，录像必须先取得0x0a状态，因为0x0c是toggle。
...
4. 返回Fly View。只要Gimbal Enabled，右侧单个纵向合并栏就应立即显示，不要求飞控或云台已连接；从上到下依次为 `+`、倍率、`-`、拍照和录像。尚未取得有效能力和倍率时倍率显示 `--`且缩放按钮禁用。视频第一帧到达后应依次看到“Installed main pulled-video resolution observers”、negotiated/observed主拉流尺寸、会话尺寸锁存和发出的命令24（0x18）日志；1920×1080/1920×1088锁存5.5x，2560×1440锁存3.5x，0x16不参与。第二次一致0x18完成后应出现“zoom controls are ready”，中心显示实际倍率，并仅点亮仍有合法目标的方向按钮：1.0x时减号保持灰色属于正确边界行为，最大倍率时加号保持灰色。录像按钮在0x0a状态未知时灰显并显示 `...`。
  ...
  -> CustomPlugin::init()
     ...
     -> FlyViewCustomSettings（FlyView/showHeadingCompassBar）
      -> GimbalControlSettings / GimbalControlManager
         ...
         -> GStreamer接收主显示sink最终协商尺寸；非GStreamer才补查VideoManager
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   - 分别在两个独立QGC会话中验证实际拉流尺寸来源：1920×1080必须锁存5.5x，2560×1440必须锁存3.5x。锁存前给出4K、720P、640×480、空尺寸或 `decoding=false`均应忽略并保持缩放锁定；一旦锁存，随后停止解码、重连或报告另一尺寸都不得改变本会话上限。需要有意更换1080P/2K时重启QGC后重新确认首次尺寸。
