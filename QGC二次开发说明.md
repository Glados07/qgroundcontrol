...
12. UniRC 10 Pro 内置SDK蓝牙的CH9拨轮变倍与CH10按键回中。
...
- **本轮云台方位角计算修正**：上一版已经绕开原生 `GimbalController` 固定用四元数yaw加 `Vehicle.heading`且不读取 `delta_yaw`的问题，但真机复测仍说明目标桥接器在Lock下声明的Vehicle frame/`delta_yaw`关系与真实运动不一致，标准转换会把基座变化再次加入方位。本轮继续不修改 `src`：`GimbalAzimuthPolicy`保留MAVLink标准公式，新增按Vehicle/source/device隔离的 `GimbalYawLockResolver`。显式Vehicle+Lock首包仍采用标准候选，随后比较标准结果、直接q和“直接q+近期Vehicle.heading”三个独立候选；兼容候选必须以8°/5°证据同时击败其余有效候选才可选中，并持续三向复核。q实际为地系时标记 `YawLockReportedYawCompatibility`；q仍为机体系但delta冻结/错误时标记 `YawLockVehicleHeadingCompatibility`。跟随、显式Earth和legacy路径不变，顶部 `GimbalIndicator` 的Az与顶部罗盘继续共用Provider；纯策略50项测试通过，完整Qt 6/Android构建与目标云台新APK回归仍待完成。
...
- SIYI A8 Mini设置区提供“通道进行云台缩放控制是否反向”开关，对应 `uniRcZoomDirectionReversed=false`，仅改变UniRC物理拨轮到A8缩放方向的映射。缺省关闭时CH9低端为缩小、高端为放大；开启时低端为放大、高端为缩小。该Fact不反转触控 `+/-`、0x0f绝对目标、A8 SDK全局方向或CH10回中；中位死区和首次回中arm保持不变。运行中修改时立即取消当前UniRC hold，并要求CH9重新经过中位再按新方向动作。
...
- 顶部世界方位角按消息坐标系解释：显式Earth frame取 `yaw(q)`；显式Vehicle frame且 `delta_yaw`有效时取 `yaw(q_delta_yaw × q)`，否则兼容回退为 `yaw(q) + Vehicle.heading`；无显式frame位时按旧协议由 `YAW_LOCK` 选择Earth/Vehicle frame，并按规范忽略 `delta_yaw`。航向锁定表示维持进入锁定时的世界朝向，不表示强制指向正北；转动基座时，正确方位角应只在控制误差和传感器误差范围内小幅波动。
...
- 顶部条的最终门禁为overlay可见、用户开关打开、活动Vehicle及 `activeGimbal` 存在、`vehicleLinkManager.communicationLost=false`，且Provider已经为该Vehicle/manager/device收到并算出有限、2秒内的新鲜世界方位角；条件不足时整条隐藏，不以0°/N伪造角度。Provider只有确认收到过飞控heading遥测后才允许heading兼容回退，不能把Fact初始值0误当作真北；整机链路丢失会清除该Vehicle的姿态和heading有效状态。这是只读MAVLink反馈层，不发送云台命令，不依赖A8/MT11私有SDK的在线状态，也不跟随右侧A8/MT11相机控制栏选中项。
...
### 1.13 UniRC 10 Pro CH9/CH10云台控制（Android正向链路真机已通过）
...
- 每次RFCOMM连接都会清零接收诊断代次。首帧watchdog根据事实分别报告“连接后零RX字节”“有蓝牙字节但无合法SDK帧”“有合法SDK帧但没有目标0x42通道帧”；已经收到合法0x42后再停止则报告流中断。合法0x42在CH值校验前续期watchdog，所以CH9/CH10未映射而返回0/越界值时只停止控制、解除arm并保留连接和实际值。
- CH9只有在初始/失联后先进入1475～1525中位死区才arm；缺省方向为小于1475缩小、大于1525放大，A8 Mini区开启“通道进行云台缩放控制是否反向”后交换为小于1475放大、大于1525缩小，回到死区始终停止。运行中改变该设置会停止当前拨轮动作并重新等待中位，不断开0x42链路。CH10只有先见到≤1250释放值才arm，随后≥1750的按下沿只触发一次回中，持续按住不重复，重新释放后才允许下一次。
...
- CH10和顶部工具栏Center共用 `GimbalCenterCoordinator`，沿活动Vehicle/Gimbal Manager的控制权确认、必要的非零pitch预激活、ACK和最终 `centerGimbal()` 流程发送MAVLink；UniRC控制器不直接写GPS2，也不绕过PX4。应用退到后台或退出会取消尚未完成的共享回中事务；PX4到A8 Mini的GPS2/UART 115200转发仍属于飞控和云台侧配置。
- 运行前必须开启Android Bluetooth，在系统蓝牙页面搜索并配对目标模块，再在UniGCS把“遥控SDK连接方式”选择为蓝牙。QGC按配置的MAC直接连接，不承担发现或配对。虽然PDF第122页没有列出“数传1=UDP、数传2=关闭、SDK=蓝牙”，但2026-09-01目标固件已在该配置下同时完成0x42、CH9缩放和CH10回中正向闭环，因此当前产品应保留这组不破坏既有数传/图传的实测配置；组合8/9仅作为固件变化后的备选排查项。Android仍需授予Bluetooth相关的Nearby devices访问权限；不再需要字符设备权限、SELinux UART放行或关闭系统蓝牙。
...
| 文件 | 详细作用 |
|---|---|
| `custom/CMakeLists.txt` | custom 构建总入口。向根工程注入 `QGC_CUSTOM_BUILD`、`CUSTOMHEADER=CustomPlugin.h` 和 `CUSTOMCLASS=CustomPlugin`，收集各custom C++、QML模块、资源和翻译，并继续通过目录递归收集GStreamer生产源码。Bluetooth被列为custom必需Qt组件并显式链接 `Qt6::Bluetooth`，不依赖原生Bluetooth Link开关的偶然传递。桌面 `QGC_BUILD_TESTING` 构建创建既有协议/媒体/UniRC测试、纯Qt `AndroidH265DecoderRoutePolicyTest`，以及包含 `GimbalAzimuthPolicy`与 `GimbalYawLockResolver`的 `GimbalAzimuthPolicyTest`/`check_gimbal_azimuth_policy`。Android configure仍把根 `android`模板复制到build目录后用 `custom/android`同路径覆盖，并只编译唯一合并源目录；生成副本注入Android 12+的 `BLUETOOTH_SCAN/CONNECT`，以及仅限API 30及以下的 `BLUETOOTH/BLUETOOTH_ADMIN/ACCESS_FINE_LOCATION`，再校验Manifest、USB管理器和媒体库overlay。UniRC本身改为按MAC直连后不启动发现器，但根工程默认启用的原生Bluetooth Link仍具有设备发现能力，因此不能仅因设置页删除扫描按钮就移除全应用的SCAN/旧版定位声明。旧Bluetooth关闭状态helper及其marker校验已删除。没有修改根 `android/AndroidManifest.xml`。外部WGS84样例目录不参与构建或安装。 |
...
| `custom/src/Android/UniRcChannelPolicy.h` | 声明CH9/CH10纯状态机、合理值范围900～2100、CH9边界1475/1525和CH10释放/按下阈值1250/1750。`update()`显式接收缩放反向布尔值，输出有效方向是否改变及是否产生一次回中请求，不依赖QObject、Bluetooth、A8 UDP或Vehicle。 |
| `custom/src/Android/UniRcChannelPolicy.cc` | 初始和 `linkLost()` 后解除两个输入的arm：CH9必须先见1475～1525中位，CH10必须先见≤1250释放。arm后先得到缺省物理映射（CH9小于1475为-1、大于1525为+1、死区为0），再按 `zoomDirectionReversed`只对非零方向取反，因此反向模式为低端+1、高端-1且中位仍为0；CH10边沿完全独立。CH9/CH10任一超出900～2100都等价失联并输出必要的缩放停止变化。 |
...
| `custom/src/Android/UniRcChannelController.cc` | Android前台且 `UniRC SDK`启用、接口选择Bluetooth后，请求Qt `QBluetoothPermission::Access`并要求系统Bluetooth开启，读取可编辑的 `uniRcSdkBluetoothAddress`（缺省 `41:42:9E:3D:A5:D2`）直接建立 `RfcommProtocol + SerialPort UUID` SPP socket，不启动QGC设备发现。连接10秒无结果、请求在本地发送队列停留10秒、socket错误/断开、写出后首帧1.5秒超时或活动流350 ms中断都会撤销UniRC缩放并按10秒退避重连。连接成功连续排队三份20 Hz请求，`bytesWritten/bytesToWrite==0`只确认本地Bluetooth写出并从该时刻重启首帧窗口，`readyRead`直接送入parser；每个合法通道帧发布全部16路值，CH9/CH10再进入控制策略，CH9方向读取 `uniRcZoomDirectionReversed`后才进入A8 Manager。反向Fact变化使用独立槽停止当前动作和重置安全arm，不关闭RFCOMM；扫描、MAC、接口或总启用变化才重建传输会话。结构化日志继续区分配对、RFCOMM、请求排队/写出、任意RX、CRC合法SDK帧、合法0x42流和失败分类；这些调试数据不再呈现在设置页。每个socket槽校验sender，旧socket排队信号不能污染新代；正常关闭先排队停止请求并用1秒有界graceful close。标准SPP UUID已由目标固件真机闭环验证。 |
| `custom/src/Gimbal/GimbalCenterCoordinator.h` | 声明顶部工具栏与UniRC CH10共用的MAVLink回中事务对象，向QML暴露busy/dispatch状态、`requestCenter()`和`cancel()`；保存活动Vehicle/Controller/Gimbal身份、控制权和两阶段ACK Timer，不读取或管理SDK Bluetooth字节流。 |
...
| `custom/src/Gimbal/GimbalAzimuthPolicy.h` | 声明无QObject依赖的云台方位角标准计算输入、结果、来源与错误类型。输入明确区分Earth/Vehicle frame位、legacy `YAW_LOCK`、`delta_yaw`是否被设备支持/当前是否有效，以及飞控heading回退是否有效；结果只输出世界方位角，不保留罗盘不再使用的相对角API。另登记 `YawLockReportedYawCompatibility` 与 `YawLockVehicleHeadingCompatibility` 两个来源，供独立兼容解析器明确标记最终修正路径。 |
| `custom/src/Gimbal/GimbalAzimuthPolicy.cc` | 校验并归一化MAVLink `[w,x,y,z]` 四元数，按 `q_earth=q_delta_yaw×q_vehicle` 完成显式坐标系转换，并把角度规范到 `[-180°,180°)`。显式Vehicle frame优先使用有限 `delta_yaw`、缺失时回退飞控heading；显式Earth frame直接输出上报yaw；legacy消息按 `YAW_LOCK`判定且强制忽略delta；两个frame位冲突、四元数非法或缺少地系参考时不猜测有效值。该纯函数保持MAVLink标准语义，不因锁定位单独把显式Vehicle q改成Earth q。 |
| `custom/src/Gimbal/GimbalYawLockResolver.h` | 声明无QObject依赖、可按单一Vehicle/source/device独立保存的锁定坐标系兼容状态。输入同时接收Policy标准候选、直接 `yaw(q)` 候选和独立的 `yaw(q)+Vehicle.heading` 候选，输出本次选择及是否发生切换；状态只保留锁定会话的选择与三个候选的运动锚，不保存或冻结云台目标角。纯helper还判断 `time_boot_ms`超过1秒的回退，当前值0被视为设备刚启动后的合法时间。 |
| `custom/src/Gimbal/GimbalYawLockResolver.cc` | 只在“明确Vehicle frame + `YAW_LOCK=1`”时处理目标桥接器frame/delta关系异常。第一份锁定样本始终采用标准候选并建立锚，避免把合规设备恰在切锁时收到yaw指令误判成坐标系切换；只有标准来源为DeltaYaw且2秒内有真实飞控heading时，第三个heading候选才独立参与。ReportedYaw或VehicleHeading兼容候选必须自身较稳定，并以至少8°运动、至少5°优势同时击败其余所有有效候选才能夺权；Standard与Heading并列稳定时保留Standard。所有有效候选都移动至少3°时才按摇杆/共同运动重建锚。三种选择均持续复核，标准来源或候选有效集合变化时重建锚，heading过期时若正在使用它则立即回Standard。两个兼容结果分别标记为 `YawLockReportedYawCompatibility` 和 `YawLockVehicleHeadingCompatibility`；单份非法姿态只令该样本无效而不丢弃已学习选择，解锁、超时、发送端重启、断链或显式reset才清除会话选择。 |
| `custom/src/Gimbal/GimbalAzimuthProvider.h` | 声明由 `CustomPlugin`持有的只读QObject Provider，向QML公开 `valid`、`absoluteYaw`、`usingDeltaYaw`和 `referenceSource`。内部缓存按Vehicle及MAVLink source component/reported device id隔离，保存消息时间、设备启动时间、扩展字段支持状态和各自的 `GimbalYawLockResolver::State`，并以Timer统一执行2秒新鲜度门禁；它只服务顶部显示，不替换原生Gimbal对象，也不发送命令。 |
| `custom/src/Gimbal/GimbalAzimuthProvider.cc` | 观察消息285并解码flags、q、`delta_yaw`、`delta_yaw_velocity`与 `gimbal_device_id`。后置的非零device id会迫使前面的默认0进入payload，因此“长度覆盖delta”本身不能区分合法首次0和旧发送端未填写字段；Provider要求消息恰好声明一个显式yaw frame，并由delta/delta速度的有限非零值、负零或NaN等线上语义非默认值首次证明扩展支持。MAVLink 2可能裁掉后续零字节，因此同一数据流一旦证明支持就保持粘性，后续合法0仍走delta路径；旧启动时间非0且新值发生超过1秒回退时重新探测，包含较大值直接回到合法0的重启首包。Provider先取得Policy标准结果；仅在显式Vehicle锁定路径再构造直接q候选，标准来源为DeltaYaw且2秒内收到真实飞控heading时还构造强制禁用delta的heading候选，再把同一路由状态交给Resolver。超过2秒的旧状态、设备启动回退和整机断链均不会复用。日志在标准来源、Resolver选择、锁定兼容条件或Resolver会话改变时同时记录flags、delta、heading、标准方位、直接q方位、heading候选与最终来源/方位。最终结果按活动Vehicle与activeGimbal身份优先选择精确manager/device路由，只有精确路由从未出现时才回退独立设备路由，避免另一组件用较新样本串扰。标准heading回退要求本链路曾观察到真实heading；第三候选还要求heading遥测不超过2秒。消息285超过2秒未更新、整机链路丢失或对象销毁时发布无效结果；Vehicle/Gimbal切换则立即按新活动路由发布，只有该路由没有新鲜有效缓存时才发布无效。 |
| `custom/test/Android/UniRcProtocolTest.cc` | 桌面纯QtTest回归，覆盖4/20 Hz及停止请求精确字节、PDF中CRC自洽的完整0x42回包向量、CTRL/CMD/32字节/16路小端解析、分片/粘包、噪声、坏CRC及伪长度重同步，以及CH9首次回中arm、默认/反向方向、1475/1525死区边界、失联后重新arm和CH10释放到按下沿。设置Fact缺省值、运行时信号槽、16路QML刷新、Android配对/RFCOMM、A8 UDP、MAVLink ACK和真机SDK路由仍必须由静态/Android验收覆盖。 |
...
| `custom/test/Gimbal/GimbalAzimuthPolicyTest.cc` | 云台方位角纯QtTest回归。覆盖显式Vehicle frame的delta优先、有效零delta与heading回退、显式Earth frame、frame位覆盖 `YAW_LOCK`、legacy锁定/跟随严格忽略delta、冲突flags、非法/非单位四元数、缺少地系参考、有限及非有限角度边界，以及连续样本中的锁定/跟随行为。Resolver序列另覆盖目标错误标帧、冻结非零delta、由velocity建立支持但delta保持错误0、合规delta/heading并列、无第三候选回退、有/无Follow历史、8°/5°判据、2°锁定漂移、锁内摇杆重建观测锚、flag/q错序、切锁同时yaw指令、三种已选结果纠正、heading过期、单份非法姿态恢复、±180°运动、解锁/reset及 `time_boot_ms`回到0的重启识别。当前使用MSVC + QtTest 5.14兼容harness实际运行50 passed、0 failed、0 skipped；正式Qt 6目标为 `check_gimbal_azimuth_policy`。该测试不替代CustomPlugin消息接线、多云台路由或目标云台真机验证。 |
...
| `custom/src/UI/AppSettings/FlyViewSettings.qml` | 保留原生Fly View设置，在Instrument Panel中以独立Loader显示 `Show Vehicle Heading Compass Bar`/“显示飞行器航向罗盘条”和 `Show Gimbal Heading Compass Bar`/“显示云台指向罗盘条”，各自绑定对应Fact并即时生效。Instrument Panel正下方仍以单层Loader固定加载Gimbal组，再加载Viewer3D组；父页缓存设置对象并在 `onLoaded` 注入。Gimbal Loader的首选/最小高度取自 `item.implicitHeight`，所以子QML若发生致命创建错误，`item=null`会使整组高度折叠为0；`onStatusChanged`只负责记录通用加载警告，修复应落在报错的子QML。当前source解析与设置对象注入链路有效，相机设置入口不依赖云台在线。 |
...
双罗盘的文件连接关系固定为：`FlyViewCustom.SettingsGroup.json` 定义两个开关元数据 -> `FlyViewCustomSettings.h/.cc` 创建、读取、保存并向QML暴露 -> `FlyViewSettings.qml` 提供两个独立用户开关 -> `CustomPlugin::mavlinkMessage()`把消息285交给 `GimbalAzimuthProvider` -> Provider按Vehicle/source/device调用 `GimbalAzimuthPolicy`取得标准世界方位，并在显式Vehicle锁定路径交给 `GimbalYawLockResolver`做运动一致性兼容 -> `FlyView.qml` 向custom overlay注入右上安全预留 -> `FlyViewCustomLayer.qml` 分别组合Vehicle heading门禁和Provider/activeGimbal/链路门禁 -> 两个Loader复用 `FlyViewCompassBar.qml` 绘制。顶部 `GimbalIndicator` 的Az也读取同一Provider。右侧A8/MT11相机选择器不在这条设置或遥测链中；任何单一文件都不能独立完成完整功能。
...
| `custom/translations/custom.ts` | 英文源字符串模板。保留既有云台、双视频、双罗盘及后台UniRC运行错误source，并同步 `UniRC SDK`、SDK接口、Bluetooth单项枚举、MAC、16通道网格和A8 CH9方向反转所需文本；设置页扫描候选、配对备注和诊断摘要source随对应UI删除。当前为21个context、185条message，全部保持unfinished模板；Qt `lrelease`可验证XML/目录结构，正式QM仍由项目Qt 6构建生成。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录，与英文模板保持完全相同的21个context、185条message及context/source pair集合；`Reverse channel gimbal zoom control`在QML和元数据context均翻译为“通道进行云台缩放控制是否反向”。全部185条均finished，0条unfinished、0条空译文。翻译完成只证明目录一致性，不替代Android按MAC自动连接或反向缩放真机验收。 |
...
方位角先按 `GIMBAL_DEVICE_ATTITUDE_STATUS.flags` 做标准计算；只有目标桥接器在显式Vehicle锁定状态下被连续样本证明frame或delta关系与真实运动不一致时，才启用按路由隔离的兼容结果。内部统一输出 `[-180°, 180°)`，罗盘绘制时再等价归一化为 `[0°, 360°)`：

| 消息情况 | 世界方位角计算 | 兼容行为 |
|---|---|---|
| 明确设置 `YAW_IN_EARTH_FRAME` | `azimuth = yaw(q_earth)` | 四元数已经相对地理北向，不叠加飞控heading。 |
| 明确设置 `YAW_IN_VEHICLE_FRAME`，且 `delta_yaw`可用 | `q_earth = q_delta_yaw × q_vehicle`，`azimuth = yaw(q_earth)` | 标准候选优先采用云台消息自己给出的地/机体系关系。跟随状态直接使用该结果。 |
| 明确设置 `YAW_IN_VEHICLE_FRAME`，但 `delta_yaw`缺失或NaN | `azimuth = wrap(yaw(q_vehicle) + Vehicle.heading)` | 标准候选保持旧设备和当前正常跟随设备的原生兼容路径。跟随状态直接使用该结果。 |
| 明确设置 `YAW_IN_VEHICLE_FRAME + YAW_LOCK`，但连续样本证明q实际已是地系 | `azimuth = yaw(q_reported)` | 目标设备兼容路径；不再把飞控heading或错误delta重复叠加到已是世界方位的q。最终来源为 `YawLockReportedYawCompatibility`。 |
| 明确设置 `YAW_IN_VEHICLE_FRAME + YAW_LOCK`，q仍为机体系但已支持的delta冻结/错误 | `azimuth = wrap(yaw(q_reported) + recent Vehicle.heading)` | 仅在标准来源为DeltaYaw且2秒内有真实飞控heading时参与比较；最终来源为 `YawLockVehicleHeadingCompatibility`。 |
| 两个新frame位都未设置 | `YAW_LOCK=1`时把q视为Earth frame，否则视为Vehicle frame | 按MAVLink向后兼容规则处理，并严格忽略 `delta_yaw`。 |
| 两个新frame位同时设置、四元数非法，或Vehicle frame又没有任何地系参考 | 结果无效 | 不猜测坐标系；罗盘隐藏，工具栏Az显示 `--`。 |

`YAW_LOCK`只表示云台维持当前地系朝向，不表示自动转向0°正北。例如进入锁定时指向72°，随后转动飞控/基座，理想方位角仍应约为72°；此时云台相对机头的角度会反向变化，但该相对角不再呈现在罗盘上。跟随状态下，如果云台相对机头角不变，飞控/基座转动会使世界方位角随之变化；摇杆改变云台yaw时，世界方位角按上述坐标变换同步变化。pitch的变化由同一姿态四元数描述，但本横向罗盘只绘制yaw方位角。

2026-09-04真机复测中，“锁定后物理朝向不变但显示仍随基座同量转动”的代码级原因是：上一版在显式Vehicle frame下无条件相信标准 `delta_yaw`/heading转换，但目标消息的frame/delta声明与锁定运动关系不一致。存在两条可产生同一现象的具体路径：一是q实际已按地系运动，标准转换又叠加一次基座关系；二是q仍按机体系反向运动，但发送端曾用非零delta或delta速度建立扩展支持后，`delta_yaw`本身冻结/错误，导致标准结果和直接q一起变化。后一种情况下只有 `yaw(q)+Vehicle.heading` 稳定，所以只比较标准与直接q仍会因两者共同达到3°而不断重建锚。不能把所有锁定设备一律改成某一个候选，因为严格按MAVLink上报的设备需要保留标准转换。当前Resolver同时比较三种运动关系：

1. 无论进入Lock前是否已有Follow样本，第一份显式Vehicle锁定姿态都采用Policy标准候选，并保存Standard=`yaw(q_delta×q)`或标准heading回退、ReportedYaw=`yaw(q)`以及可用时VehicleHeading=`yaw(q)+heading`的各自运动锚。Standard本来就是heading回退时不重复加入第三候选。单包连续性不作为覆盖依据，因为坐标系切换和切锁时恰有yaw指令可能产生相同数据。
2. 后续样本中，ReportedYaw或VehicleHeading只有在其他每个有效候选相对各自锚均累计运动至少8°，且自身比它们各少运动至少5°时才能夺权。这样允许真实锁定方位有约2°等小幅漂移，也不会因Standard与VehicleHeading同样稳定就覆盖标准delta。只有全部有效且去重后的候选都移动至少3°时，才按锁内摇杆/共同运动处理并重建三路锚。
3. Standard、ReportedYaw或VehicleHeading选中后仍持续做三向复核，后续反向证据可以纠正旧选择。标准来源由heading变成delta或反向变化时首包立即回Standard；第三候选出现/过期时重建观察窗，若正在使用已过期的VehicleHeading则立即回Standard。单份非法姿态不训练也不丢弃已学习选择。锁内拨动yaw摇杆时方位角仍随真实云台朝向变化，并非冻结进入锁定时的数值；解锁立即恢复标准跟随计算并清除锁定选择。
4. Resolver状态按Vehicle、MAVLink source component和reported device id隔离。消息间隔超过2秒、`time_boot_ms`明显回退或整机链路丢失后不复用旧选择，避免多云台、重启和重连串扰。VehicleHeading候选还要求2秒内收到飞控heading承载消息，不能用陈旧Fact训练兼容结果。

每次新Lock会话都需要保持云台物理朝向、不要拨yaw摇杆，并让基座形成足够的可区分运动；通常至少需要约8°，且获胜兼容候选必须相对其余每路取得至少5°的运动优势，因此8°不是存在漂移或重建锚时的严格识别上限。日志类别 `qgc.custom.gimbal.azimuth` 会在进入/退出兼容条件、重开Resolver会话或选源变化时打印 `protocol reference/protocol azimuth/reported q yaw/heading candidate/lock resolver/reference/azimuth`。若q实际为地系，目标路径应看到 `lock resolver ReportedYaw` 和 `reference YawLockReportedYawCompatibility`；若q仍为机体系而delta错误，应看到 `lock resolver VehicleHeading` 和 `reference YawLockVehicleHeadingCompatibility`。若始终为 `Undecided`，应核对基座转量、每个竞争候选的5°优势、近期heading及摇杆是否同时在动；若消息没有 `YAW_LOCK` 或显式Vehicle位，则兼容条件不会启用，应继续检查设备flags。
...
custom Provider逐帧接收 `GIMBAL_DEVICE_ATTITUDE_STATUS`，为每个Vehicle/manager/device保存单调接收时间，并每250毫秒检查一次；活动样本超过2秒未更新时同时令顶部工具栏Az无效并隐藏顶部罗盘，下一份消息也从新的Resolver状态开始。整机 `vehicleLinkManager.communicationLost` 会立即清除该Vehicle缓存；恢复通信后必须重新收到消息285，需要标准heading回退或VehicleHeading兼容候选时还必须收到本次连接的新heading遥测。新鲜度只按消息接收时间判断，不能根据角度是否变化判断，因为静止云台可持续上报相同值。
...
5. 在遥控器通道映射中把自动回中拨轮映射为CH9，把小摇杆按键映射为CH10。目标值为CH9最小/中位/最大约1050/1500/1950，CH10释放1050、按下1950。连接后设置页下方的自适应网格必须更新CH1～CH16全部真实值，其中CH9/CH10再用于物理动作验收；网格本身只表示最近的合法0x42通道帧，断流状态不得把旧会话数值冒充为当前实时输入。
...
UniGCS：遥控SDK -> Bluetooth
  ...
  -> 20 Hz的16路通道帧
     ...
     -> CH10 -> 共享GimbalCenterCoordinator -> MAVLink Gimbal Manager回中
```
...
| `stream-active` / `SDK_ROUTE_ACTIVE` | 已收到CTRL 0、CMD 0x42、32字节通道帧，Bluetooth SDK路由和0x42回包闭环成立 | CH9/CH10映射正确以及A8/MAVLink物理动作完成 |
...
| 输入 | 线值与门禁 | 动作 |
|---|---|---|
| CH9自动回中拨轮 | 初始、Bluetooth重连、失联、异常值或运行中改变方向Fact后，必须先见到1475～1525；未见中位时即使值为1050/1950也不动作 | Fact缺省false：小于1475调用A8缩小-1，大于1525调用放大+1；Fact为true：小于1475调用放大+1，大于1525调用缩小-1；1475～1525两种模式都停止。±25死区吸收中位抖动。 |
| CH10小摇杆按键 | 初始、Bluetooth重连、失联或异常值后，必须先见到≤1250释放值；未见释放时启动值1950不会回中 | arm后首次达到≥1750产生一次回中沿；保持按下不重复，回到≤1250后才允许下一次。 |
| CH9或CH10异常 | 任一值小于900或大于2100 | 样本按失联处理，停止UniRC缩放并解除两个输入arm；合法0x42仍续期Bluetooth链路，后台日志记录实际值和映射错误，16通道网格继续以活动/失效状态区分实时值。 |

CH9不按20 Hz重复发送A8方向包；通道策略只在方向改变时启动/停止，同方向帧只刷新Manager安全watchdog。反向只在CH9策略结果写入前改变符号，Manager和SiyiProtocol仍统一保持 `+1=放大、-1=缩小`，因此触控缩放、绝对目标和边界规划不会被反转。物理拨轮可先停止触控hold并接管；UniRC持有期间触控tap/绝对变倍被拒绝，触控release/cancel不能误停拨轮。拨轮回中、反向、方向Fact变化、通道失联、Bluetooth断开和生命周期退出只释放UniRC自己的动作。运行中改变Fact不会让镜头原地反向，必须先回中重新arm。若A8已在有效端点停止，本次仍偏转的拨轮不会反复重启，必须先回中或反向形成新边沿。

CH10与顶部工具栏Center共用 `GimbalCenterCoordinator`。协调器冻结当前Vehicle/Controller/Gimbal身份，必要时请求控制权；对已记录需要预激活的云台，先发送边界内非零pitch目标，等待对应命令1000 Accepted ACK和400 ms稳定窗口，再调用原生 `centerGimbal()`。对象切换、失权、拒绝、超时或销毁会取消，迟到ACK不能发到新云台。

#### 8.4.5 当前验证状态与真机验收

`UniRcProtocolTest`只覆盖与传输无关的精确请求、CRC、严格回包门禁、16路解析、分片/粘包/重同步及CH9/CH10纯状态机，包括缺省/反向方向与反向模式重新arm；它不能验证Fact信号槽、16路QML刷新、Android权限、系统配对、按MAC直连、SPP UUID、固件路由、A8 UDP或MAVLink ACK。合并前必须完成Qt 6.8.3 Desktop及Android arm64完整构建，并按以下顺序真机验收：

2026-09-01目标遥控器真机已完成核心功能闭环。`unirc-20260901-103053.tar.gz`记录同一Bluetooth attempt依次到达 `pairing-confirmed -> connected -> request-transmitted -> first-rx -> sdk-frame-valid -> stream-active`；QGC向 `BLUE-A201156781 [41:42:9E:3D:A5:D2]`写出三份0x42请求共33/33字节，随后收到一帧11字节合法确认和689帧42字节通道数据，`RX=28949=11+689×42`，持续频率约19.7 Hz，期间没有SDK超时、解析失败或通道越界。恢复正常飞控和图传通信后，CH9拨轮的缩小、放大与回中停止以及CH10按键的云台回中均已通过物理动作验证。
...
7. CH10每次释放到按下只产生一个共享回中事务；无Vehicle/activeGimbal不发送。存在PX4/GPS2/A8链时同时记录MAVLink命令1000 ACK、PX4下游输出和最终物理回中。
8. 关闭Bluetooth、断开BLUE、停用SDK流、切后台、关闭开关、修改地址和退出，确认350 ms断流撤权、通道网格不把旧值冒充实时输入、无残留镜头运动、旧socket信号不影响新连接、正常关闭停止请求与1秒有界释放符合预期。
9. 顶部Center与CH10交替、RC夺权、多Vehicle/多云台、按键长按和快速重复时，共享协调器不得并发、串目标或绕过8.3.3控制权门禁。不要同时建立QGC原生Bluetooth MAVLink Link连接同一个BLUE设备，以免两个RFCOMM客户端竞争同一SPP服务。
10. Ubuntu主机通过USB连接遥控器后，一键抓取必须先保存 `getprop/dumpsys package/appops/dumpsys bluetooth_manager`前置快照，再清空all buffer并冷启动 `org.mavlink.qgroundcontrol`；用户只需等待按缺省MAC自动连接，再完成CH9两方向/回中和CH10两次按下后按Enter。脚本保存后置快照、完整logcat、UniRC事件时间线、Bluetooth系统过滤日志、权限摘要并打包 `unirc_bt_YYYYmmdd_HHMMSS.tar.gz`及SHA-256。分析时以同一attempt内 `pairing-confirmed -> connect-start -> connected -> request-transmitted -> first-rx -> sdk-frame-valid -> stream-active`的最远到达点定位，不跨attempt拼接证据。
...
28. 顶部条必须通过门禁与坐标系矩阵：开关打开但无Vehicle、无activeGimbal、`vehicleLinkManager.communicationLost=true`或Provider方位角无效时隐藏。显式Earth frame直接核对q的yaw；显式Vehicle frame分别核对有效 `delta_yaw`（包括已确认支持后的0值）优先和缺失/NaN时heading回退；在尚未收到真实飞控heading前不得把Fact初始0值用于回退；无frame位分别核对legacy锁定/跟随且确认delta被忽略；两个frame位同时设置必须拒绝。航向锁定必须做三类上报序列：q实际保持世界方位而Standard/Heading随基座变化时，应选择 `YawLockReportedYawCompatibility`；q按机体系反向变化但已支持的delta冻结为非零或错误0、Standard/Reported一起变化时，应选择 `YawLockVehicleHeadingCompatibility`；合规设备Standard保持世界方位、Reported变化且Heading与Standard可能并列时，应保留 `DeltaYaw`或 `VehicleHeadingFallback`。兼容候选必须同时以至少8°运动和至少5°优势击败其余每个有效候选，首个Lock样本仍须为Standard/Undecided。测试另覆盖有/无Follow历史、2°锁定漂移、flag/q错序、切锁同时yaw指令、全部候选共同运动才重建锚、三向反向纠正、heading出现/过期、标准来源改变、单份非法姿态恢复及±180°。锁内拨yaw摇杆后方位必须继续变化，不能冻结。目标真机应先锁在非零世界方位，再转动飞控/云台基座且不拨yaw摇杆；通常至少转动约8°并持续到某一兼容候选对其余各路均取得5°优势，进入上述两个兼容来源之一后，物理朝向不变时罗盘和顶部Az应基本不变。跟随测试保持云台相对机头角不变并转动飞控，两处世界方位应同步变化。多云台环境从顶部MAVLink栏切换activeGimbal，方位角必须优先按精确Vehicle/manager/device身份切换，且各路Resolver选择不得串用；右侧A8/MT11相机选择、私有SDK断网、视频工作模式及RTSP有无不得改变数据源。保持飞控链路正常而单独停止消息285时，两处应在2秒门槛后的下一次250毫秒检查内失效，恢复消息后的Resolver不得复用超时前选择；VehicleHeading兼容候选的飞控heading超过2秒未更新时必须退出该来源。整机链路丢失、`time_boot_ms`从较大值回退到0或其他超过1秒的回退后也必须等待本次会话的新消息285；依赖heading的路径还必须等待本次会话的新heading遥测。真机日志应保存flags、protocol/reference、reported q yaw、heading candidate与最终azimuth，确认不是选错manager/device路由。
...
35. UniRC 10 Pro验收必须先在UniGCS把SDK分配到Bluetooth，保持已通过实测的“数传1=UDP、数传2=关闭、SDK=蓝牙”产品基线并核对CH9/CH10映射；Android系统须开启Bluetooth、授予Nearby devices权限并预先配对BLUE设备。净安装/清除设置后确认 `UniRC SDK`缺省开启、接口只有缺省Bluetooth、MAC缺省 `41:42:9E:3D:A5:D2`且自动直连；页面没有扫描候选、备注或调试摘要，CH1～CH16在窄/宽布局按两/三列更新且断流不冒充实时值。按第8.4节覆盖无效MAC、未配对、连接/认证/服务失败、10秒连接超时、三份0x42请求、零RX、有字节无合法帧、正常20 Hz、350 ms停流、前后台、关闭开关和修改地址；确认旧socket信号不污染重连，完整分类仍可由后台日志定位。CH9覆盖启动偏转不动作、首次回中arm、1475/1525边界、正反方向、回中停止、失联停止和触控hold竞争；CH10覆盖启动按下不动作、首次释放arm、长按只一次、释放后再按、无Vehicle/无Gimbal、RC夺权和共享顶部Center。A8侧同时抓取0x05 UDP，PX4侧同时记录命令1000 ACK与GPS2 UART，最终以合法 `stream-active`和物理缩放/回中闭环为准。完成Qt 6桌面测试时先运行 `check_unirc_protocol`；在Android真机完成上述矩阵前，不得把“Bluetooth已连接”、请求进入Qt队列、设置页出现通道值或MAVLink请求已受理单独描述为功能验收通过。
...
ctest --test-dir <desktop-build>/custom -R '^(SiyiProtocolTest|Mt11ProtocolTest|GimbalMediaSessionPolicyTest|GimbalPhotoCapturePolicyTest|GimbalAzimuthPolicyTest|UniRcProtocolTest|AndroidH265DecoderRoutePolicyTest)$' --output-on-failure
```
...
截至2026-09-04，针对“偏航锁定时云台物理朝向不变、顶部方位仍随飞控/基座转动”的真机复测完成第二轮代码修正，改动仍只位于 `custom` 与本说明，未修改 `src`。代码级根因是上一版在显式Vehicle消息下无条件相信标准 `delta_yaw`/heading转换，而目标桥接器的frame/delta声明与锁定运动关系不一致：q可能实际已按地系运动，也可能仍按机体系反向运动但已被判定支持的delta冻结/错误；后一种情况下Standard与直接q会一起变化，原二候选方案仍无法修正。为避免破坏合规设备，本轮保持 `GimbalAzimuthPolicy`标准公式不变，新增按Vehicle/source/device隔离的 `GimbalYawLockResolver`，同时比较Standard、ReportedYaw和使用2秒内真实飞控航向的VehicleHeading。首个Lock样本使用Standard建三路锚；兼容候选只有同时以至少8°运动、至少5°优势击败其余各路才夺权；所有有效候选共同运动3°才重建锚；已选来源持续三向复核；标准来源变化时首包回Standard，VehicleHeading过期时立即退出该来源，单份非法姿态不丢失已学习选择。两个兼容结果分别标记 `YawLockReportedYawCompatibility` 和 `YawLockVehicleHeadingCompatibility`。解锁、2秒消息间隔、设备启动时间回退和断链后不复用选择；Provider的重启判断也修正了较大 `time_boot_ms`直接回到合法0时漏判的问题，并记录标准、直接q、heading候选和最终来源。当前源码使用MSVC + QtTest 5.14兼容harness重新编译运行50 passed、0 failed、0 skipped，覆盖错误frame、冻结非零/错误零delta、合规body q、三向纠正、heading过期、有/无Follow历史、2°锁定漂移、锁内摇杆、flag/q错序、切锁同时yaw指令、±180°、reset及0 ms重启首包；Policy、Resolver及测试源码在 `/W4` 下无自身警告（仅Qt 5.14测试头保留既有C4127），`git diff --check`通过。该结果尚不替代Qt 6.8.3完整工程/Android构建和目标真机新APK复测；复测时通常至少需要在不拨yaw摇杆的情况下转动基座约8°，并持续到某一兼容候选对其余各路均取得5°优势，确认日志进入上述两个兼容来源之一。

截至2026-09-03，本轮云台方位角修正只改动 `custom` 与本说明，未修改 `src`。`CustomPlugin`只读观察MAVLink消息并把每个Vehicle/manager/device的消息285姿态交给 `GimbalAzimuthProvider`；`GimbalAzimuthPolicy`按显式frame位、`delta_yaw`和legacy `YAW_LOCK`优先级输出世界方位，顶部工具栏Az与顶部罗盘共用结果。Provider要求真实heading遥测后才允许兼容回退，精确manager/device路由优先，姿态超过2秒或整机链路丢失即失效。罗盘的 `secondaryDegrees`、`secondaryPrefix`、`REL` QML及英中翻译条目均已删除。纯策略使用MSVC + QtTest 5.14兼容harness重新编译运行27 passed、0 failed、0 skipped，包含有效零delta、非有限边界、锁定时基座转动方位保持和跟随时方位随基座转动回归；Policy本体以 `/W4`编译无警告。英中TS删除REL后均为21个context、184条message、175个唯一source、183个唯一context/source pair，pair集合一致；英文184条保持unfinished模板，中文184条均finished，两份Qt 5.14 `lrelease`均exit 0。完整Qt 6工程/Android构建、QML运行加载、真实设备flags/delta上报、多云台路由、2秒超时和航向锁定物理稳定性仍须在目标环境验收。
