...
- **代码已集成，待目标遥控器真机回归验收**：本轮A8 Mini缩放即时单击/原生连续长按状态机、SD卡与本地媒体双支路、UniPod MT11私有SDK与变焦/热成像/变焦+热成像拼接三种工作模式、通用独立第二路RTSP、Map/Video 1/Video 2三视图、统一视觉的双相机右栏、底部飞行器航向与顶部云台指向双罗盘条、Android 86%界面缩放缺省值、Android H.264/H.265厂商MediaCodec强制策略和Android USB串口管理器已经进入当前工作树。Android第二路显示已补入当前上游QGC采用的启动顺序：先把带QML Item的video sink加入receiver管线并置PAUSED，再创建/同步decoder，decoder动态src pad只负责连接已存在的sink；这消除了第二个MediaCodec先协商、第二个 `qml6glsink` 后动态加入的已知黑屏风险。硬解开关默认开启时，H.264/H.265限定为兼容的厂商MediaCodec候选；全局H.265 rank继续让A8既有 `hvc1 -> Annex-B/AU` adapter硬解优先。每个新的receiver/URI组合首代都走adapter。watchdog入口仍要求同代H.265 source媒体已到、adapter已实例化，并在没有decoder输出和sink帧时超时；另有严格的decoder分支bus入口：同代 `!rtspSourceError && decoderBranchError`、已识别H.265、adapter已选且无decoder/sink帧时，允许在core stop前准备direct，即使source首buffer尚未记录，以覆盖MediaCodec在CAPS/CSD阶段的not-negotiated/Failed configure。direct选择对同一URI保持sticky，URI改变即复位；仅缺source且无上述decoder分支证据、启动失败、RTSP/source错误、非decoder分支或sink分支bus错误，以及已有decoder输出但sink无帧都不会切换。所有重建均不改变进程级rank，也不会转到 `avdec_h265`。双路是否能各自创建并持续驱动独立MediaCodec实例且分别到达sink首帧，仍须按第12章在目标设备验证，不能仅凭代码审查、候选rank、adapter注册或路由切换日志判定整机验收通过。
...
- **本轮 Android MT11 黑屏逐流硬解修正与安全边界**：MT11地址换到URL 1仍无画面，已排除Video 2专用QML/PIP接线作为共同根因；A8与MT11实际都是H.265，不能用全局direct-first破坏A8已能显示的adapter链。现继续保持 `forceAndroidH265HardwareDecoder=true`、adapter `PRIMARY+100=356` 优先、direct-hvc1厂商硬解 `PRIMARY+3=259` 次级以及所有软件候选为NONE；在此基础上增加按receiver/URI冻结的硬解路由。每个新URI首代仍由全局rank选择adapter。第一条切换入口是H.265真实source已到、实际decoder为adapter、首帧watchdog超时且decoder/sink都没有输出；第二条只接受同代 `!rtspSourceError && decoderBranchError`、已确认H.265和adapter且decoder/sink均无帧的decoder分支bus错误，可在source首buffer尚未记录时于core stop前准备direct，用于覆盖MediaCodec在CAPS/CSD阶段的not-negotiated/Failed configure。两条入口都只让下一完整generation显式创建策略预先筛选的direct-hvc1厂商MediaCodec。同URI后续重建保持该direct路由，URI改变时清除；仅缺source而没有上述decoder分支证据、同步start/startDecoding失败、RTSP/source错误、非decoder分支或sink分支bus错误，以及decoder已有输出但sink无帧都只按原路完整重建，不切换decoder。该判定依据本路事实而非Video 1/2槽位、URL后缀、IP或分辨率，不改变另一路。当前只是代码静态修正，尚无新APK与目标遥控器四阶段日志，不能宣称MT11 Android已经显示或双路已经验收通过。若新APK仍无画面，继续按无source且无decoder分支错误查Android图传路由/RTSP/RTP/depay/parsebin；若出现adapter decoder分支错误或source后watchdog日志，则核对下一代explicit factory；有decoder输出但无sink帧继续查GL/surface。应用设置－视频页面的五个顶层组同时保留50个默认字体字符的preferredWidth，并以同值maximumWidth真正封顶、`Qt.AlignHCenter`居中；长硬解说明只在组内换行，不能再通过implicitWidth把页面撑满。
- **Android MT11旧版黑屏的提交级程序根因**：`7ef4bbff0` 中direct-hvc1为 `PRIMARY+3=259`、adapter为 `PRIMARY+2=258`，direct可先参与；`6ab1e2445` 为恢复A8 Mini既有兼容链把adapter提升到 `PRIMARY+100=356`，`b2239ed65`继续继承356/259。由此旧版每个H.265 decodebin都会先实例化adapter；若MT11真实码流已经进入adapter但其内部没有产出raw帧，decodebin不会把“运行期无输出”当作autoplug创建失败再去尝试rank 259的direct候选。旧首帧watchdog又只以相同全局rank完整重建，下一代仍选择同一adapter，因而形成MT11无限重走adapter、direct永远没有机会的代码级闭环。这是已由提交和控制流确认的程序原因；MT11码流的CSD/profile/封装究竟在adapter哪一层失配，及显式direct是否能在目标遥控器真实出帧，仍必须用新APK四阶段日志确认，不能伪称direct已真机通过。
...
- 开关开启时，兼容adapter以 `GST_RANK_PRIMARY + 100=356` 注册并优先接收播放支路的 `hvc1`，通过GStreamer `h265parse`转换为 `video/x-h265,stream-format=byte-stream,alignment=au` 后送入厂商MediaCodec；direct-hvc1厂商MediaCodec固定为 `GST_RANK_PRIMARY + 3=259` 并保留为显式次级硬解路径。每个新receiver/URI首代仍按全局rank进入adapter；H.265 source已到、adapter已实例化、首帧watchdog超时且decoder/sink均无帧时，下一完整generation显式创建direct factory。另仅当同代非RTSP-source错误而确认为decoder分支bus错误、已识别H.265与adapter且decoder/sink均无帧时，也可在core stop前准备同一下一代direct；该入口允许source首buffer尚未记录，以覆盖MediaCodec处理CAPS/CSD时提前失败。同URI保持direct直到URI改变，且全程不改全局rank。READY预检不输入真实码流，不能验证CSD、profile/level、Surface或第二个MediaCodec实例资源；实际选择与输出仍须用四阶段日志确认。
- 同一编码下所有通过筛选的direct厂商候选仍统一为259，因此自动插拔时原有rank差异不再决定先后；策略会在覆盖rank之前按“原始rank降序、稳定factory名升序”缓存首个兼容vendor androidmedia direct-hvc1 factory，供逐receiver显式重试查询。没有该factory时不会切direct，仍按adapter路径重建。这不会引入软件解码，但必须通过 `Decoder element instantiated` 记录实际factory；不能仅凭候选数量或缓存日志判断播放成功。
...
- 通用receiver记录每个实际decoder的选择及其src首个buffer，字段包括receiver、URI、generation、选择方式、plugin/factory、instance和caps；显式direct和decodebin自动插拔使用同一诊断路径。还需看到对应sink首帧，才能证明该receiver从实际decoder输出走通到显示端。候选rank、显式路由属性或adapter的 `vendor MediaCodec candidate` 首帧字样都不是系统级硬件确认；必要时仍以Android API 29 `isHardwareAccelerated()`核实硬件属性。
...
- 通用receiver现为每个真正启动的pipeline分配单调generation，以低频信号报告CAPS协商出的实际codec、真实source buffer、实际decoder plugin/factory、decoder首输出、sink首帧及bus error前的一致快照。所有诊断都携带generation，同URI旧代迟到事件不能污染新代；core只提供事实与“按本代冻结的factory显式创建H.265 decoder”通用钩子，不包含A8/MT11判断。Video 1由 `AndroidVideoDecoderRecovery` 消费，Video 2由 `DualVideoManager` 消费。Timer仅从真实source首帧启动，超时为 `max(rtspTimeout, 8秒) + 3秒`；`streaming=true`会重新检查Timer，覆盖source首帧先于streaming确认的时序。watchdog入口同时要求H.265、source已到、adapter已选且decoder/sink均无帧；bus入口则只接受同代 `!rtspSourceError && decoderBranchError`、H.265、adapter已选且decoder/sink均无帧，可早于source首buffer。两者都由custom helper在stop前为同receiver/URI写入direct factory，下一代由core显式创建；同URIsticky，URI改变复位。同步启动失败、RTSP/source错误、非decoder分支或sink分支bus错误、仅缺source且没有decoder分支证据，以及decoder已有输出但sink无帧都不写该路由，只按原路径完整重建。任何路径都不改rank或使用软件decoder，健康另一路不受影响。
...
- 二次开发主体位于 `custom`，目录和命名参照 `src` 模块树；当前共 156 个文件。
...
- custom 翻译加载、简体中文目录和 `lupdate` 更新脚本已经接入；视频层文案保留通用 Video 1/Video 2、第二路独立receiver/留空禁用提示，以及沿用旧Fact键但显示为Android H.264/H.265必须硬解的开关。两路RTSP-over-TCP开关及其元数据说明已经从英文模板和简体中文目录同时删除；硬解说明现明确写出新安装默认开启、每个新H.265 receiver/URI首代adapter，以及两条唯一切换入口：source已到的adapter首帧watchdog超时，或同代非RTSP-source且确属adapter decoder分支、H.265已识别、decoder/sink均无帧的bus错误；后一种允许早于source首buffer。direct同URIsticky而URI变化复位，其他启动/source/bus/sink失败不切换并绝不使用 `avdec_h265`。视频设置页的五个顶层组保留50字符preferredWidth，同时以相同maximumWidth封顶并用 `Qt.AlignHCenter`居中；maximumWidth才是阻止长说明通过implicitWidth把页面撑大的硬约束。两份TS的最终统计与完成状态见第4.12节。
...
- Android前台运行时直接打开UniRC SDK Serial 2字符设备，默认路径 `/dev/ttyHS0`、PDF明确波特率115200；8N1是当前实现采用的常规串口格式，PDF该处没有明确数据位/校验位/停止位，仍需真机确认。非Android构建不启动该控制器；Android也不按未验证的 `Build.MODEL` 硬编码开启，用户必须显式打开默认关闭的功能开关。设置页允许即时关闭功能或调整设备路径，并显示串口打开、等待数据、CH9/CH10实时值及错误状态。
- 0x42请求使用 `55 66`帧头、control 0x01、小端长度/sequence、CRC16/XMODEM（poly 0x1021、init 0、CRC小端）；启用20 Hz与关闭输出都严格连续发送三份。周期回包只接受control 0、command 0x42和精确32字节payload，并按小端有符号16位解析CH1～CH16。
...
| `custom/src/Android/UniRcChannelController.cc` | 仅在Android前台且用户显式开启开关时以POSIX `open()`打开默认 `/dev/ttyHS0`，使用termios raw 115200及当前实现假设的8N1、`QSocketNotifier`异步读和 `flock/TIOCEXCL`尽力阻止并发占用；PDF明确115200但未在该项明确8N1，后者仍须真机确认。打开后把20 Hz请求完整帧连续写三份，关闭时同样尽力写三份停止请求；首帧等待1.2秒，活动流watchdog到350 ms时先排空内核中因GUI事件循环延迟而尚未处理的数据，仍无合法0x42才取消UniRC缩放、关串口并3秒后重连。合法CH9变化调用A8 Manager的UniRC专用0x05入口，CH10按下沿调用共享回中协调器；应用后台、禁用、路径改变和退出都会停止输入并关闭fd。它故意不走Android QGC的USB-only `QSerialPort/QGCUsbSerialManager`链：`/dev/ttyHS0` 是SoC内置字符设备，不是Android USB Host枚举出的usb-serial-for-android端口。 |
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义13个Gimbal Fact；新增安全默认关闭的 `uniRcChannelControlEnabled=false` 和 `uniRcSdkSerialPort=/dev/ttyHS0`，前者必须由目标遥控器用户显式开启且不按未验证的Android型号硬编码，后者只表示内置SDK字符设备，不是USB端口。独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，只决定tap目标和长按期间的目标参考网格，不参与原生长按物理速度。longDesc明确短按精确30x端点、0x05没有rate字段、hold-only也使用显式420 ms且短按零命令、同按压100 ms启动重试、所有hold整次按住450 ms方向保活、活动hold不因0x18/端点、反馈失效、SDK静默或单次写失败提前结束，以及严格端点资格只允许普通release省略停止、cancel/生命周期仍强制停止；不改变设备上限或RTSP URL。 |
...
| `custom/src/VideoManager/DualVideoManager.cc` | 监听通用 `[Video]/secondaryRtspUrl` 与原生Video Source/URL/启用状态，为实际 `secondaryVideoContent` Item创建独立receiver和sink；GL Item先经过 `BeforeSynchronizingStage` 与 `itemInitialized` 门禁。它独立维护start/stop、1～15秒封顶且持续的退避重试、URL换代、pause、PIP/surface销毁重建及主receiver starting/active/releasing重复源保护；被动主路通知不能绕过既有deadline。每次 `videoPipelineGenerationStarted(uri,generation)` 都清空上代诊断事实，后续source/decoder/sink/bus通知必须同时匹配冻结URI和generation，因此同一URL重启时的迟到信号也不会污染新管线。tee CAPS、source buffer、decoder选择/输出和sink首帧形成独立里程碑。只有真实source到达后才启动 `max(rtspTimeout,8秒)+3秒` watchdog；若同代为H.265、adapter已实例化且decoder/sink均无帧，超时会在完整stop前调用逐receiver fallback helper。另在同代bus回调中，只有 `!rtspSourceError && decoderBranchError`、H.265、adapter已选且decoder/sink均无帧时，才可在core stop前准备下一代direct；该路径可早于source首buffer。同步start失败、RTSP/source错误、非decoder分支或sink分支bus错误、仅缺source且无上述证据，以及decoder已有输出但sink无帧都不切路，只按封顶退避持续重建。同URI保持direct，URL改变即复位；全程不改rank、不使用软件decoder，也不改变主路。非GStreamer后端继续使用原有10～30秒watchdog且不执行该硬解路由。相机SDK在线不参与RTSP判定，URL为空/重复时不启流，cleanup与Item换代仍按receiver先销毁、sink后释放，退出阶段不重建。 |
...
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidH265HardwareDecoderAdapter.cc` | 在启动阶段枚举实际属于 `androidmedia` 插件、能接受 `video/x-h265,stream-format=byte-stream,alignment=au` 且通过厂商名称筛选的MediaCodec候选，排除secure、software/FFmpeg、`*.sw.dec`、Qualcomm `*swvdec`、OMX Google及C2 Android/Google/Goldfish，优先名称含low-latency变体的组件，再按原rank/名称排序；预检只证明元素可创建、静态链可链接且bin能进READY，不证明真实profile/level已解码。选中后缓存厂商factory并以rank `PRIMARY+100=356` 注册 `qgcandroidh265hwdec`，高于固定为 `PRIMARY+3=259` 的direct-hvc1厂商候选，以保持A8 Mini既有H.265兼容路径。每个播放管线都会创建独立adapter实例，实例各自拥有parser、capsfilter、厂商MediaCodec element和downstream-leaky raw queue（最多2 buffer）；共享项仅为启动时选定后只读的factory名称、类型注册和rank，不共享decoder element、buffer或逐流状态。单实例内部为：外部hvc1 ghost sink -> `h265parse(config-interval=-1)` -> Annex-B byte-stream/AU capsfilter -> 厂商MediaCodec -> raw queue -> `video/x-raw(ANY)` ghost src；probe记录真实输入caps和首个raw buffer的caps/PTS/bytes/GLMemory，首帧使用 `vendor MediaCodec candidate` 术语。用户确认A8在Android端实际为H.265且可显示，但具体运行factory及完整四阶段仍以同代日志为准。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidH265DecoderFallback.h` | 声明逐receiver的Android H.265硬解fallback：幂等安装URI复位监听，并在调用方提供本代URI/generation、实际codec、adapter选择及source/decoder/sink事实时判断是否为下一完整代准备direct。它不持有GStreamer element、不修改rank，也不提供软件解码路径。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidH265DecoderFallback.cc` | 使用receiver动态属性保存当前路由URI和可选显式decoder factory。新receiver/URI属性为空，首代仍按全局rank走adapter。`prepareDirectRetry()`公共门禁始终要求H.265、adapter已实例化、decoder/sink均无帧且存在兼容direct factory；其证据入口可以是source首buffer已到，也可以是owner已确认同代decoder分支bus失败。watchdog只传source证据；bus入口还必须由owner先证明 `!rtspSourceError && decoderBranchError`，因此允许MediaCodec在CAPS/CSD阶段、source首buffer前失败，但不会把RTSP/source、非decoder分支或sink错误误判为adapter失败。写入值在对应receiver同URI后续generation中保持sticky，`uriChanged`时清空；同步start失败、decoder已有输出或sink已有帧均返回false。写入发生在完整stop前的receiver Qt线程，运行中的pipeline不被修改。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidVideoDecoderPolicy.h` | 声明进程级 `apply(forceHardwareDecoding)` 启动入口及 `preferredDirectHvc1DecoderFactoryName()` 查询。策略在 `VideoManager`完成 `GStreamer::initialize()` 之后、创建receiver之前应用；getter只返回启动期冻结的兼容vendor androidmedia direct-hvc1 factory名，不执行运行期切换。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidVideoDecoderPolicy.cc` | `CustomPlugin::init()` 读取 `forceAndroidH265HardwareDecoder`；false时清空首选direct factory、不注册adapter、不改原生rank，交回QGC/GStreamer自动选择，仅用于诊断。true时在改rank前按原始rank降序、稳定factory名升序缓存首个兼容direct-hvc1厂商factory，再把接受H.264 `avc`/H.265 `hvc1` 的厂商direct候选固定为 `PRIMARY+3=259`，把可用H.265 adapter注册为优先硬解rank `PRIMARY+100=356`，并无条件将同codec的非兼容候选降为NONE。找不到硬件路径时critical并明确失败，不保留软件rank。启动后全局rank固定；getter只供逐receiver显式硬件重试，不会提升或返回 `avdec_h265`。非Android/非GStreamer构建返回空。 |
...
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidVideoDecoderRecovery.cc` | 仅Android+GStreamer由 `CustomPlugin::createVideoSink()` 为Video 1幂等安装，并同时安装逐receiver fallback helper。它只消费与当前URI/generation匹配的实际codec、source、decoder选择/输出、sink和结构化bus事实；只有start与startDecoding受理、streaming成立且真实source已到，才以 `max(rtspTimeout,8秒)+3秒` 等待首个sink帧。若超时时本代为H.265、adapter已实例化且decoder/sink均无帧，则在完整stop前为同receiver/URI准备下一代显式direct。bus error会先停Timer；仅同代 `!rtspSourceError && decoderBranchError`、H.265、adapter已选且decoder/sink均无帧时在core stop前准备direct，即使source首buffer尚未记录，随后立即标记stopping以避免与watchdog双重stop。同步start/startDecoding失败、其他bus/source/sink故障及decoder已有输出但sink无帧都不切换。原生VideoManager保留desired-running并在 `onStopComplete` 后按既有退避启动下一代；桌面端无动作。 |

本轮对既有 `DualVideoManager.cc` 的扩展集中在generation诊断消费和逐流首帧恢复：Video 2精确绑定每代URI/generation并安装同一个fallback helper。第一入口是“H.265 source已到、adapter已选、watchdog超时且decoder/sink均无帧”；第二入口是同代 `!rtspSourceError && decoderBranchError`、H.265、adapter已选且decoder/sink均无帧，后者可在source首buffer前为下一代准备本receiver/URI的direct factory。bus回调在准备后立即进入stopping并等待core stop；start失败、RTSP/source错误、非decoder分支或sink分支错误、仅缺source且无第二入口证据，以及decoder已有输出但sink无帧仍按原路径重建。所有失败都只由本路owner按封顶退避持续stop/restart，不调用进程级rank调整，因而不会让一路故障改变另一路decoder候选或路由。原有receiver所有权、PIP/surface和重复URL判定均不改。

Android选择链为：`CustomPlugin::init()`读取重启后生效的兼容Fact；新安装缺省true，由 `AndroidVideoDecoderPolicy` 将H.264/H.265限定为兼容厂商MediaCodec，H.265 adapter维持 `PRIMARY+100=356`、direct-hvc1候选维持 `PRIMARY+3=259`、其他对应codec候选均为NONE，并在覆盖rank前缓存确定的direct factory -> 新receiver/URI没有显式factory，先按全局rank进入adapter -> 每路以URI+generation记录codec、source、实际decoder、decoder输出、sink帧及bus来源。source watchdog入口要求H.265、source已到、adapter已实例化且decoder/sink均无帧；decoder-bus入口要求同代 `!rtspSourceError && decoderBranchError`、H.265、adapter已选且decoder/sink均无帧，可在source首buffer前触发 -> custom在stop前把direct factory冻结到该receiver -> 下一代core优先使用parsebin动态pad current/query CAPS；显式factory接受全部structure均为 `video/x-h265` 且可与 `hvc1` 相交的非ANY CAPS，late valve ANY/EMPTY只有本代tee已确认H.265才放行 -> 同URI后续重建保持direct，URI变化清空并重新从adapter开始。同步start失败、其他bus/source/sink故障、仅缺source且无decoder分支证据，以及decoder已有输出但sink无帧均不切换。所有路径都完整重建而不改rank，不会影响健康另一路，也绝不提升 `avdec_h265`。关闭Fact才恢复原生自动选择，且只用于诊断。rank、READY、路由准备日志或factory创建都不能替代四阶段里程碑与实际画面。

为避免离线相机持续重试时淹没Application Messages，Video 2的逐次启动、成功回调、surface、streaming/decoding、尺寸、handoff和退避过程日志，以及主路observer安装日志均已删除；失败回调、首帧watchdog超时和“为下一代准备direct”仍可见。core `GstVideoReceiver` 保留四个低频成功里程碑：source媒体首帧、实际decoder实例、该decoder首个输出buffer和sink首帧。自动插拔与显式direct都沿用 `Decoder element instantiated` 短语并带receiver/URI/generation/factory/instance；只有显式direct额外带 `selection receiver-specific explicit factory`，普通decodebin日志不增加selection字段。旧日志中的 `STATUS_MIN/STATUS_OK=0`只代表异步请求受理，本轮不再把该过程值当作播放证据。
...
| `custom/translations/custom.ts` | 英文源字符串模板。保留既有云台、双视频与双罗盘source，并同步加入UniRC设置状态、串口/依赖错误、zoom ownership和两个Fact说明；当前21个context、179条message和170个唯一source，179条均unfinished。Qt 5.14 `lrelease`验证exit 0并按模板语义忽略179条未翻译source；仍需项目Qt 6构建生成正式QM。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录，与英文模板保持相同的21个context、179条message和170个唯一source，context/source pair集合一致；179条均finished，unfinished和空译文为0。新增翻译覆盖UniRC CH9/CH10设置、实时状态、POSIX串口/依赖错误、拨轮回中提示和Fact元数据；Qt 5.14 `lrelease`实际生成179条译文并exit 0。翻译完成不代表完整QGC、Android串口或真机云台已验收。 |
...
当前分支 `SecDev/ft/control` 的二次开发业务仍位于 `custom`。`src` 差异登记仍为以下十个文件：前两个是custom PX4模块正常链接所需的受控例外，中间两个为所有原生VideoReceiver提供通用串行生命周期与RTSP退避，随后四个提供通用启动URI快照、OPTIONS请求兼容、teardown诊断、无媒体恢复与逐代显式H.265 factory创建钩子，另一个修正通用日志级别过滤，最后一个修正模拟相机对VideoManager通知信号的连接。GStreamer/GIO默认直连及Android逐receiver fallback策略都位于 `custom`；本轮只扩展已登记的 `GstVideoReceiver.h/.cc`，没有新增 `src` 例外文件，也没有把A8/MT11身份或切换条件写入core：
...
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.h` | 保存上次RTSP URI、同URI的OPTIONS兼容状态、active pipeline URI、stop代次、teardown/method和错误去重状态；新增单调generation counter、当前active generation、tee CAPS得到的实际codec，以及逐代复位的source首buffer、decoder首输出、sink首帧原子事实。另以互斥量保护当前decoder root引用和已选择decoder的plugin/factory；保留原decodebin诊断回调，仅新增显式decoder观察和复用输出probe的辅助函数。`_start()` 接收已经冻结的可选显式H.265 factory，`_makeDecoder(caps,sink)`据实际CAPS决定显式创建或沿用decodebin。它不实现rank调整、URI sticky或产品恢复策略。仍声明sink先于decoder进入管线的通用 `_ensureVideoSinkInPipeline()`。 |
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.cc` | GUI start先分配单调generation，并冻结URI、low-latency与可选动态属性 `customAndroidH265DecoderFactory`，再交给私有worker；只有worker确认当前没有活动管线且冻结URL非空时，才把generation设为active。URI/generation/显式factory上下文绑定到本代pipeline，旧代回调不能冒充当前管线。parsebin动态pad建decoder时先取pad current CAPS、没有则query，并把该hint直接传给decoder分支；late `startDecoding()`没有动态pad时才退回valve current/query。显式factory接受非ANY/EMPTY、所有structure均为 `video/x-h265` 且整体能与 `hvc1` 相交的CAPS，无需完全fixed；若late valve只给ANY/EMPTY，则必须由本代tee CAPS里程碑已确认H.265才放行。非H.265、混合媒体CAPS或没有本代H.265证据的ANY/EMPTY均明确拒绝，未设置factory时保持原生 `decodebin3`。普通decodebin继续由既有deep-element-added回调记录 `Decoder element instantiated`，不增加selection字段；显式root由新增辅助函数记录同一短语并额外写入 `selection receiver-specific explicit factory`，两者都安装decoder输出probe。core还把同代bus错误标记为 `rtspSourceError` 与“消息源为decoder root或其后代”的 `decoderBranchError`，随codec/decoder/source/输出/sink快照先发给owner，再停止本代pipeline；RTSP Auto、OPTIONS兼容、teardown及owner退避不变。core不解释动态属性来源，不决定何时写入或清除，不引用custom policy、不改rank，也不识别MT11/A8。 |
...
| `forceAndroidH265HardwareDecoder` | bool / `true` | 兼容保留的旧QSettings键，仅Android生效；UI显示为“Android H.264/H.265必须使用硬件解码”。新安装缺省开启，将对应codec限定为经过筛选的厂商androidmedia decoder；H.265全局rank保持adapter `PRIMARY+100` 优先、direct-hvc1 `PRIMARY+3` 次级，其他候选为NONE。每个新receiver/URI首代使用adapter。下一代只在两种证据之一成立时锁定同receiver/URI的direct factory：H.265 source已到、adapter已实例化且首帧watchdog超时时仍无decoder/sink帧；或同代非RTSP-source且确属decoder分支bus错误、H.265与adapter已识别且decoder/sink均无帧，后一种可早于source首buffer。URI改变复位；start失败、其他bus/source/sink故障、仅缺source且无decoder分支证据，以及decoder已有输出但sink无帧均不切换。没有硬件路径时critical失败，任何路径都不使用 `avdec_h265`。关闭仅用于诊断，恢复原生自动选择。修改后必须重启QGC；已有安装保存值继续优先于metadata缺省。双H.265/双MediaCodec仍须目标遥控器新APK验证。 |
...
- 若设备厂商H.265解码器本身直接接受 `hvc1`，该direct候选rank固定为 `GST_RANK_PRIMARY + 3=259`。策略在统一rank之前按原始rank降序、稳定factory名升序保存首个兼容direct factory；它不改变adapter 356的全局优先级，只为后续逐receiver显式硬件重试提供确定名称。
- 每个新receiver/URI没有显式factory，首代仍由 `decodebin3` 按全局rank选择adapter。watchdog入口要求本代已收到H.265 source buffer、实际选择 `qgcandroidh265hwdec`、首帧watchdog超时且decoder/sink均无帧；bus入口要求同代 `!rtspSourceError && decoderBranchError`、已识别H.265与adapter且decoder/sink均无帧，可在source首buffer前覆盖MediaCodec处理CAPS/CSD时的not-negotiated/Failed configure。任一入口成立后，custom才在完整stop前把上述direct factory写入该receiver；下一代由通用core在hvc1兼容H.265 CAPS或本代tee H.265证据下显式创建。该选择对同一receiver/URI保持sticky，URI改变清空并重新从adapter开始。
- 同步start/startDecoding失败、RTSP/source错误、非decoder分支或sink分支bus error、仅缺source且没有已确认decoder分支失败、decoder已有输出但sink无帧、实际codec不是H.265、实际decoder不是adapter或没有兼容direct factory，均不得写入显式路由，只按当前路径完整重建。适配器和direct都不包含软件解码；非兼容候选始终为NONE，绝不自动切换 `avdec_h265`。关闭开关才恢复原生rank与QGC/GStreamer自动选择，且仅用于诊断。
...
1. 在UniGCS的接口分配中把 **SDK接口** 选择为 **Serial 2**；Serial 2不能同时被遥测链路、另一应用或串口调试工具占用。UniRC 10 Pro文档给出的SDK Serial 2设备为 `/dev/ttyHS0`、115200波特率；当前实现另采用8N1，但PDF该处没有明确数据位/校验位/停止位，必须以目标遥控器实测或厂商补充说明确认。
...
- 打开串口后首个合法通道帧最长等待1.2秒；进入活动流后350 ms没有合法帧即按失联处理，取消UniRC缩放、关闭设备并在功能仍开启时3秒后重连。应用进入后台、路径变化、禁用或退出同样执行安全收尾。
...
| CH9或CH10异常 | 任一值小于900或大于2100 | 整份控制样本按失联处理；活动UniRC缩放回到0方向，两个输入解除arm。 |
...
已新增 `UniRcProtocolTest`，覆盖20 Hz/停止请求精确hex、CRC错误、CTRL和32字节严格门禁、16路小端解析、分片/粘包、噪声/坏帧/伪长度重同步，以及CH9死区和首次中位arm、CH10释放到按下单沿、合理值边界和失联复位；已用Qt 5.14兼容构建实际运行10 passed、0 failed。当前工作区仍没有Qt 6项目构建目录或 `compile_commands.json`，也没有生成新Android APK或连接目标UniRC、A8和PX4，因此以下结果全部仍待真机：

1. UniGCS正确分配Serial 2后，应用是否具备 `/dev/ttyHS0` 的DAC/SELinux权限、能否独占打开并持续收到20 Hz合法0x42。
2. CH9从中位到两端及回中是否显示正确值、方向是否符合“小于1500缩小/大于1500放大”，A8 UDP抓包是否只出现必要的0x05开始/停止且触控所有权不串扰。
3. 拔掉/停用SDK流、切后台、关闭开关、修改路径和串口被占用时，350 ms watchdog、停止动作和3秒重连是否符合预期，断流期间不得保持镜头运动。
4. CH10每次释放到按下只产生一次共享回中事务；无Vehicle/无activeGimbal时不发送；存在PX4/GPS2/A8链时需要同时记录MAVLink命令1000 ACK、PX4下游串口输出和最终物理回中，不能只看QGC日志中的“request accepted”。
5. 顶部Center与CH10交替、RC夺权、多Vehicle/多云台、按键长按和快速重复时，共享协调器不得并发、串目标或绕过8.3.3的控制权/预激活门禁。
  ...
  -> CustomPlugin::init()
     ...
     -> AndroidVideoDecoderPolicy::apply()
        -> 新安装默认true：H.264/H.265限定为通过厂商筛选的androidmedia MediaCodec候选
           ...
           -> H.265：qgcandroidh265hwdec保持PRIMARY+100=356优先；direct-hvc1保持PRIMARY+3=259
           -> 覆盖rank前按原始rank/稳定名称保存首选direct-hvc1 factory，供逐receiver显式重试
     ...
     -> AndroidH265DecoderFallback安装到Video 1/Video 2 receiver
        -> 新receiver/URI显式factory为空；URI改变清空旧direct sticky路由
     -> AndroidVideoDecoderRecovery（Video 1）/ DualVideoManager watchdog（Video 2）
        -> 按冻结URI + generation隔离同URL重启；tee CAPS确认实际codec，分别记录source/decoder输出/sink里程碑及decoder plugin/factory
         -> 同代bus error先停止本路Timer；仅!rtspSourceError + decoderBranchError + H.265 + adapter已选 + decoder/sink均无帧时，在core stop前为下一代准备direct，随后标记stopping
        -> watchdog路径：只在真实source buffer后启动；max(rtspTimeout,8秒)+3秒仍无sink帧时完整重建
           -> H.265 + adapter已实例化 + decoder/sink均无帧：为下一代冻结首选direct factory
            -> decoder已有输出但sink无帧不切换，转查显示/GL/surface；无source且没有已确认decoder分支失败时不切换，转查路由/RTSP/RTP
        -> 完整stop；各自owner按原有生命周期和退避重建receiver
           -> 同URI direct路由保持sticky；URI改变后下一代重新从adapter开始
   ...
   -> VideoManager::init()
      ...
      -> 创建 VideoReceiver / decodebin3
         -> 普通decodebin沿用deep-element-added实例日志；显式direct沿用同一短语并额外记录selection=receiver-specific explicit factory
        ...
        -> CustomPlugin::createVideoSink()复用原生qgcvideosinkbin
         ...
         -> 满足严格adapter首帧超时条件后的同URI下一代：显式direct-hvc1厂商 amcviddec-*
...
Application Settings / Video
  ...
  -> Video Stream Integration
     ...
     -> forceAndroidH265HardwareDecoder（兼容旧键；UI为H.264/H.265必须硬解，新安装默认true；仅Android生效且修改后重启）
        -> true：对应codec只允许厂商MediaCodec；H.265 adapter保持356优先、direct保持259；其余候选NONE，无硬件路径则critical失败
        -> false：仅用于诊断；不注册custom adapter、不改rank，恢复QGC/GStreamer原生自动选择
        -> URI+generation隔离实际codec、source、decoder身份/输出、sink与bus事实
        -> 新receiver/URI首代走adapter；同URI在H.265 source已到的adapter watchdog超时，或同代非RTSP-source的adapter decoder分支bus错误成立时，才让下一代显式direct；两者都要求H.265且decoder/sink均无帧
        -> direct对同receiver/URI保持sticky，URI改变复位；全程不改rank、不使用avdec_h265
        -> start失败、其他bus/source/sink故障、无source且无decoder分支证据，以及decoder已有输出但sink无帧都不切换，只按原路重建并分层排查
```
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - 双路并行解码、全屏和叠加层：URL 1/2配置为两个不同且可用的RTSP endpoint，使用新安装默认 `forceAndroidH265HardwareDecoder=true` 冷启动。两路都必须依次出现各自 `First source media frame reached the receiver`、`Decoder element instantiated`、`Decoder produced its first output frame`和 `First decoded video frame reached the sink`；每条日志应带同一冻结URI/generation，receiver与decoder instance必须各自不同，实际decoder必须是厂商MediaCodec或H.265 adapter。正常A8首代应保持adapter并到sink；针对MT11分别注入“source已到+H.265+adapter+decoder/sink无帧+watchdog超时”和“同代!rtspSourceError+decoderBranchError+H.265+adapter+decoder/sink无帧”两类严格条件，后者同时覆盖source首buffer前的MediaCodec CAPS/CSD失败。任一条件成立后应看到下一代准备direct，且下一代实例日志额外包含 `selection receiver-specific explicit factory`，随后必须重新取得decoder输出与sink首帧。普通decodebin实例日志不要求selection字段。同URI再次重建仍应direct，改变URI再换回时首代应恢复adapter。仅缺source且没有decoder分支证据、start失败、RTSP/source错误、非decoder分支或sink分支bus错误，以及decoder已有输出但sink无帧不得准备direct。所有重建前后rank一致，任何代次都不得实例化 `libav/avdec_h265`，一路路由变化不得影响另一路。其余仍需验证持续画面、PIP/主视图交换、全屏、断流重连、独立窗口及surface销毁重建；rank、READY、路由准备或factory创建均不等于播放成功。
   - Android第二显示面启动顺序回归：冷启动、Video 2断流重建、Fly View退出重进及桌面跨window重挂后，都必须再次看到本路decoder输出和sink首帧，且画面持续刷新。代码静态顺序必须保持 `sink add/set PAUSED -> add decoder（decodebin3或显式factory） -> decoder pad link sink -> sink sync parent`，不得恢复成decoder动态pad出现后才 `gst_bin_add` sink。sink probe必须在安装时冻结本代URI+generation不可变context、由destroy notify释放；同URI持续重建时故意制造旧probe迟到回调，不得把旧代sink首帧计入新代，也不得泄漏context。parsebin动态pad必须优先把current/query CAPS传给decoder；显式factory应接受所有structure都是 `video/x-h265` 且可与 `hvc1` 相交的非ANY CAPS，不要求完全fixed。late `startDecoding()`让valve只返回ANY/EMPTY时，只有本代tee已确认H.265才可放行；非H.265、混合CAPS或没有本代H.265证据的ANY/EMPTY必须拒绝。若MT11只有SDK绿点而没有source首帧且没有decoder分支bus错误，先查RTSP/RTP；若有adapter decoder分支错误或source但无decoder输出，查H.265/CAPS/CSD/MediaCodec；只有decoder输出存在而sink首帧缺失才判为显示端问题。
...
11. Android保持默认true，分别覆盖MT11单独放在URL 1且URL 2留空、MT11放在URL 2且URL 1为不同有效地址、H.264+H.264、H.265+H.265及产品实际A8 H.265+MT11 H.265组合，连续播放至少10分钟。每个新receiver/URI的H.265首代都应以356的adapter压过259的direct候选；健康A8不得因为另一条流失败而改变factory。对具备兼容direct的目标机，单独制造MT11 adapter首帧watchdog条件，确认下一代显式direct、同URI sticky、URI变化复位adapter，并分别记录四阶段、开始/5分钟/10分钟延迟、CPU、丢帧、前后台、PIP和断流重连。URL 1/2填入同一地址会触发重复源保护，不能作为解码A/B。用户确认旧版A8 Android H.265可显示；新代码A8不回归、MT11显示及双H.265/双MediaCodec仍须新APK真机验收。
12. 仅为诊断临时关闭硬解开关并重启做A/B，确认不注册custom adapter、首选direct getter为空、不改rank并由QGC/GStreamer原生自动选择；诊断中可能出现的软件decoder只能作为对照，不能成为产品配置或自动恢复路径。随后重新开启并重启。默认严格模式首代预期进入 `qgcandroidh265hwdec`；若满足watchdog或decoder分支bus两条fallback入口之一，下一代应出现 `H.265 adapter failed before producing a decoded frame` 与 `Using receiver-specific explicit H.265 decoder factory`，并用helper日志的 `sourceFrame`、`decoderBranchFailure`区分入口，再核对direct `androidmedia/amcviddec-*` 的输出和sink首帧。同URI后续失败仍须direct，不能误称全局rank已翻转；URI改变后应清除receiver-specific路由。双路H.265期间分别开始/停止录像并回放，确认播放支路转换不影响原生 `hvc1` 录像。rank、READY、getter、路由准备或factory创建均不能代替首帧与画面。
13. 分别让URL 1和URL 2覆盖首帧失败矩阵并核对URI/generation不串代：第一类保持真实H.265 source buffer持续进入、确认adapter已实例化、不产生decoder输出且不注入bus错误，直到首帧watchdog超时，必须为下一代准备direct；第二类在source首buffer前让已识别H.265的adapter decoder分支报not-negotiated/Failed configure，确认 `!rtspSourceError && decoderBranchError` 且decoder/sink均无帧，也必须准备direct；第三类在source首buffer后注入同样decoder分支错误，结果应与第二类一致。再分别注入RTSP/source错误、非decoder分支bus错误、sink分支错误、同步start/startDecoding失败、只有source pad而无buffer且无decoder分支错误，以及decoder已有输出但sink无帧，这些都必须保持本receiver当前路由。所有bus error先停止Timer，允许的decoder分支路径在core stop前冻结direct，随后统一标记stopping，不能与watchdog形成双重stop。所有重试前后adapter/direct/软件候选rank保持不变且不得实例化 `avdec_h265`。另验证direct sticky后同URIbus/首帧失败仍保持direct，URI改变才复位；让Video 1失败而Video 2持续正常、再反向执行，确认健康路decoder实例和路由不受影响。
...
RTSP黑屏必须先做“URL/SDP -> transport/RTP -> decode -> render item”分层，不得一开始就归因于H.265或双路同步。相机栏MT11绿点仅代表私有SDK UDP合法回包，不能跳过任何视频层检查。本轮Android程序审查发现显示sink晚于decoder动态加入，已经按当前上游QGC改为sink先进入PAUSED、decoder后创建和pad直连；由于用户尚未提供本轮Android四阶段日志，现场故障归属仍必须按以下里程碑确认。Ubuntu对照命令为：
...
4. receiver/decode/render层：成功status、`Secondary video streaming/decoding`过程日志已删除。`First source media frame reached the receiver`证明媒体buffer进入receiver；随后必须看到同一receiver的 `Decoder element instantiated`、`Decoder produced its first output frame`和 `First decoded video frame reached the sink`。普通decodebin实例日志没有selection字段；只有显式direct实例额外带 `selection receiver-specific explicit factory`。没有source首帧且没有已确认adapter decoder分支错误时，结合GST_DEBUG/pcap查RTSP/RTP/depay/parser；若同代日志显示 `!rtspSourceError`、`decoderBranchError=true`、H.265、adapter已选且decoder/sink均无帧，则允许在source首buffer前为下一代准备direct，应查CAPS/CSD/not-negotiated/Failed configure；有source首帧但没有decoder输出时同样查codec、caps与实例资源。decoder有输出、sink尚未收到首帧时查显示链、window和可见性，该情况不会触发direct fallback。start/startDecoding、RTSP/source、非decoder分支或sink分支错误也不会切路。
...
1. policy候选：H.264日志应显示实际plugin为androidmedia、厂商factory接受 `avc`且rank为259；H.265 adapter应出现 `Registered qgcandroidh265hwdec ... rank 356 ... hvc1 -> byte-stream/au conversion` 并优先于rank 259的direct-hvc1候选；有兼容direct时还应出现 `Selected receiver-specific H.265 direct hardware retry factory`。这些只证明候选、adapter和显式重试名称已准备；`avdec_h264/avdec_h265`只能用于核对其rank为NONE，不能成为实际decoder。
2. 实际选择：每路都要看到 `Decoder element instantiated`，并核对receiver、URI、generation、plugin/factory和instance。新URI首代H.265应由普通decodebin实例化 `qgcandroidh265hwdec`，此日志不要求selection字段；watchdog或decoder分支bus两条fallback入口之一成立后，下一代应先出现helper准备日志，再看到direct厂商decoder的同名实例日志且额外包含 `selection receiver-specific explicit factory`。helper中的 `sourceFrame=true, decoderBranchFailure=false`表示watchdog入口，`decoderBranchFailure=true`表示已确认同代decoder分支错误，后者允许 `sourceFrame=false`。adapter内部 `selected actual decoder ... byte-stream ... alignment=au`只说明内部候选与输入caps，不代替外层四阶段。
...
若adapter已选择、H.265 source已到但直到watchdog超时仍无decoder/sink帧，下一代允许显式direct；同代非RTSP-source的adapter decoder分支bus错误在H.265已识别且decoder/sink均无帧时也允许，即使source首buffer尚未记录。这仍只是有界硬解路由改变，不是播放成功。若已有decoder输出而sink无帧，则只转查显示端并保持当前路由；start失败、RTSP/source错误、非decoder分支或sink分支bus错误，以及无source且无decoder分支证据也保持当前路由。所有情况都按封顶退避完整重建且不改变rank。任何 `vendor MediaCodec candidate`、direct getter或explicit factory创建都只代表筛选/选择结果，系统级硬件属性仍以Android API 29 `isHardwareAccelerated()`为准。
...
ctest --test-dir <desktop-build>/custom -R '^(SiyiProtocolTest|Mt11ProtocolTest|GimbalMediaSessionPolicyTest|GimbalPhotoCapturePolicyTest|UniRcProtocolTest)$' --output-on-failure
```

截至2026-08-27，UniRC协议/策略测试目标和用例源码已经加入，20 Hz请求 `55660101000000420552b0` 的CRC由独立计算复核，并已使用Qt 5.14兼容构建实际运行 `UniRcProtocolTest`：10 passed、0 failed；相关JSON和两份TS XML可解析，context/source集合一致，英文/中文Qt 5.14 `lrelease`均exit 0且中文生成179条译文，`git diff --check`没有本轮空白错误。当前机器仍没有Qt 6项目构建目录，也没有Android APK、目标遥控器或adb/串口会话。POSIX设备访问、PDF未明确的8N1假设、20 Hz持续帧、350 ms失联、A8原生0x05运动、触控/拨轮ownership、共享MAVLink回中、PX4 GPS2 UART和物理云台结果全部必须按8.4.5与第35项完成真机验证。
...
截至2026-08-27，Android黑屏程序审查形成三层静态修正。第一层沿用当前上游QGC顺序：先把每路QML sink加入receiver管线并置PAUSED，再创建decoder，动态pad只连接已有sink。第二层维持严格厂商硬解：H.265 adapter保持 `PRIMARY+100=356` 并压过rank 259的direct候选，其他对应codec候选为NONE，不使用 `avdec_h265`，因此A8既有H.265 adapter链不因MT11改变。第三层解决单一进程级rank无法为两条流表达不同路径的问题：每个新receiver/URI首代仍adapter；H.265 source已到的adapter首帧watchdog超时，或同代 `!rtspSourceError && decoderBranchError`、H.265、adapter已选且decoder/sink均无帧的bus错误，才让该receiver同URI的下一代显式direct，并保持到URI改变。后一入口覆盖MediaCodec在CAPS/CSD阶段、source首buffer前的失败。core负责冻结factory、传递parsebin动态pad current/query CAPS、识别全H.265且可与hvc1相交的非ANY CAPS，并仅在late valve ANY/EMPTY且本代tee已确认H.265时放行；fallback条件、sticky状态和factory筛选仍在custom。
...
截至2026-08-27，用户确认严格adapter优先构建中MT11无论放在URL 1还是原Video 2链路都无画面，而A8 H.265正常。当前代码保持adapter 356、direct 259和所有软件候选NONE，并增加上述逐receiver/URI direct fallback，避免用全局direct-first改变A8成功链。更换到URL 1已排除Video 2专用显示接线作为共同根因，但用户尚未提供新实现的Android四阶段日志；因此目前只能称为代码静态修正，不能确定MT11现场是否触发watchdog或decoder分支bus入口、direct是否产生raw输出或画面是否到达sink，更不能宣称Android双路已通过。若新APK没有source且没有decoderBranchError，应转查RTSP/RTP/depay/parsebin；若出现adapter source后watchdog或source前decoderBranchError日志，应核对下一代explicit factory的decoder输出；若decoder已有输出但sink无帧，路由不会切换，应查GL/surface。本轮同时让Application Settings -> Video五个顶层组使用50字符preferredWidth与maximumWidth，并以 `Qt.AlignHCenter`居中；maximumWidth才负责封顶，长说明只在组内换行。

本轮Android验收以默认开启的厂商MediaCodec双硬解为主线：全局adapter 356/direct 259不变，新URI首代adapter；严格的adapter首帧watchdog入口和同代非RTSP-source的adapter decoder分支bus入口都可让下一代逐receiver显式direct，同URIsticky、URI改变复位。临时关闭开关后的QGC/GStreamer原生自动选择只用于诊断A/B；任何产品路径都不能加入 `avdec_h265`。URL 1与URL 2必须分别覆盖adapter健康、adapter watchdog超时、source前/后的decoder分支bus错误、direct同URI重建、URI复位、无source且无decoder分支证据、RTSP/source/非decoder/sink bus错误、start失败以及decoder输出但sink无帧，确认每个分支都符合切换门禁且一路失败不影响另一路。RTSP传输仍保持GStreamer原生Auto。完成新APK干净构建、目标遥控器四阶段日志和实际双画面前，本说明不把该静态实现表述为真机修复完成。
