...
- 在 `custom` 中实现思翼私有 UDP SDK 和协议封装，包括帧组装、CRC16、固定为0的协议sequence字段，以及0x05、0x0a、0x0b、0x0c、0x0f、0x16和0x18。0x05只保留协议兼容接口，当前Manager/UI不调用；0x0f执行绝对倍率，0x16读取设备倍率能力，0x18读取实际倍率。0x0f的一字节ACK只表示受理或拒绝，不能更新显示；由于现有设备不递增sequence，最终结果始终以串行0x18实际回读为准。Manager对用户目标保留单目标/单查询在途、350 ms旧包隔离和两次目标命中。明确负ACK后用两份新的一致回读快速恢复；无负ACK时必须先有五份连续一致非目标回读，再分别满足合法停点3秒、非网格停点5秒的命令年龄门槛，不能再把约1秒内重复的1.6误判为最终值。
...
- Gimbal关闭时才在存在活动飞行器的前提下回退QGC原生 `PhotoVideoControl`。缩放手势统一为Idle/Pressed/Holding/Consumed状态：短按向串行队列增加一个合法停点；长按也不使用自由运行的0x05，而是在上一0x0f经过两次0x18目标确认后串行发送下一合法停点，包含5.5/3.5精确末端上限。松开、取消、移出或外部安全停止只取消尚未发送的后续长按步骤，已经发出的唯一目标继续核对，不发送0x05停止包，也不会在释放后补发短按。保留60秒长按看门狗。
- 在Fly View设置页以 `SIYI Gimbal Camera` 标题提供云台相机启用、SDK IP、SDK端口和缩放步长设置，并明确短按按步长排队、长按重复已确认目标、合法停点为 `1.0 + n × zoomStep` 与分辨率精确上限的并集；实际拉流尺寸单独决定已连接会话的缩放上限。
...
| `custom/src/FlightDisplay/GimbalZoomControl.qml` | 合并栏顶部的思翼缩放子控件，使用单列GridLayout从上到下显示加号、当前倍率和减号，并把完整列高传给外层布局。从Manager读取 `zoomControlsUnlocked/currentZoom/zoomStatusKnown/zoomValueUncertain/zoomInAvailable/zoomOutAvailable`；命令pending时保留上一份已确认合法倍率，初次倍率未知或值不确定时显示 `--`，只显示Manager两次0x18确认过的合法停点。按钮不依赖 `sdkResponding`，pending也不会将两方向一起禁用；方向availability只反映串行计划倍率是否还有下一合法停点。两侧MouseArea使用Idle/Pressed/Holding/Consumed手势状态：短按排入一个分度，长按启动串行重复，释放只取消后续长按步骤且不补发短按。 |
...
| 文件 | 详细作用 |
|---|---|
| `custom/src/Gimbal/A8MiniZoomPolicy.h` | A8 Mini缩放策略的纯静态接口，将视频尺寸映射、合法末端上限、严格双向步进和目标观察器从Manager状态机中分离，便于独立测试；不访问网络、QSettings或UI。`TargetTracker`要求两次目标命中；明确负ACK后要求两份新的相同非目标值，无负ACK时要求五份连续相同非目标值并交由Manager继续检查3秒/5秒命令年龄门。 |
...
| `custom/test/Gimbal/SiyiProtocolTest.cc` | custom独立QtTest协议与策略回归用例。覆盖封包量化、ACK control、坏CRC、严格整帧长度、0x05/0x0f/0x16/0x18新版与真机旧版倍率payload、越界和非法载荷、720P/1080P/2K/4K及coded-height映射、包含5.5/3.5末端的双向序列、0.7x步长短末段、非网格双向恢复，以及“普通非目标五份证据/目标双命中/负ACK后两份新样本快速恢复”观察器状态。测试只在桌面 `QGC_BUILD_TESTING`注册，并提供 `check_siyi_protocol`独立目标。 |

云台相机完整调用链为：`VideoReceiver`解码当前视频 -> 真实首帧CAPS或最终 `GstVideoInfo` 隐式尺寸 -> 无直接结果时使用稳定1秒的 `VideoManager::videoSize` -> 得到并锁存本次流会话上限/解锁控件 -> `A8MiniZoomPolicy`生成“步长网格+精确上限”合法停点 -> `GimbalControlManager`查询0x18并只发布两份一致的合法实际倍率 -> 单击方向进入串行队列或长按启动逐步泵 -> 每个0x0f目标由两次0x18命中后提交并继续下一步；运动中间值只内部记录，负ACK、达到五份样本与分级年龄门的稳定未达值、或最终超时均只进入有界恢复/清理事务，不锁定仍在线的视频会话。
...
短按从“当前在途目标或最后确认/raw参考”计算下一合法停点并加入方向队列；快速连点不会并发发包，而是在每个目标双确认后逐步发送。普通步进保持 `current ± zoomStep`，若下一完整步越过分辨率上限则精确上限作为最后停点，例如5.0→5.5、3.0→3.5；反向从精确上限回到上一常规网格。首次倍率尚未知时，排队方向直接从两次确认的raw值计算本次目标，不能先自动归整后再把同一次点击多走一步。0x0f ACK只表示受理或拒绝；两个串行0x18都命中目标后才更新 `currentZoom`。1.6/1.8等运动样本继续查询；无负ACK时需五份连续一致回读，且合法替代停点至少经过3秒、非网格值至少经过5秒才进入恢复，约1秒内的重复值不能结束pending。明确负ACK用两份新的一致回读快速恢复；非网格结果最多执行两次有界归整，任何路径都不公开非法数字。pending期间继续显示上一份已确认合法倍率，首次未知或不确定时才显示 `--`；两个按钮始终按计划倍率边界可操作。长按复用相同串行路径：上一目标双确认后等待180 ms再发下一合法停点；松开、取消、移出、隐藏、销毁或60秒看门狗只取消后续长按步骤，已发送目标继续核对且不发送0x05。设置变化、SDK通信异常或心跳超时会清理当前事务与队列并重新核对，但视频在线时不撤销控件解锁。10秒整体超时同样清除事务和短按队列、保留最后合法值/内部raw参考并后台重读，视频仍在线时按钮不会锁定。0x0c拍照/录像不带直接ACK：照片由0x0b确认；录像约400 ms查询0x0a并仅接受目标一致状态，2.5秒无结果标记unknown。
...
5. 短按 `+`/`-` 每次排入一个合法停点。步长1.0x时，1080P序列为1→2→3→4→5→5.5及5.5→5→4→3→2→1，2K为1→2→3→3.5及3.5→3→2→1。一个目标确认前继续显示上一份已确认合法倍率，初次未知或超时核对状态显示 `--`；按钮不因pending一起变灰。快速点击 `+++`应严格串行发出2、3、4，不能并发。按住约420 ms后按相同规则逐项到达精确上限，每个目标需两个0x18命中；松手或移出只停止下一目标，长按不附带短按，也不应抓到0x05缩放包。模拟负ACK、重复1.6/1.8或丢包时，不得公开非合法数字；pending经实际核对或10秒总超时结束后，只要视频仍在解码，缩放按钮仍可再次操作。
  ...
  -> CustomPlugin::init()
     ...
     -> FlyViewCustomSettings（FlyView/showHeadingCompassBar）
      -> GimbalControlSettings / GimbalControlManager
         ...
         -> 一个0x0f目标在途；两个串行0x18都命中才发布current，pending保留上一合法显示
         -> 短按方向队列和长按共用串行泵；busy/超时/心跳不重新锁定视频会话
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - `-`/`+`每次短按只增加一个方向意图，释放不得重复触发。步长1.0x且1080P上限5.5x时严格覆盖1→2→3→4→5→5.5和反向序列；2K上限3.5x时覆盖1→2→3→3.5和反向序列。1.8→2.8、2.8→1.8等漂移目标不得发送；从1.8x人工放大/缩小只能去2.0x/1.0x。首次倍率未知且稳定raw为1.6x时，一次 `+`只能发送2.0x，不能先归整2.0x后又发送3.0x。快速点击 `+++`可累计计划目标，但抓包必须在每一步双确认后才依次看到2.0、3.0、4.0，绝不并发；pending期间两方向不能因busy一起变灰，数字继续显示上一份合法确认值。
   - 按住420 ms后抓包应看到当前序列的一个0x0f目标；只有两个串行0x18查询都命中该目标后，才允许在180 ms后发送下一完整网格步。松开、取消、移出、控件隐藏/销毁或60秒看门狗只取消后续长按目标，已经发送的0x0f继续确认，不得补发短按。SDK通信异常、心跳超时或endpoint/Enabled变化应清理当前事务与队列，但视频仍在线时不能撤销流会话解锁；应用退出同样不得额外发送0x05。所有路径都不发送任何0x05方向或停止包。
   - 验证查询串行化和失败恢复：0x0f后先有约350 ms旧包隔离，同一时刻最多一个0x18查询在途；单次查询1秒超时后重新隔离再查，10秒整体deadline不被重试刷新。目标2.0时依次构造1.2、1.6、1.6、1.6、1.8，所有样本都只能内部记录，不能结束pending、自动重发0x0f或显示非法数字；随后两份2.0才提交显示。无负ACK时，五份连续一致非目标值只是稳定证据：合法停点需命令年龄至少3秒才恢复，1.6等非网格值需至少5秒且只能触发最多两次合法归整，不能发布1.6；任一样本变化应重新累计五份。明确负ACK后清空ACK前候选，只允许两份新的相同实际值快速恢复。若10秒仍无可确认结果，则清除pending和短按队列、保留合法公开值/不确定状态，视频在线的按钮仍须可点。另需覆盖：SDK heartbeat false不锁、0x16抖动不锁、stablePending超时不锁、同一会话异常尺寸不锁、真实 `decoding=false`才锁，以及发送失败后下一次用户操作可重试。
...
初始镜头1.0x却显示错误、实际与显示不一致或缩放按钮始终灰色时，先查看0x18原始payload。新版 `01 00`表示1.0x、`01 08`表示1.8x；A8真机旧版会返回 `0A 00`表示1.0x、`12 00`表示1.8x。日志若持续出现 `Rejected invalid SIYI current zoom payload "0a 00"`，说明运行的仍是未加入兼容解析的旧构建；新实现应记录 `Decoded SIYI current zoom 1 encoding little-endian tenths compatibility`。0x0f出站仍必须使用官方“整数byte+小数byte”，2.0x为 `02 00`，不能因旧版0x18回读兼容而改成 `14 00`。目标2.0期间出现1.6，表示设备当时报告运动中间值，不是2.0被误解码；两份2.0会立即提交，约1秒内即使累计多份1.6也不能判定终值。没有负ACK时，1.6必须同时满足五份连续一致和5秒命令年龄才会触发有界合法归整，永远不会提交为显示倍率。pending时保留上一份已确认合法显示，不确定时才显示 `--`，两种状态下按钮都继续可用。拉流上限720P/1080P/2K/4K分别为6.0x/5.5x/3.5x/1.0x；默认步长1.0x必须发送并显示5.5/3.5末端。若10秒仍无法命中绝对目标，事务会清理但视频会话不锁，需进一步确认相机设置中的Zoom Mode为Absolute Zoom。长按日志只包含串行的“0x0f目标、0x18查询、0x18查询”，释放不应出现0x05。紧邻应用退出出现的 `PhotoVideoControl.qml`中 `cameraManager/currentCameraInstance`空对象警告来自QGC原生相机面板销毁时序，不参与custom缩放状态机，也不是本次锁定原因。
