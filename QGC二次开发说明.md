...
- MT11的 `currentZoom`发布从1.0x锚定且不超过物理上限的合法步长目标；`actualZoom/actualZoomKnown`继续在Manager内部保存0x18权威实测值，供确认、端点判断和诊断使用，但相机控制栏与A8一致，只显示一个目标倍率数值，不再显示Target/Actual双行。默认步长1.0x且产品物理上限为165.1x时，目标最高165.0x，内部实测仍可到165.1x；步长0.1x时165.1x本身也是合法目标。短按按独立 `mt11ZoomStep`从上一目标计算1.0～30.0x的0x0F，快速连点替换在途目标；不足完整步长时把精确30.0x追加为最后短按目标，例如step 2.0使用1、3、…、29、30并沿同一序列反向返回。实测大于30.0x时两方向tap关闭。SDK的0x05只有 `-1/0/+1`方向/停止字段，没有速度字段；已结算的普通hold在420 ms成立后只发送一次原生方向并持续到release/cancel或0x18确认物理端点，不再用周期stop/start或逐档0x0F伪造速度。tap、绝对控制结束、hold停止/端点停止都会留下3秒controller-handoff grace，使这些瞬时pending标志即使早于QML的420 ms阈值清除，第一次新hold仍进入受保护接管。接管时取消旧hold尚未发出的安全stop且不再补0；只有仍pending的0x0F发送一份0x05(0)。新方向要保证距最近一次stop/controller transition已满300 ms：本次新发stop时完整等待，其他情况扣除QML等待420 ms长按阈值期间已经过去的时间。首次方向发出450 ms后固定发送第一份受保护取得副本并建立反馈基线；此后若尚未出现两次前向0x18变化且中间无反向变化，则每450 ms继续补纯方向，取得副本合计最多六份。副本之间没有stop，一旦确认运动立即取消余下副本，成功取得控制权后的完整hold仍是单一原生连续运动。运动期间每100 ms主动查询0x18，并只把单调对齐后的合法目标发布给控制栏；新运动起步300 ms后才允许端点自动停止，且要求连续两份同方向端点反馈，单个乱序0x18不能移动目标、虚假确认启动或吞掉新hold。release/cancel同时取消handoff和全部方向副本，立即停止并安排一份150 ms有界安全停止副本；settled期间tap继续受门禁保护，但物理方向允许时hold不再被禁用。60秒watchdog按“无进展”重置，165.1x端点仍允许反向退出。
...
- 2026-08-25的首次长按无运动根因不是QML吞触点：`GimbalZoomControl`在gesture非idle时保持按钮enabled，tap pending和post-hold期间Manager也允许物理可行方向的hold。真正竞态是MT11的0x05(0)同时执行“停止变倍并自动对焦”，而production sequence固定0使UDP写成功和0x05 ACK都不能证明随后方向已经取得控制权；旧的立即方向＋150 ms单副本可能全部落在相机busy窗口内。更关键的是QML到420 ms才调用Manager，届时150 ms stop Timer和两份settled 0x18常已清除瞬时pending状态，旧程序会误走无副本的ordinary路径。当前以跨越420 ms的handoff grace、保证距最近transition满300 ms、首份450 ms取得副本后的0x18进展确认和最多六份有限纯方向副本修复；它只解决启动取得控制权，不改变成功后的原生连续速度，也不恢复周期stop/start。
...
- 当前代码已完成扩展后的两套独立QtTest编译运行：`Mt11ProtocolTest`为11 passed、0 failed（9个业务槽加init/cleanup），`SiyiProtocolTest`为42 passed、0 failed（40个业务槽加init/cleanup）；`Mt11Sdk.cc`也已独立编译通过，Manager头文件moc成功且diff-check无错误。MT11测试除覆盖1～30x短按网格、step 2.0的1/3/…/29/30精确端点及对称返回、30.0/30.1协议门禁、0x18实测对齐和165.1物理端点显示边界外，还覆盖三种0x11官方payload、0x10通用main/sub合法域与main=0/2/3三态识别；默认1.0步长拒绝把物理165.1发布为Target，0.1步长允许165.1。该结果仍只证明Protocol/Policy和接口静态范围，不覆盖Manager的3秒handoff grace、300/450/150 ms Timer、0x18启动取得确认、完整QGC Qt 6构建或MT11真机原生连续运动/模式切换。
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义11个Gimbal Fact；独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，只决定tap目标和长按期间的目标参考网格，不参与原生长按物理速度。longDesc明确短按精确30x端点、0x05没有rate字段、普通hold单方向原生连续运动、100 ms的0x18内部确认、控制栏目标单值，以及近期控制器切换后补足300 ms交接、首次方向后的450 ms首份取得副本和总计最多六份纯方向取得副本；从首份副本建立的基线观察到两次前向变化且无反向变化后不再重复，不改变设备上限或RTSP URL。 |
...
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11 QML业务门面；合法目标由 `currentZoom`发布，0x18实测仍由 `actualZoom/actualZoomKnown`保留作内部确认与可选诊断；继续暴露设备上限、独立步长、tap/hold availability、媒体与错误状态。长按状态保留原生方向、3秒controller-handoff grace、300 ms交接、450 ms有界方向取得控制权、150 ms安全停止、100 ms反馈以及端点/watchdog Timer；仍不持有pace Timer或drive-running PWM状态。三模式QML契约为 `videoModeKnown`、`videoMode`、`videoModePending` 和 `setVideoMode(int)`，枚举值0/2/3与协议 `main_stream` 一致；旧 `thermalModeKnown/thermalModeEnabled/thermalCommandPending/toggleThermalMode()` 只为已有调用者保留兼容，新UI不以它们表达三态。常量区分0x0F上限30.0x、产品物理上限165.1x和线反馈上限255.9x。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 短按维持1～30x相邻0x0F，并在最后不足step时直接到精确30x。0x05协议没有速度字段；已结算普通hold在全倍率范围只发送一次原生方向并保持到release/cancel/端点，不再用周期stop/start降低平均速度，也不发送逐档0x0F。tap发送/确认、hold停止及端点停止均启动3秒handoff grace，避免QML的420 ms长按阈值晚于瞬时pending状态；新hold取消旧手势尚未发出的stop而不再发送会重启自动对焦的0，仍pending的0x0F只发一份stop。程序保证距最近stop/controller transition满300 ms再发方向；若QML的420 ms阈值已覆盖该间隔则不额外等待。首方向450 ms后发送第一份受保护取得副本并建立基线；其后仅在尚未观察到两次前向0x18变化且中间无反向变化时继续补方向，取得副本合计最多六份且中间不发stop。确认后立即退休全部副本，后续仍是单一原生连续运动。100 ms查询0x18并把控制栏目标沿运动方向单调对齐到合法参考档；起步300 ms后连续两次同方向端点反馈才允许自动停止，post-hold端点settle同样拒绝单个乱序包。release/cancel先取消handoff及全部方向副本，再发送stop和一份150 ms安全停止副本，松手后不会迟发start。60秒watchdog按0x18进展刷新，高倍端点仍允许反向hold恢复。模式切换、main/sub与媒体隔离语义保持。 |
...
MT11完整缩放调用链为：合法SDK包维持6秒在线 -> 0x16能力与0x18实际位置各维持6.5秒新鲜度 -> Policy对齐合法步长Target -> tap在1～30x发送0x0F，最后不足一步直接到精确30x -> 已结算普通hold在420 ms后发送一份原生0x05方向并保持连续运动 -> tap/绝对完成/hold停止留下3秒handoff grace -> grace内的新hold退休旧stop副本，仍pending的0x0F只发一份stop，并保证距最近stop/transition满300 ms再发方向（已过去时间计入） -> 每100 ms查询0x18；首次方向450 ms后发送第一份受保护取得副本并建立基线，此后在两次前向变化且中间无反向变化确认前，每450 ms继续补方向，取得副本合计最多六份且不插入stop -> 内部保存实测值并沿运动方向单调发布合法目标，起步300 ms后连续两份端点反馈才停止 -> release/cancel退休handoff与全部方向副本、立即发送0x05停止及一份150 ms安全副本。0x05 ACK只维持可达性，0x18是唯一位置真值；UI只显示目标倍率。默认step 1.0时目标上限165.0而内部实测可到165.1，step 0.1时165.1亦为目标。step不参与原生物理速度；除控制器接管所需的有界启动取得阶段外，1.0与2.0使用相同物理命令策略，模式/析构隔离和不读取A8能力的边界保持不变。
...
4. 右栏在A8与MT11同时启用时显示带独立在线点的分段胶囊选择器；切换只更换Manager、会先关闭当前弹层，不切换中心视频。两个相机面板共享高透明蓝灰背景、青色强调、统一浅蓝外边缘、圆角按钮和悬停/按压动效；通用错误不再形成整栏红色外圈。MT11倍率与A8一致，只显示目标数值：tap发送1～30x绝对目标且不足一步直接到30x；普通hold全域只发送一次原生方向并连续运动。若hold紧接tap或端点停止，则由3秒grace捕获，只补足距最近transition尚未过去的300 ms交接时间；首方向450 ms后发送第一份受保护取得副本并建立基线，随后仅在尚未观察到两次前向0x18变化且中间无反向变化时有界补方向，取得副本最多六份且中间绝不发stop；确认后立即恢复单一原生运动。release不得补tap，并会在任何handoff/补发阶段先取消未来方向。MT11拍照上方的模式按钮打开左侧优先的紧凑Overlay Popup，三个模式项与主栏按钮同为 `actionSize`正方形，以0x11选择变焦、热成像或变焦+热成像拼接，仅回包确认后更新当前模式。拍照、录像与媒体徽标语义不变。
...
当前 `Mt11ProtocolTest` 独立运行11 passed、0 failed，覆盖9个业务slot及init/cleanup，包括1～30x tap、step 2.0的1/3/…/29/30精确端点和对称返回、0x18对齐、165.1显示边界、三种0x11精确payload、0x10通用合法域及UI三态识别；`SiyiProtocolTest`为42 passed、0 failed。Manager moc检查0错误、diff-check通过。Manager的3秒grace、300/450/150 ms Timer、0x18启动确认、真实UDP、原生连续镜头运动、三模式实机画面及全域物理端点仍待验收。SDK没有速度字段，因此成功启动后的单次0x05提供设备原生平滑速度；当前450 ms方向副本只在控制器busy导致启动未确认时有界发送且不夹stop，不是长按限速或周期PWM。
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> 长按：已结算普通路径全域只启动一次原生0x05方向并保持连续运动，step不参与物理速度；近期控制器切换在首方向450 ms后发送首份受保护取得副本并建立基线，随后最多再补五份纯方向
         -> 100 ms轮询0x18并单调更新合法目标；起步300 ms后连续两份端点反馈才停止，单个乱序端点不移动UI目标
         -> tap pending后长按：取消旧安全stop副本 -> 对仍pending的0x0F只发一份0x05(0) -> 距该stop满300 ms发新方向 -> 首份取得副本后出现两次前向0x18变化且中间无反向变化时取消余下副本
         -> hold结算后立即再hold：3秒grace跨过QML的420 ms阈值；取消待发旧stop而不再触发自动对焦，只补足距最近transition尚缺的300 ms时间后进入同一启动取得流程，第一次手势不禁用
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - MT11 tap↔hold与首按接管矩阵：任意0x0F tap仍pending时开始同向或反向hold，都必须取消上一轮待发stop副本、仅向仍pending的绝对控制器发送一份0x05 `0`，从该stop起满300 ms后只发所按方向。若tap已由0x18确认或hold端点settle已清除瞬时flag，但仍处3秒grace内，则必须从300 ms中扣除transition已过去的时间；QML的420 ms阈值通常已覆盖该间隔，第一份方向可立即发送。分别在handoff Timer前、首次方向后和任一450 ms取得副本前验证release/cancel/移出/后台：不得补tap、不得迟发方向。受保护hold持续按住时，首次方向450 ms后应发送第一份取得副本并以当时0x18建立证明基线；注入无进展、重复、逆向乱序和前向变化，只有首份副本之后累计两次前向变化且中间无反向变化才能取消余下副本，静止样本不清零，逆向样本清零，无进展时取得副本合计最多六份且副本之间不得出现0。post-hold端点结算仍要求连续两份端点样本；release立即停止并仅保留一份150 ms安全副本；settled门禁只阻止tap，不阻止可行方向的新hold。
   - MT11原生连续长按矩阵：420 ms成立后，从1x到设备物理上限的任意位置和方向，远离3秒controller grace的普通hold抓包只能出现一份方向start，整个按住期间不得周期stop/start或发送逐档0x0F。接管近期tap/hold/端点控制器时，允许首方向及最多六份450 ms方向取得副本；首份取得副本固定建立新基线，此后只在尚未出现两次前向0x18变化且中间无反向变化时继续补发，所有副本之间不得发送stop，确认后直到release/端点不得再出现方向包。分别把step设为1.0和2.0，取得控制权后的0x05序列和连续采样0x18得到的真实倍率—时间斜率应基本一致，只有控制栏目标网格粗细不同。每100 ms查询权威0x18，UI只显示沿运动方向单调对齐的合法目标，迟到反馈不得让目标反向跳动或虚假取消取得副本。release/cancel立即取消handoff/方向Timer、stop并仅有一份150 ms安全副本，达到1x或0x16物理上限也自动stop。60秒watchdog按0x18进展重置；无进展、发送失败、模式切换及SDK离线必须可靠停止。约11x前后需单独记录画面与0x18斜率，以区分11x光学/165x混合成像链路的固件曲线；SDK没有rate字段，若原生速度仍不满足，只能向厂商索取带rate命令或固件，不能重新用step或周期停止伪造平滑速度。Manager Timer仍需fake SDK或真机验证。
...
选择MT11时视频有画面但缩放按钮仍灰色，应检查SDK在线、0x16/0x18新鲜度和模式pending；RTSP、A8的0x20及分辨率不能解锁MT11。settled等待只应锁定tap，物理方向可行的hold必须保持可用。>30x短点无动作是保护，应按住420 ms。远离近期控制器切换的普通长按抓包应只看到一份方向start，按住期间不能出现周期stop/start或逐档0x0F；紧接tap、release或165.1端点反向时，3秒grace会进入启动取得流程：旧安全stop只取消不补发，仍pending的0x0F只stop一次；若transition尚不足300 ms则补足后发方向，首方向450 ms后发送首份受保护取得副本并建立基线，随后在出现两次前向0x18变化且中间无反向变化前每450 ms继续补方向，取得副本最多六份且中间无stop。step 1.0与2.0只改变目标网格，不改变这套物理策略。若首次长按仍无运动，应抓包确认grace是否跨过QML的420 ms阈值、方向副本是否持续到0x18确认、逆向乱序样本是否被拒绝，以及release后是否仍有迟发方向。控制栏只显示单个目标值；165.1端点且step 1.0时显示165.0，内部0x18仍可为165.1，重新按住减号必须能反向退出。日志不再逐包显示unmatched ACK，抓包才是方向、停止与时间线证据。
...
A8 hold仍按原有单次0x05与按压时长目标策略。MT11成功取得控制权后同样使用单次原生0x05连续运动，但不使用A8的按压时长目标估算，而以100 ms的0x18实测更新单值目标；近期tap/stop由3秒grace跨过QML的420 ms阈值，旧安全stop取消不补发，仍pending的0x0F只stop一次，未满300 ms则补足交接时间。首方向450 ms后发送首份受保护取得副本并建立基线，随后仅在尚未出现两次前向0x18变化且中间无反向变化时每450 ms有界补方向，取得副本最多六份且中间不发stop。release/cancel先取消handoff及全部待发方向、发送stop，并在150 ms后发送一份有界安全副本。
...
截至2026-08-25，本轮MT11首按取得修正的静态验证为：Gimbal JSON和两份TS XML均可解析，Manager头文件Qt moc exit 0，英文/中文lrelease均exit 0，`git diff --check` exit 0且只有行尾转换提示；新增Timer成员、构造连接、析构、release/cancel、状态失效及错误退出路径已逐项审查，旧 `_flushContinuousZoomStopRetry` 无残留。当前机器没有Qt 6构建目录或 `compile_commands.json`，现有 `Mt11ProtocolTest`也不覆盖Manager的QTimer状态机，因此仍需Qt 6完整构建和MT11真机验证。现行算法以本段和2026-08-25根因条目为准；下面“截至2026-08-24”段落中的立即start＋150 ms单方向副本仅保留为历史记录，已被3秒grace、补足300 ms交接及450 ms纯方向取得方案取代。
