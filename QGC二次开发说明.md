...
- **已有实测基础与最新日志边界**：Ubuntu 24.04 下 A8 Mini 云台控制及 H.265 RTSP 播放曾正常；Android 下同一云台使用 H.264 编码曾正常。MT11 `rtsp://192.168.144.24:8554/video1` 的SDP已由外部工具确认为标准RTSP、H.265 Main、1920×1080、30 fps，用户也已在官方QGC验证同一URL可播放。2026-08-19 11:58最新附件来自Desktop Debug：MT11作为Video 2共9次start，其中8轮完整失败、最后一轮被退出中止；8轮均为第一次 `OPTIONS` 后服务端EOF、GStreamer内部自动重连再发同一 `OPTIONS`、第二次仍EOF，合计17条外发 `OPTIONS`，`DESCRIBE/SETUP/PLAY`和source媒体buffer、decoder输出、sink首帧均为0。首轮effective TCP/`0x4`与后续effective Auto/`0x7`结果相同，证明失败发生在RTP transport协商之前。该日志还显示未连接A8主路的状态通知绕过了Video 2已计划的2/4/8/15秒退避；本轮已按精确OPTIONS失败增加两级兼容和副路Timer门禁，但尚无修正后的真机日志，不能宣称MT11画面或Android双硬解已经恢复。
...
- MT11实机SDP确认 `video1` 为H.265 Main、1920×1080、30 fps。11:58最新附件把此前笼统的RTSP控制/读取EOF精确定位到初始 `OPTIONS`：8轮完整失败均在外发标准 `OPTIONS`、GStreamer内部自动重连并重发一次后收到第二个EOF，尚未发出 `DESCRIBE`。Connection组仍为两路提供独立的“优先TCP”开关，两路缺省均为Auto；开关只影响取得SDP之后的RTP/RTCP transport首选及回退，不改URI、SDP、编码格式、OPTIONS兼容状态或解码策略。
- `GstVideoReceiver` 对RTSP的设置超时值和运行时watchdog值分开处理：设置Fact不被回写，实际TCP首选管线的超时下限为5秒，Auto及TCP回退到Auto后的超时下限为8秒。Auto始终不设置 `rtspsrc.protocols`，保留原生协商，不再自定义切换到TCP；显式TCP首轮限制为interleaved TCP。尚未收到媒体帧时，如果显式TCP发生符合条件的资源读取错误、source EOS或source timeout，同一URI和同一请求模式最多单向回退Auto一次；回退后不会切回TCP，URI或用户transport设置变化时才清除状态。
- `GstVideoReceiver` 以成功启动代次记录stop completion：每个成功start最多完成一次stop；即使URI已清空也会释放旧pipeline，重复stop在没有活动pipeline和待完成代次时被忽略，从而避免同一次停止重复启动重连Timer。通用 `VideoManager` 为原生主/thermal receiver保存唯一生命周期状态，冷启动不再由初始化和render job各发一次start；异常停止后的RTSP重试改为1/2/4/8/15秒有上限退避。每个延迟回调携带generation，URI、transport、期望运行状态或新启动代次变化后，旧generation不得再次启动管线；只有首个解码/sink帧成立时才清零退避，单有source pad但无媒体buffer不会把每轮重试重置回1秒。Video 2继续使用自己的1～15秒有上限退避；11:58日志发现被动的主receiver active/releasing通知会绕过该Timer，当前实现只有URI、启用状态、重复源判定或transport等真实用户配置变化才取消旧延迟，被动通知只保留并打印剩余时间。
- 每次RTSP启动都记录requested/effective transport、requested/effective timeout、`rtspsrc.protocols`、`teardownTimeoutMs 1000`及OPTIONS兼容级别。标准级别0保持原生头部、`udp-reconnect=true`和RTSP keepalive；首帧前精确发生 `GST_RESOURCE_ERROR_READ + lastRtspMethod=OPTIONS + Received end-of-file` 后，同一URI下一轮进入级别1，仅设置 `short-header=true`并发送基本RTSP头；若基本OPTIONS仍收到同类EOF，再进入级别2，由 `before-send` 抑制OPTIONS并直接继续DESCRIBE，同时关闭RTSP keepalive，防止PLAY后以OPTIONS重新探活。三个级别都保留 `rtspsrc` 原生的 `udp-reconnect=true`，以免破坏成功建立UDP媒体会话后的RTSP控制连接恢复能力；级别1的一次失败attempt因此仍可能由GStreamer内部重发一次基本OPTIONS。持久级别与每轮active快照分离，compare-exchange保证同一attempt的重复error最多推进一级；OPTIONS EOF不会触发TCP到Auto回退，因为尚未发生SETUP/下层transport协商。该状态跟随URI、跨Auto/TCP切换保留，只有URI变化或转为非RTSP才复位，不把一次设备兼容结论泄漏给其他流。
- `rtspsrc` 的 `before-send` 回调记录每个实际外发请求的receiver、请求URI、method、OPTIONS兼容级别、User-Agent头数量、RealMedia扩展头数量及是否抑制，不记录认证或header值；bus error的critical除 `uri/source/domain/code/message/debug` 外还输出 `lastRtspMethod`。停止进入NULL期间只抑制中间态PAUSE，TEARDOWN仍允许发送并受1秒 `teardown-timeout` 约束；teardown标志从请求NULL起一直保持到下一轮start，pipeline引用清除后的迟到PAUSE也必须被抑制。不能把“抑制PAUSE”或“兼容性跳过OPTIONS”误写成跳过已建立RTSP session的正常TEARDOWN。
- 11:58日志证明上轮唯一首次start和同URI交接保护已生效，但也登记两类新修正：初始OPTIONS EOF采用“基本头部 -> 跳过OPTIONS”的同URI两级自适应；Video 2的2/4/8/15秒退避不得被未连接A8主路的被动状态通知提前取消。上述仍属于待真机验收的代码修正；没有新日志中的DESCRIBE、SETUP、PLAY、source媒体帧、decoder首输出和sink首帧，不得由静态diff推定播放恢复。
...
- 仅保留 `src/CMakeLists.txt`、`src/Vehicle/VehicleSetup/VehicleSummary.qml` 两处feature必需例外，`VideoManager.h/.cc` 的通用串行生命周期与退避，`VideoReceiver.h`/`GstVideoReceiver.h/.cc`/`QtMultimediaReceiver.cc` 的通用启动URI快照、RTSP transport、OPTIONS EOF兼容、teardown与解码诊断扩展，以及 `QGCLogging.cc` 的通用日志级别过滤修复；其余功能通过custom C++、QRC、独立custom QML模块、QML URL拦截和Android overlay接入。请求Auto时始终使用 `rtspsrc` 原生自动协商，不做Auto到TCP的自定义切换；仅请求TCP时，首帧前符合条件的资源读错误、timeout或EOS可使同URI后续重启单向回退Auto。OPTIONS兼容状态只依据通用RTSP错误、method与URI推进，不引入相机型号、产品IP或custom设置依赖。
...
当前分支 `SecDev/ft/gimbal` 的二次开发业务仍位于 `custom`。`src` 差异登记为以下九个文件：前两个是custom PX4模块正常链接所需的受控例外，中间两个为所有原生VideoReceiver提供通用串行生命周期与RTSP退避，随后四个是所有VideoReceiver可复用的启动URI快照及GStreamer RTSP transport、OPTIONS兼容、teardown诊断与无媒体恢复扩展，最后一个修正通用日志级别过滤：
...
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.h` | 保存上次RTSP URI与用户请求transport、本轮requested/effective transport快照、显式TCP到Auto的单向fallback状态和是否收到source frame；不保留Auto到TCP状态。另以原子整数分别保存同URI持久OPTIONS兼容级别和本attempt active快照（0标准、1基本头部、2跳过OPTIONS），以成功start代次保存待完成stop标志，使用原子状态记录“正在进入NULL的teardown”和最后一次允许发送的RTSP method，以互斥快照保存实际pipeline URI，并声明 `before-send`、OPTIONS兼容CAS推进、decodebin深层元素与decoder src首buffer探针。`_makeSource()`只使用本轮transport与兼容快照，用户transport切换不清除同URI已确认的OPTIONS兼容，URI变化则复位。 |
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.cc` | GUI调用start时先冻结URI、transport和low-latency并随任务传入私有worker，worker不再读取可能由设置线程并发修改的URI，也不会把旧attempt误记为新地址；实际pipeline URI以互斥副本供bus、watchdog、fallback和before-send线程读取。仅在创建标准 `rtsp://` 的 `rtspsrc` 时检查上述通用选项。requested Auto始终不设 `protocols`，完整保留 `rtspsrc` 原生协商；requested TCP首轮设置并回读 `protocols=GST_RTSP_LOWER_TRANS_TCP`。尚未收到媒体帧时，符合条件的资源读取错误、source timeout或EOS只会让显式TCP同一URI后续重启单向回退Auto，直到URI/设置变化，不会从Auto切到TCP；精确OPTIONS EOF不执行该transport回退。首帧前只有 `RESOURCE_READ + OPTIONS + Received end-of-file` 才以active快照和CAS推进同URI兼容：标准请求失败后下一轮仅设置 `short-header=true`；基本OPTIONS仍失败后再由before-send跳过OPTIONS、直接进入DESCRIBE并关闭RTSP keepalive，避免后续重新发被拒绝的方法。各级均保留 `rtspsrc` 原生 `udp-reconnect=true`，保护成功UDP会话后的控制连接恢复能力。每轮记录requested/effective transport、protocols、timeout、`teardownTimeoutMs 1000`、compatibility、shortHeader、udpReconnect和rtspKeepAlive；`before-send`按receiver记录request URI、method、兼容级别、User-Agent/Real扩展头数量和是否抑制，但不输出header值，bus error的critical记录含 `uri/source/domain/code/lastRtspMethod/message/debug`。停止进入NULL时只拦截rtspsrc中间态自动产生的PAUSE，TEARDOWN保持允许；teardown原子标志保持到下一次start，覆盖异步NULL后的迟到PAUSE；1秒 `teardown-timeout` 用于等待正常关闭而不是跳过关闭。成功start代次最多发送一次stop completion，即使URI已清空也清理旧pipeline，无活动pipeline且无待完成代次的重复stop会被忽略，避免重复重连Timer。decodebin `deep-element-added`记录实际decoder，decoder src探针记录首个输出buffer及caps，显示sink仍记录自己的首帧。RTSP运行时将Auto/TCP的watchdog下限分别钳到8/5秒，不回写用户Fact。该通用hook不识别MT11/A8，不改URI、caps、延迟或decoder选择。 |
...
除上述九个已登记文件外，当前二次开发功能没有其他 `src` 差异。两路URL、transport Fact、默认策略、设置UI、receiver识别及第二路编排仍位于 `custom`；核心只提供与产品无关的原生receiver串行生命周期、退避、启动URI快照、transport、基于通用错误的OPTIONS兼容及诊断，不知道A8/MT11或任何产品地址。顶部云台栏自动接管、底部航向罗盘条、Android H.264/H.265优先策略与 USB 飞控连接修复都完全位于 `custom`；根目录 `android/src` 保持原样，Android APK 通过构建目录 overlay 使用 custom Java 实现。
   ...
   -> VideoManager::init()
     ...
     -> GstVideoReceiver::_makeSource(rtsp://...)
        ...
        -> 显式TCP在首帧前符合条件的失败可单向回退Auto；Auto不自定义切到TCP
         -> 同URI OPTIONS EOF按0标准、1基本头部、2跳过OPTIONS逐级兼容；各级保留原生udp-reconnect，跳过级关闭RTSP keepalive
         -> before-send记录RTSP method/兼容级别；进入NULL后teardown flag保持到下一start，仅抑制PAUSE、允许TEARDOWN，teardown-timeout=1s
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - MT11下一轮RTSP日志前置：先彻底退出QGC及其他RTSP客户端，再断电重启MT11并等待其视频服务完成启动；进入设置后逐字符核对URL 2必须精确为 `rtsp://192.168.144.24:8554/video1`，不得保留14:01日志中短暂出现的 `192.168.144.2`。URL 1必须与URL 2不同；产品双路基线保持URL 1 `.25/main.264`、URL 2 `.24/video1`且两路均Auto。保存设置后完整退出并只冷启动一次干净重编译的程序，采集期间不要再编辑URL或transport。11:58日志虽然物理上只连接MT11，应用仍同时运行未连接A8的主receiver；它不再与MT11争抢同一URI，但主路状态通知曾提前触发副路重启。若只做MT11单路隔离，应明确只启用一个指向 `.24/video1` 的receiver；若保留离线A8主路，则日志必须证明Video 2在 `Keeping scheduled secondary video restart ... remainingMs ...` 期间没有被主路事件提前start。
   ...
   - transport切换与停止生命周期验收：切换URL 2 Auto/TCP或在两个非空、非重复URL间修改时，上述“停止/重建”必须表现为stop/start同一 `VideoReceiver`并重建其GStreamer管线，不应触发 `videoObjectsAboutToBeReleased`或新建receiver。只有URL变空/重复、Video Source/全局stream关闭、cleanup或实际surface Item换代才进入receiver释放/新建路径。每个成功start代次最多只产生一次stop completion；清空URI时仍须释放旧pipeline并完成该代次，随后重复stop应记录无活动pipeline而不再次完成。冷启动时同一主URI在首个错误/stop前只能出现一次首次start，不得重现14:01日志的双start。主路连续无媒体失败应只有一条1/2/4/8/15秒有上限退避序列，Video 2仍只有Dual manager的一条1～15秒退避Timer；11:58附件中的8个副路计划延迟全部被提前绕过，修正后除真实URI、启用状态、重复源判定或transport变化外，任何主路active/releasing通知都必须打印保留的 `remainingMs`，且下次start不得早于当前deadline。修改URI、transport或stream状态后，旧generation Timer即使到期也不得start。把活动主URI从 `.24/video1` 交给Video 2时，必须先看到active URI跟踪与stop后1000 ms hold；hold释放前Video 2不得start同一URI，新主同URI恢复则取消清除。不得再出现主/副在旧主管线释放窗口内同时打开 `.24/video1`。
   - RTSP运行包与日志验收：修改 `GstVideoReceiver.cc` 后删除旧桌面构建产物或至少强制重新configure并全量重编译，确认最终可执行文件时间戳来自本轮构建。默认Auto每次重启都应显示 `requestedTransport Auto effectiveTransport Auto effectiveProtocols 0x7`（具体允许位以目标GStreamer回读为准），且不得出现 `TCP (Auto fallback)` 或强制TCP日志。开启TCP优先时依次观察 `Starting ... transport TCP`、`Forcing RTSP-over-TCP ... by configuration`和 `Configured ... requestedTransport TCP effectiveTransport TCP effectiveProtocols 0x4`；若在首帧前报符合条件的资源读错误、source timeout或EOS，必须记录回退原因，下一次重启显示 `requestedTransport TCP effectiveTransport Auto`且不再强制protocols。回退后同URI/设置保持Auto，不得再切回TCP；修改URI或用户transport选择后才清除。每次配置还必须输出 `optionsCompatibility/shortHeader/udpReconnect/rtspKeepAlive`：新URI的首轮为 `0/false/true/true`；精确OPTIONS EOF后下一轮为 `1/true/true/true`；基本OPTIONS仍同样EOF后下一轮为 `2/true/true/false`，该轮OPTIONS必须显示 `suppressed true`，随后应首次看到DESCRIBE。三个级别的udpReconnect均须保持true；transport变化不能把同URI兼容级别降回0，URI变化必须复位。每次实际外发RTSP请求必须有 `RTSP request ... method ... optionsCompatibility ... suppressed ...`，每条bus error critical必须同时含 `uri/source/domain/code/lastRtspMethod/message/debug`。正常停止进入NULL时允许看到PAUSE的 `suppressed true`，即使NULL异步完成且pipeline引用已清除，迟到PAUSE仍须抑制；TEARDOWN必须为 `suppressed false`。分别把持久化 `rtspTimeout` 设为0、1、5、20，Auto实际值不得低于8秒，TCP不得低于5秒，20秒不得被降低。`statusCode 0 success true` 后仍必须继续等到DESCRIBE、SETUP、PLAY、streaming、首个source media frame、实际decoder首输出及sink首帧，不能把兼容级别推进、接口受理、source pad或候选rank当成画面成功。
...
9. Ubuntu 24.04下对MT11做transport分层验收：用SDP/discoverer确认 `rtsp://192.168.144.24:8554/video1` 仍是标准RTSP、H.265 Main、1920×1080、30 fps；先将该URL单独放入URL 1并使用默认Auto，必须与官方QGC一样播放，所有重启都保留原生Auto。随后开启URL 1 TCP优先，确认 `effectiveProtocols=0x4`；若TCP于source pad/首帧前失败，下一次重启应显示requested TCP、effective Auto并恢复原生协商，之后不能反向切回TCP。同时保留A8回归：URL 1默认Auto下须独立成功。Ubuntu/虚拟机代理需将A8的 `192.168.144.25`和MT11的 `192.168.144.24`同时加入忽略列表。
...
当前可确认的基线是：外部工具能从该endpoint识别标准RTSP和H.265 Main 1080p30；用户已在官方QGC验证同一URL可播放。2026-08-19 11:58最新附件是Desktop Debug，不是Android硬解验收。物理上只连接MT11，但应用逻辑上仍同时运行未连接A8的主receiver `.25/main.264`与MT11副receiver `.24/video1`；二者不再请求同一endpoint，因此该日志可以排除上轮同URI交接竞争，却仍暴露离线主路通知干扰副路Timer。必须以同一QGC构建内的精确URL、receiver身份、requested/effective transport、OPTIONS兼容级别、实际RTSP method、结构化bus error、source媒体帧、decoder输出和sink首帧为准。如需保留QGC桌面日志，使用：
...
11:58附件首次给出完整 `before-send/lastRtspMethod` 证据。MT11副receiver共9次start，其中前8轮完整失败，第9轮在程序退出时中止；8轮完整失败全部是：第一次外发 `OPTIONS`，约0.4～0.5秒后第二次外发同一 `OPTIONS`，随后 `GST_RESOURCE_ERROR_READ code 9`、`lastRtspMethod "OPTIONS"`和 `Received end-of-file`。9轮合计17条OPTIONS，`DESCRIBE/SETUP/PLAY/PAUSE/TEARDOWN`均为0，source pad/媒体buffer、decoder实例/输出和sink首帧也均为0。首轮为effective TCP/`0x4`，其余为显式TCP失败后的effective Auto/`0x7`；两者在OPTIONS阶段完全相同，因为 `protocols` 只决定取得SDP后SETUP所允许的RTP/RTCP下层传输，不能改变RTSP控制连接的初始OPTIONS。每轮两条OPTIONS也不是两个receiver：GStreamer `gst_rtspsrc_try_send()` 源码在第一次响应读取为EEOF、尚未interleaved且 `udp-reconnect=true` 时会内部重连并重发同一请求一次，第二次仍EEOF才向bus报告。由此可确定服务端在本程序的初始OPTIONS阶段关闭控制连接；日志尚不能单独确定其拒绝的是method本身还是某个请求头。

GStreamer源码与打包清单提供了一条需要真机抓包验证的兼容性推断：动态Linux安装含 `rtspreal` 的扩展插件时，`short-header=false` 会在标准请求发送前调用RTSP extension链；RealMedia扩展可能在尚未取得服务端响应的首个OPTIONS中追加第二个RealMedia Player User-Agent以及 `ClientChallenge/CompanyID/GUID/RegionData/PlayerStarttime/ClientID` 等旧扩展头。官方/Android静态QGC的插件清单不含对应ugly扩展，这可以解释“官方QGC能播、开发版动态Linux在OPTIONS被关”的差异，但当前附件没有请求报文抓包，不能把“rtspreal一定已加载或这些头一定触发MT11断开”写成事实。另一个仍可能成立的情况是MT11固件本身不接受OPTIONS。当前通用修正因此不依赖设备型号：同URI标准OPTIONS精确EOF后先启用 `short-header=true`去掉扩展头，同时保留GStreamer原生 `udp-reconnect`；基本OPTIONS仍精确EOF后再由before-send跳过OPTIONS、利用rtspsrc的mandatory-method fallback继续DESCRIBE，并关闭RTSP keepalive以免播放后重新发OPTIONS。保留 `udp-reconnect` 是为了不牺牲成功UDP媒体会话后的控制连接恢复能力，而非依赖它解决初始OPTIONS兼容问题。

较早附件缺少精确GStreamer错误，是因为旧 `QGCLogging::msgHandler` 对debug、info、warning、critical一律调用 `isDebugEnabled()`；当前代码已改为 `isEnabled(type)`。11:58附件证明before-send与结构化bus error已进入实际二进制，也证明14:01之后追加的唯一首次start和starting/active/releasing URI交接保护生效：本轮没有重复首启或主/副同URI并发。停止进入NULL时仍只抑制rtspsrc自动PAUSE、允许TEARDOWN；新修正把teardown标志保持到下一次start，避免 `set_state(NULL)`异步完成、pipeline引用已清除后迟到PAUSE漏发。MT11 SDK每约2秒出现的invalid ACK属于独立UDP控制链路，不能用于判断RTSP是否到达SDP。

11:58附件同时暴露Video 2退避绕过：8次失败分别计划2、4、8、15、15、15、15、15秒重启，但实际约0.548、2.322、0.171、1.452、2.119、0.835、6.120、2.621秒后就再次start，没有一次等到deadline。触发点均与离线A8主路的start/error或1000 ms releasing guard通知重合。当前修正使 `_applyDesiredState()` 在 `_restartTimer` 活动时只打印剩余时间并返回；只有secondary URI、启用状态、重复判定或transport等真实配置变化才显式取消Timer。该修正控制请求压力和生命周期确定性，但第一轮干净OPTIONS本身也独立失败，因此不能把退避绕过当作唯一协议根因。
...
2. source/transport层：每次RTSP启动都必须出现 `Starting RTSP receiver ... requestedTimeout ... effectiveTimeout ...` 和 `Configured RTSP source ... requestedTransport ... effectiveTransport ... effectiveProtocols ... teardownTimeoutMs ... optionsCompatibility ... shortHeader ... udpReconnect ... rtspKeepAlive ...`。冷启动首个终止事件前同一主URI只能有一个首次start；连续失败后的主路间隔必须为1/2/4/8/15秒封顶，URL、transport、stream状态或新start代次变化后旧generation回调不能再启动。只有用户显式选择TCP的首轮才应出现 `Forcing RTSP-over-TCP transport for ... by configuration`；TCP失败回退Auto后不再出现该行。TCP下effectiveProtocols必须等于TCP-only标志 `0x4`；Auto每轮都应保留rtspsrc默认允许集合。逐条检查OPTIONS自适应：级别0精确EOF后下一轮为级别1基本头部且 `udpReconnect=true`，级别1再次同类EOF后下一轮为级别2并抑制OPTIONS，且该级仍须保持 `udpReconnect=true`、只把 `rtspKeepAlive` 置为false，紧接着必须出现DESCRIBE；其他error、非OPTIONS method或已有source frame不得推进该状态。同URI切换Auto/TCP保留级别，换URI复位。逐条检查 `RTSP request` 的method、compatibility和suppressed：进入NULL后迟到PAUSE仍可被抑制，TEARDOWN必须继续允许。用Wireshark过滤 `ip.addr == 192.168.144.24 && (rtsp || rtp || rtcp || tcp.port == 8554)` 对比本程序级别0/1/2与官方QGC工作会话的实际请求头，确认动态Linux是否加载rtspreal扩展；进入SETUP后再证明Auto是否有UDP RTP、TCP是否持续出现interleaved媒体，并检查TEARDOWN/session释放时序。
...
11:58最新QGC日志已经把MT11失败精确到初始OPTIONS，并证明唯一首次start和同URI交接保护生效；它也证明Video 2原有退避可被离线主路通知绕过。该附件采集时尚未包含“标准 -> 基本头部 -> 跳过OPTIONS”两级兼容、被动通知保留副路deadline及teardown flag延长，因此仍没有DESCRIBE、SETUP、PLAY或实际画面成功证据。下一轮必须先重启MT11、确认URL 2精确为 `.24/video1`并冷启动干净构建，检查兼容级别按条件单向推进且最终进入DESCRIBE，同时验证副路每次实际start不早于已记录deadline；随后再分别验证URL 1单路MT11、URL 2单路MT11、A8+MT11双路、Auto/显式TCP回退、停止释放、PIP跨窗口和断流重连。不能用静态检查、兼容级别推进、接口 `success true` 或单独 `gst-launch` 成功宣称整机验收通过。
...
截至2026-08-18，本轮已独立编译并运行 `Mt11ProtocolTest`，QtTest报告9 passed、0 failed：7个业务slot加框架init/cleanup，覆盖SDK文档命令帧、RGB/热成像模式帧、严格CRC/长度/control解析、UDP合包原子性和业务payload。该结果仅属于纯 `Mt11Protocol`，不覆盖 `Mt11Sdk`真实UDP、Manager计时、QML、GStreamer、双receiver或真机固件。MT11 `rtsp://192.168.144.24:8554/video1` 已确认为H.265 Main 1920×1080@30 fps的标准RTSP endpoint。当前Video 1/Video 2分别使用 `[Video]/rtspUrl` 与 `[Video]/secondaryRtspUrl`，新安装两路都默认Auto并始终保留 `rtspsrc` 原生协商；只有兼容“优先TCP”键为true时，首帧前符合条件的失败才单向回退Auto。core为成功start代次去重stop completion、清空URI时仍清理旧pipeline，`before-send`记录method/OPTIONS兼容级别；同URI标准OPTIONS精确EOF后依次尝试基本头部、必要时跳过OPTIONS并关闭keepalive继续DESCRIBE。进入NULL后teardown flag保持到下一start，只抑制PAUSE、允许TEARDOWN，`teardown-timeout` 为1秒。通用 `VideoManager` 保证原生主路唯一首次start，并用generation约束1/2/4/8/15秒RTSP退避；custom第二receiver保持1～15秒退避，主路被动通知不得绕过已有deadline，且以primary active URI和stop后至少1000 ms hold保护同endpoint交接。`QGCLogging`按真实消息级别过滤，GStreamer日志同时给出transport、OPTIONS compatibility、`teardownTimeoutMs 1000`、`lastRtspMethod`、结构化错误、实际decoder创建/首输出和sink首帧。Android沿用旧Fact键但UI表示H.264/H.265硬解优先：H.264 direct avc与H.265 direct hvc1均为rank259，H.265 adapter为rank258，软件rank保留。`DualPipView`保留固定槽精确交换、原生PIP显隐、独立窗口与拖拽resize。两份TS均为18个context、146条message且context/source一致；英文模板146条保持unfinished，简体中文146条全部finished、unfinished/空译文均为0，`lrelease`验证通过。

11:58最新附件日志的确定结论是：它来自Desktop而非Android；物理上只连接MT11，但应用仍同时运行未连接A8的 `.25/main.264` 主receiver和MT11 `.24/video1` 副receiver，二者没有同URI并发。MT11共9次start、8次完整 `gst_rtspsrc_try_send` EOF和1次退出中止；17条外发请求全部为OPTIONS，8条bus error全部为 `lastRtspMethod OPTIONS`，DESCRIBE/SETUP/PLAY及source媒体buffer、decoder输出、sink首帧均为0。每轮第二条OPTIONS是GStreamer在首个EEOF后的内部 `udp-reconnect`重发，不是另一个receiver。首轮effective TCP/`0x4`与后续Auto/`0x7`结果相同，进一步排除RTP transport、H.265、MediaCodec、QML或双路同步作为当前阶段根因。日志还证明8个Video 2计划退避全部被A8主路被动通知提前绕过。基本头部/跳过OPTIONS兼容、副路deadline门禁和teardown flag延长均是在该附件之后追加，尚无真机成功日志。

当前开发环境仍未完成这次通用双视频与OPTIONS兼容修正后的全工程Qt 6桌面/Android干净构建，因此不能把静态检查扩大为“MT11或QGC双路已验收”。下一轮先关闭全部客户端、断电重启MT11、确认URL 2精确为 `.24/video1`且两路Auto，再冷启动一次干净桌面程序/APK。日志必须证明：同URI唯一首次start；主路失败间隔按1/2/4/8/15秒封顶且旧generation不复活；Video 2的已计划deadline不被主路被动通知提前绕过；主/副同URI交接先hold active URI至少1000 ms；标准OPTIONS失败后按条件推进到基本头部，若仍失败再抑制OPTIONS并实际出现DESCRIBE、SETUP和PLAY；停止时NULL后的迟到PAUSE仍可抑制但TEARDOWN允许；随后依次得到source媒体帧、实际decoder首输出和sink首帧。完整矩阵仍包括MT11在URL 1 Auto单路、MT11在URL 2 Auto单路、显式TCP失败后的Auto fallback、A8 URL 1 Auto + MT11 URL 2 Auto同时显示、两路transport独立切换、stop代次与TEARDOWN释放、Video 2 Item/window重挂载、连续PIP交换/拖拽及断流重连。Android双路必须分别看到不同receiver/decoder instance的实际decoder首输出，再看到各自sink首帧；`vendor MediaCodec candidate`、rank或adapter注册都不能单独证明硬解或双实例成功。当前本地媒体仍映射Video 1 -> A8、Video 2 -> MT11；录像/媒体发布期间修改URL 2、URL 2 transport、Video Source或触发重复URL保护时，同步DirectConnection释放路径可能短时阻塞UI，该风险未改为异步状态机。A8+MT11真实ACK时序、RGB/热成像往返、双路长期解码/重连、三视图/全屏与叠加层、两套本地媒体receiver隔离、Android性能及媒体发布、SDK/RTSP各自故障和退出收尾仍须按本章矩阵在目标设备验收。
