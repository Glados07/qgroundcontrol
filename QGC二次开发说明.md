...
- 在 `custom` 中实现思翼私有 UDP SDK 和协议封装，包括帧组装、CRC16、固定为0的协议sequence字段，以及0x05、0x0a、0x0b、0x0c、0x0f、0x16和0x18。0x05只保留协议兼容接口，当前Manager/UI不调用；0x0f执行绝对倍率，0x16读取设备当前配置真正支持的倍率上限，0x18读取实际倍率。0x0f的一字节ACK只表示受理或拒绝，不能更新显示；由于现有设备不递增sequence，最终结果始终以串行0x18实际回读为准。Manager对用户目标保留单目标/单查询在途、350 ms旧包隔离和两次目标命中。
- A8 Mini数字变焦的物理上限优先由QGC实际解码拉流尺寸确认。GStreamer路径有两条互相独立的首选可信来源：custom在主视频 `qgcvideosinkbin` 的sink pad等待真实首帧，并依次检查外层pad、已连接解码器peer及ghost target的current CAPS；同时观察 `GstGLQt6VideoItem` 在最终 `GstVideoInfo` 生效后写入的 `implicitWidth/implicitHeight`。任一来源得到受支持尺寸都会直接投递Manager，pad路径还会修正原生 `VideoManager::videoSize`。两个custom观察器都未回调时，已解码且连续稳定1秒的 `VideoManager::videoSize`作为第二级后备。尺寸物理上限映射为1280×720（及coded-height 736）→6.0x、1920×1080（及1088）→5.5x、2560×1440→3.5x、3840/4096×2160→1.0x。Manager同时每2秒查询0x16，并以“拉流物理上限与设备当前能力的较小值”为安全上限，既保证上限来自实际拉流，又避免拉流为1080P而相机当前配置只接受1.0x时首个2.0x命令被持续拒绝。尺寸未知时0x16可单独恢复能力；其他拉流尺寸不作猜测。短暂断流保留最近确认的拉流能力，新的受支持尺寸会替换旧值，新的不支持尺寸会释放旧映射并由0x16恢复。
- `zoomStep`网格固定锚定1.0x，合法目标统一为 `1.0 + n × zoomStep`，协议和显示按0.1x量化。可用最大倍率是“不超过上述安全上限的最高完整网格点”：步长1.0x时，720P/1080P/2K/4K分别为6.0x/5.0x/3.0x/1.0x；步长0.5x时1080P和2K可分别到5.5x和3.5x。不存在小于配置步长的末端吸附，因此正常命令目标及确认后的显示一定在同一网格上。
- `currentZoom`只保存经过协议校验和状态机确认的0x18设备回读，不在0x0f发送成功或ACK成功后乐观写入请求目标。0x16/0x18逐包优先按新版“整数字节+一位小数字节”解析；若该候选超出A8协议范围，再兼容真机固件使用的小端uint16/10格式，例如 `01 00`与 `0a 00`都可表示1.0x。采用兼容格式时日志记录 `encoding little-endian tenths compatibility`，不会因某一命令的格式锁定整个会话。启动、重连、能力更新及外部倍率变化要求两次一致0x18后发布；用户目标要求两次0x18命中目标。目标pending期间保留并显示上一份已确认实际值，同时追加省略号，绝不把requested值冒充实际倍率。
- `GimbalControlManager` 直接接收上述主显示sink的协商尺寸；GStreamer构建还监听 `VideoManager::videoSizeChanged/decodingChanged`，但只用于无直接结果时的1秒稳定兜底，非GStreamer构建则直接使用这两个原生状态。绝对目标由可恢复状态机接管：正常过程值继续查询；连续两次目标值完成；相机稳定停在另一值时有限样本内退出pending；负ACK立即进入实际值核对；非网格稳定值最多发送两次归整命令。若相机仍拒绝归整，则显示真实0x18值并恢复至少一个安全操作方向，不伪造网格值也不永久锁控件。单次查询1秒无响应会重试；10秒整体超时会恢复操作前的最后确认值并后台重读，而不是清空已有倍率、无限保持unknown。该探测不依赖控件可见或活动飞行器。接收端以 `QHostAddress::isEqual()`比较来源逻辑IP，并要求精确帧长、CRC及ACK control严格等于0x02；只有业务payload也合法的回包才置 `sdkResponding=true`。
- 飞行界面右侧使用单个始终可见的纵向半透明控制栏：只要Gimbal功能已启用，无论是否连接飞控或云台都会显示；从上到下依次为放大、当前倍率、缩小、横向分隔线、思翼机内拍照和录像。离线、能力未知或初次倍率尚未确认时显示 `--`；用户目标pending时短暂禁用两个方向以防命令重叠，但中心继续显示最后确认的 `x…`，目标完成、拒绝、稳定未达或总超时均有明确恢复出口。到网格上限或1.0x下限时只禁用没有下一目标的方向。4K安全上限为1.0x时两个方向都不可用是设备无可用数字变焦范围，不属于状态机锁死。
...
- Gimbal关闭时才在存在活动飞行器的前提下回退QGC原生 `PhotoVideoControl`。缩放手势统一为Idle/Pressed/Holding/Consumed状态：短按发送一个0x0f目标；长按也不使用自由运行的0x05，而是在上一0x0f经过两次0x18目标确认后串行发送下一完整网格目标。松开、取消、移出或外部安全停止只取消尚未发送的后续步骤，已经发出的唯一目标继续核对，不发送0x05停止包，也不会在释放后补发短按。保留60秒长按看门狗。
- 在Fly View设置页以 `SIYI Gimbal Camera` 标题提供云台相机启用、SDK IP、SDK端口和缩放步长设置，并明确短按发送一个完整分度、长按重复已确认目标、所有正常目标位于 `1.0 + n × zoomStep` 网格；实际拉流尺寸提供物理上限，0x16持续提供设备当前能力安全限制。
...
| `custom/src/FlightDisplay/GimbalZoomControl.qml` | 合并栏顶部的思翼缩放子控件，使用单列GridLayout从上到下显示加号、当前倍率和减号，并把完整列高传给外层布局。从Manager读取 `currentZoom/zoomStatusKnown/zoomCommandPending/zoomValueUncertain/sdkResponding/zoomInAvailable/zoomOutAvailable`；离线或初次倍率未知时显示 `--`，命令pending时保留最后确认倍率并显示 `x…`，总超时解除pending后以 `x?` 表示后台仍在重新核对，不显示requested目标；一个目标核对期间短暂禁用两方向以防叠加，到边界后只禁用没有下一网格目标的方向。两侧MouseArea都使用Idle/Pressed/Holding/Consumed手势状态：短按只调用一次目标接口，长按启动串行完整分度重复，释放只取消后续步骤且不补发短按。后台2秒探测已移到Manager。 |
...
| 文件 | 详细作用 |
|---|---|
| `custom/src/Gimbal/A8MiniZoomPolicy.h` | A8 Mini缩放策略的纯静态接口，将视频尺寸映射、有效网格上限、严格步进和目标观察器从Manager状态机中分离，便于独立测试；不访问网络、QSettings或UI。`TargetTracker`区分继续等待、目标双命中和稳定停在其他倍率三种结果，使Manager可在命令未达时有限恢复。 |
| `custom/src/Gimbal/A8MiniZoomPolicy.cc` | 显式映射A8 Mini拉流规格：1280×720/736→6.0x、1920×1080/1088→5.5x、2560×1440→3.5x、3840/4096×2160→1.0x（4K不可继续缩放），其他尺寸返回不支持。倍率计算使用十分之一整数并固定锚定1.0x；有效上限取不超过物理/设备安全上限的最高完整网格点，所有正常目标严格相差完整 `zoomStep`。若硬件最终留下非网格真实值，人工操作只去该方向最近合法网格点，不会从1.8x继续计算2.8x漂移网格。 |
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | `GimbalControl` 设置组的元数据源，不执行任何云台或视频操作。定义6个Fact及约束：`enabled=true`（短描述为启用思翼云台相机控制）、数值IP `sdkHost=192.168.144.25`、`sdkPort=37260`（1-65535）、`zoomStep=1.0x`（0.1-4.5）、`mavlinkAutoVideoStream=false`、`forceAndroidH265HardwareDecoder=true`；后两项标记需重启。`zoomStep`说明明确合法目标为 `1.0 + n × zoomStep`，有效最大值是物理上限以下的最高完整分度；步长1.0x且物理上限为5.5x/3.5x时，有效上限分别为5.0x/3.0x。`SettingsFact` 将用户值自动保存为 `[GimbalControl]/同名键`；硬解开关虽在所有平台存在，策略只在Android+GStreamer路径执行。 |
...
| `custom/src/Gimbal/GimbalControlManager.h` | 思翼云台相机业务的QML门面和运行态声明。`currentZoom`只表示设备确认值，另向QML暴露按固定网格计算的 `zoomInAvailable/zoomOutAvailable`、只读 `zoomCommandPending`及超时后非阻塞核对标记 `zoomValueUncertain`。能力状态分别保存拉流物理上限、0x16设备当前上限和两者交集后的完整分度上限；倍率状态保存唯一绝对目标、可恢复目标观察器、最多两次归整计数、归整失败抑制键、稳定值候选、查询在途门禁、350 ms隔离Timer、1秒单次查询Timer和10秒操作Timer。默认值仅是能力未知时的内部占位，不能用于发送缩放命令。 |
| `custom/src/Gimbal/GimbalControlManager.cc` | 由 `CustomPlugin`创建一次并持有 `SiyiSdk`，没有修改或复制 `src/VideoManager`。GStreamer构建按“主显示sink直接尺寸→稳定1秒的VideoManager尺寸”确认拉流物理上限，同时始终轮询0x16并取设备当前能力的较小值，尺寸未知时0x16可独立恢复。能力或步长变化会计算最高完整网格上限并重新同步0x18。启动、重连和外部变化要求两次稳定0x18，用户0x0f目标要求两次目标命中；稳定未达、负ACK、非网格值和整体超时均有有限恢复路径。pending期间保留上一实际显示；若硬件拒绝两次网格归整，则公开真实回读并恢复可操作方向。单击和长按共用串行0x0f目标路径，一个目标在途时拒绝追加；状态机保证同一时刻最多一个0x18查询。 |
...
| `custom/test/Gimbal/SiyiProtocolTest.cc` | custom独立QtTest协议与策略回归用例。覆盖封包量化、ACK control、坏CRC、严格整帧长度、0x05/0x0f/0x16/0x18新版与真机旧版倍率payload、越界和非法载荷、720P/1080P/2K/4K及coded-height映射、完整分度上限、严格双向步进、非网格安全恢复，以及“目标命中/稳定未达/负ACK”观察器状态。测试只在桌面 `QGC_BUILD_TESTING`注册，并提供 `check_siyi_protocol`独立目标。 |

云台相机完整调用链为：`VideoReceiver`解码当前视频 -> 真实首帧CAPS或最终 `GstVideoInfo` 隐式尺寸 -> 无直接结果时使用稳定1秒的 `VideoManager::videoSize` -> 得到拉流物理上限 -> 与持续轮询的0x16设备当前上限取较小值 -> `A8MiniZoomPolicy`向下对齐到完整步长网格 -> `GimbalControlManager`查询0x18并以两份一致实际倍率恢复显示 -> 单击/长按发送串行0x0f目标 -> 两次0x18命中则发布；拒绝、稳定未达或超时则恢复真实值和控件。
...
| `zoomStep` | 0.1-4.5 / `1.0x` | 短按一次及长按重复一次的完整网格分度。合法目标为 `1.0 + n × zoomStep`；可用上限是不超过拉流/设备安全上限的最高完整网格点。步长1.0x且物理上限为5.5x/3.5x时，有效上限分别为5.0x/3.0x，不发送半步目标。 |
...
Gimbal启用后，Manager即使不在Fly View也会持续运行，并每2秒向 `sdkHost:sdkPort`探测：GStreamer构建优先采用主显示sink直接报告的最终协商尺寸，仅在没有任何有效直接结果时用 `VideoManager::videoSizeChanged/decodingChanged` 启动1秒稳定兜底；非GStreamer构建直接使用这两个原生状态。每轮始终发送0x16查询设备当前支持缩放范围，并发送0x0a相机状态；拉流尺寸已知时0x16只会收紧安全上限，尺寸未知时可独立恢复能力。取得任一有效能力后才开始查询0x18当前倍率。绝对目标和稳定确认由专用状态机管理：先留350 ms隔离窗口，再发送唯一在途的0x18；1秒未收到合法倍率才安排下一次隔离重试，10秒整体deadline不会被重试刷新。Fly View右侧纵向合并栏不等待探测结果，无论是否连接飞控或云台都立即显示。只有来源逻辑IP匹配配置endpoint、通过帧头、精确长度、CRC、control=0x02及对应payload校验的SDK响应，`sdkResponding`才变为true。

拉流物理上限取QGC实际解码尺寸：1280×720/736→6.0x、1920×1080/1088→5.5x、2560×1440→3.5x、3840/4096×2160→1.0x且不可继续数字变焦。GStreamer主视频在真实首帧到达时从sink、解码器peer或ghost target的current CAPS读取宽高，同时以主 `GstGLQt6VideoItem` 根据最终 `GstVideoInfo` 设置的隐式尺寸作为第二条直接来源。两条路径任一取得受支持尺寸就直接交给Manager，并由pad路径同步校正 `VideoManager::videoSize`。若两条直接路径都没有报告有效尺寸，Manager仅在原生视频状态已解码、尺寸属于白名单且连续稳定1秒时以 `VideoManager`兜底；有效直接尺寸会立即取消该兜底。0x16表示相机当前配置实际允许的范围，最终安全上限取其与拉流物理上限的较小值；若拉流尺寸尚未确认，则合法0x16可单独恢复。短暂断流不清空拉流能力；新的不支持直接尺寸会释放旧尺寸映射但继续使用0x16。安全上限还会向下对齐到 `1.0 + n × zoomStep` 完整网格；每次能力或步长改变均重新同步0x18。普通SDK在线探测使用1.5秒响应窗口；正在进行的倍率确认可在10秒整体deadline内重试。拍照仍可重试，录像必须先取得0x0a状态，因为0x0c是toggle。

短按从已确认的 `currentZoom` 计算唯一下一目标：在1.0x锚定网格内部按0.1x协议精度执行完整的 `current ± zoomStep`；下一完整步超过安全上限时，该方向没有目标，不通过通用钳制制造半步。发送后保留上一份确认显示并追加 `…`，0x0f ACK只表示受理或拒绝；两个串行0x18都命中目标后才更新 `currentZoom`。运动中间值继续查询；若连续样本证明相机稳定停在其他倍率，则立即结束本次pending并按实际值恢复。负ACK会加速实际值核对；非网格稳定值最多尝试两次最近完整网格归整，仍失败时显示真实值并保留朝合法网格移动的方向，避免自动校正死循环。长按复用相同目标路径：上一目标双确认后等待180 ms再计算下一完整网格步；松开、取消、移出、隐藏、销毁、设置变化、SDK异常或60秒看门狗只取消后续调度，已发送目标继续核对，不发送0x05启动或停止包。10秒整体超时保留操作前最后确认倍率、清除pending并后台重读，不会让两个按钮永久锁住。0x0c拍照/录像不带直接ACK：照片由0x0b确认；录像约400 ms查询0x0a并仅接受目标一致状态，2.5秒无结果标记unknown。
...
2. Application Settings -> Fly View -> SIYI Gimbal Camera 中确认IP、端口、缩放分度值和Enabled。页面标题说明保持简短，以免撑坏设置页布局；短按/长按、完整步长网格、拉流物理上限与0x16设备安全上限等完整规则以本节和Fact元数据为准。
3. Application Settings -> Video -> Video Stream Integration 中选择是否使用 MAVLink 自动视频流；Android 设备确认 H.265 硬解开关开启，然后重启 QGC。
4. 返回Fly View。只要Gimbal Enabled，右侧单个纵向合并栏就应立即显示，不要求飞控或云台已连接；从上到下依次为 `+`、倍率、`-`、拍照和录像。尚未取得有效能力和倍率时倍率显示 `--`且缩放按钮禁用。视频第一帧到达后，正常直接路径应依次看到“Installed main pulled-video resolution observers”、negotiated/observed主拉流尺寸、“Latched pulled video resolution”和命令22/24（0x16/0x18）日志；运行期切换尺寸时应出现“Updated pulled video resolution”。若直接观察器未回调但原生尺寸有效，则先看到“Waiting for stable VideoManager pulled-video resolution fallback”，约1秒后source为“stable VideoManager fallback”；若尺寸链仍无结果，0x16应单独激活能力。日志 `Activated SIYI zoom capability` 同时给出raw maximum、step-aligned maximum和step。第二次一致0x18后应出现“zoom controls are ready”，中心显示实际倍率，并仅点亮仍有合法网格目标的方向。步长1.0x时，720P/1080P/2K/4K的分辨率网格上限分别为6.0x/5.0x/3.0x/1.0x；设备0x16更低时以更低完整网格点为准。录像按钮在0x0a状态未知时灰显并显示 `...`。
5. 短按 `+`/`-` 只发送一个完整步长目标。步长1.0x时，1080P序列为1→2→3→4→5及5→4→3→2→1，2K为1→2→3及3→2→1；若步长0.5x，则可到5.5x/3.5x。一个目标确认前按钮短暂禁用，中心保留上一实际倍率并显示 `…`，快速连点不会追加或累计。按住约420 ms后按相同规则串行重复，每个目标需两个0x18命中；松手或移出只停止下一目标，长按不附带短按，也不应抓到0x05缩放包。模拟负ACK、连续稳定旧值或丢包时，pending必须经实际核对或10秒总超时自动结束，不能永久锁定。
  ...
  -> CustomPlugin::init()
     ...
     -> FlyViewCustomSettings（FlyView/showHeadingCompassBar）
      -> GimbalControlSettings / GimbalControlManager
         ...
         -> 每2秒查询0x16当前支持范围；尺寸已知时作为安全收紧，未知时可恢复能力
         -> 拉流映射：720P→6.0x、1080P→5.5x、2K→3.5x、4K→1.0x
         -> 拉流物理上限与0x16设备能力取较小值，再向下对齐完整zoomStep网格
         -> 每轮发送0x0a状态；取得能力后，缩放空闲时发送0x18
         -> 新拉流尺寸可安全更新，短暂断流保留旧拉流能力
         -> 来源逻辑IP、精确帧长、CRC、control=0x02及payload均合法才置sdkResponding
         -> 一个0x0f目标在途；两个串行0x18都命中才发布current，pending显示上一实际值…
         -> 短按和长按均串行完整zoomStep；拒绝/未达/超时均有限恢复，不永久pending
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   - 验证实际拉流尺寸来源：1280×720/736、1920×1080/1088、2560×1440、3840/4096×2160必须分别确认6.0x、5.5x、3.5x、1.0x物理上限；步长1.0x且0x16不更低时，对外网格上限分别为6.0x、5.0x、3.0x、1.0x。先验证任一直接观察器报告尺寸时立即取消 `VideoManager`兜底；再模拟两个直接观察器均不回调，要求 `decoding=true`且同一受支持 `VideoManager::videoSize`连续稳定1秒后才采用，尺寸在1秒内变化时必须重新计时。短暂停止解码应保留已确认能力但清除旧直接尺寸标记；重连后即使新管线的直接观察器失效，稳定1秒的VideoManager新尺寸也必须能替换旧上限。同一QGC进程内依次执行4K→1080P和1080P→4K，必须更新并重新双确认，不得要求重启。新的640×480或1366×768直接协商尺寸应释放旧映射并使用0x16，空尺寸不应误清空能力。
   - 无论拉流尺寸是否已确认，抓包都应看到Manager每2秒发送0x16。尺寸未知时合法范围可单独激活能力；尺寸已知时最终raw安全上限取拉流映射和0x16的较小值，再对齐到最高完整分度。0x16 `00 00`、超过6.0x、错误小数、错误长度/CRC/control必须拒绝。同一完整网格内的raw抖动（例如步长1.0x时5.4/5.5）只能更新内部raw值，不得取消用户pending、打断长按或把倍率重新置unknown；跨越网格点时才重新同步。相机当前配置若使0x16降为1.0x，即使拉流为1080P也必须安全收紧为1.0x，而不是重复发送硬件会拒绝的2.0x。
   ...
   - `-`/`+`短按只发送一次目标0x0f，释放不得重复触发。步长1.0x且raw上限为5.5x时严格覆盖1→2→3→4→5及5→4→3→2→1；raw上限为3.5x时覆盖1→2→3及3→2→1；步长0.5x才允许5.5x和3.5x。1.8→2.8、2.8→1.8等漂移目标不得发送；若自动归整最终失败，从1.8x人工放大/缩小只能去2.0x/1.0x。快速点击 `+++`在首目标确认前不得追加第二个0x0f；pending期间显示最后确认实际倍率和省略号，不显示requested值。
   - 按住420 ms后抓包应看到当前序列的一个0x0f目标；只有两个串行0x18查询都命中该目标后，才允许在180 ms后发送下一完整网格步。松开、取消、移出、控件隐藏/销毁、SDK异常、endpoint/Enabled变化和应用退出只取消后续目标，不发送任何0x05方向或停止包，也不得补发短按。60秒看门狗必须兜底。
   - 验证查询串行化和失败恢复：0x0f后先有约350 ms旧包隔离，同一时刻最多一个0x18查询在途；单次查询1秒超时后重新隔离再查，10秒整体deadline不被重试刷新。构造“1.8、1.8、2.0、2.0”回包，目标2.0时前两包不得发布，只有后两个独立目标命中才显示2.0。构造目标2.0后连续三次稳定1.0，必须结束pending、恢复1.0显示和放大按钮；负0x0f ACK后两份稳定实际值也必须恢复。非网格值最多归整两次；硬件持续拒绝后必须锁存“暂不自动归整”，后续空闲2秒轮询不能周期性重锁，直到实际值、步长、有效上限变化或用户主动操作。另需覆盖：`zoomValueUncertain`已经为true时再次目标超时仍能解除视觉锁；稳定确认第二包持续丢失时10秒后清除stablePending；超上限校正首轮即超时时后续轮询不再重发；0x16在同一网格内抖动期间长按不中断。任何发送失败都必须保持communicationError设置的离线/unknown状态，不能被恢复分支重新置known。
...
视频有画面且SDK已有0x0a回包、但缩放按钮仍灰色时，分别检查拉流能力和设备能力日志。拉流直接路径应出现 `Installed main pulled-video resolution observers`，随后出现 `Negotiated main pulled-video resolution` 或 `Observed negotiated main pulled-video resolution`；若两路直接观察器均未报告，则应出现 `Waiting for stable VideoManager pulled-video resolution fallback`，约1秒后以 `stable VideoManager fallback`为source确认。无论尺寸是否已知，`gcs.custom.gimbal.sdk`每2秒都应发出命令22（0x16）；尺寸未知时它可单独恢复能力，尺寸已知时它与拉流物理上限取较小值。`Activated SIYI zoom capability`会同时打印raw maximum、step-aligned maximum和step；首次尺寸记录 `Latched pulled video resolution`，运行期变化记录 `Updated pulled video resolution`。能力激活后应在350 ms隔离后发出命令24（0x18），收到两次一致合法0x18后出现 `Confirmed stable SIYI zoom ... zoom controls are ready`。若0x16有发送却无合法响应，检查设备固件和原始回包；若命令24不能稳定确认，检查0x18 payload与UDP丢包。

拉流尺寸白名单为1280×720/736、1920×1080/1088、2560×1440及3840/4096×2160，对应6.0x、5.5x、3.5x及1.0x物理上限。实际对外上限还必须不超过0x16并落在完整步长网格；默认步长1.0x时，前述拉流规格在0x16不更低的情况下对应6.0x、5.0x、3.0x、1.0x。未到边界时至少一个方向应可用；1.0x时减号灰显、有效上限时加号灰显是正确边界。4K或0x16当前能力为1.0x时两个方向都灰显是无可用数字变焦范围，不属于状态机锁死。若设置页出现分组拉伸、顶部裁切或无法滚到底，检查 `FlyViewSettings.qml` 外层Loader的 `Layout.preferredHeight/minimumHeight`、两个设置组根Loader的 `implicitHeight` 透传，以及是否重新引入了用长说明撑大隐式宽度或手工写 `item.width` 的逻辑。

初始镜头1.0x却显示错误、实际与显示不一致或缩放按钮始终灰色时，先查看0x18原始payload。新版 `01 00`表示1.0x、`01 08`表示1.8x；A8真机旧版会返回 `0A 00`表示1.0x、`12 00`表示1.8x。日志若持续出现 `Rejected invalid SIYI current zoom payload "0a 00"`，说明运行的仍是未加入兼容解析的旧构建；新实现应记录 `Decoded SIYI current zoom 1 encoding little-endian tenths compatibility`，并在稳定确认后记录 `zoom controls are ready`。0x0f ACK与requested目标都不能直接成为显示值；目标pending时显示上一份实际值加 `…`，10秒总超时后显示 `?`直到后台再次确认，不能把未知的新实际值伪装成旧值。拉流物理上限720P/1080P/2K/4K分别为6.0x/5.5x/3.5x/1.0x，最终上限取它与持续0x16设备能力的较小值并向下对齐完整分度；步长1.0x时不会发送5.5x或3.5x半步。启动或外部控制留下非网格倍率时最多自动归整两次；若硬件拒绝，显示真实0x18值并停止后台重复归整，用户仍可向最近合法网格点操作。点击后若出现 `Started SIYI absolute zoom target 2`，但0x18持续稳定为1.0，状态机应在有限样本内记录“target was rejected/did not settle”、恢复1.0和放大按钮；不应等待无限循环。长按日志只包含串行的“0x0f目标、0x18查询、0x18查询”，释放不应出现0x05。紧邻应用退出出现的 `PhotoVideoControl.qml`中 `cameraManager/currentCameraInstance`空对象警告来自QGC原生相机面板销毁时序，不参与custom缩放状态机，也不是本次锁定原因。
