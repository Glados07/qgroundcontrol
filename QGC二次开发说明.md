...
- 当前代码已完成扩展后的两套独立QtTest编译运行：`Mt11ProtocolTest`为11 passed、0 failed（9个业务槽加init/cleanup），`SiyiProtocolTest`为42 passed、0 failed（40个业务槽加init/cleanup）；`Mt11Sdk.cc`也已独立编译通过，Manager头文件moc成功且diff-check无错误。MT11纯策略测试覆盖1～30x短按网格、step 2.0的1/3/…/29/30精确端点及对称返回、30.0/30.1协议门禁、0x18实测对齐和165.1物理端点显示边界；默认1.0步长拒绝把物理165.1发布为Target，0.1步长允许165.1。该结果仍只证明Protocol/Policy和接口静态范围，不覆盖Manager的100 ms反馈、150 ms handoff/安全停止Timer、完整QGC Qt 6构建或真机原生连续运动。
...
| `custom/test/Gimbal/Mt11ProtocolTest.cc` | MT11纯协议与策略QtTest，共9个业务slot；覆盖帧、CRC、0x05/0x0F/0x16/0x18倍率格式、1～30x tap门禁、step 2.0的1/3/…/29/30精确边界及对称返回、Target对齐和165.1物理端点规则。当前11 passed、0 failed（含init/cleanup），Manager moc成功、diff-check无错误；仍不覆盖Manager的100/150 ms Timer、真实UDP或真机原生连续运动。 |
...
| `custom/src/UI/AppSettings/GimbalControlSettingsGroup.qml` | 根节点直接为 `ColumnLayout`，组内依次显示A8/MT11独立Zoom Step与两套SDK设置。MT11的 `mt11ZoomStep`决定1～30x tap目标和hold期间Target参考，不控制单次原生0x05的物理速度；页面不依赖设备在线，也不把SDK地址同步成RTSP URL。 |
...
| `custom/translations/custom.ts` | 英文源字符串模板。保留 `GimbalZoomControl` 的Target/Actual两条，删除已不再存在的逐档0x18超时字符串，并同步改写MT11 step longDesc；当前19个context、148条message和141个唯一source，148条均unfinished。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录，与英文模板保持相同的19个context、148条message和141个唯一source；148条均finished，unfinished和空译文为0。保留“目标/实际”并加入精确30x短按端点、全域原生连续hold和150 ms handoff说明；完整QGC/真机验收仍未因此扩大。 |
...
4. 右栏在A8与MT11同时启用时显示相机选择器；切换只更换Manager。MT11控制栏同时显示Target与Actual：tap发送1～30x绝对目标且不足一步直接到30x；任意tap仍pending时转hold都先用0x05停止退休绝对控制器，150 ms后发所按方向，仍按住时最多补一份方向。hold本身全域只保持一次原生0x05连续运动；release不得补tap。RGB/IR、拍照、录像与媒体徽标语义不变。
...
当前 `Mt11ProtocolTest` 独立运行11 passed、0 failed，覆盖9个业务slot及init/cleanup，包括1～30x tap、step 2.0的1/3/…/29/30精确端点和对称返回、0x18对齐及165.1显示边界；`SiyiProtocolTest`为42 passed、0 failed。Manager moc检查0错误、diff-check通过。Manager的100 ms反馈Timer、150 ms handoff/安全停止Timer、真实UDP、镜头平滑运动与全域物理端点仍待验收。
...
截至2026-08-23，`Mt11ProtocolTest`为11 passed、0 failed，`SiyiProtocolTest`为42 passed、0 failed；`Mt11Sdk.cc`独立编译、Manager moc和diff-check均通过。MT11现以Target/Actual双值区分合法目标参考和0x18实测位置；tap在1～30x按step发送0x0F，最后不足一步直接到精确30x。hold在全域只启动一次原生0x05，以相机原生速度持续平滑运动；100 ms轮询0x18更新Actual并使Target沿方向单调对齐合法参考，step不再调节物理速度，也不再逐档0x0F或pulse停启。tap pending转hold使用stop→150 ms方向handoff，仍按住时最多一份方向副本；release/cancel或物理端点立即stop并只有一份150 ms安全副本，60秒watchdog按实测进展重置，165.1端点允许反向hold恢复。附件136条unmatched ACK debug（0x05 125、0x18 11）已停止逐包输出，严格丢弃和重要warning仍保留。Viewer3D设置组已删除对Qt 6只读隐式尺寸的赋值。两份custom TS现均为19个context、148条message和141个唯一source；英文148条unfinished，中文148条finished且空译文为0，context/source集合一致。上述结果仍不代表Manager的100/150 ms Timer、完整QGC Qt 6构建或MT11真机连续运动通过。RTSP原生Auto/GIO直连、Android硬解优先、双PIP、本地媒体和其他既有状态保持前文结论不变。
