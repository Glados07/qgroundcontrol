...
- 飞行界面右侧使用单个始终可见的纵向深色半透明控制栏：只要Gimbal功能已启用，无论是否连接飞控或云台都会显示；从上到下依次为放大、当前目标倍率、缩小、横向分隔线、MT11可选的三模式按钮、拍照/录像图标按钮以及SD/LOCAL状态徽标。空闲时模式/拍照/录像按钮均为 `actionSize` 圆角方形触控区，与缩放按钮共享青色强调、悬停描边和按压动效；录像图标与拍照图标等大且不显示“录像/REC”，开始录像后仅为计时、pending或失败状态文字按内容展开胶囊。受支持拉流会话或卡录能力尚未确认时缩放按钮锁定；两者确认后按钮启用状态只由当前目标在同一合法表中的方向边界决定。到上限仅禁用加号，到1.0x仅禁用减号；真实断流重新锁定两键并显示 `--`，同分辨率重连后会用已确认或重新查询的卡录能力解锁。本地录像按钮可用性另由本地开关与流状态决定，不要求云台SD能力。
...
- MT11的 `currentZoom`发布从1.0x锚定且不超过物理上限的合法步长参考，新增 `actualZoom/actualZoomKnown`只发布0x18权威实测值；控制栏分别显示“目标”和“实际”，不再把参考目标伪装成镜头已经到位。默认步长1.0x且设备/产品物理上限为165.1x时，目标最高165.0x、实际可显示165.1x；步长0.1x时165.1x本身也是合法目标。短按按独立 `mt11ZoomStep`从上一目标计算1.0～30.0x的0x0F，快速连点替换在途目标；不足完整步长时把精确30.0x追加为最后短按目标，例如step 2.0使用1、3、…、29、30并沿同一序列反向返回。实测大于30.0x时两方向tap关闭。长按420 ms成立后在全倍率范围只启动一次原生0x05方向命令，保持相机原生平滑匀速运动，直至release/cancel或0x18确认物理端点；长按不再逐档发送0x0F，也不再按step反复启停0x05。运动期间每100 ms主动查询0x18：Actual发布一位小数实测值，Target只随实测位置单调对齐到最近合法参考档，不控制物理速度。若长按接管仍pending的0x0F，先发0x05 `0`退休绝对控制器，等待150 ms再发方向；仍按住时最多补一份方向副本。release/cancel取消待发handoff或方向副本，立即停止并安排一份150 ms有界安全停止副本，settled 0x18到达前继续锁定新手势。60秒watchdog按“无进展”重置；165.1x端点在停止后的settled反馈丢失时仍允许重新按住反向退出端点，但tap继续受门禁保护。
- `mt11ZoomStep=1.0x`只决定短按绝对目标和长按期间Target的合法参考网格，不设置原生0x05的物理运动速度。绝对目标沿1、2、3、4…线性增加；若30x边界不足一步则精确追加30x。在同一成像链路中，从 `z`到 `z+1`的线性尺寸比为 `(z+1)/z`：1→2是2.0倍（100%增量）、2→3是1.5倍（50%）、3→4约1.33倍（33%）、4→5是1.25倍（25%），因此越往后固定 `+1.0x`看起来越小是数学上的必然结果，不是QGC改了步长。《UniPod MT11 v1.2》p16另列出广角4.5 mm/等效24 mm和变焦镜头15～50 mm/等效81～270 mm，`15/4.5 ≈ 3.33`；p37的8K拍照逻辑也以3.3x和11x分段。由此可推断设备双镜头焦距段在约3.3x附近还会形成一个额外体感拐点，但这是设备光学/成像链路的手册参数推断，不是SDK重映射或程序在4x后换了步长。
...
- Fly View右侧栏在A8和MT11都启用时显示分段胶囊式 `A8 Mini / MT11`选择器，每段自带绿/灰SDK在线点，只启用一路时自动归一到该后端。两栏共用 `GimbalCameraControl.qml`的深色半透明面板、青色强调色、统一圆角按钮、悬停/按压动效、缩放、拍照、录像、SD/LOCAL徽标和移动端最小触控尺寸；视觉更新不改变原有缩放tap/420 ms hold/release/cancel手势契约。`MT11CameraControl.qml`只负责注入MT11 Manager并开启三模式控件；点击模式按钮时，基于 `QtQuick.Controls.Overlay` 的非模态Popup优先在按钮左侧展示“变焦/热成像/变焦 + 热成像”三项，按Overlay边界钳制，窄屏左侧不足时改为按钮下方优先、上方回退。点外部、Esc、SDK离线、应用进入后台、窗口隐藏、控件销毁或A8/MT11切换都会关闭弹层。模式按钮以 `videoModeKnown/videoMode/videoModePending`显示已确认值和pending，通过 `setVideoMode()`发送0x11；仅匹配的0x10/0x11回包确认成功，2.5秒超时后标记未确认并重查实际模式，不以本地乐观值冒充切换成功。
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义11个Gimbal Fact；独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，只决定tap目标和长按期间Target参考网格，不控制原生0x05运动速度。longDesc明确短按精确30x端点、全域单次0x05平滑长按、100 ms的0x18 Target/Actual更新以及tap→hold的150 ms handoff；不改变设备上限或RTSP URL。 |
...
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11 QML业务门面；除合法目标 `currentZoom`外新增只由0x18更新的 `actualZoom/actualZoomKnown`，继续暴露设备上限、独立步长、tap/hold availability、媒体与错误状态。三模式QML契约为 `videoModeKnown`、`videoMode`、`videoModePending` 和 `setVideoMode(int)`，枚举值0/2/3与协议 `main_stream` 一致；旧 `thermalModeKnown/thermalModeEnabled/thermalCommandPending/toggleThermalMode()` 只为已有调用者保留兼容，新UI不以它们表达三态。常量区分0x0F上限30.0x、产品物理上限165.1x和线反馈上限255.9x。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 短按维持1～30x相邻0x0F，并在最后不足step时直接到精确30x；长按在全倍率范围只发送一次原生0x05方向，让相机持续平滑匀速运动，100 ms查询0x18并把Actual原样发布、把Target沿运动方向单调对齐到合法参考档，仅在release/cancel或物理端点停止。step不再控制长按速度，也不再逐档发0x0F或反复启停0x05。tap pending转hold时先stop，150 ms后发方向，仍按住时最多一份方向副本；release/cancel取消handoff并立即stop，150 ms后仅一份安全停止副本。60秒watchdog按0x18进展刷新，高倍端点在settled反馈丢失时仍允许反向hold恢复。模式切换前停止正在运动或pending的缩放、清除旧镜头0x16/0x18能力并发送三态0x11；发送本身不更改显示模式，只有匹配目标的合法0x10/0x11回包才结束pending，2.5秒超时后改为未确认并重查。Manager保存最后一组原始main/sub，但工作模式与镜头倍率generation以 `main_stream` 为权威：只有main变化才立即退休旧缩放generation和能力；同一main下固件对sub做0↔2等归一化只记录/诊断，不中断长按、不使倍率失效。析构和媒体隔离语义保持。 |
...
MT11完整缩放调用链为：合法SDK包维持6秒在线 -> 0x16能力与0x18实际位置各维持6.5秒新鲜度 -> Policy对齐合法步长Target -> tap在1～30x发送0x0F，最后不足一步直接到精确30x -> hold在420 ms后全域只发一次0x05方向并保持原生平滑匀速 -> 每100 ms查询0x18，Actual发布实测值，Target沿运动方向单调对齐合法参考 -> release/cancel或物理端点发送0x05停止及一份150 ms安全副本。tap仍pending时，hold先stop退休0x0F、等待150 ms，再发方向且仍按住时最多一份方向副本。0x05 ACK只维持可达性；0x18是唯一位置真值。默认step 1.0时目标上限165.0而Actual可到165.1，step 0.1时165.1亦为目标；端点停止后若settled反馈丢失，高倍反向hold仍可退出端点。step不控制长按物理速度；模式/析构隔离和不读取A8能力的边界保持不变。
...
| `mt11ZoomStep` | 0.1–29.0 / `1.0x` | MT11 tap目标和hold期间Target参考所用的合法步长。tap只在1～30x使用0x0F，最后不足一步时把精确30.0x作为终点；hold全域只启动一次0x05，以相机原生速度平滑运动，step不控制物理长按速度。默认step 1.0时Target最高165.0、Actual可到165.1；step 0.1时165.1也可成为Target。 |
...
默认步长1.0x时，Target严格为1.0、2.0、3.0、4.0…；Actual可显示0x18报告的非网格中间值。固定 `+1.0x` 的相对画面增量按 `(z+1)/z` 递减：1→2为100%、2→3为50%、3→4约33%、4→5为25%。手册p16的4.5 mm广角与15～50 mm变焦镜头还表明约3.3x可能有设备成像链路体感拐点；这不是QGC改变步长或重映射0x0F。
...
1. Application Settings -> Video配置独立 `RTSP URL 1/2`，两路均使用原生Auto。Application Settings -> Fly View的仪表板正下方固定显示单层Gimbal设置组，不依赖云台在线；A8/MT11独立Zoom Step置于第一项。MT11默认1.0x，决定tap目标和hold期间Target参考；tap最后不足一步直接到30x，hold全域只发一次原生0x05并保持平滑匀速，物理上限仍取 `min(0x16,165.1x)`。
...
4. 右栏在A8与MT11同时启用时显示带独立在线点的分段胶囊选择器；切换只更换Manager、会先关闭当前弹层，不切换中心视频。两个相机面板共享深色半透明、青色强调、圆角按钮和悬停/按压动效，但不改变缩放手势。MT11控制栏同时显示Target与Actual：tap发送1～30x绝对目标且不足一步直接到30x；任意tap仍pending时转hold都先用0x05停止退休绝对控制器，150 ms后发所按方向，仍按住时最多补一份方向。hold本身全域只保持一次原生0x05连续运动；release不得补tap。MT11拍照上方的模式按钮打开左侧优先的Overlay Popup，以0x11选择变焦、热成像或变焦+热成像拼接，仅回包确认后更新当前模式。拍照、录像与媒体徽标语义不变。
...
当前 `Mt11ProtocolTest` 独立运行11 passed、0 failed，覆盖9个业务slot及init/cleanup，包括1～30x tap、step 2.0的1/3/…/29/30精确端点和对称返回、0x18对齐、165.1显示边界、三种0x11精确payload、0x10通用合法域及UI三态识别；`SiyiProtocolTest`为42 passed、0 failed。Manager moc检查0错误、diff-check通过。Manager的100 ms反馈Timer、150 ms handoff/安全停止Timer、模式2.5秒确认Timer、真实UDP、镜头平滑运动、三模式实机画面与全域物理端点仍待验收。
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> Mt11ZoomPolicy对齐合法Target，0x18另发布Actual；默认step 1.0、物理165.1时Target 165.0/Actual 165.1
         ...
         -> 长按：全域只启动一次原生0x05方向，保持相机平滑匀速运动；100 ms轮询0x18更新Actual和单调合法Target，只在释放/取消或物理端点停止
         -> tap pending后长按：0x05(0)退休绝对控制器 -> 150 ms handoff后发方向 -> 仍按住时最多一份方向副本；step不控制物理长按速度
     ...
     -> A8或MT11任一enabled
        ...
        -> 合并栏始终显示，sdkResponding不决定可见性；SDK超时会失效卡录能力并锁缩放，待新0x20恢复
         -> 顶部 GimbalZoomControl.qml（统一深色圆角按钮、青色强调的 + / 倍率 / -，调用当前选择的Manager）
            ...
            -> MT11：tap在1～30x按step发0x0F并追加30x精确端点，hold全域只保持一次0x05原生运动，100 ms轮询0x18更新Actual/参考Target
        -> 下部双目标相机控制
...
1. Application Settings -> Fly View -> Instrument Panel正下方必须立即显示Gimbal组，再往下才是Viewer3D；Gimbal入口不按在线状态隐藏。A8/MT11独立Zoom Step分别保存。MT11 step只决定tap目标和hold期间Target参考，不能改变原生0x05物理速度或0x16/165.1物理上限；Viewer3D组不得再出现向只读implicit尺寸赋值的Qt 6 warning。
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - 合并栏必须保持纵向单栏结构：A8按 `+ -> 当前倍率 -> - -> 分隔线 -> 拍照 -> 录像 -> SD/LOCAL徽标` 排列，MT11只在分隔线与拍照之间多一个三模式按钮；两者均使用QGC相机图标且不出现原生缩放滑块。面板、缩放、模式、拍照和录像应统一为深色半透明+青色强调+圆角方形视觉，悬停描边、按压反色/缩放动效不得改变原手势调用次数。空闲拍照和录像按钮必须同为 `actionSize` 触控区，录像图标固定为 `actionSize * 0.48`并与拍照图标等大，空闲录像按钮不得显示“录像/REC”；计时、pending和 `FAILED` 仅在相应状态下出现。验证SD/LOCAL绿色实际录制、黄色pending、红色失败、灰色不可用/未录制及错误提示互不覆盖。Gimbal启用时右侧Column宽度和Loader高度必须始终随控制栏完整隐式尺寸扩展；在Android 86%/100%、桌面、横竖屏、地图/视频主窗口、PIP、Viewer3D和小屏触摸场景检查不裁切、不重叠、按钮达到移动端最小触控尺寸。
   ...
   - MT11协议与策略边界：抓包核对0x05/0x0F/0x16/0x18及sequence/CRC；0x05 `73 06`与0x16/0x18 `a5 01`均代表165.1x，线反馈可到255.9x，但产品上限钳制165.1x。默认step 1.0时Target上限165.0、Actual可显示165.1；step 0.1时Target也可到165.1。0x0F只允许1～30x，且30.0是唯一允许追加的协议精确端点。纯测试11/0不代表Manager Timer或真机通过。
   - MT11短按与双值矩阵：默认step 1.0，另覆盖0.1/2.0/29.0；1～30x短按使用0x0F，Target在发送成功时更新，Actual只随0x18更新。最后不足一步必须直接到30.0，例如step 2.0正向为1/3/…/29/30，反向严格按30/29/…/3/1返回；快速tap不按actual重建网格，>30x双向tap关闭。验证在途Target 2.0/Actual 1.x、最终近似对应、Actual非网格中间值以及离线 `--`，0x05 ACK不得更新Actual。
   - MT11 tap↔hold矩阵：任意0x0F tap仍pending时开始同向或反向hold，都必须先用0x05 `0`退休绝对控制器，等待150 ms后只发所按方向；仍按住时最多补一份方向副本。分别在handoff前、首次方向后和副本前验证release/cancel/移出/后台：不得补tap、不得迟发方向。hold release后立即停止、150 ms安全停止副本和settled 0x18门禁仍须验证。
   - MT11原生连续长按矩阵：420 ms成立后，从1x到设备物理上限的任意位置和方向都只启动一次0x05方向，整个按住期间不得发送逐档0x0F，也不得为step反复发送0x05 stop/start；以实机画面确认运动连续、平滑且保持相机原生匀速。每100 ms查询权威0x18，Actual显示原始一位小数值，Target沿运动方向单调对齐到最近合法参考档；迟到反馈不得让Target反向跳动。release/cancel立即stop并仅有一份150 ms安全副本，达到1x或0x16物理上限也自动stop。60秒watchdog按0x18进展重置；无进展、发送失败、模式切换及SDK离线必须可靠停止。物理165.1/step 1.0时Target为165.0、Actual为165.1；加方向关闭而反向hold可直接连续退出端点。Manager Timer仍需fake SDK或真机验证。
...
选择MT11时视频有画面但缩放按钮仍灰色，应检查SDK在线、0x16/0x18新鲜度、settled门禁和模式pending；RTSP、A8的0x20及分辨率不能解锁MT11。>30x短点无动作是保护，应按住420 ms。无论当前倍率是否超过30x，正常长按只应看到一次0x05方向并保持原生连续运动；若接管pending的0x0F，则允许先有一次0x05停止、150 ms后方向以及仍按住时最多一份方向副本。Actual每100 ms由0x18更新，Target只沿运动方向对齐到合法step参考；step不会改变物理长按速度。若长按没有运动，抓包检查方向命令、SDK endpoint和0x18新鲜度，不能再等待已删除的逐档pulse超时提示。165.1端点的step 1.0应显示Target 165.0/Actual 165.1，重新按住减号必须能够连续反向退出；日志不再逐包显示unmatched ACK，抓包才是方向与payload证据。
...
A8仍只显示目标，运动中raw不覆盖UI。MT11则明确双值显示：tap期间Target是已下发的合法绝对目标；hold期间Target是根据0x18沿运动方向单调对齐的合法step参考；Actual始终是0x18实测且可出现5.6等非网格值。两者不同表示镜头仍在运动或实测量化存在偏差，不是步长失效。MT11的step不设置hold物理速度。
...
A8 hold仍按原有单次0x05与按压时长目标策略。MT11同样以单次原生0x05实现平滑长按，但不使用A8的按压时长目标估算或80 ms时序：MT11以100 ms的0x18实测更新Actual/Target，tap pending接管等待150 ms，release/cancel立即stop并在150 ms后发送一份有界安全副本。
