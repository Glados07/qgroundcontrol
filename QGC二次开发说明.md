...
- **代码已集成，待目标遥控器真机回归验收**：本轮A8 Mini缩放即时单击/原生连续长按状态机、SD卡与本地媒体双支路、UniPod MT11私有SDK与三种工作模式、通用独立第二路RTSP、Map/Video 1/Video 2三视图、统一双相机右栏、双罗盘条、Android 86%界面缩放缺省值、Android H.264/H.265厂商MediaCodec强制策略和Android USB串口管理器已经进入当前工作树。Android两路都先把带QML Item的video sink加入各自receiver管线并置PAUSED；A8既有 `decodebin3 -> qgcandroidh265hwdec -> h265parse -> Annex-B/AU -> 厂商MediaCodec` 首代路径保持不变。逐流恢复严格限定为硬解：每个新receiver/URI先走共享A8 adapter；满足H.265 source首帧watchdog、严格decoder分支bus错误或decoder/sink分支 `startDecoding`失败且decoder/sink均无输出时，后续完整generation依次尝试有序direct-hvc1/AU厂商MediaCodec候选，每个候选最多一次；全部direct失败后回到共享A8 adapter最终路由，URI改变时清除候选序号、耗尽标记和显式factory并重新从首代adapter开始。普通receiver/pipeline start失败、RTSP/source错误、非decoder分支或sink分支bus错误、仅缺source且无decoder分支证据，以及decoder已有输出但sink无帧都不推进路由。显式MediaCodec先连接压缩输入和到已PAUSED sink的静态输出，再依次同步decoder与sink，添加、连接和状态同步的每个返回值都检查。所有重建均不改变进程级rank，也不会转到 `avdec_h265`。双路是否能各自持续驱动独立MediaCodec实例并到达sink首帧，仍须按第12章在目标设备验证，不能仅凭静态代码、候选rank、adapter注册或路由切换日志判定整机验收通过。
...
- **本轮 Android MT11 黑屏逐流硬解修正与安全边界**：MT11换到URL 1仍无画面，已排除Video 2专用QML/PIP接线作为共同根因；A8与MT11实际都是H.265，不能用全局direct-first破坏A8已能显示的adapter链。现继续保持 `forceAndroidH265HardwareDecoder=true`、adapter `PRIMARY+100=356` 优先、direct-hvc1/AU厂商硬解 `PRIMARY+3=259` 次级以及软件候选为NONE。策略在覆盖rank前建立有序direct列表：adapter内部选中的厂商MediaCodec若也直接接受 `hvc1,alignment=au` 则排第一，其余按原始rank降序、factory名升序排列。每个新receiver/URI的路由为“首代共享adapter -> direct候选1…N各最多一次 -> direct耗尽后共享adapter最终路由”，URI改变全复位；无direct候选时始终留在adapter。推进证据包括source已到后的首帧watchdog、严格decoder分支bus错误，以及decoder/sink分支 `startDecoding`失败；普通pipeline start失败不推进。adapter路由同时识别外层 `qgcandroidh265hwdec` 与它选中的内部厂商MediaCodec身份，避免deep-element诊断只报告内部factory时漏判。显式direct先完成输入和静态输出连接再进入PLAYING，不再让MediaCodec在下游未就绪时提前configure。判定依据本路事实而非Video 1/2槽位、URL、IP或分辨率，不改变另一路。当前仍只是代码静态修正，尚无新APK与目标遥控器四阶段日志，不能宣称MT11 Android或双路已经验收通过。应用设置－视频已恢复原生自适应居中内容宽度，RTSP URL输入保留约40个默认字体字符的首选宽度；固定50字符封顶和大段MediaCodec说明均已删除。
- **Android MT11旧版黑屏的提交级程序根因（分阶段）**：第一阶段是 `b2239ed65` 继承adapter 356/direct 259后，每个H.265 decodebin首代都实例化adapter；若MT11真实码流在adapter内部没有产出raw帧，decodebin不会把运行期无输出当作autoplug创建失败，旧watchdog又按相同rank重建，形成无限重走adapter、direct永远没有机会的闭环。第二阶段是后续 `ffe3ecdbb` 虽加入逐receiver显式direct，但仍有四个程序缺陷：显式MediaCodec先同步到父PLAYING、后连接压缩输入与显示输出，可能在 `set_format/configure` 时上下游尚未就绪；`gst_bin_add`、输入/输出连接及decoder/sink状态同步的失败未完整向上传递；只冻结一个direct factory且同URI无限重建同一失败候选；adapter身份只认外层factory，deep-element回调报告内部MediaCodec时可能漏掉adapter失败事实。本轮通过“先连接后同步、全部返回值检查、有序候选每个一次后回adapter、外层/内部双重identity”修正这些代码级闭环。MT11码流的CSD/profile/封装究竟在哪一层触发失败，以及新路径能否真机出帧，仍必须用新APK四阶段日志确认。
...
- 开关开启时，兼容adapter以 `GST_RANK_PRIMARY + 100=356` 注册并优先接收播放支路的 `hvc1`，通过GStreamer `h265parse`转换为 `video/x-h265,stream-format=byte-stream,alignment=au` 后送入厂商MediaCodec；能直接接受 `video/x-h265,stream-format=hvc1,alignment=au` 的厂商MediaCodec固定为 `GST_RANK_PRIMARY + 3=259` 并组成显式次级硬解列表。每个新receiver/URI首代仍按全局rank进入adapter；之后仅在source首帧watchdog、严格decoder分支bus错误或decoder/sink分支 `startDecoding`失败且decoder/sink均无输出时推进到下一direct候选。每个direct候选只试一次，全部失败后清除显式factory并进入共享adapter最终路由；URI改变才全复位。普通pipeline start失败不推进。READY预检不输入真实码流，不能验证CSD、profile/level、Surface或第二个MediaCodec实例资源；实际选择与输出仍须用四阶段日志确认。
- 同一编码下所有通过筛选的direct厂商候选自动插拔rank仍统一为259，因此运行期显式顺序必须在覆盖rank前冻结。adapter内部选中的厂商MediaCodec若也接受hvc1/AU则排第一，其余按“原始rank降序、稳定factory名升序”组成有序列表。列表为空时不离开adapter；列表非空时按receiver/URI记录candidate index和exhausted状态。该顺序不会引入软件解码，但必须通过 `Decoder element instantiated` 记录每代实际factory，不能仅凭候选列表或路由日志判断播放成功。
...
- Android双路都先把自己的 `qgcvideosinkbin/qml6glsink`加入本管线、重新绑定本路QML Item、设置sync并置为PAUSED。A8使用的普通decodebin/adapter分支保留既有动态pad行为；显式direct厂商MediaCodec通常具有静态pad，因此core先add但不把它同步到父PLAYING，随后连接压缩输入、把静态输出连接到已PAUSED的sink，最后依次同步decoder与sink。`gst_bin_add`、输入连接、输出/sink连接、decoder同步及sink同步全部检查结果并把失败返回owner，避免MediaCodec在下游未连接时提前进入 `set_format/configure`。旧版“sink晚于decoder动态加入”的修正仍保留，但它不是本轮已测试版本仍黑屏的完整原因。
- 通用receiver为每个真正启动的pipeline分配单调generation，以低频信号报告CAPS协商出的实际codec、真实source buffer、实际decoder plugin/factory、decoder首输出、sink首帧及bus error前的一致快照。所有诊断都携带generation，同URI旧代迟到事件不能污染新代；core只提供事实与“按本代冻结的factory显式创建H.265 decoder”通用钩子，不包含A8/MT11判断。Video 1由 `AndroidVideoDecoderRecovery` 消费，Video 2由 `DualVideoManager` 消费。Timer仅从真实source首帧启动，超时为 `max(rtspTimeout, 8秒) + 3秒`。watchdog入口要求H.265、source已到、当前硬解路由decoder已识别且decoder/sink均无帧；bus入口只接受同代 `!rtspSourceError && decoderBranchError`、H.265且decoder/sink均无帧。decoder分支既包括decoder root及其后代，也包括严格限定的上游sourcebin `not-negotiated`：本代已有decoder root、错误来自source分支、非RTSP source错误、domain为 `GST_STREAM_ERROR`、codec已确认为H.265、decoder/sink均无帧且debug含 `not-negotiated`。该分类覆盖decoder拒绝CAPS/CSD却由上游h265parse/parsebin报错的情况，不把普通RTSP/RTP错误误归类。watchdog、bus及 `startDecoding`失败都可推进到下一硬解路由；普通pipeline start失败、其他source/bus/sink故障及decoder已有输出但sink无帧不推进。任何路径都不改rank或使用软件decoder，健康另一路不受影响。
...
| `custom/src/VideoManager/DualVideoManager.cc` | 监听通用 `[Video]/secondaryRtspUrl` 与原生Video Source/URL/启用状态，为实际 `secondaryVideoContent` Item创建独立receiver和sink；GL Item先经过 `BeforeSynchronizingStage` 与 `itemInitialized` 门禁。它独立维护start/stop、1～15秒封顶退避、URL换代、pause、PIP/surface销毁重建及主receiver重复源保护。每次 `videoPipelineGenerationStarted(uri,generation)` 都清空上代诊断事实，后续source/decoder/sink/bus通知必须同时匹配冻结URI和generation。真实source到达后启动 `max(rtspTimeout,8秒)+3秒` watchdog；严格decoder分支bus错误及decoder/sink分支 `startDecoding`失败也可在完整stop前调用逐receiver硬解路由helper。adapter路由同时识别外层adapter及其内部厂商decoder；每次失败只推进到下一个direct候选，direct全部耗尽后回共享adapter最终路由，URL改变全复位。普通pipeline start、RTSP/source、非decoder或sink bus错误、仅缺source且无decoder证据，以及decoder已有输出但sink无帧只按退避重建而不推进。全程不改rank、不使用软件decoder，也不改变主路。非GStreamer后端继续使用原有10～30秒watchdog且不执行该硬解路由。相机SDK在线不参与RTSP判定，URL为空/重复时不启流，cleanup与Item换代仍按receiver先销毁、sink后释放，退出阶段不重建。 |
...
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidH265DecoderFallback.h` | 声明逐receiver的Android H.265有界硬解路由：幂等安装URI复位监听，报告当前是否使用共享adapter，并在调用方提供本代URI/generation、codec、当前路由身份及source/decoder/sink事实时推进下一硬解候选。direct factory按策略顺序每个最多一次，耗尽后回共享adapter；它不持有GStreamer element、不修改rank，也不提供软件解码路径。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidH265DecoderFallback.cc` | 使用receiver动态属性保存路由URI、显式decoder factory、direct candidate index和direct exhausted状态。新receiver/URI首代按全局rank走adapter；source首帧watchdog、严格decoder分支失败或decoder/sink分支 `startDecoding`失败在decoder/sink均无帧时可调用 `prepareDirectRetry()`。adapter初始路由推进到direct候选1，direct失败依次推进到下一候选，候选耗尽后清空显式factory并锁定共享adapter最终路由；URI改变清除全部状态。显式factory本身可在source或decoder选择回调前证明当前代为H.265 direct；普通pipeline start、RTSP/source、非decoder或sink bus错误不作为证据。写入发生在完整stop前的receiver Qt线程，运行中的pipeline不被修改。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidVideoDecoderPolicy.h` | 声明进程级 `apply(forceHardwareDecoding)`、兼容的首项查询及 `directHvc1DecoderFactoryNames()` 有序列表查询。策略在 `VideoManager`完成 `GStreamer::initialize()` 之后、创建receiver之前应用；列表在启动期冻结，运行期只读。 |
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidVideoDecoderPolicy.cc` | `CustomPlugin::init()` 读取 `forceAndroidH265HardwareDecoder`；false时清空direct列表、不注册adapter、不改原生rank，交回QGC/GStreamer自动选择，仅用于诊断。true时先注册adapter并取得其内部厂商factory，再筛选直接接受 `hvc1,alignment=au` 的vendor androidmedia decoder：与adapter内部factory相同者优先，其余按原始rank降序、factory名升序组成有序列表；之后把H.264 `avc`/H.265 `hvc1/AU` direct候选固定为 `PRIMARY+3=259`、adapter固定为 `PRIMARY+100=356`，同codec非兼容候选降为NONE。找不到硬件路径时critical并明确失败，不保留软件rank。启动后全局rank固定，查询接口不会提升或返回 `avdec_h265`。非Android/非GStreamer构建返回空。 |
...
| `custom/src/VideoManager/VideoReceiver/GStreamer/AndroidVideoDecoderRecovery.cc` | 仅Android+GStreamer由 `CustomPlugin::createVideoSink()` 为Video 1幂等安装，并同时安装逐receiver路由helper。它只消费与当前URI/generation匹配的codec、source、decoder选择/输出、sink和结构化bus事实。source watchdog、严格decoder分支bus错误及decoder/sink分支 `startDecoding`失败都可在decoder/sink均无帧时推进下一硬解路由；普通pipeline start失败不推进。当前路由为adapter时，外层 `qgcandroidh265hwdec` 或其选中的内部MediaCodec factory都能建立adapter身份；当前路由为显式direct时，显式属性本身提供H.265路由证明。owner随后完整stop，原生VideoManager保留desired-running并在 `onStopComplete` 后按既有退避启动下一代；桌面端无动作。 |

本轮对既有 `DualVideoManager.cc` 的扩展集中在generation诊断消费和逐流首帧恢复：Video 2精确绑定每代URI/generation并安装同一个硬解路由helper。推进入口包括“H.265 source已到且decoder/sink均无帧”的watchdog、同代 `!rtspSourceError && decoderBranchError` 且decoder/sink均无帧的bus错误，以及decoder/sink分支 `startDecoding`失败；普通receiver/pipeline start失败仍不推进。bus中的decoder分支除decoder root/后代外，也接受core按H.265、`GST_STREAM_ERROR`、sourcebin来源、无decoder/sink帧及debug `not-negotiated`严格归类的上游协商错误。每次推进只改变本receiver/URI下一完整generation：首代adapter之后依次尝试有序direct候选，每个一次，耗尽后回adapter最终路由。所有失败都只由本路owner按封顶退避stop/restart，不调用进程级rank调整，因而不会让一路故障改变另一路decoder候选或路由。原有receiver所有权、PIP/surface和重复URL判定均不改。

Android选择链为：`CustomPlugin::init()`读取重启后生效的兼容Fact；新安装缺省true，由 `AndroidVideoDecoderPolicy` 将H.264/H.265限定为兼容厂商MediaCodec，H.265 adapter维持 `PRIMARY+100=356`、direct-hvc1/AU候选维持 `PRIMARY+3=259`、其他对应codec候选均为NONE，并在覆盖rank前冻结“adapter内部同factory优先、其余原始rank/名称排序”的direct列表 -> 新receiver/URI没有显式factory，先按全局rank进入adapter -> 每路以URI+generation记录codec、source、实际decoder、decoder输出、sink帧及bus来源 -> source watchdog、严格decoder-bus或 `startDecoding`失败满足门禁时，custom在stop前把下一direct factory冻结到该receiver -> core先把显式MediaCodec输入和静态输出连接到已PAUSED sink，再同步decoder/sink并检查所有结果 -> direct失败推进下一候选 -> 全部候选失败后清空显式factory并回共享adapter最终路由 -> URI变化清除路由URI之外的candidate index/exhausted/factory状态并重新从首代adapter开始。普通pipeline start、其他source/bus/sink故障、仅缺source且无decoder证据，以及decoder已有输出但sink无帧不推进。所有路径都完整重建而不改rank，不会影响健康另一路，也绝不提升 `avdec_h265`。关闭Fact才恢复原生自动选择，且只用于诊断。rank、READY、路由准备日志或factory创建都不能替代四阶段里程碑与实际画面。
...
| `custom/translations/custom.ts` | 英文源字符串模板。保留既有云台、双视频、双罗盘及UniRC运行状态source；运行页删除MediaCodec长说明、UniGCS静态提示和MT11 zoom longDesc后，当前为21个context、175条message和166个唯一source，175条均unfinished。Qt 5.14 `lrelease`验证exit 0并按模板语义忽略未翻译source；仍需项目Qt 6构建生成正式QM。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录，与英文模板保持相同的21个context、175条message和166个唯一source，context/source pair集合一致；175条均finished，unfinished和空译文为0。翻译覆盖UniRC CH9/CH10实时状态、POSIX串口/依赖错误、拨轮回中提示和保留的Fact短描述；Qt 5.14 `lrelease`实际生成175条译文并exit 0。翻译完成不代表完整QGC、Android串口或真机云台已验收。 |
...
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.cc` | GUI start先分配generation并冻结URI、low-latency与 `customAndroidH265DecoderFactory`。parsebin动态pad优先传递current/query CAPS；late `startDecoding()`才退回valve current/query。显式factory只接受H.265/hvc1兼容证据；创建时先确保sink已在pipeline且PAUSED，add显式MediaCodec但不先sync，依次连接压缩输入、静态src到既有sink，再同步decoder和sink；bin add、输入/输出连接及两次状态同步任一失败都返回false。未设置factory时保持A8既有 `decodebin3 -> adapter` 路径。自动与显式decoder统一记录实例和输出probe。bus除了decoder root/后代，还把同时满足“本代decoder root存在、sourcebin来源、非RTSP source错误、`GST_STREAM_ERROR`、H.265、decoder/sink均无帧、debug含not-negotiated”的错误严格标为decoder分支，以覆盖上游parser代报协商失败；其他RTSP/RTP/source错误不归类。core不解释动态属性来源，不决定候选推进或URI复位，不引用custom policy、不改rank，也不识别MT11/A8。 |
...
| `forceAndroidH265HardwareDecoder` | bool / `true` | 兼容保留的旧QSettings键，仅Android生效；UI显示为“Android H.264/H.265必须使用硬件解码”。新安装缺省开启，将对应codec限定为经过筛选的厂商androidmedia decoder；H.265全局rank保持adapter `PRIMARY+100` 优先、direct-hvc1/AU `PRIMARY+3` 次级，其他候选为NONE。每个新receiver/URI首代使用adapter；source watchdog、严格decoder分支bus错误或decoder/sink分支 `startDecoding`失败可推进到有序direct列表的下一项，每项最多一次，耗尽后回adapter最终路由。URI改变清除candidate index/exhausted/显式factory并重新开始；普通pipeline start、其他source/bus/sink故障和decoder已有输出但sink无帧不推进。没有硬件路径时critical失败，任何产品路径都不使用 `avdec_h265`。关闭仅用于诊断，恢复原生自动选择。修改后必须重启QGC；已有安装保存值继续优先于metadata缺省。双H.265/双MediaCodec仍须目标遥控器新APK验证。 |
...
- 若设备厂商H.265解码器直接接受 `hvc1,alignment=au`，该direct候选rank固定为 `GST_RANK_PRIMARY + 3=259`。策略在统一rank前保存全部兼容候选：adapter内部所用factory若也兼容hvc1/AU则排第一，其余按原始rank降序、稳定factory名升序；该列表不改变adapter 356的全局首代优先级，只为逐receiver有界硬件重试提供确定顺序。
- 每个新receiver/URI没有显式factory，首代仍由 `decodebin3` 按全局rank选择adapter。watchdog入口要求本代已收到H.265 source、当前硬解decoder已识别且decoder/sink均无帧；bus入口要求同代 `!rtspSourceError && decoderBranchError`、H.265且decoder/sink均无帧，可早于source首buffer覆盖MediaCodec CAPS/CSD失败。core还会把满足严格H.265/sourcebin/GST_STREAM_ERROR/无输出/debug not-negotiated条件的上游协商错误归为decoder分支。`startDecoding`失败作为已确认decoder/sink分支失败也可推进；当前adapter identity可来自外层adapter或其内部厂商factory，当前显式direct则由冻结factory证明H.265路由。
- 路由按“adapter首代 -> direct候选1…N各最多一次 -> adapter最终”推进，URI改变全复位。显式direct add后先连接输入和到PAUSED sink的静态输出，再同步decoder/sink并检查所有返回值。普通pipeline start失败、RTSP/source错误、非decoder或sink bus error、仅缺source且无decoder证据、decoder已有输出但sink无帧及非H.265均不推进。adapter和direct都不包含软件解码；非兼容候选始终为NONE，绝不自动切换 `avdec_h265`。关闭开关才恢复原生rank与QGC/GStreamer自动选择，且仅用于诊断。
  ...
  -> CustomPlugin::init()
     ...
     -> AndroidVideoDecoderPolicy::apply()
        -> 新安装默认true：H.264/H.265限定为通过厂商筛选的androidmedia MediaCodec候选
           ...
           -> 覆盖rank前建立有序direct列表：adapter内部factory若兼容hvc1/AU则优先，其余按原始rank/稳定名称排序
     ...
     -> AndroidH265DecoderFallback安装到Video 1/Video 2 receiver
        -> 新receiver/URI显式factory为空、candidate=-1、exhausted=false；首代共享adapter
        -> URI改变清空显式factory、candidate与exhausted并重新从首代adapter开始
     -> AndroidVideoDecoderRecovery（Video 1）/ DualVideoManager watchdog（Video 2）
        -> 按冻结URI + generation隔离同URL重启；tee CAPS确认实际codec，分别记录source/decoder输出/sink里程碑及decoder plugin/factory
         -> adapter路由同时识别外层qgcandroidh265hwdec及其内部厂商MediaCodec；显式factory证明direct H.265路由
         -> 同代bus error先停止本路Timer；!rtspSourceError + decoderBranchError + H.265 + decoder/sink均无帧时推进下一硬解候选
        ...
        -> watchdog路径：只在真实source buffer后启动；max(rtspTimeout,8秒)+3秒仍无sink帧时完整重建
           -> H.265 + 当前硬解路由已识别 + decoder/sink均无帧：推进下一硬解候选
        -> decoder/sink分支startDecoding失败同样推进；普通pipeline start失败不推进
        -> 显式MediaCodec：add但不先sync -> link input -> link static output到PAUSED sink -> sync decoder -> sync sink；每步检查
        -> decoder已有输出但sink无帧不推进，转查显示/GL/surface；无source且无decoder分支失败时转查路由/RTSP/RTP
        -> 完整stop；各自owner按原有生命周期和退避重建receiver
           -> adapter首代 -> direct候选1…N各最多一次 -> direct耗尽后adapter最终；全程仅硬解
     ...
     -> DualVideoManager
        ...
        -> adapter首代、direct候选各一次、direct耗尽后adapter最终；decoder已输出但sink无帧转查显示链
...
Application Settings / Video
  ...
  -> Video Stream Integration
     ...
     -> forceAndroidH265HardwareDecoder（兼容旧键；UI为H.264/H.265必须硬解，新安装默认true；仅Android生效且修改后重启）
        ...
        -> true：对应codec只允许厂商MediaCodec；H.265 adapter保持356优先、direct-hvc1/AU保持259；其余候选NONE，无硬件路径则critical失败
        -> false：仅用于诊断；不注册custom adapter、不改rank，恢复QGC/GStreamer原生自动选择
        -> URI+generation隔离实际codec、source、decoder身份/输出、sink与bus事实
        -> 新receiver/URI首代走adapter；source watchdog、严格decoder分支bus错误或decoder/sink分支startDecoding失败可推进
        -> 有序direct候选各最多一次，耗尽后回adapter最终；URI改变清除candidate/exhausted/factory并重新开始
        -> 普通pipeline start、其他source/bus/sink故障、无source且无decoder分支证据，以及decoder已有输出但sink无帧不推进
        -> 全程不改rank、不使用avdec_h265；显式MediaCodec先连接输入/静态输出到PAUSED sink，再同步decoder/sink
```
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - 双路并行解码、全屏和叠加层：URL 1/2配置为两个不同且可用的RTSP endpoint，使用新安装默认 `forceAndroidH265HardwareDecoder=true` 冷启动。两路都必须依次出现各自source、decoder实例、decoder输出和sink首帧，且receiver/decoder instance各自独立。正常A8首代应保持adapter并到sink；针对MT11分别注入source watchdog、decoder root/后代bus错误、严格sourcebin not-negotiated和decoder/sink分支 `startDecoding`失败。每次允许失败后应看到硬解路由只推进一项，显式代实例日志包含 `selection receiver-specific explicit factory`；连续失败必须按有序direct候选各一次运行并在耗尽后回adapter最终。普通pipeline start、RTSP/source、非decoder或sink bus错误、无source且无decoder证据，以及decoder已有输出但sink无帧不得推进。改变URI再换回时首代恢复adapter。所有重建前后rank一致，任何代次都不得实例化 `libav/avdec_h265`，一路路由变化不得影响另一路。其余仍需验证持续画面、PIP/主视图交换、全屏、断流重连、独立窗口及surface销毁重建；rank、READY、路由准备或factory创建均不等于播放成功。
   - Android第二显示面启动顺序回归：冷启动、Video 2断流重建、Fly View退出重进及桌面跨window重挂后，都必须再次看到本路decoder输出和sink首帧，且画面持续刷新。共同前置是 `sink add/set PAUSED`；普通decodebin/adapter继续走A8既有动态pad路径，显式direct必须严格保持 `add MediaCodec但不sync -> link compressed input -> link static src到PAUSED sink -> sync decoder -> sync sink`，并分别故障注入验证bin add、输入连接、输出/sink连接、decoder sync和sink sync任一false都使 `startDecoding`失败并推进下一硬解候选。不得恢复成sink晚于decoder加入，也不得让显式MediaCodec在连接完成前进入父PLAYING。sink probe必须冻结本代URI+generation；迟到旧代回调不得污染新代。parsebin动态pad优先传current/query CAPS；显式factory接受H.265/hvc1兼容的非ANY CAPS，late valve ANY/EMPTY只有本代tee已确认H.265才可放行。若MT11只有SDK绿点而没有source首帧且没有decoder分支错误，先查RTSP/RTP；若有decoder分支错误或source但无decoder输出，查H.265/CAPS/CSD/MediaCodec；只有decoder输出存在而sink首帧缺失才判为显示端问题。
...
11. Android保持默认true，分别覆盖MT11单独放在URL 1且URL 2留空、MT11放在URL 2且URL 1为不同有效地址、H.264+H.264、H.265+H.265及产品实际A8 H.265+MT11 H.265组合，连续播放至少10分钟。每个新receiver/URI的H.265首代都应以356的adapter压过259的direct候选；健康A8不得因为另一条流失败而改变factory。对具备多个兼容direct的目标机，连续制造无decoder/sink输出的允许失败，确认路由严格为adapter首代、策略有序direct候选各最多一次、全部耗尽后adapter最终；URI变化后应重新从adapter首代开始。分别记录四阶段、开始/5分钟/10分钟延迟、CPU、丢帧、前后台、PIP和断流重连。URL 1/2填入同一地址会触发重复源保护，不能作为解码A/B。用户确认旧版A8 Android H.265可显示；新代码A8不回归、MT11显示及双H.265/双MediaCodec仍须新APK真机验收。
12. 仅为诊断临时关闭硬解开关并重启做A/B，确认不注册custom adapter、有序direct列表为空、不改rank并由QGC/GStreamer原生自动选择；诊断中可能出现的软件decoder只能作为对照，不能成为产品配置或自动恢复路径。重新开启后应看到 `Selected ordered receiver-specific H.265 direct hardware retry factories`；每次允许推进应看到 `Advancing the receiver-specific H.265 hardware route`，显式代仍看到 `Using receiver-specific explicit H.265 decoder factory`，direct全部耗尽后应看到 `All receiver-specific direct H.265 MediaCodec routes failed ... return to the shared hardware adapter`。owner日志以 `hardwareRouteAdvanced` 表示是否实际改变路由，并用 `sourceFrame`、`decoderBranchFailure`区分证据。逐代核对direct `androidmedia/amcviddec-*`、decoder输出与sink首帧，不能把全局rank误称为翻转。双路H.265期间分别开始/停止录像并回放，确认播放支路转换不影响原生 `hvc1` 录像。rank、READY、列表、路由准备或factory创建均不能代替首帧与画面。
13. 分别让URL 1和URL 2覆盖首帧失败矩阵并核对URI/generation不串代：第一类保持真实H.265 source持续进入、当前硬解decoder已识别且decoder/sink均无帧，直到watchdog超时，必须推进下一硬解候选；第二类让decoder root/后代在source首buffer前后分别报not-negotiated/Failed configure，确认 `!rtspSourceError && decoderBranchError`、H.265且decoder/sink均无帧；第三类让h265parse/parsebin在sourcebin上报 `GST_STREAM_ERROR` 且debug含not-negotiated，只有本代decoder root存在、非RTSP source错误、H.265且decoder/sink均无帧时才必须被严格归类为decoderBranchError并推进；第四类分别使显式decoder add、输入连接、静态输出/sink连接、decoder sync或sink sync失败，`startDecoding`失败必须推进。另注入普通pipeline start失败、RTSP/source错误、非decoder分支bus错误、sink分支bus错误、只有source pad而无buffer且无decoder证据，以及decoder已有输出但sink无帧，这些都不得推进。验证每个direct factory只试一次、耗尽后回adapter最终、URI改变全复位；所有重试前后rank不变且不得实例化 `avdec_h265`。让Video 1失败而Video 2持续正常、再反向执行，确认健康路decoder实例和路由不受影响。
...
1. policy候选：H.264日志应显示实际plugin为androidmedia、厂商factory接受 `avc`且rank为259；H.265 adapter应出现 `Registered qgcandroidh265hwdec ... rank 356 ... hvc1 -> byte-stream/au conversion` 并优先于rank 259的direct-hvc1/AU候选；有兼容direct时还应出现 `Selected ordered receiver-specific H.265 direct hardware retry factories`，顺序中adapter内部同factory在兼容时应排第一。这些只证明候选、adapter和显式重试列表已准备；`avdec_h264/avdec_h265`只能用于核对其rank为NONE，不能成为实际decoder。
2. 实际选择：每路都要看到 `Decoder element instantiated`，并核对receiver、URI、generation、plugin/factory和instance。新URI首代H.265应由普通decodebin实例化 `qgcandroidh265hwdec`，此日志不要求selection字段；watchdog、严格decoder分支bus或 `startDecoding`失败成立后，应先看到 `Advancing the receiver-specific H.265 hardware route`，再看到有序列表下一direct厂商decoder且带 `selection receiver-specific explicit factory`。连续失败时candidate序号必须递增且每项一次，耗尽后看到回共享adapter日志。`sourceFrame=true, decoderBranchFailure=false`表示watchdog证据，`decoderBranchFailure=true`表示decoder分支或startDecoding证据并允许 `sourceFrame=false`。adapter内部 `selected actual decoder ... byte-stream ... alignment=au`只说明内部候选与输入caps，不代替外层四阶段。
...
若H.265当前硬解路由已识别、source已到但watchdog超时仍无decoder/sink帧，下一代推进；严格decoder分支bus错误在decoder/sink均无帧时也推进，即使source首buffer尚未记录；decoder/sink分支 `startDecoding`失败同样推进。每次只前进一个有序direct候选，候选各一次，全部失败后回adapter最终。若已有decoder输出而sink无帧，则只转查显示端；普通pipeline start、RTSP/source、非decoder或sink bus错误，以及无source且无decoder分支证据不推进。所有情况都按封顶退避完整重建且不改变rank。任何 `vendor MediaCodec candidate`、direct列表或explicit factory创建都只代表筛选/选择结果，系统级硬件属性仍以Android API 29 `isHardwareAccelerated()`为准。
