...
- MT11的 `currentZoom`发布从1.0x锚定且不超过物理上限的合法步长目标；`actualZoom/actualZoomKnown`继续在Manager内部保存0x18权威实测值，供确认、端点判断和诊断使用，但相机控制栏与A8一致，只显示一个目标倍率数值，不再显示Target/Actual双行。默认步长1.0x且产品物理上限为165.1x时，目标最高165.0x，内部实测仍可到165.1x；步长0.1x时165.1x本身也是合法目标。短按按独立 `mt11ZoomStep`从上一目标计算1.0～30.0x的0x0F，快速连点替换在途目标；不足完整步长时把精确30.0x追加为最后短按目标，例如step 2.0使用1、3、…、29、30并沿同一序列反向返回。实测大于30.0x时两方向tap关闭。SDK的0x05只有 `-1/0/+1`方向/停止字段，没有速度字段；长按420 ms成立后只发送一次原生方向启动并持续到release/cancel或0x18确认物理端点，不再用周期stop/start或逐档0x0F伪造速度。普通hold不重复发送方向；仅接管pending 0x0F或上一轮待发安全stop时，先串行发送stop、立即发送新方向，并在同一hold仍活动时于150 ms后最多补一份有界方向副本。运动期间每100 ms主动查询0x18，并只把单调对齐后的合法目标发布给控制栏；新运动起步300 ms后才允许端点自动停止，且要求连续两份同方向端点反馈，单个乱序0x18不能移动目标或吞掉新hold。release/cancel取消待发方向副本，立即停止并安排一份150 ms有界安全停止副本；settled期间tap继续受门禁保护，但物理方向允许时hold不再被禁用。60秒watchdog按“无进展”重置，165.1x端点仍允许反向退出。
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义11个Gimbal Fact；独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，只决定tap目标和长按期间的目标参考网格，不参与原生长按物理速度。longDesc明确短按精确30x端点、0x05没有rate字段、长按只启动一次原生方向、100 ms的0x18内部确认、控制栏目标单值，以及tap或上一轮settle被新hold立即接管并保留一份150 ms有界方向副本；不改变设备上限或RTSP URL。 |
...
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11 QML业务门面；合法目标由 `currentZoom`发布，0x18实测仍由 `actualZoom/actualZoomKnown`保留作内部确认与可选诊断；继续暴露设备上限、独立步长、tap/hold availability、媒体与错误状态。长按状态只保留原生方向、150 ms有界方向/停止副本、100 ms反馈和端点/watchdog Timer，不再持有pace Timer或drive-running PWM状态。三模式QML契约为 `videoModeKnown`、`videoMode`、`videoModePending` 和 `setVideoMode(int)`，枚举值0/2/3与协议 `main_stream` 一致；旧 `thermalModeKnown/thermalModeEnabled/thermalCommandPending/toggleThermalMode()` 只为已有调用者保留兼容，新UI不以它们表达三态。常量区分0x0F上限30.0x、产品物理上限165.1x和线反馈上限255.9x。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 短按维持1～30x相邻0x0F，并在最后不足step时直接到精确30x。0x05协议没有速度字段；长按在全倍率范围只发送一次原生方向并保持到release/cancel/端点，不再用周期stop/start降低平均速度，也不发送逐档0x0F。普通hold没有方向重复包；tap pending或上一轮安全stop被新hold接管时先串行stop、立即发送新方向，仅同一hold仍活动时于150 ms补一份方向，从而同时避免420～570 ms释放无运动和旧stop越过新start。100 ms查询0x18并把控制栏目标沿运动方向单调对齐到合法参考档，实测值留作内部确认；起步300 ms后连续两次同方向端点反馈才允许自动停止，post-hold端点settle也要求连续两次，从而拒绝单个乱序端点包。release/cancel立即退休方向副本、发送stop，150 ms后仅一份安全停止副本。60秒watchdog按0x18进展刷新，高倍端点仍允许反向hold恢复。模式切换前停止正在运动或pending的缩放、清除旧镜头0x16/0x18能力并发送三态0x11；发送本身不更改显示模式，只有匹配目标的合法0x10/0x11回包才结束pending，2.5秒超时后改为未确认并重查。Manager保存最后一组原始main/sub，但工作模式与镜头倍率generation以 `main_stream` 为权威：只有main变化才立即退休旧缩放generation和能力；同一main下固件对sub做0↔2等归一化只记录/诊断，不中断长按、不使倍率失效。析构和媒体隔离语义保持。 |
...
MT11完整缩放调用链为：合法SDK包维持6秒在线 -> 0x16能力与0x18实际位置各维持6.5秒新鲜度 -> Policy对齐合法步长Target -> tap在1～30x发送0x0F，最后不足一步直接到精确30x -> hold在420 ms后发送一次原生0x05方向并保持连续运动 -> 每100 ms查询0x18，内部保存实测值并沿运动方向单调发布合法目标，起步300 ms后连续两份端点反馈才停止 -> release/cancel退休方向副本、立即发送0x05停止及一份150 ms安全副本。tap仍pending或上一轮安全stop仍待发时，新hold先串行stop、立即发方向，同一hold仍活动时150 ms后最多补一份方向；只剩settled反馈迟到时直接启动，post-hold端点反馈同样要求连续两份才解除门禁。0x05 ACK只维持可达性，0x18是唯一位置真值；UI只显示目标倍率。默认step 1.0时目标上限165.0而内部实测可到165.1，step 0.1时165.1亦为目标。step不参与原生物理速度，1.0与2.0的0x05命令序列相同；模式/析构隔离和不读取A8能力的边界保持不变。
...
4. 右栏在A8与MT11同时启用时显示带独立在线点的分段胶囊选择器；切换只更换Manager、会先关闭当前弹层，不切换中心视频。两个相机面板共享高透明蓝灰背景、青色强调、统一浅蓝外边缘、圆角按钮和悬停/按压动效；通用错误不再形成整栏红色外圈。MT11倍率与A8一致，只显示目标数值：tap发送1～30x绝对目标且不足一步直接到30x；任意tap仍pending时转hold都先用0x05停止退休绝对控制器，随即发送所按方向，仅同一hold仍活动时于150 ms后补一份方向。普通hold全域只发送一次原生方向并连续运动；上一轮hold结算中的第一次新长按可立即接管，不再因settled门禁或150 ms空窗丢失，release不得补tap。MT11拍照上方的模式按钮打开左侧优先的紧凑Overlay Popup，三个模式项与主栏按钮同为 `actionSize`正方形，以0x11选择变焦、热成像或变焦+热成像拼接，仅回包确认后更新当前模式。拍照、录像与媒体徽标语义不变。
...
当前 `Mt11ProtocolTest` 独立运行11 passed、0 failed，覆盖9个业务slot及init/cleanup，包括1～30x tap、step 2.0的1/3/…/29/30精确端点和对称返回、0x18对齐、165.1显示边界、三种0x11精确payload、0x10通用合法域及UI三态识别；`SiyiProtocolTest`为42 passed、0 failed。Manager moc检查0错误、diff-check通过。Manager的100 ms反馈Timer、150 ms方向/安全停止副本、真实UDP、原生连续镜头运动、三模式实机画面及全域物理端点仍待验收。SDK没有速度字段，因此单次0x05提供设备原生平滑速度；周期stop/start虽然能改变平均运动时间，却必然引入停走和停止后对焦，不再作为长按限速方案。
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> 长按：全域只启动一次原生0x05方向并保持连续运动，step不参与物理速度；仅接管旧命令时允许一份150 ms有界方向副本
         -> 100 ms轮询0x18并单调更新合法目标；起步300 ms后连续两份端点反馈才停止，单个乱序端点不移动UI目标
         -> tap pending后长按：0x05(0)退休绝对控制器 -> 立即发新方向；同一hold仍活动时150 ms后最多补一份方向
         -> hold结算后立即再hold：安全stop仍待发则先同步兑现再立即发新方向；若只剩0x18迟到同样直接启动，第一次手势不禁用
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - MT11原生连续长按矩阵：420 ms成立后，从1x到设备物理上限的任意位置和方向，普通hold抓包只能出现一份方向start，整个按住期间不得周期stop/start或发送逐档0x0F。若接管pending tap或上一轮待发安全stop，允许在旧stop之后立即发第一份新方向，并且仅同一hold仍活动时在约150 ms后补一份方向；420～570 ms之间释放也必须已经产生运动，release/cancel/移出/后台或端点后不得再迟发方向。分别把step设为1.0和2.0，0x05命令序列和连续采样0x18得到的真实倍率—时间斜率应基本一致，只有控制栏目标网格粗细不同。每100 ms查询权威0x18，UI只显示沿运动方向单调对齐的合法目标，迟到反馈不得让目标反向跳动。release/cancel立即stop并仅有一份150 ms安全副本，达到1x或0x16物理上限也自动stop。60秒watchdog按0x18进展重置；无进展、发送失败、模式切换及SDK离线必须可靠停止。约11x前后需单独记录画面与0x18斜率，以区分11x光学/165x混合成像链路的固件曲线；SDK没有rate字段，若原生速度仍不满足，只能向厂商索取带rate命令或固件，不能重新用step或周期停止伪造平滑速度。Manager Timer仍需fake SDK或真机验证。
...
选择MT11时视频有画面但缩放按钮仍灰色，应检查SDK在线、0x16/0x18新鲜度和模式pending；RTSP、A8的0x20及分辨率不能解锁MT11。settled等待只应锁定tap，物理方向可行的hold必须保持可用。>30x短点无动作是保护，应按住420 ms。普通长按抓包应只看到一份方向start，按住期间不能出现周期stop/start或逐档0x0F；若接管pending的0x0F或上一轮安全stop，则允许先stop、立即start，并在仍按住时于150 ms补一份方向。step 1.0与2.0必须使用相同hold命令序列，只改变目标网格。若长按没有运动，特别是上一轮释放后第一次按住无效，应抓包确认：待发安全停止是否先被同步兑现、420 ms阈值后是否立即发方向、需要接管时150 ms副本是否到达、单个旧端点0x18是否被300 ms起步保护和连续两份确认拒绝，以及release后是否还有迟发方向。控制栏只显示单个目标值；165.1端点且step 1.0时显示165.0，内部0x18仍可为165.1，重新按住减号必须能反向退出。日志不再逐包显示unmatched ACK，抓包才是方向、停止与时间线证据。
...
A8 hold仍按原有单次0x05与按压时长目标策略。MT11同样使用单次原生0x05连续运动，但不使用A8的按压时长目标估算，而以100 ms的0x18实测更新单值目标；tap pending或旧安全stop接管时先stop并立即发新方向，同一hold仍活动时150 ms后最多补一份方向，上一轮若只剩settled反馈迟到则直接启动。release/cancel先取消待发方向、发送stop，并在150 ms后发送一份有界安全副本。
