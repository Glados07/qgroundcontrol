...
- 飞行界面右侧使用单个始终可见的纵向半透明控制栏：只要Gimbal功能已启用，无论是否连接飞控或云台都会显示；从上到下依次为放大、当前目标倍率、缩小、横向分隔线、思翼机内拍照和录像。受支持拉流会话或卡录能力尚未确认时缩放按钮锁定；两者确认后按钮启用状态只由当前目标在同一合法表中的方向边界决定。到上限仅禁用加号，到1.0x仅禁用减号；真实断流重新锁定两键并显示 `--`，同分辨率重连后会用已确认或重新查询的卡录能力解锁。
...
- 真机确认RC接管后的首个 `Center(0°,0°)` 仍可能被飞控到云台的下游链路当成旧的全零目标而不产生物理更新；`Tilt 90` 或Yaw模式命令先执行后Center恢复，说明控制权切换本身已经成功。为保持精确0°语义且不制造可见俯仰动作，只有“本次自动接管后待执行动作仍为Center”时，先缓存当前pitch、对应Yaw模式的当前yaw和Yaw模式，再调用原生 `sendPitchBodyYaw()` 或 `sendPitchAbsoluteYaw()` 发送无位移预激活；这两个接口同时停止原生500 ms速率发送Timer，避免另一条命令1000干扰ACK归属。严格等待预激活的 `COMMAND_ACK=ACCEPTED`，再延迟150 ms、复核同一对象和控制权后调用原生 `centerGimbal()`。不能固定延时连续发送两个命令号1000的命令，否则QGC会在首条ACK返回前把第二条判为重复命令。
- 自动接管不是循环争抢。等待总时限为10秒，用于覆盖最慢约5秒一次的状态兜底、普通链路3秒命令ACK窗口和150 ms稳定窗口；切换活动Vehicle、GimbalController或活动云台、对象销毁、手动Acquire/Release、预激活ACK失败以及超时都会取消待执行动作，迟到状态或ACK不能跨对象重放。接管等待期间快速点击多个姿态按钮采用last-click-wins，只保留最后一个动作且不重复发送Configure；若RC持续输入重新取得控制，QGC不会在后台反复抢权。速率控制、屏幕拖动和摇杆连续输入不进入延迟重放。
...
| `custom/src/Gimbal/GimbalControlManager.h` | 思翼云台相机业务的QML门面和运行态声明。`currentZoom`表示当前合法目标：tap取成功发送的0x0f目标，hold取按时间和步长计算的显示目标；最近0x18实际值另行保存。`zoomControlsUnlocked`表示受支持的解码流会话，并暴露基于当前目标边界计算的 `zoomInAvailable/zoomOutAvailable`、命令pending和长按状态。公共接口区分普通 `stopZoom()`与安全 `cancelZoom()`。 |
| `custom/src/Gimbal/GimbalControlManager.cc` | 由 `CustomPlugin`创建一次并持有 `SiyiSdk`，没有修改或复制 `src/VideoManager`。拉流尺寸只确认受支持会话；0x20的 `stream_type=0`卡录分辨率决定能力，合法0x16只通过取较小值安全收紧；卡录能力另有4.5秒新鲜度期限，不能由0x16、0x0a或其他正常回包续期。tap从唯一合法表取相邻一档并立即发送0x0f，本地发送成功即更新 `currentZoom`，快速tap从上一成功目标继续规划。hold在420 ms成立时锁存起点与方向并通常仅发送一次0x05方向命令，随后按总按压时长 `qRound(totalMs / 600.0)` 计算和显示合法档数；到有效端点立即停止，起步即为端点时改用单次同方向0x0f。release/cancel发送0x05停止及一份有界安全重复，停止后不发送0x0f归整。0x18反馈保留在独立实际值状态中用于核对，不覆盖当前目标显示。 |
...
| `custom/src/UI/toolbar/GimbalIndicator.qml` | 原生 `src/UI/toolbar/GimbalIndicator.qml` 的同路径custom覆盖，保留遥测、设置、多云台选择和显式Acquire/Release界面。`Yaw Lock/Follow`、`Center`、`Tilt 90`、`Retract` 在控制权不足时建立一个带Vehicle/Controller/Gimbal身份和代次token的待执行动作，只发送一次Configure；同一上下文后续点击只替换动作内容。待两个控制权属性经下一事件循环共同确认后，普通姿态动作在受保护窗口中重放一次；自动接管后的Center先缓存当前姿态和Yaw模式，按模式调用 `sendPitchBodyYaw()` 或 `sendPitchAbsoluteYaw()` 发送无位移预激活并停止原生速率Timer。它监听同一Vehicle的 `mavCommandResult`，同时核对代次、Vehicle id、manager component和命令1000，只在ACK Accepted后等待150 ms，再复核上下文/控制权并执行最新动作。10秒Timer、ACK失败、上下文变化、对象销毁、显式Acquire/Release和Point Home都会使旧动作失效；pending或姿态执行窗口内由原生 `_tryGetGimbalControl()` 产生的确认信号只被静默抑制，不触发重试；自动流程不活跃时，其他调用来源仍保留原生确认框。Point Home继续直发Vehicle ROI；本文件不实现RC输入、不修改MAVLink协议、也不调用思翼UDP SDK。 |
...
4. 返回Fly View。只要Gimbal Enabled，右侧单个纵向合并栏就应立即显示，不要求飞控或云台已连接；从上到下依次为 `+`、当前目标倍率、`-`、拍照和录像。尚未确认受支持视频流或卡录能力时倍率显示 `--`且缩放按钮禁用。视频第一帧只建立拉流会话门控；随后应看到卡录分辨率、映射上限及最终有效上限的能力摘要；若该时刻已收到0x16，摘要也包含设备上限，否则显示为未确认值。只有0x16与卡录规则不一致时才额外告警。最终上限按卡录4K=1.0、2K=3.5、1080P=5.5、720P=6.0确定，0x16只允许收紧。
...
源码对比和真机结果同时表明，控制权问题与Center专属问题是两个阶段：QGC的Center和Tilt 90都无条件调用 `MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW(1000)`，相同target、NaN角速度和相同flags下，主要差别只有pitch为 `0` 或 `-90`。MAVLink规范用NaN表示未设置，`0°`是合法角度；标准PX4输入实现也不会过滤 `0,0`。因此“自动接管后Tilt 90/Yaw Lock首次可用，但Center首次无物理动作，执行任一前者后Center恢复”最符合飞控输出桥或厂商云台固件仍缓存旧MAVLink `0,0` 目标、没有把首个全零目标判为一次新输入。若失败Center的命令1000 ACK确认为Accepted，可基本把问题定位到Gimbal Manager之后的输出/设备链；若ACK为Denied，则仍是控制权被RC抢回；无ACK则先处理链路。

1. 点击 `Yaw Lock/Follow`、`Center`、`Tilt 90` 或 `Retract`。如果同一活动云台已经满足 `gimbalHaveControl && !gimbalOthersHaveControl`，直接执行原生命令，不额外发送Configure。
2. 如果控制权属于RC/其他控制端，QGC静默调用一次 `acquireGimbalControl()`，发送 `MAV_CMD_DO_GIMBAL_MANAGER_CONFIGURE`，不显示“是否接管”确认框，也不立即盲发姿态目标。
...
4. 如果最新待执行动作不是Center，直接重放一次。如果仍为Center，则先缓存当前遥测pitch、当前Yaw模式以及该模式对应的body/absolute yaw，然后调用原生 `sendPitchBodyYaw(..., false)` 或 `sendPitchAbsoluteYaw(..., false)`。它保持当前物理姿态和Yaw模式，预激活时不应造成可见位移，也不会像Tilt 90那样先转动云台；同时会停止原生500 ms速率Timer，防止它并发发送同一个命令1000。
5. 预激活和真正Center都是命令1000。必须监听同一Vehicle的 `mavCommandResult`，严格匹配manager component、命令号和当前代次；Vehicle在发出结果信号前已经移除在途项。只有ACK Accepted才启动150 ms稳定窗口，窗口结束后再次确认对象和控制权，再调用一次原生 `centerGimbal()`。ACK失败或RC重抢立即取消，不能用固定Timer盲发第二条。
...
6. 验证云台在线发现与飞控解耦：
   - Gimbal Enabled但飞控和云台都未接入时，合并栏仍必须立即显示并占用完整布局尺寸；Manager继续在后台探测，状态点灰显、倍率显示 `--`，两个缩放按钮禁用，tap不得发送0x0f且hold不得发送0x05。拍照仍可尝试发送，录像保持 `...` 和禁用。错误来源逻辑IP、短包、错误帧头/长度/CRC、control不是0x02或业务payload非法都不能把 `sdkResponding`置true。
...
8. 使用 `MNT_MODE_IN=Auto (0)` 验证顶部原生云台姿态栏自动接管：
   - QGC已拥有控制权时，Center/Tilt 90/Yaw Lock/Follow/Retract各点击一次只产生一次原生动作，不额外发送Configure。
   ...
   - 单独重复RC移动后点击Center：状态确认后先出现一条命令1000预激活，参数来自当时实际pitch/yaw且Yaw模式保持不变；其ACK Accepted后约150 ms出现第二条命令1000，pitch/yaw必须为 `0,0`。预激活不应造成可见位移，第二条应使云台居中。
   - 分别注入预激活ACK Accepted、Denied和无ACK：Accepted才允许第二条Center；Denied立即取消；无ACK由总超时取消。两条命令不能在首条ACK前同时进入Vehicle pending列表。
...
RC控制后顶部Center仍在第一次点击弹确认框或无动作时，先确认APK/桌面程序已重新编译 `custom.qrc`，其中存在 `QGroundControl/Toolbar/GimbalIndicator.qml` alias；该URL由拦截器从原生 `qrc:/qml/QGroundControl/Toolbar/GimbalIndicator.qml` 重定向，旧资源缓存仍会运行原生逻辑。随后抓取 `MAV_CMD_DO_GIMBAL_MANAGER_CONFIGURE`、`GIMBAL_MANAGER_STATUS`、两条 `MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW` 及各自 `COMMAND_ACK`：Configure只有一次但10秒内始终没有状态确认，应检查MAVLink转发和Gimbal Manager状态上报；状态确认后立刻又回到RC或命令1000返回Denied，说明摇杆仍在持续产生输入，本实现按安全边界不循环争抢。自动接管后的Center应先出现一条使用当前姿态/当前Yaw模式的预激活命令，ACK Accepted约150 ms后才出现 `0,0,NaN,NaN` 的真正Center；只有一条1000说明预激活ACK未匹配或超时。两条均ACK Accepted但第二条仍无物理动作时，应检查 `MNT_MODE_OUT`、飞控到云台的下行MAVLink及厂商固件，不再归因于QGC按钮或控制权弹窗。
