...
- **已有实测基础**：Ubuntu 24.04 下 A8 Mini 云台控制及 H.265 RTSP 播放正常；Android 下同一云台使用 H.264 编码播放正常。本轮对 MT11 `rtsp://192.168.144.24:8554/video1` 的独立探测已确认：URI是标准 `rtsp://`，SDP给出 H.265 Main、1920×1080、30 fps；当前测试网络中默认UDP未收到RTP媒体，而强制RTSP interleaved TCP的独立探测可以收流。最新自定义QGC日志则表明第二receiver在source pad建立前异常停止，尚未进入H.265解码；其精确GStreamer bus error此前被日志级别过滤缺陷隐藏，因此不能只凭独立探测把QGC黑屏最终归因于UDP，也不能归因于双路帧同步。
...
- 仅保留 `src/CMakeLists.txt`、`src/Vehicle/VehicleSetup/VehicleSummary.qml` 两处feature必需例外，`VideoReceiver.h`/`GstVideoReceiver.h/.cc` 三处通用RTSP transport与无媒体恢复最小扩展，以及 `QGCLogging.cc` 的通用日志级别过滤修复；其余功能通过custom C++、QRC、独立custom QML模块、QML URL拦截和Android overlay接入。`Auto` 首次启动仍使用原生自动协商，无媒体时才在下一次重启显式回退TCP；core不引入相机型号、产品IP或custom设置依赖。
...
- custom 翻译加载、简体中文目录和 `lupdate` 更新脚本已经接入；视频层文案已同步为通用 Video 1/Video 2以及两路RTSP-over-TCP开关。本轮最终校验统计为：`custom.ts` 与 `custom_zh_CN.ts` 均为18个context、146条message，context/source完全一致；英文模板146条保持unfinished，中文146条全部finished，unfinished/空译文均为0，`lrelease` 验证通过。

## 2. 开发边界

1. 二次开发业务仍位于 `custom`。`src` 只允许已登记的受控例外：`src/CMakeLists.txt`、`src/Vehicle/VehicleSetup/VehicleSummary.qml`，为所有VideoReceiver提供 `Auto/Tcp` 选项、RTSP安全超时与无媒体TCP fallback的 `src/VideoManager/VideoReceiver/VideoReceiver.h`、`GStreamer/GstVideoReceiver.h`、`GStreamer/GstVideoReceiver.cc`，以及按实际 `QtMsgType` 保留info/warning/critical的 `src/Utilities/QGCLogging.cc`。这些改动保持通用且首次Auto协商兼容原生；URL分路、产品默认值、设置UI和重启编排仍全部在 `custom`。未登记的新 `src` 改动不允许并入。
...
| `custom/src/Settings/VideoCustom.SettingsGroup.json` | 定义三个通用RTSP Fact：第二路URL `secondaryRtspUrl`（缺省 `rtsp://192.168.144.24:8554/video1`，空值禁用Video 2）、第一路 `primaryRtspTcpOnly=false`（Auto）和第二路 `secondaryRtspTcpOnly=true`（TCP）。元数据不含MT11型号语义，不创建receiver，也不把 `rtsp://` 改写为其他scheme。 |
...
| `custom/src/UI/AppSettings/VideoSettings.qml` | Application Settings -> Video 的同路径覆盖页。保留原生Video Source、Connection、播放设置和Local Video Storage；当选择RTSP源时，在同一 `Connection` 组内将原生 `[Video]/rtspUrl` 标记为 `RTSP URL 1`，将 `[Video]/secondaryRtspUrl` 标记为 `RTSP URL 2`，并在各URL下方分别提供 `Use RTSP-over-TCP for URL 1/2` 开关。默认组合是URL 1 Auto、URL 2 TCP；设置页始终保留标准 `rtsp://` 地址，说明TCP用于UDP媒体包无法穿过图传/网络的情况。第二路为空时不显示Video 2；URL 2与配置中的URL 1或主receiver当前实际URI相同时显示警告并由Manager禁用重复接收器。Local Video Storage继续提供共享 `localMediaStorageEnabled`，只控制A8/MT11各自本地附加支路，不关闭相机SD动作；实际双路receiver、相机命令、录像、截图和解码策略由对应Manager执行。 |
...
| `custom/translations/custom.ts` | `lupdate` 生成的英文源字符串模板，供各语言目录对齐context/source，CMake不把它编译为运行时 `.qm`。它已包含通用RTSP URL 1/2、两路RTSP-over-TCP开关与说明、重复URL警告、双相机选择、MT11 SDK、RGB/热成像、本地媒体和连续变焦安全超时文本。最终校验为18个context、146条message，英文模板146条保持unfinished，与简体中文目录的context/source完全一致。 |
...
| `src/Utilities/QGCLogging.cc` | 自定义消息处理器按收到的 `QtMsgType` 调用 `QLoggingCategory::isEnabled(type)`。旧实现无论消息级别都检查 `isDebugEnabled()`，当 `qgc.*.debug=false` 时会错误吞掉同类别的info、warning和critical，导致GStreamer effective transport及bus error在Application Messages中消失；修复不改变各类别自身的过滤规则。 |
...
| `[Video]/primaryRtspTcpOnly` | bool / `false` | URL 1独立transport选择。`false`=原生Auto，不限制 `rtspsrc` 自动协商；`true`=在标准 `rtsp://` 上强制interleaved TCP。更改后由custom通知原生Video 1 receiver停止并按新传输重启。 |
| `[Video]/secondaryRtspTcpOnly` | bool / `true` | URL 2独立transport选择。产品缺省为TCP，用于已确认默认UDP收不到RTP、interleaved TCP可播放的MT11/图传路径；其他设备仍可关闭以回到Auto。更改后只停止并重启Video 2 receiver，以重建其播放管线。 |
...
1. Application Settings -> Video先选择RTSP源，再在同一Connection组配置 `RTSP URL 1` 与 `RTSP URL 2`。产品默认分别为 `rtsp://192.168.144.25:8554/main.264` 与 `rtsp://192.168.144.24:8554/video1`；URL 1严格对应Video 1，URL 2严格对应Video 2，两者都不使用相机型号作为设置名。每个URL下方的RTSP-over-TCP开关只影响该路transport；缺省为URL 1 Auto（关）、URL 2 TCP（开）。URL仍填 `rtsp://...`，不填 `rtspt://...`。修改URL 2或它的transport时 `DualVideoManager`有序停止并重启该receiver，以按新值重建播放管线；URL 2与配置URL 1或主receiver实际生效URI相同时显示明确警告并禁用Video 2。Application Settings -> Fly View仍分别配置A8与MT11的Enabled/SDK Host/SDK Port/Zoom Step，但这些只控制右栏SDK。
...
QGCApplication
  ...
  -> CustomPlugin::init()
     ...
     -> VideoCustomSettings
        ...
        -> `[Video]/secondaryRtspTcpOnly=true`：Video 2默认interleaved TCP
        -> 两路都保留标准rtsp:// URI；开关选择rtspsrc transport，不改写为rtspt://
...
Application Settings / Video
  ...
  -> 保留原生 Video Source / Connection / Settings / Local Video Storage
     -> Connection（选择RTSP源时）
        ...
        -> Use RTSP-over-TCP for URL 1 = `[Video]/primaryRtspTcpOnly`，默认false/Auto
        -> RTSP URL 2 = `[Video]/secondaryRtspUrl` -> Video 2，默认rtsp://192.168.144.24:8554/video1
        -> Use RTSP-over-TCP for URL 2 = `[Video]/secondaryRtspTcpOnly`，默认true/TCP
        -> TCP开关只选择interleaved transport，URL仍为rtsp://
...
2. Application Settings -> Video保留全部原生设置组；选择RTSP源后，Connection同组同时显示 `RTSP URL 1` 和 `RTSP URL 2`，分别对应Video 1与Video 2，每路下方都有独立 `Use RTSP-over-TCP` 开关。新安装默认必须分别为 URL 1 `rtsp://192.168.144.25:8554/main.264` + Auto（TCP关）、URL 2 `rtsp://192.168.144.24:8554/video1` + TCP（开）。两个地址都必须保持标准 `rtsp://`，不得用 `rtspt://` 冒充transport设置。分别切换两路开关并确认只重启目标receiver，值持久化后重启应用仍保持。清空URL 2后Video 2移除；URL 2与URL 1设置值或主receiver实际URI相同时，页面必须告警且第二receiver不得启动。Local Video Storage保留本地照片/录像开关；Video Stream Integration在所有平台显示两个原有开关，H.265强制硬解缺省为true且仅Android生效。
...
9. Ubuntu 24.04下对MT11做transport分层验收：用SDP/discoverer确认 `rtsp://192.168.144.24:8554/video1` 仍是标准RTSP、H.265 Main、1920×1080、30 fps；默认Auto/UDP对照应复现无媒体数据，同一URL启用TCP后应出现 `Forcing RTSP-over-TCP transport for ...`，随后streaming、decoding和画面全部成立。再将该URL单独放入URL 1并开启URL 1 TCP，必须同样可播放，用于证明修复不依赖Video 2 UI。同时保留A8 H.265回归：URL 1默认Auto下仍正常。Ubuntu/虚拟机代理需将A8的 `192.168.144.25`和MT11的 `192.168.144.24`同时加入忽略列表。
...
2026-08-18附件日志中，Video 2每轮都以 `rtsp://192.168.144.24:8554/video1`、`transport TCP`、`requestedTimeout 20` 请求启动；`start/startDecoding statusCode 0 success true` 只是异步请求被受理。整段运行没有 `Secondary video streaming ... true`、首个source媒体帧、decoding true或视频尺寸，并在约1–5秒后异常停止、按2/4/8/15秒退避重试。该停止早于20秒source watchdog，也早于只在streaming后启动的decode watchdog，因此故障点位于RTSP协商或 `rtspsrc -> parsebin` 建立source pad之前，尚未进入H.265硬/软解码及QML surface。

同一附件无法给出精确GStreamer错误，是因为旧 `QGCLogging::msgHandler` 对debug、info、warning、critical一律调用 `isDebugEnabled()`；运行规则中的 `qgc.*.debug=false` 因而把 `GstVideoReceiver` 的effective protocols、EOS和bus error全部错误丢弃。当前代码已改为 `isEnabled(type)`，关闭debug后仍保留对应类别允许的info/warning/critical。附件52秒处还显示主 Video 1 receiver的实际URI也为 `.24/video1`，但这发生在前几轮Video 2失败之后，不能单独解释所有早期失败；它仍证明旧重复检测只比较设置Fact不够。当前DualVideoManager已同时监听配置URL 1和主receiver实际URI，任一与URL 2相同都会停止Video 2并显示警告。下一轮必须干净重编译后取得未被过滤的 `GStreamer error`，才能最终区分服务端会话拒绝、transport、插件或parsebin协商。MT11 SDK的ACK告警属于独立UDP控制链路，不能用于判断RTSP是否到达。
...
截至2026-08-18，本轮已独立编译并运行 `Mt11ProtocolTest`，QtTest报告9 passed、0 failed：7个业务slot加框架init/cleanup，覆盖SDK文档命令帧、RGB/热成像模式帧、严格CRC/长度/control解析、UDP合包原子性和业务payload。该结果仅属于纯 `Mt11Protocol`，不覆盖 `Mt11Sdk`真实UDP、Manager计时、QML、GStreamer、双receiver或真机固件。本轮对MT11 `rtsp://192.168.144.24:8554/video1` 的独立Ubuntu探测确认它是H.265 Main 1920×1080@30 fps的标准RTSP endpoint，当前网络下TCP对照可收流；最新自定义QGC日志则确认其Video 2在source pad前异常停止，精确bus error因旧日志处理器错误过滤尚未取得，因此QGC黑屏最终根因仍须用修复后的日志闭环。当前工作树已将视频层改为通用Video 1/Video 2：`[Video]/rtspUrl` 与 `[Video]/secondaryRtspUrl` 同属Connection组，并分别使用 `primaryRtspTcpOnly=false`（URL 1 Auto）与 `secondaryRtspTcpOnly=true`（URL 2 TCP）；两路均保留标准 `rtsp://`。`VideoReceiver/GstVideoReceiver`只增加默认Auto、向后兼容的通用transport hook，产品策略、Fact与重启编排仍在custom；`QGCLogging`按真实消息级别过滤，不再因关闭debug而丢失GStreamer warning/critical。第二receiver具有配置URL和主receiver实际URI双重重复源保护、Loader实际Item/window直接初始化、真实GL Item门控、解码启动结果处理和首帧watchdog，GL Item在WAITING背景下仍保留于场景图；Android强制H.265厂商硬解默认开启，每条播放管线创建独立adapter、MediaCodec element和输出队列。`DualPipView`使用固定上/下槽与主视图精确交换，并保留原生PIP显隐、独立窗口与拖拽resize；`FlyViewSecondaryVideo`/`FlightDisplayViewSecondaryVideo` 已取代型号化显示类。两份TS均为18个context、146条message且context/source一致；英文模板146条保持unfinished，简体中文146条全部finished、unfinished/空译文均为0，`lrelease`验证通过。

最新附件日志的确定结论是：Video 2的start/startDecoding请求均以数值0受理，但整段运行没有streaming或decoding true；receiver输出source pad尚未建立或仍在建立过程中，且没有首个媒体buffer进入receiver的证据，因此不是双H.265同步、MediaCodec争用或QML渲染阶段。网络上是否已出现RTP仍须由修复后的bus日志或抓包判断；旧日志中的 `STATUS_MIN` 就是数值0的 `STATUS_OK` 别名。工作树已修复按debug状态误删全部GStreamer日志级别的问题，并让重复源保护同时比较URL 1设置值和主receiver实际URI；RTSP安全超时钳制、Auto无媒体后的下一轮显式TCP fallback、protocols回读，以及Video 2 transport/timeout/statusCode/success日志继续保留。这些修正只有完成core干净重编译并取得明确bus error、effective protocols、source frame和decoding链路后，才能视为进入真机验证。
