...
- **本轮云台方位角计算修正**：问题路径位于当前原生 `src/Gimbal/GimbalController.cc`：当 `GIMBAL_DEVICE_ATTITUDE_STATUS` 明确声明 `YAW_IN_VEHICLE_FRAME` 时，原生实现固定使用四元数yaw加 `Vehicle.heading`，没有读取同一消息的 `delta_yaw`。具有独立航向估计的云台，其 `delta_yaw` 与飞控heading可能不完全一致；航向锁定时直接叠加飞控heading会使显示方位角随基座明显转动。为保持后续merge友好，本轮不修改 `src`，而在 `custom/src/Gimbal` 新增只读 `GimbalAzimuthProvider` 与纯计算 `GimbalAzimuthPolicy`：显式Vehicle frame优先按MAVLink关系 `q_earth = q_delta_yaw * q_vehicle` 求世界方位，`delta_yaw`不可用时才回退原生的飞控heading算法；显式Earth frame直接使用上报四元数；无新frame位的旧设备继续严格按 `YAW_LOCK` 判定并忽略扩展字段。顶部 `GimbalIndicator` 的Az值和顶部罗盘共用该Provider，跟随模式原有计算路径保持兼容，锁定模式则可使用云台自身的地/机体系关系保持世界朝向稳定。代码级策略测试已通过，完整Qt 6/Android构建与目标云台真机回归仍待完成。
...
| 文件 | 详细作用 |
|---|---|
| `custom/CMakeLists.txt` | custom 构建总入口。向根工程注入 `QGC_CUSTOM_BUILD`、`CUSTOMHEADER=CustomPlugin.h` 和 `CUSTOMCLASS=CustomPlugin`，收集各custom C++、QML模块、资源和翻译，并继续通过目录递归收集GStreamer生产源码。Bluetooth被列为custom必需Qt组件并显式链接 `Qt6::Bluetooth`，不依赖原生Bluetooth Link开关的偶然传递。桌面 `QGC_BUILD_TESTING` 构建创建既有协议/媒体/UniRC测试、纯Qt `AndroidH265DecoderRoutePolicyTest`，以及本轮新增的 `GimbalAzimuthPolicyTest`/`check_gimbal_azimuth_policy`。Android configure仍把根 `android`模板复制到build目录后用 `custom/android`同路径覆盖，并只编译唯一合并源目录；生成副本注入Android 12+的 `BLUETOOTH_SCAN/CONNECT`，以及仅限API 30及以下的 `BLUETOOTH/BLUETOOTH_ADMIN/ACCESS_FINE_LOCATION`，再校验Manifest、USB管理器和媒体库overlay。UniRC本身改为按MAC直连后不启动发现器，但根工程默认启用的原生Bluetooth Link仍具有设备发现能力，因此不能仅因设置页删除扫描按钮就移除全应用的SCAN/旧版定位声明。旧Bluetooth关闭状态helper及其marker校验已删除。没有修改根 `android/AndroidManifest.xml`。外部WGS84样例目录不参与构建或安装。 |
...
| `custom/src/Gimbal/GimbalAzimuthPolicy.h` | 声明无QObject依赖的云台方位角计算输入、结果、来源与错误类型。输入明确区分Earth/Vehicle frame位、legacy `YAW_LOCK`、`delta_yaw`是否被设备支持/当前是否有效，以及飞控heading回退是否有效；结果只输出世界方位角，不保留罗盘不再使用的相对角API。 |
| `custom/src/Gimbal/GimbalAzimuthPolicy.cc` | 校验并归一化MAVLink `[w,x,y,z]` 四元数，按 `q_earth=q_delta_yaw×q_vehicle` 完成显式坐标系转换，并把角度规范到 `[-180°,180°)`。显式Vehicle frame优先使用有限 `delta_yaw`、缺失时回退飞控heading；显式Earth frame直接输出上报yaw；legacy消息按 `YAW_LOCK`判定且强制忽略delta；两个frame位冲突、四元数非法或缺少地系参考时不猜测有效值。 |
| `custom/src/Gimbal/GimbalAzimuthProvider.h` | 声明由 `CustomPlugin`持有的只读QObject Provider，向QML公开 `valid`、`absoluteYaw`、`usingDeltaYaw`和 `referenceSource`。内部缓存按Vehicle及MAVLink source component/reported device id隔离，保存消息时间、设备启动时间和扩展字段支持状态，并以Timer统一执行2秒新鲜度门禁；它只服务顶部显示，不替换原生Gimbal对象，也不发送命令。 |
| `custom/src/Gimbal/GimbalAzimuthProvider.cc` | 观察消息285并解码flags、q、`delta_yaw`、`delta_yaw_velocity`与 `gimbal_device_id`。后置的非零device id会迫使前面的默认0进入payload，因此“长度覆盖delta”本身不能区分合法首次0和旧发送端未填写字段；Provider要求消息恰好声明一个显式yaw frame，并由delta/delta速度的有限非零值、负零或NaN等线上语义非默认值首次证明扩展支持。MAVLink 2可能裁掉后续零字节，因此同一数据流一旦证明支持就保持粘性，后续合法0仍走delta路径；设备启动时间回退时重新探测。Provider调用Policy计算后，按活动Vehicle与activeGimbal身份优先选择精确manager/device路由，只有精确路由从未出现时才回退独立设备路由，避免另一组件用较新样本串扰。heading回退要求已观察到真实heading消息；消息285超过2秒未更新、整机链路丢失、Vehicle/Gimbal切换或对象销毁时统一发布无效结果，顶部工具栏Az与罗盘不会继续显示旧值。 |
...
| `custom/test/Gimbal/GimbalAzimuthPolicyTest.cc` | 云台方位角纯QtTest回归。覆盖显式Vehicle frame的delta优先、有效零delta与heading回退、显式Earth frame、frame位覆盖 `YAW_LOCK`、legacy锁定/跟随严格忽略delta、冲突flags、非法/非单位四元数、缺少地系参考、有限及非有限角度边界，以及连续样本中的“锁定时基座转动而世界方位保持”“跟随时相对角固定而世界方位随基座变化”。当前使用MSVC + QtTest 5.14兼容harness实际运行27 passed、0 failed、0 skipped；正式Qt 6目标为 `check_gimbal_azimuth_policy`。该测试不替代CustomPlugin消息接线、多云台路由或目标云台真机验证。 |
...
双罗盘的文件连接关系固定为：`FlyViewCustom.SettingsGroup.json` 定义两个开关元数据 -> `FlyViewCustomSettings.h/.cc` 创建、读取、保存并向QML暴露 -> `FlyViewSettings.qml` 提供两个独立用户开关 -> `CustomPlugin::mavlinkMessage()`把消息285交给 `GimbalAzimuthProvider` -> Provider按活动Vehicle/manager/device调用 `GimbalAzimuthPolicy`发布世界方位 -> `FlyView.qml` 向custom overlay注入右上安全预留 -> `FlyViewCustomLayer.qml` 分别组合Vehicle heading门禁和Provider/activeGimbal/链路门禁 -> 两个Loader复用 `FlyViewCompassBar.qml` 绘制。顶部 `GimbalIndicator` 的Az也读取同一Provider。右侧A8/MT11相机选择器不在这条设置或遥测链中；任何单一文件都不能独立完成完整功能。
...
方位角计算严格遵循 `GIMBAL_DEVICE_ATTITUDE_STATUS.flags`，内部统一输出 `[-180°, 180°)`；罗盘绘制时再等价归一化为 `[0°, 360°)`：

| 消息情况 | 世界方位角计算 | 兼容行为 |
|---|---|---|
| 明确设置 `YAW_IN_EARTH_FRAME` | `azimuth = yaw(q_earth)` | 四元数已经相对地理北向，不叠加飞控heading。 |
| 明确设置 `YAW_IN_VEHICLE_FRAME`，且 `delta_yaw`可用 | `q_earth = q_delta_yaw × q_vehicle`，`azimuth = yaw(q_earth)` | 优先采用云台消息自己给出的地/机体系关系；这是本轮锁定显示修正的主路径。 |
| 明确设置 `YAW_IN_VEHICLE_FRAME`，但 `delta_yaw`缺失或NaN | `azimuth = wrap(yaw(q_vehicle) + Vehicle.heading)` | 保持旧设备和当前正常跟随设备的原生兼容路径。 |
...
custom Provider逐帧接收 `GIMBAL_DEVICE_ATTITUDE_STATUS`，为每个Vehicle/manager/device保存单调接收时间，并每250毫秒检查一次；活动样本超过2秒未更新时同时令顶部工具栏Az无效并隐藏顶部罗盘。整机 `vehicleLinkManager.communicationLost` 会立即清除该Vehicle缓存，恢复通信后必须重新收到heading（仅兼容回退需要）和消息285才能恢复显示。新鲜度只按消息接收时间判断，不能根据角度是否变化判断，因为静止云台可持续上报相同值。
...
28. 顶部条必须通过门禁与坐标系矩阵：开关打开但无Vehicle、无activeGimbal、`vehicleLinkManager.communicationLost=true`或Provider方位角无效时隐藏。显式Earth frame直接核对q的yaw；显式Vehicle frame分别核对有效 `delta_yaw`（包括已确认支持后的0值）优先和缺失/NaN时heading回退；在尚未收到真实飞控heading前不得把Fact初始0值用于回退；无frame位分别核对legacy锁定/跟随且确认delta被忽略；两个frame位同时设置必须拒绝。航向锁定测试应先锁在一个非零世界方位，再同时转动飞控与云台基座，物理朝向不变时罗盘和顶部Az应基本不变；航向跟随测试保持云台相对机头角不变并转动飞控，两处世界方位应同步变化。多云台环境从顶部MAVLink栏切换activeGimbal，方位角必须优先按精确Vehicle/manager/device身份切换；右侧A8/MT11相机选择、私有SDK断网、视频工作模式及RTSP有无不得改变数据源。保持飞控链路正常而单独停止消息285时，两处应在2秒门槛后的下一次250毫秒检查内失效，随后罗盘隐藏且顶部Az显示 `--`；恢复消息后自动恢复。整机链路丢失后重连也必须等待本次连接的新heading和消息285，不得复用断链前缓存。
