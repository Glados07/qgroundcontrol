...
- 在 `custom` 中实现思翼私有 UDP SDK 和协议封装，包括帧组装、CRC16、固定为0的协议sequence字段，以及0x05、0x0a、0x0b、0x0c、0x0f、0x16和0x18。0x05与0x16只保留协议兼容接口，当前Manager/UI不调用；缩放控制统一使用0x0f绝对倍率和0x18实际倍率查询。0x0f的一字节ACK只表示命令受理，不能更新显示。为兼容现有设备当前仍未递增sequence，因此不能在协议层关联同命令迟到包；Manager以单目标/单查询在途、350 ms隔离和两次目标命中降低该风险。
- A8 Mini数字变焦上限只以QGC实际解码拉流尺寸确认，不能使用机内卡录分辨率或0x16替代。GStreamer路径有两条互相独立的首选可信来源：custom在主视频 `qgcvideosinkbin` 的sink pad等待真实首帧，并依次检查外层pad、已连接解码器peer及ghost target的current CAPS；同时观察 `GstGLQt6VideoItem` 在最终 `GstVideoInfo` 生效后写入的 `implicitWidth/implicitHeight`。任一来源首次得到受支持尺寸都会直接投递Manager，pad路径还会修正原生 `VideoManager::videoSize`。针对部分平台视频已经正常显示、`VideoManager`也已发布真实尺寸、但两个custom观察器均未回调的情况，Manager增加低优先级兜底：仅在解码已开始、没有收到任何有效直接观察尺寸、且 `VideoManager::videoSize` 为受支持尺寸并连续稳定1秒后才锁存；一旦出现有效直接观察结果（包括不受支持尺寸）就立即取消兜底，避免暂态值覆盖真实sink结果。当前用户场景只接受1920×1080（以及设备解码链可能暴露的coded-height对齐值1920×1088）→5.5x、2560×1440→3.5x；4K、720P及其他尺寸不映射。Manager生命周期内首次受支持尺寸即锁存一次，后续断流、重连和尺寸变化都不重算；锁存前尺寸无效或不受支持时保持unknown并锁定缩放。Manager不发送0x16查询。
- `currentZoom`只保存经过协议校验和状态机确认的0x18设备回读，不在0x0f写入成功或ACK成功后乐观显示请求目标；命令确认期间 `zoomStatusKnown=false`，界面显示 `--`。同一时刻只允许一个0x0f目标在途，快速连点会被按钮和Manager共同拒绝，不再按requested累计。每个绝对目标必须由两个串行0x18查询都命中后才发布；任意1.8x、2.8x等运动中间值只会重置目标命中计数。`zoomStep`网格固定锚定1.0x：所有中间移动严格使用完整步长；只有下一完整步会越过相机真实小数上限时，才允许一次边界吸附。步长1.0x时1080P序列为1/2/3/4/5/5.5x，2K为1/2/3/3.5x；从小数上限缩小时先回到5.0x或3.0x，再继续完整步长。
- `GimbalControlManager` 直接接收上述主显示sink的协商尺寸；GStreamer构建还监听 `VideoManager::videoSizeChanged/decodingChanged`，但只用于无直接结果时的1秒稳定兜底，非GStreamer构建则直接使用这两个原生状态。2秒轮询入口只负责补查尚未完成的首次锁存。能力锁存会显式通知QML重新计算两个方向的可用性；功能已启用时立即启动0x18稳定同步，后台每2秒在空闲时继续查询0x18并查询0x0a相机状态。绝对目标或稳定确认期间由专用串行查询状态机接管。每次先等待350 ms隔离旧包，同一时刻只允许一个0x18查询在途；单次查询1秒无响应才重试，整次缩放操作10秒未确认才置unknown。该探测不依赖控件可见或活动飞行器。接收端以 `QHostAddress::isEqual()`比较来源逻辑IP，并要求精确帧长、CRC及ACK control严格等于0x02；只有业务payload也合法的回包才置 `sdkResponding=true`。
- 飞行界面右侧使用单个始终可见的纵向半透明控制栏：只要Gimbal功能已启用，无论是否连接飞控或云台都会显示；从上到下依次为放大、当前倍率、缩小、横向分隔线、思翼机内拍照和录像。离线、拉流尺寸未知、倍率尚未确认或命令pending时显示 `--`并禁用缩放；已到真实上限或下限且该方向没有下一目标时保持显示真实倍率，只禁用该方向。最后一个整数网格点仍可向小数上限吸附，小数上限也可先回到相邻整数网格点。取得锁存的1080P/2K能力及稳定倍率后，短按和长按才按同一目标序列工作。
...
- 在Fly View设置页以 `SIYI Gimbal Camera` 标题提供云台相机启用、SDK IP、SDK端口和缩放步长设置，并明确短按发送一个目标、长按重复已确认目标、中间移动使用完整步长、最后一次可吸附真实小数上限并在反向时先返回相邻网格点；首次受支持的QGC实际解码拉流尺寸在本应用会话锁存一次，机内卡录分辨率和0x16不参与上限。
...
| 文件 | 详细作用 |
|---|---|
| `custom/src/Gimbal/A8MiniZoomPolicy.h` | A8 Mini缩放策略的纯静态接口，将视频尺寸到最大倍率映射、固定网格和真实边界吸附计算从Manager状态机中分离，便于独立测试；不访问网络、QSettings或UI。实际拉流尺寸来源优先级、稳定兜底和会话锁存由Manager强制执行。 |
| `custom/src/Gimbal/A8MiniZoomPolicy.cc` | 只实现用户实际使用的两种拉流规格：2560×1440→3.5x、1920×1080→5.5x；同时把设备解码链可能暴露的coded-height对齐值1920×1088明确视为1080P→5.5x，其他尺寸返回不支持。倍率网格以1.0x为固定锚点并使用十分之一整数计算：中间目标严格相差完整 `zoomStep`；若下一完整步会越过小数上限，则唯一一次吸附到真实上限；从该上限缩小时先回到最高合法网格点。该规则是显式边界转换，不使用通用 `qBound`在其他位置制造部分步长。 |
...
| `custom/src/Gimbal/GimbalControlManager.h` | 思翼云台相机业务的QML门面和运行态声明。`currentZoom`只表示设备确认值，另向QML暴露按固定网格及真实边界转换计算的 `zoomInAvailable/zoomOutAvailable`。能力状态保存首次拉流尺寸锁存、直接观察尺寸、`VideoManager`兜底候选及1秒稳定Timer；倍率状态分别保存唯一绝对目标、目标命中次数、稳定值候选、查询在途门禁、350 ms隔离Timer、1秒单次查询Timer和10秒操作Timer。默认5.5x仅是能力未知时的界面占位，不能用于发送缩放命令。 |
| `custom/src/Gimbal/GimbalControlManager.cc` | 由 `CustomPlugin`创建一次并持有 `SiyiSdk`，没有修改或复制 `src/VideoManager`。GStreamer构建优先接受主显示sink直接报告的最终协商尺寸；当两个直接观察器均没有给出任何有效尺寸时，才允许已解码且连续稳定1秒的受支持 `VideoManager::videoSize`兜底，避免“画面正常但能力永远unknown”，同时防止原生 `query_caps` 早期值抢先锁错。任何有效直接尺寸都会取消该兜底，即使该尺寸不受支持；非GStreamer构建继续直接只读复用 `VideoManager::decoding/videoSize`。首次观察到1920×1080、1920×1088或2560×1440即锁存上限并显式通知QML重算方向可用性，后续断流、重连和尺寸变化均忽略，0x16完全不参与；锁存日志包含尺寸、来源和上限，倍率双确认完成时另有“zoom controls are ready”日志。单击和长按共用串行0x0f目标路径；中间移动使用完整步长，最后一个网格点与真实小数上限之间允许显式边界吸附及反向返回。一个目标在途时拒绝追加操作，只有两个独立0x18查询都命中目标才更新 `currentZoom`。状态机保证同一时刻最多一个0x18查询，超时经隔离后重试；启动或外部控制留下的其他非网格稳定值会在显示解锁前吸附到最近合法停点（固定网格点或真实上限）。功能关闭后忽略关闭前请求的迟到回包，不能重新点亮在线状态或触发自动倍率修正命令。 |
...
| `custom/src/Gimbal/SiyiProtocol.cc` | 实现 `55 66`帧头、control 0x01、小端payload长度、固定0的seq、command、payload和小端CRC16（多项式0x1021、初值0）。整帧长度必须与payload声明精确相等，尾随字节也会拒绝；入站业务ACK必须严格为control 0x02。0x05 ACK单独按小端uint16/10解析；0x16/0x18严格按官方“整数字节+一位小数字节”解析，`12 00`、`1c 00`、`37 00`等不再猜测为1.8x、2.8x、5.5x。0x0f ACK只接受单字节0/1。 |
| `custom/src/Gimbal/SiyiSdk.h` | `QUdpSocket`传输层接口声明，保留0x05/0x16兼容接口并提供绝对缩放、当前倍率查询、拍照、录像和状态查询；它不负责拉流尺寸、步长网格、手势状态或设置持久化，Manager/UI不会调用0x05/0x16。 |
| `custom/src/Gimbal/SiyiSdk.cc` | 将Protocol帧通过 `QUdpSocket::writeDatagram()`发到配置endpoint；接收侧按逻辑IP、精确帧长、CRC和control 0x02过滤，并且只在对应业务payload解析成功后发出 `packetReceived`。`gcs.custom.gimbal.sdk`记录每个收发命令和原始payload；0x0f受理反馈、0x16和0x18分别按各自格式分发，不能交叉解释。空包、短写或无效endpoint通过 `communicationError`交给Manager。 |
| `custom/test/Gimbal/SiyiProtocolTest.cc` | custom独立QtTest协议与策略回归用例。覆盖封包量化、ACK control、坏CRC、严格整帧长度、0x05/0x0f/0x16/0x18格式隔离、非法legacy载荷、2K/1080P及1920×1088编码对齐映射、固定网格对齐，以及短按/长按共用的中间完整步长和双向小数上限吸附序列。测试只在桌面 `QGC_BUILD_TESTING`注册，并提供 `check_siyi_protocol`独立目标。 |

云台相机完整调用链为：`VideoReceiver`解码当前视频 -> `PulledVideoResolutionProbe`在真实首帧从sink/decoder-peer/ghost-target current CAPS取得实际尺寸，或 `GstGLQt6VideoItem` 从最终 `GstVideoInfo` 发布隐式尺寸；若两者均未报告，则以已解码且稳定1秒的受支持 `VideoManager::videoSize`低优先级兜底 -> `GimbalControlManager`锁存首次受支持的1080P/2K拉流能力并显式通知QML，功能已启用时启动0x18同步 -> `A8MiniZoomPolicy`计算以1.0x为锚点的中间完整步长及双向真实边界吸附 -> `GimbalZoomControl.qml`把短按和长按都映射到同一串行0x0f目标路径 -> `SiyiSdk/SiyiProtocol`收发0x0f ACK与0x18 -> Manager只在两次查询都命中目标后发布倍率并点亮可用方向按钮。机内卡录配置和0x16不在该调用链中。
...
Gimbal启用后，Manager即使不在Fly View也会持续运行，并每2秒向 `sdkHost:sdkPort`探测：GStreamer构建优先采用主显示sink直接报告的最终协商尺寸，仅在没有任何有效直接结果时用 `VideoManager::videoSizeChanged/decodingChanged` 启动1秒稳定兜底；非GStreamer构建直接使用这两个原生状态。每轮发送0x0a相机状态，只有缩放状态空闲且没有0x18查询在途时才查询当前倍率，Manager不发送0x16。绝对目标和稳定确认由专用状态机管理：先留350 ms隔离窗口，再发送唯一在途的0x18；1秒未收到合法倍率才安排下一次隔离重试，10秒整体deadline不会被重试刷新。Fly View右侧纵向合并栏不等待探测结果，无论是否连接飞控或云台都立即显示。只有来源逻辑IP匹配配置endpoint、通过帧头、精确长度、CRC、control=0x02及对应payload校验的SDK响应，`sdkResponding`才变为true。

有效上限只取QGC实际解码的拉流尺寸，且当前支持1920×1080→5.5x、设备解码链coded-height对齐形式1920×1088→5.5x、2560×1440→3.5x。这里不是相机机内卡录分辨率，也不读取0x16；GStreamer主视频在真实首帧到达时从sink、解码器peer或ghost target的current CAPS读取宽高，同时以主 `GstGLQt6VideoItem` 根据最终 `GstVideoInfo` 设置的隐式尺寸作为第二条直接来源。两条路径任一取得受支持尺寸就直接交给Manager，并由pad路径同步校正 `VideoManager::videoSize`。若两条直接路径都没有报告任何有效尺寸，Manager仅在原生视频状态已解码、尺寸属于上述白名单且连续稳定1秒时以 `VideoManager`兜底；有效直接尺寸会立即取消兜底，因此不受支持的真实拉流不会被另一暂态尺寸绕过。该尺寸在Manager/QGC应用会话内只锁存一次，之后断流、重连或尺寸变化不重算；锁存前视频未解码或尺寸不受支持时保持unknown，单击和长按均锁定。能力确认会独立发出可用性变更通知，并立即启动同步，在350 ms旧包隔离后发出0x18；需两次串行稳定确认并在必要时对齐到固定网格点或真实上限才解锁。普通SDK在线探测使用1.5秒响应窗口；正在进行的倍率确认可在10秒整体deadline内重试。拍照仍可重试，录像必须先取得0x0a状态，因为0x0c是toggle。
...
2. Application Settings -> Fly View -> SIYI Gimbal Camera 中确认IP、端口、缩放分度值和Enabled。页面标题说明保持简短，以免撑坏设置页布局；短按/长按、完整步长、双向边界吸附、首次1080P/2K实际拉流尺寸会话锁存，以及卡录分辨率和0x16不参与等完整规则以本节和Fact元数据为准。
3. Application Settings -> Video -> Video Stream Integration 中选择是否使用 MAVLink 自动视频流；Android 设备确认 H.265 硬解开关开启，然后重启 QGC。
4. 返回Fly View。只要Gimbal Enabled，右侧单个纵向合并栏就应立即显示，不要求飞控或云台已连接；从上到下依次为 `+`、倍率、`-`、拍照和录像。尚未取得有效能力和倍率时倍率显示 `--`且缩放按钮禁用。视频第一帧到达后，正常直接路径应依次看到“Installed main pulled-video resolution observers”、negotiated/observed主拉流尺寸、会话尺寸锁存和发出的命令24（0x18）日志；若直接观察器未回调但原生尺寸有效，则先看到“Waiting for stable VideoManager pulled-video resolution fallback”，约1秒后锁存日志的source为“stable VideoManager fallback”。1920×1080/1920×1088锁存5.5x，2560×1440锁存3.5x，0x16不参与。第二次一致0x18完成后应出现“zoom controls are ready”，中心显示实际倍率，并仅点亮仍有合法目标的方向按钮：1.0x时减号保持灰色属于正确边界行为，最大倍率时加号保持灰色。录像按钮在0x0a状态未知时灰显并显示 `...`。
  ...
  -> CustomPlugin::init()
     ...
     -> FlyViewCustomSettings（FlyView/showHeadingCompassBar）
      -> GimbalControlSettings / GimbalControlManager
         ...
         -> 首次锁存1080P→5.5x或2K→3.5x，并立即启动两次0x18稳定确认
         -> 每轮发送0x0a状态，缩放空闲时发送0x18；Manager不发送0x16
         -> 首次受支持的实际拉流尺寸是唯一上限来源；卡录配置不参与，锁存后不重算
...
6. 验证云台在线发现与飞控解耦：
   ...
   - 只连接A8 Mini网络、正常拉流并接通私有SDK、完全不连接飞控时，合法回包必须把状态点切为绿色；实际拉流尺寸加两份一致0x18恢复实时倍率，0x0a恢复机内录像状态。连接或断开飞控不得影响控制栏可见性和后端选择。
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   - 分别在两个独立QGC会话中验证实际拉流尺寸来源：1920×1080必须锁存5.5x，2560×1440必须锁存3.5x。先验证任一直接观察器报告尺寸时立即取消 `VideoManager`兜底；再模拟两个直接观察器均不回调，要求 `decoding=true`且同一受支持 `VideoManager::videoSize`连续稳定1秒后才锁存，尺寸在1秒内变化时必须重新计时。锁存前给出4K、720P、640×480、空尺寸、`decoding=false`，或先给出有效但不受支持的直接尺寸，均应忽略并保持缩放锁定，不能由兜底绕过；一旦锁存，随后停止解码、重连或报告另一尺寸都不得改变本会话上限。需要有意更换1080P/2K时重启QGC后重新确认首次尺寸。
   - 抓包确认Manager运行期间不发送0x16；只改变机内卡录分辨率或注入0x16响应不得改变5.5x/3.5x上限。0x16 `00 00`及错误格式在Protocol测试中仍应拒绝，但该命令不进入Manager能力状态。
   - 验证0x18只按官方“整数字节+小数字节”解析：`01 00`→1.0x、`01 08`→1.8x、`02 08`→2.8x、`03 05`→3.5x、`05 05`→5.5x；`0A 00`、`12 00`、`1C 00`、`37 00`、`FF FF`、非法小数字节、短/长payload及带尾随字节的完整帧均不得改变倍率。只有0x05 ACK可按小端uint16/10解析，而Manager/UI不调用0x05。
...
Gimbal Enabled但合并栏不显示时，不要检查飞控、云台回包或 `activeVehicle`：当前可见性已完全与连接状态解耦，只要Enabled为true就必须显示。优先检查设置值、custom QRC命中、Loader错误及资源是否重新构建。若控制栏显示但状态点持续灰色，再确认A8 Mini供电和网络、`sdkHost/sdkPort`、本机路由及2秒轮询；RTSP视频与私有UDP SDK是独立链路。接收端接受逻辑等价的IPv4/IPv4-mapped IPv6来源，也不强制回包源端口为37260，但要求来源逻辑IP、帧头、精确长度、CRC、control=0x02及业务payload全部合法。开启 `gcs.custom.gimbal.sdk` 后，日志会显示0x0a/0x0f/0x18等命令和原始payload；正常Manager流程不应出现发出的0x16。视频有画面且SDK已有0x0a回包、但缩放按钮仍灰色时，先找 `gcs.custom.customplugin` 的 `Installed main pulled-video resolution observers`：主流应显示非空receiver/widget类型，正常情况下 `videoItem` 与 `padProbe` 都为true；两路任一路成功取得受支持尺寸即可完成锁存，另一条为false时仍可继续按后续日志判断。随后正常直接路径应出现 `gcs.custom.gimbal.pulledvideoresolution` 的 `Negotiated main pulled-video resolution` 或 `gcs.custom.gimbal.control` 的 `Observed negotiated main pulled-video resolution`；若两路均未报告但原生视频已解码出受支持尺寸，则应出现 `Waiting for stable VideoManager pulled-video resolution fallback`，约1秒后以 `stable VideoManager fallback`为source锁存。再往后应看到 `Latched pulled video resolution for this application session` 和350 ms后 `gcs.custom.gimbal.sdk` 发出的命令24。若直接观察到有效但不受支持的真实尺寸，兜底会被取消，不能用另一尺寸越过白名单；若所有路径都没有受支持尺寸，优先检查安装包是否重新包含最新custom目标和原生 `VideoManager`尺寸状态，而不是从卡录规格或0x16猜测。收到两次一致合法0x18后必须出现 `Confirmed stable SIYI zoom ... zoom controls are ready`；此时1.0x仅加号可用、最大倍率仅减号可用是正确边界，不属于控件未点亮。受支持尺寸为1920×1080、1920×1088或2560×1440；4K、720P和其他尺寸在锁存前会被忽略。若设置页出现分组拉伸、顶部裁切或无法滚到底，检查 `FlyViewSettings.qml` 外层Loader的 `Layout.preferredHeight/minimumHeight`、两个设置组根Loader的 `implicitHeight` 透传，以及是否重新引入了用长说明撑大隐式宽度或手工写 `item.width` 的逻辑。

初始镜头1.0x却显示5.5x、实际与显示不一致或缩小方向异常时，先查看0x18原始payload：官方格式 `01 00`才是1.0x，`01 08`是1.8x；`0A 00`、`12 00`、`1C 00`和`37 00`不得按小端兼容猜测。0x0f发送后UI在两个0x18查询都命中目标前必须为 `--`，不能显示requested冒充实际值。若镜头或按钮上限为3.5，查看control日志中首次锁存的实际拉流尺寸：2560×1440应为3.5，1920×1080必须为5.5；卡录分辨率和0x16不能影响结果。锁存后更换拉流规格不会在本会话重算，如需有意更换应重启QGC。步长1.0时，5.0↔5.5和3.0↔3.5产生0.5距离是预期的真实上限双向吸附；其他中间位置仍必须保持1.0完整步长，1.8→2.8等非网格值不能直接成为用户目标。启动或外部控制留下其他非网格倍率时会自动归整到最近合法停点，该过程显示 `--`且完成后才解锁按钮。长按日志应只包含串行的“0x0f目标、0x18查询、0x18查询”，释放不应出现0x05。
