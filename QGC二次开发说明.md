...
- MT11的 `currentZoom`发布从1.0x锚定且不超过物理上限的完整步长目标，新增 `actualZoom/actualZoomKnown`只发布0x18权威实测值；控制栏分别显示“目标”和“实际”，不再把命令目标伪装成镜头已经到位。默认步长1.0x且设备/产品物理上限为165.1x时，目标最高165.0x、实际可显示165.1x；步长0.1x时165.1x本身也是合法目标。短按仍按独立 `mt11ZoomStep`从上一目标计算1.0～30.0x的0x0F，快速连点替换在途目标；实测大于30.0x时两方向tap关闭。长按420 ms成立后严格逐档运行：实测与目标均不超过30.0x时，每档使用精确0x0F；跨越或处于30.0x以上时，每档使用一个有界0x05脉冲，以100 ms查询0x18，达到或越过目标便立即0x05停止、150 ms后补一份停止，并至少再留50 ms处理窗口后才进入下一档。单个脉冲500 ms仍未到档则硬停并提示错误。目标在每档命令发出时更新，实际值只随0x18更新；step 1.0/2.0的名义最短目标节奏分别为600/1200 ms/档，其他值限制在350～2000 ms；等待实测到档及停止/稳定序列会延长实际档间隔。tap pending后同向hold重发相同目标而不跳档，反向hold发送相邻反向目标；只有跨30且仍有0x0F pending时才先发0x05 `0`、等待150 ms再发方向，仍按住时最多再补一份方向副本。release/cancel会取消所有待发档位和方向，立即停止且不会补tap。60秒watchdog按“无进展”重置，不会把从1x正常长按到165x误判为总时长超时；165.1x端点在停止后的settled反馈丢失时仍允许重新按住反向退出端点，但tap继续受门禁保护。
...
- 当前代码已完成扩展后的两套独立QtTest编译运行：`Mt11ProtocolTest`为11 passed、0 failed（9个业务槽加init/cleanup），`SiyiProtocolTest`为42 passed、0 failed（40个业务槽加init/cleanup）；`Mt11Sdk.cc`也已独立编译通过，Manager头文件moc成功且diff-check无错误。MT11纯策略测试新增持按单档目标、tap pending同向不跳档/反向相邻档、step 1.0=600 ms与step 2.0=1200 ms节奏、350～2000 ms边界、0x18到档判定和165.0反向164.0；默认1.0步长拒绝把物理165.1发布为目标，0.1步长允许165.1。该结果仍只证明Protocol/Policy和接口静态范围，不覆盖Manager的100/150/500 ms Timer状态机、完整QGC Qt 6构建或真机运动；尤其500 ms pulse上限对MT11真机2.0步长与网络抖动仍须验收。
...
| `custom/src/FlightDisplay/GimbalZoomControl.qml` | A8/MT11共享手势壳，短按仅在release调用一次，420 ms长按先消费手势再启动，release/cancel/移出/隐藏/后台/销毁可靠停止且不补tap。新增 `showActualZoom`仅由MT11 wrapper开启：A8保留单值，MT11同时显示 `Target currentZoom`与 `Actual actualZoom`，后者只来自0x18；QML不复制设备协议、档位Timer或上限策略。 |
...
| `custom/src/Gimbal/Mt11ZoomPolicy.h` | 声明MT11纯策略：显示上限、实测对齐、tap目标、hold单档目标、pending同/反向交互、档位节奏及0x18到档判定；不发送UDP，也不使用A8能力表。 |
| `custom/src/Gimbal/Mt11ZoomPolicy.cc` | 在通用网格之上实现MT11显示上限、tap目标、hold手势下一档、pending同向目标保留/反向相邻档、0x18到档判定，以及按 `step×600 ms`计算并钳制350～2000 ms的名义最短节奏。协议选择和实测到档耗时留给Manager。 |
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义11个Gimbal Fact；独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，同时决定tap/hold合法目标与hold节奏。longDesc明确≤30x逐档0x0F、>30x反馈闭环0x05 pulse、Target/Actual双值及1x/2x的600/1200 ms名义最短节奏，并说明实测到档与停止窗口可延长间隔；不改变设备上限或RTSP URL。 |
...
| `custom/src/Gimbal/Mt11Sdk.cc` | 发送Protocol帧并按配置逻辑IP/端口过滤回包；普通响应须匹配同command最近1.5秒请求窗口。无匹配/过期ACK仍严格丢弃，但不再逐包打印debug：本轮附件136条噪声中0x05为125条、0x18为11条，来自固件运动反馈与迟到查询响应。非法datagram及不同非法payload的去重warning仍保留。 |
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11 QML业务门面；除合法目标 `currentZoom`外新增只由0x18更新的 `actualZoom/actualZoomKnown`，继续暴露设备上限、独立步长、tap/hold availability、媒体、RGB/IR与错误状态。常量区分0x0F上限30.0x、产品物理上限165.1x和线反馈上限255.9x。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 短按维持1～30x逐档0x0F；长按改为离散档位状态机：≤30x逐档0x0F，>30x逐档0x05 pulse，100 ms查询0x18，到档立即停止，150 ms安全停止副本后留50 ms窗口，单pulse 500 ms无到档反馈则硬停。名义最短节奏为 `step×600 ms`并钳制350～2000 ms，实际间隔还受本档到位耗时及200 ms停止窗口约束；每完成一档重置60秒无进展watchdog。tap pending同向hold重发原目标不跳档，反向发相邻目标；仅跨30且0x0F仍pending才用stop→150 ms方向handoff。Target随命令更新，Actual只随0x18；release/cancel取消未来档位与Timer，高倍端点在settled反馈丢失时仍允许反向hold恢复。既有模式切换、析构和媒体隔离语义保持。 |
...
| `custom/test/Gimbal/Mt11ProtocolTest.cc` | MT11纯协议与策略QtTest，共9个业务slot；在既有帧、边界、网格和tap门禁上新增held单档目标、pending同向目标保留/反向相邻目标、600/1200 ms节奏与350/2000 ms边界、0x18到档判定和165→164反向。当前11 passed、0 failed（含init/cleanup），Manager moc成功、diff-check无错误；仍不覆盖Manager Timer、真实UDP或真机运动。 |
...
MT11完整缩放调用链为：合法SDK包维持6秒在线 -> 0x16能力与0x18实际位置各维持6.5秒新鲜度 -> Policy对齐完整步长目标 -> tap在1～30x发送0x0F -> hold在420 ms后逐档推进，≤30x发0x0F，跨越或高于30x发0x05 pulse并每100 ms查询0x18 -> Target在发档时更新，Actual只由0x18更新 -> pulse到档立即停止，150 ms停止副本和50 ms处理窗口后按350～2000 ms目标节奏进入下一档，500 ms未到档硬停 -> release/cancel取消所有未来档位并停止。tap pending同向hold重发同一目标、反向发相邻目标；仅跨30且仍pending才stop→150 ms方向handoff。0x05 ACK只维持可达性；0x18是唯一位置真值。默认step 1.0时目标上限165.0而Actual可到165.1，step 0.1时165.1亦为目标；端点停止后若settled反馈丢失，高倍反向hold仍可退出端点。模式/析构隔离和不读取A8能力的边界保持不变。
...
| `custom/src/UI/AppSettings/GimbalControlSettingsGroup.qml` | 根节点直接为 `ColumnLayout`，组内依次显示A8/MT11独立Zoom Step与两套SDK设置。MT11的 `mt11ZoomStep`同时决定tap/hold合法目标和hold节奏：≤30x逐档0x0F，>30x逐档0x05 pulse；页面不依赖设备在线，也不把SDK地址同步成RTSP URL。 |
...
| `custom/translations/custom.ts` | 英文源字符串模板。本轮新增 `GimbalZoomControl` 的Target/Actual两条及0x18未到下一档的held错误，并同步改写MT11 step longDesc；当前19个context、149条message和142个唯一source，149条均unfinished。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录，与英文模板保持相同的19个context、149条message和142个唯一source；149条均finished，unfinished和空译文为0。新增“目标/实际”、held错误和新版step说明译文；完整QGC/真机验收仍未因此扩大。 |
...
| `mt11ZoomStep` | 0.1–29.0 / `1.0x` | MT11 tap和hold共用的合法目标步长。tap只在1～30x使用0x0F；hold在≤30x逐档0x0F，跨越/高于30x逐档0x05 pulse。step 1.0/2.0的名义最短节奏为600/1200 ms，并钳制350～2000 ms；等待实际到档会延长间隔。默认step 1.0时Target最高165.0、Actual可到165.1；step 0.1时165.1也可成为Target。 |
...
1. Application Settings -> Video配置独立 `RTSP URL 1/2`，两路均使用原生Auto。Application Settings -> Fly View的仪表板正下方固定显示单层Gimbal设置组，不依赖云台在线；A8/MT11独立Zoom Step置于第一项。MT11默认1.0x，同时决定tap/hold合法目标和600 ms/档名义最短节奏；hold在≤30x逐档0x0F，>30x逐档0x05 pulse，物理上限仍取 `min(0x16,165.1x)`。
...
4. 右栏在A8与MT11同时启用时显示相机选择器；切换只更换Manager。MT11控制栏同时显示Target与Actual：tap后同向hold重发同一pending目标、不跳档，反向hold发相邻反向目标；只有跨30且仍pending时才进入0x05停止→150 ms方向交接。release不得补tap。RGB/IR、拍照、录像与媒体徽标语义不变。
...
当前 `Mt11ProtocolTest` 独立运行11 passed、0 failed，覆盖9个业务slot及init/cleanup，新增单档目标、pending同/反向交互、档位节奏、到档判定和165反向；`SiyiProtocolTest`为42 passed、0 failed。Manager moc检查0错误、diff-check通过。Manager的100/150/50/500 ms Timer、真实UDP、镜头运动与真机2.0步长网络抖动仍待验收。
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> 长按：≤30x逐档0x0F；>30x逐档0x05 pulse，100 ms轮询0x18，到档停止并按step节奏继续
         -> tap pending后长按：同向重发原0x0F目标、反向发相邻目标；仅跨30时0x05(0)抢占 -> 150 ms后方向 -> 最多一份方向副本
...
1. Application Settings -> Fly View -> Instrument Panel正下方必须立即显示Gimbal组，再往下才是Viewer3D；Gimbal入口不按在线状态隐藏。A8/MT11独立Zoom Step分别保存。MT11 step同时决定tap/hold档位和hold节奏，不能改变0x16/165.1物理上限；Viewer3D组不得再出现向只读implicit尺寸赋值的Qt 6 warning。
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - MT11协议与策略边界：抓包核对0x05/0x0F/0x16/0x18及sequence/CRC；0x05 `73 06`与0x16/0x18 `a5 01`均代表165.1x，线反馈可到255.9x，但产品上限钳制165.1x。默认step 1.0时Target上限165.0、Actual可显示165.1；step 0.1时Target也可到165.1。0x0F只允许1～30x。纯测试11/0不代表Manager Timer或真机通过。
   - MT11短按与双值矩阵：默认step 1.0，另覆盖0.1/2.0/29.0；1～30x短按逐档0x0F，Target在发送成功时更新，Actual只随0x18更新。快速tap不按actual重建网格；>30x双向tap关闭。验证在途Target 2.0/Actual 1.x、最终同值、Actual非网格中间值以及离线 `--`，0x05 ACK不得更新Actual。
   - MT11 tap↔hold矩阵：tap pending后同向hold必须重发同一个0x0F目标而不跳档，反向hold发送相邻反向目标；分别验证release/cancel/移出/后台不补tap。仅当目标跨越30且旧0x0F仍pending时，才允许0x05 `0`→150 ms方向→仍按住最多一份方向副本；每个等待段取消后均不得迟发。hold release后立即停止、150 ms安全副本和settled 0x18门禁仍须验证。
   - MT11按step长按矩阵：420 ms成立后每次只推进一个配置档。≤30x每档0x0F；跨越/高于30x每档0x05 pulse，每100 ms查询权威0x18，到达/越过目标立即stop，150 ms后一次stop副本并至少留50 ms再发下一档；500 ms未到档必须硬停并显示对应错误。step 1.0/2.0的名义最短节奏为600/1200 ms，实际档间隔取该节奏与“本档物理耗时+200 ms停止窗口”中的较大值；覆盖350/2000 ms钳制、跨多档反馈不跳目标、发送失败、pulse-stop-next序列和模式切换。60秒watchdog必须按每档进展重置。物理165.1/step 1.0时Target为165.0、Actual为165.1；加方向关闭而反向hold可用，反向Target先到164.0，Actual下降到/越过164.0才完成该档。Manager Timer仍需fake SDK或真机验证。
   - MT11日志降噪矩阵：附件136条unmatched debug由0x05 125条和0x18 11条组成。正常长按不得再逐包打印该文本，但传输仍必须拒绝无匹配/过期ACK；非法CRC/长度/control/datagram和不同非法payload warning、网络发送失败及低频手势超时提示须保留。30秒长按的日志量应随手势而非固件反馈包数量增长。
...
选择MT11时视频有画面但缩放按钮仍灰色，应检查SDK在线、0x16/0x18新鲜度、settled门禁和模式pending；RTSP、A8的0x20及分辨率不能解锁MT11。>30x短点无动作是保护，应按住420 ms；此后Target按配置step推进，Actual每100 ms查询0x18。≤30x抓包看逐档0x0F，>30x看有界0x05方向/停止；若出现“no 0x18 feedback reached the next step”，说明单pulse在500 ms内未到档，需记录真机RTT和step后再判断是否调整动态上限。165.1端点的step 1.0应显示Target 165.0/Actual 165.1，重新按住减号必须能够反向退出；日志不再逐包显示unmatched ACK，抓包才是方向与payload证据。
...
A8仍只显示目标，运动中raw不覆盖UI。MT11则明确双值显示：Target是已下发的合法档，Actual是0x18实测且可出现5.6等非网格值；两者不同表示镜头仍在运动，不是步长失效。MT11 hold的目标按step节奏发出，不采用A8按总按压时长估算或精确非整步末端。
...
A8 hold仍按原有单次0x05与按压时长目标策略。MT11不使用该策略或80 ms时序：其≤30x逐档0x0F、>30x逐档0x05 pulse、100 ms反馈、150 ms停止副本、50 ms处理窗口和500 ms单pulse上限见上文。
...
截至2026-08-21，`Mt11ProtocolTest`为11 passed、0 failed，`SiyiProtocolTest`为42 passed、0 failed；`Mt11Sdk.cc`独立编译、Manager moc和diff-check均通过。MT11现以Target/Actual双值区分已下发合法档位和0x18实测位置；hold在≤30x逐档0x0F、>30x逐档0x05 pulse，100 ms轮询，到档后150 ms停止副本与50 ms窗口，step 1.0/2.0的名义最短节奏分别为600/1200 ms，单pulse 500 ms未到档硬停，60秒watchdog按进展重置。tap pending同向hold不跳档、反向发相邻目标；165.1端点允许反向hold恢复。附件136条unmatched ACK debug（0x05 125、0x18 11）已停止逐包输出，严格丢弃和重要warning仍保留。Viewer3D设置组已删除对Qt 6只读隐式尺寸的赋值。两份custom TS现均为19个context、149条message、142个唯一source；英文149条unfinished，中文149条finished且空译文为0，context/source集合一致。上述结果仍不代表Manager Timer、完整QGC Qt 6构建或MT11真机通过，500 ms pulse对2.0步长和网络抖动须重点回归。RTSP原生Auto/GIO直连、Android硬解优先、双PIP、本地媒体和其他既有状态保持前文结论不变。
