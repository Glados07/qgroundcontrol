...
- `currentZoom`只保存经过协议校验和状态机确认的0x18设备回读，不在0x0f写入成功或ACK成功后乐观显示请求目标。启动、重连、能力来源恢复、外部倍率变化及每个用户绝对目标都要求两个串行0x18查询得到一致实际值后才发布；命令确认期间 `zoomStatusKnown=false`并显示 `--`，避免单个迟到包或运动中间值冒充实际倍率。`zoomStep`网格固定锚定1.0x：所有中间移动严格使用完整步长；只有下一完整步会越过相机真实小数上限时，才允许一次边界吸附。
...
| `custom/test/Gimbal/SiyiProtocolTest.cc` | custom独立QtTest协议与策略回归用例。覆盖封包量化、ACK control、坏CRC、严格整帧长度、0x05/0x0f/0x16/0x18格式隔离、非法legacy载荷、720P/1080P/2K/4K及coded-height映射、固定网格对齐，以及短按/长按共用的中间完整步长和双向小数上限吸附序列。测试只在桌面 `QGC_BUILD_TESTING`注册，并提供 `check_siyi_protocol`独立目标。 |
