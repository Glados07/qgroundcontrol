...
- 飞行界面右侧使用单个始终可见的纵向浅一档蓝灰半透明控制栏：只要Gimbal功能已启用，无论是否连接飞控或云台都会显示；从上到下依次为放大、当前目标倍率、缩小、横向分隔线、MT11可选的三模式按钮、拍照/录像图标按钮以及SD/LOCAL状态徽标。空闲时模式/拍照/录像按钮均为 `actionSize` 圆角方形触控区，与缩放按钮共享青色强调、悬停描边和按压动效；录像图标与拍照图标等大且不显示“录像/REC”，开始录像后仅为计时、pending或失败状态文字按内容展开胶囊。受支持拉流会话或卡录能力尚未确认时缩放按钮锁定；两者确认后按钮启用状态只由当前目标在同一合法表中的方向边界决定。到上限仅禁用加号，到1.0x仅禁用减号；真实断流重新锁定两键并显示 `--`，同分辨率重连后会用已确认或重新查询的卡录能力解锁。本地录像按钮可用性另由本地开关与流状态决定，不要求云台SD能力。
...
- MT11 SDK V0.1.0已在 `custom/src/Gimbal` 转换为 `Mt11Protocol`、`Mt11Sdk` 和 `Mt11ControlManager` 三层：Protocol只做严格帧/payload编解码，Sdk持有独立UDP socket并校验来源IP、ACK和1.5秒命令窗口，Manager负责2秒常规轮询、缩放、拍照、录像、三种视频工作模式以及当前产品映射下 Video 2 的本地媒体协调。新增通用 `ZoomStepPolicy` 统一十分之一倍率和1.0x最小值锚定网格，`Mt11ZoomPolicy`在其上实现MT11显示目标、1～30x短按协议域、30x精确边界以及0x18实测对齐；A8只把共用网格接口薄包装到该通用策略，同时保留自己的卡录分辨率能力与按时长hold策略。MT11长按运动阶段以100 ms轮询0x18实测倍率；0x16能力和0x18位置各有独立6.5秒新鲜度门禁。由于production sequence固定为0，0x05停止与方向ACK不能可靠关联到具体代次，其内嵌倍率不推进目标、实测值或边界；只有主动查询得到的0x18是权威位置来源。两套Manager并存，MT11不使用A8的卡录分辨率倍率映射。
- MT11帧使用 `55 66`头、control、payload length LE、production请求固定sequence 0、command、payload和末尾CRC16 LE；CRC多项式为 `0x1021`、初值为0。当前接入命令为0x05手动变倍/停止、0x0A相机系统状态、0x0B异步功能反馈、0x0C拍照/录像切换、0x0F绝对倍率、0x10查询视频模式、0x11设置视频模式、0x16最大倍率和0x18当前倍率。SDK PDF规定0x0F绝对命令只覆盖1.0～30.0x；0x05倍率反馈为十分之一倍率的 `uint16 LE`，0x16/0x18则为“整数byte + 一位小数byte”。因此0x16/0x18 payload `a5 01`是合法165.1x，不是非法165.1样式值。Protocol/Sdk线格式层允许反馈解析至255.9x，Manager再按当前MT11产品策略把可操作混合变倍上限封顶165.1x（标称约165x）。0x11按SDK官方payload分别发送变焦 `[0,2]`、热成像 `[2,0]` 和变焦+热成像拼接 `[3,2]`。0x10兼容解析 `main_stream=0..5`以及 `sub_stream=0/1/2/6`；UI只将合法回包的main值0/2/3识别为上述三态，main值1/4/5仍可解析但显示为未确认模式。
- MT11的 `currentZoom`发布从1.0x锚定且不超过物理上限的合法步长目标；`actualZoom/actualZoomKnown`继续在Manager内部保存0x18权威实测值，供确认、端点判断和诊断使用，但相机控制栏与A8一致，只显示一个目标倍率数值，不再显示Target/Actual双行。默认步长1.0x且产品物理上限为165.1x时，目标最高165.0x，内部实测仍可到165.1x；步长0.1x时165.1x本身也是合法目标。短按按独立 `mt11ZoomStep`从上一目标计算1.0～30.0x的0x0F，快速连点替换在途目标；不足完整步长时把精确30.0x追加为最后短按目标，例如step 2.0使用1、3、…、29、30并沿同一序列反向返回。实测大于30.0x时两方向tap关闭。SDK的0x05只有 `-1/0/+1`方向/停止字段，没有速度字段；长按420 ms成立后采用固定且与step无关的360 ms运行/240 ms暂停节奏降低平均物理速度，直到release/cancel或0x18确认物理端点，整个过程中不发送逐档0x0F。每个360 ms运行段在仍为运行态时于150 ms最多补发一份同方向命令，避免首包UDP丢失；每次进入240 ms暂停前连续发送两份有界停止，方向副本Timer在暂停前取消且暂停态拒绝迟到副本。运动期间每100 ms主动查询0x18，并只把单调对齐后的合法目标发布给控制栏；新运动起步300 ms后才允许端点自动停止，且要求连续两份同方向端点反馈，单个乱序0x18不能移动目标或吞掉新hold。新长按可以直接接管上一轮的settled反馈等待：若安全停止副本仍待发，先同步兑现并等待一次150 ms交接；若只是0x18迟到且停止早已完成，则在420 ms阈值到达后立即启动。接管pending的0x0F时仍先发0x05 `0`退休绝对控制器、等待150 ms再发方向。release/cancel取消所有待发启动，立即停止并安排一份150 ms有界安全停止副本；settled期间tap继续受门禁保护，但物理方向允许时hold不再被禁用，因此第一次按住不会被QML吞掉。60秒watchdog按“无进展”重置，165.1x端点仍允许反向退出。
- 为关闭tap之后的同类乱序窗口，0x0F目标现在必须由连续两份匹配0x18确认；首份命中后立即再查，第二份消耗最后的0x18请求窗口后才退休absolute pending。长按若由“raw处于端点、保留目标表明仍可运动”的争议状态进入，必须先看到本代次非端点反馈才累计端点确认；若真实就在端点，则900 ms失效保护后仍可凭连续两份端点反馈自动停止，避免永久运行。
- `mt11ZoomStep=1.0x`只决定短按绝对目标和控制栏目标的合法参考网格，不参与长按360/240 ms节奏；设为2.0x只会让显示目标以更粗的合法网格推进，不会让镜头长按更快。绝对目标沿1、2、3、4…线性增加；若30x边界不足一步则精确追加30x。在同一成像链路中，从 `z`到 `z+1`的线性尺寸比为 `(z+1)/z`：1→2是2.0倍（100%增量）、2→3是1.5倍（50%）、3→4约1.33倍（33%）、4→5是1.25倍（25%），因此越往后固定 `+1.0x`看起来越小是数学上的必然结果，不是QGC改了步长。《UniPod MT11 v1.2》p16另列出广角4.5 mm/等效24 mm和变焦镜头15～50 mm/等效81～270 mm，`15/4.5 ≈ 3.33`；p37的8K拍照逻辑也以3.3x和11x分段。由此可推断设备双镜头焦距段在约3.3x附近还会形成一个额外体感拐点，但这是设备光学/成像链路的手册参数推断，不是SDK重映射或程序在4x后换了步长。
...
- Fly View右侧栏在A8和MT11都启用时显示分段胶囊式 `A8 Mini / MT11`选择器，每段自带绿/灰SDK在线点，只启用一路时自动归一到该后端。两栏共用 `GimbalCameraControl.qml`的浅一档蓝灰半透明面板（在线 `#b83b4b58`、离线 `#aa303c47`）、青色强调色、统一圆角按钮、悬停/按压动效、缩放、拍照、录像、SD/LOCAL徽标和移动端最小触控尺寸；选择器同步使用在线 `#b83b4b58`。整栏外边框固定为中性蓝灰，不再把Manager的通用缩放/模式错误扩展成红色外圈；录像和本地媒体失败仍由按钮、FAILED、SD/LOCAL徽标及重要提示承担。视觉更新不改变原有缩放tap/420 ms hold/release/cancel手势契约。`MT11CameraControl.qml`只负责注入MT11 Manager并开启三模式控件，倍率与A8一样只显示目标数值；点击模式按钮时，基于 `QtQuick.Controls.Overlay` 的非模态Popup优先在按钮左侧展示“变焦/热成像/变焦 + 热成像”三项，按Overlay边界钳制，窄屏左侧不足时改为按钮下方优先、上方回退。点外部、Esc、SDK离线、应用进入后台、窗口隐藏、控件销毁或A8/MT11切换都会关闭弹层。模式按钮以 `videoModeKnown/videoMode/videoModePending`显示已确认值和pending，通过 `setVideoMode()`发送0x11；仅匹配的0x10/0x11回包确认成功，2.5秒超时后标记未确认并重查实际模式，不以本地乐观值冒充切换成功。
...
- 当前代码已完成扩展后的两套独立QtTest编译运行：`Mt11ProtocolTest`为11 passed、0 failed（9个业务槽加init/cleanup），`SiyiProtocolTest`为42 passed、0 failed（40个业务槽加init/cleanup）；`Mt11Sdk.cc`也已独立编译通过，Manager头文件moc成功且diff-check无错误。MT11测试除覆盖1～30x短按网格、step 2.0的1/3/…/29/30精确端点及对称返回、30.0/30.1协议门禁、0x18实测对齐和165.1物理端点显示边界外，还覆盖三种0x11官方payload、0x10通用main/sub合法域与main=0/2/3三态识别；默认1.0步长拒绝把物理165.1发布为Target，0.1步长允许165.1。该结果仍只证明Protocol/Policy和接口静态范围，不覆盖Manager的100 ms反馈、150 ms交接/安全停止、360/240 ms长按节奏Timer、完整QGC Qt 6构建或MT11真机运动/模式切换。
...
| `custom/src/FlightDisplay/GimbalCameraControl.qml` | A8与MT11共用的相机面板，Manager可由外层注入，不依赖飞控、活动Vehicle或SDK在线状态决定可见性。圆角面板改为更浅、透明度更低的蓝灰背景（在线 `#b83b4b58`、离线 `#aa303c47`），统一使用青色强调色、中性蓝灰外描边、圆角方形按钮及悬停/按压动效；通用Manager错误不再把整栏外圈染红，录像/媒体失败仍保留局部按钮、FAILED、SD/LOCAL徽标与重要提示。面板纵向排列通用 `GimbalZoomControl`、可选三模式按钮、拍照、录像和SD/LOCAL徽标。A8不显示模式按钮；MT11按钮显示已确认的 `ZOOM/IR/MIX`以及已知/pending点，点击后在 `QtQuick.Controls.Overlay.overlay` 上打开非模态三项Popup。Popup优先位于按钮左侧并在Overlay内钳制；窄屏时与按钮右边对齐，优先放在下方、空间不足放上方。`CloseOnPressOutside/CloseOnEscape`、离线、后台、窗口隐藏、面板隐藏/销毁、Manager变更以及模式pending均会关闭弹层；Overlay尺寸变化时重新定位。选项仅调用Manager `setVideoMode()`，不乐观更改当前模式。空闲拍照/录像图标尺寸一致，录像计时、pending或失败时才扩展文字胶囊。命令调用当前Manager的 `takePhoto()/toggleVideoRecording()`，拍照反馈观察机内和本地计数；录像读取组合会话available/active/capturing，本地支路不被SD无卡覆盖。该栏不实例化原生 `PhotoVideoControl`。 |
| `custom/src/FlightDisplay/GimbalZoomControl.qml` | A8/MT11共享手势壳和视觉参数：加/减按钮与面板其他操作一致，使用深色圆角方形、青色悬停描边及浅色按压缩放；倍率读数为胶囊内的单个青色目标数值。视觉参数由共享面板注入，手势逻辑保持短按仅在release调用一次，420 ms长按先消费手势再启动，release/cancel/移出/隐藏/后台/销毁可靠停止且不补tap。`showActualZoom`只保留为可选诊断能力，当前A8与MT11 wrapper均关闭；QML不复制设备协议、节奏Timer或上限策略。 |
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | 定义11个Gimbal Fact；独立 `mt11ZoomStep=1.0x`允许0.1～29.0x，只决定tap目标和长按期间的目标参考网格，不参与固定360/240 ms节奏。longDesc明确短按精确30x端点、0x05没有rate字段、全域固定pace、100 ms的0x18内部确认、控制栏目标单值，以及tap或上一轮settle被新hold接管的150 ms有界handoff；不改变设备上限或RTSP URL。 |
...
| `custom/src/Gimbal/Mt11Protocol.h` | 声明UniPod MT11 SDK V0.1.0纯协议接口、命令枚举、视频源枚举和payload结构，覆盖0x05手动变倍、0x0A相机状态、0x0B异步功能反馈、0x0C拍照/录像、0x0F绝对倍率、0x10查询视频模式、0x11设置视频模式、0x16最大倍率、0x18当前倍率。`VideoSource` 保留main 0～5与sub 0/1/2/6通用协议枚举，`VideoWorkMode`只声明UI对应的0变焦、2热成像和3变焦+热成像拼接，数值故意与 `main_stream` 一致。倍率常量分开表达 `MinimumZoom=1.0`、0x0F命令上限 `MaximumAbsoluteZoom=30.0` 与线反馈上限 `MaximumFeedbackZoom=255.9`；不把产品165.1x策略或A8能力映射放进协议层。 |
| `custom/src/Gimbal/Mt11Protocol.cc` | 按SDK构造并严格解析 `55 66 + control + payload length LE + sequence LE + command + payload + CRC16 LE`。生产请求sequence固定0；CRC覆盖CRC字段之前的完整帧，多项式 `0x1021`、初值0、逐位高位优先。0x0F出站只允许1.0～30.0x并编码为“整数byte + 一位小数byte”；0x05 ACK按十分之一倍率的 `uint16 LE`解析，`73 06`为165.1x；0x16/0x18按“整数byte + 一位小数byte”解析，`a5 01`为165.1x。两类反馈线格式都可校验到255.9x，产品上限由Manager另行收紧。0x11严格编码三个官方工作模式：变焦 `[0,2]`、热成像 `[2,0]`、变焦+热成像拼接 `[3,2]`；未知模式拒绝编码，旧二态helper仅作兼容包装。0x10回包可解析main 0～5及sub 0/1/2/6；`videoWorkMode()`只用main 0/2/3归纳三态，允许固件对合法sub做归一化。 |
| `custom/src/Gimbal/Mt11Sdk.h` | 声明MT11独立 `QUdpSocket`传输对象，默认endpoint为 `192.168.144.24:37260`，提供协议九类命令的发送接口和状态信号；`setVideoMode(VideoWorkMode)`承接三态0x11，旧 `setThermalMode(bool)`仅保留为变焦/热成像二态调用者的兼容包装。`setZoomRange()`只约束0x0F绝对命令，`setFeedbackZoomRange()`独立约束0x05/0x16/0x18反馈，默认反馈范围为1.0～255.9x；该endpoint只代表私有SDK控制链路，不决定RTSP URL 1/2，也不创建视频receiver。 |
| `custom/src/Gimbal/Mt11Sdk.cc` | 发送Protocol帧并按配置逻辑IP/端口过滤回包；`setVideoMode()`先让Protocol为三个合法枚举生成0x11，空包/未知枚举直接拒绝，兼容 `setThermalMode()` 委托到变焦或热成像枚举。普通响应须匹配同command最近1.5秒请求窗口。无匹配/过期ACK仍严格丢弃，但不再逐包打印debug：本轮附件136条噪声中0x05为125条、0x18为11条，来自固件运动反馈与迟到查询响应。非法datagram及不同非法payload的去重warning仍保留。 |
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11 QML业务门面；合法目标由 `currentZoom`发布，0x18实测仍由 `actualZoom/actualZoomKnown`保留作内部确认与可选诊断；继续暴露设备上限、独立步长、tap/hold availability、媒体与错误状态。长按状态新增独立pace Timer和drive-running状态，使360 ms运行/240 ms暂停的固定节奏不与QML手势状态或step混用。三模式QML契约为 `videoModeKnown`、`videoMode`、`videoModePending` 和 `setVideoMode(int)`，枚举值0/2/3与协议 `main_stream` 一致；旧 `thermalModeKnown/thermalModeEnabled/thermalCommandPending/toggleThermalMode()` 只为已有调用者保留兼容，新UI不以它们表达三态。常量区分0x0F上限30.0x、产品物理上限165.1x和线反馈上限255.9x。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 短按维持1～30x相邻0x0F，并在最后不足step时直接到精确30x。0x05协议没有速度字段，因此长按在全倍率范围使用固定、与step无关的360 ms方向运行/240 ms停止节奏降低平均速度；整个hold保持 `continuousZoomActive=true`，内部pace stop不会让QML丢失手势，release/cancel/端点会取消pace与所有待发方向。每个运行段在150 ms保留一份仅允许穿行于drive-running状态的方向副本，每次内部停止立即双发且在pause前退休方向副本，降低UDP单包丢失造成的首按无动作或pause失效。100 ms查询0x18并把控制栏目标沿运动方向单调对齐到合法参考档，实测值留作内部确认；起步300 ms后连续两次同方向端点反馈才允许自动停止，post-hold端点settle也要求连续两次，从而拒绝单个乱序端点包。tap pending转hold时先stop、150 ms后发方向；上一轮hold结算期间的新hold会先同步兑现仍排队的安全stop，必要时做一次150 ms交接，仅0x18反馈迟到时直接启动，避免第一次长按被禁用窗口吞掉。release/cancel立即stop，150 ms后仅一份安全停止副本。60秒watchdog按0x18进展刷新，高倍端点仍允许反向hold恢复。模式切换前停止正在运动或pending的缩放、清除旧镜头0x16/0x18能力并发送三态0x11；发送本身不更改显示模式，只有匹配目标的合法0x10/0x11回包才结束pending，2.5秒超时后改为未确认并重查。Manager保存最后一组原始main/sub，但工作模式与镜头倍率generation以 `main_stream` 为权威：只有main变化才立即退休旧缩放generation和能力；同一main下固件对sub做0↔2等归一化只记录/诊断，不中断长按、不使倍率失效。析构和媒体隔离语义保持。 |
...
| `custom/test/Gimbal/Mt11ProtocolTest.cc` | MT11纯协议与策略QtTest，共9个业务slot；覆盖帧、CRC、0x05/0x0F/0x16/0x18倍率格式、1～30x tap门禁、step 2.0的1/3/…/29/30精确边界及对称返回、Target对齐和165.1物理端点规则；同时精确比对0x11的 `[00 02]`/`[02 00]`/`[03 02]`完整帧，验证0x10对main 0～5、sub 0/1/2/6的兼容解析以及只将main 0/2/3映射到三个UI工作模式。当前11 passed、0 failed（含init/cleanup），Manager moc成功、diff-check无错误；仍不覆盖Manager的100/150/360/240 ms Timer、模式2.5秒Timer、真实UDP或真机节奏运动/视频模式切换。 |
...
MT11完整缩放调用链为：合法SDK包维持6秒在线 -> 0x16能力与0x18实际位置各维持6.5秒新鲜度 -> Policy对齐合法步长Target -> tap在1～30x发送0x0F，最后不足一步直接到精确30x -> hold在420 ms后以固定360 ms方向/240 ms停止节奏全域运行，每个run在150 ms最多补一份方向、每个pace stop立即双发 -> 每100 ms查询0x18，内部保存实测值并沿运动方向单调发布合法目标，起步300 ms后连续两份端点反馈才停止 -> release/cancel取消pace与方向副本并发送0x05停止及一份150 ms安全副本。tap仍pending时，hold先stop退休0x0F、等待150 ms再发方向；上一轮安全stop仍待发时同步兑现并交接，只剩settled反馈迟到时不额外等待，post-hold端点反馈同样要求连续两份才解除门禁。0x05 ACK只维持可达性，0x18是唯一位置真值；UI只显示目标倍率。默认step 1.0时目标上限165.0而内部实测可到165.1，step 0.1时165.1亦为目标。step不参与pace，1.0与2.0长按的物理节奏相同；模式/析构隔离和不读取A8能力的边界保持不变。
...
| `custom/src/UI/AppSettings/GimbalControlSettingsGroup.qml` | 根节点直接为 `ColumnLayout`，组内依次显示A8/MT11独立Zoom Step与两套SDK设置。MT11的 `mt11ZoomStep`决定1～30x tap目标和hold期间的目标参考网格，不参与固定360/240 ms长按节奏；MT11 SDK提示文案明确Host/Port还承载三种视频工作模式控制，RTSP地址仍在Video -> Connection -> RTSP URL 2中单独配置。页面不依赖设备在线，也不把SDK地址同步成RTSP URL。 |
...
| `mt11ZoomStep` | 0.1–29.0 / `1.0x` | MT11 tap目标和hold期间目标参考所用的合法步长。tap只在1～30x使用0x0F，最后不足一步时把精确30.0x作为终点；hold使用固定360 ms方向运行/240 ms停止节奏，step不参与物理长按速度。默认step 1.0时UI目标最高165.0、内部0x18实测可到165.1；step 0.1时165.1也可成为目标。 |
...
1. Application Settings -> Video配置独立 `RTSP URL 1/2`，两路均使用原生Auto。Application Settings -> Fly View的仪表板正下方固定显示单层Gimbal设置组，不依赖云台在线；A8/MT11独立Zoom Step置于第一项。MT11默认1.0x，决定tap目标和hold期间目标参考；tap最后不足一步直接到30x，hold采用与step无关的固定360/240 ms节奏，物理上限仍取 `min(0x16,165.1x)`。
...
4. 右栏在A8与MT11同时启用时显示带独立在线点的分段胶囊选择器；切换只更换Manager、会先关闭当前弹层，不切换中心视频。两个相机面板共享浅一档蓝灰半透明背景、青色强调、中性外边框、圆角按钮和悬停/按压动效；通用错误不再形成整栏红色外圈。MT11倍率与A8一致，只显示目标数值：tap发送1～30x绝对目标且不足一步直接到30x；任意tap仍pending时转hold都先用0x05停止退休绝对控制器，150 ms后发所按方向，仍按住时最多补一份方向。hold全域使用固定360 ms方向/240 ms停止节奏；上一轮hold结算中的第一次新长按可立即接管，不再因settled门禁丢失，release不得补tap。MT11拍照上方的模式按钮打开左侧优先的Overlay Popup，以0x11选择变焦、热成像或变焦+热成像拼接，仅回包确认后更新当前模式。拍照、录像与媒体徽标语义不变。
...
当前 `Mt11ProtocolTest` 独立运行11 passed、0 failed，覆盖9个业务slot及init/cleanup，包括1～30x tap、step 2.0的1/3/…/29/30精确端点和对称返回、0x18对齐、165.1显示边界、三种0x11精确payload、0x10通用合法域及UI三态识别；`SiyiProtocolTest`为42 passed、0 failed。Manager moc检查0错误、diff-check通过。Manager的100 ms反馈Timer、150 ms handoff/安全停止Timer、360/240 ms pace Timer、真实UDP、镜头节奏与平滑性、三模式实机画面及全域物理端点仍待验收。SDK没有速度字段，因此软件节奏只能降低平均速度，真机是否满足视觉平滑要求必须按抓包和0x18曲线验收。
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> 长按：全域按固定360 ms方向/240 ms停止节奏运行，step不参与pace；每个run在150 ms最多一份方向副本，每个内部stop立即双发且副本不得穿透pause
         -> 100 ms轮询0x18并单调更新合法目标；起步300 ms后连续两份端点反馈才停止，单个乱序端点不移动UI目标
         -> tap pending后长按：0x05(0)退休绝对控制器 -> 150 ms handoff后发方向；run自己的有界副本只在drive-running时允许
         -> hold结算后立即再hold：安全stop仍待发则同步兑现并做150 ms交接；若只剩0x18迟到则420 ms阈值后直接启动，第一次手势不禁用
         -> 0x05 ACK不按代次更新倍率，只有主动查询的0x18是位置真值
         -> 停止立即发0x05(0)，150 ms只安全重复一次；有效0x18 settled反馈到达前tap锁定，但物理方向可行的hold保持可用
     ...
     -> A8或MT11任一enabled
        ...
        -> 合并栏始终显示，sdkResponding不决定可见性；SDK超时会失效卡录能力并锁缩放，待新0x20恢复
         -> 顶部 GimbalZoomControl.qml（统一深色圆角按钮、青色强调的 + / 倍率 / -，调用当前选择的Manager）
            -> 共享手势：tap仅release调用一次，hold 420 ms成立；普通release调用stopZoom，取消/移出/离线/销毁调用cancelZoom，不补短按
            -> A8：tap走卡录能力的唯一min锚表，hold一次0x05连续运动并按总按压时长每600 ms更新显示目标；2K为1/2/3/3.5，1080P为1/2/3/4/5/5.5，720P为1/2/3/4/5/6
            -> MT11：tap在1～30x按step发0x0F并追加30x精确端点；hold全域按固定360/240 ms节奏运行，100 ms轮询0x18并只显示单调合法目标
        -> 下部双目标相机控制
...
1. Application Settings -> Fly View -> Instrument Panel正下方必须立即显示Gimbal组，再往下才是Viewer3D；Gimbal入口不按在线状态隐藏。A8/MT11独立Zoom Step分别保存。MT11 step只决定tap目标和hold期间目标参考，不参与固定360/240 ms长按节奏，也不能改变0x16/165.1物理上限；分别设为1.0与2.0时抓包中的hold节奏和0x18真实运动斜率应基本一致。Viewer3D组不得再出现向只读implicit尺寸赋值的Qt 6 warning。
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - 合并栏必须保持纵向单栏结构：A8按 `+ -> 当前目标倍率 -> - -> 分隔线 -> 拍照 -> 录像 -> SD/LOCAL徽标` 排列，MT11只在分隔线与拍照之间多一个三模式按钮；MT11同样只显示单个目标倍率，不出现Target/Actual双行。两者均使用QGC相机图标且不出现原生缩放滑块。面板与选择器使用更浅、较低不透明度的蓝灰半透明背景（在线 `#b83b4b58`），缩放、模式、拍照和录像保持青色强调+圆角方形视觉；整栏外边框不得因通用Manager错误变红，局部FAILED、SD/LOCAL红色失败态和重要提示仍须保留。悬停描边、按压反色/缩放动效不得改变原手势调用次数。空闲拍照和录像按钮必须同为 `actionSize` 触控区，录像图标固定为 `actionSize * 0.48`并与拍照图标等大，空闲录像按钮不得显示“录像/REC”；计时、pending和 `FAILED` 仅在相应状态下出现。验证SD/LOCAL绿色实际录制、黄色pending、红色失败、灰色不可用/未录制及错误提示互不覆盖。Gimbal启用时右侧Column宽度和Loader高度必须始终随控制栏完整隐式尺寸扩展；在Android 86%/100%、桌面、横竖屏、地图/视频主窗口、PIP、Viewer3D和小屏触摸场景检查不裁切、不重叠、按钮达到移动端最小触控尺寸。
   ...
   - MT11 tap↔hold与首按接管矩阵：任意0x0F tap仍pending时开始同向或反向hold，都必须先用0x05 `0`退休绝对控制器，等待150 ms后只发所按方向。分别在handoff前、首次方向后和该run的150 ms方向副本前验证release/cancel/移出/后台：不得补tap、不得迟发方向。另覆盖hold release后0～150 ms内立刻开始下一次同向/反向按住，以及安全副本已发但settled 0x18延迟/丢失两种时序：按钮在物理方向可行时必须接受第一次触点，安全stop仍排队时同步兑现并有界交接，只剩反馈迟到时420 ms阈值后立即发方向，不得要求松手再按第二次。向新hold注入一份旧端点0x18时不得移动目标或停止；起步300 ms后只有连续两份同方向端点样本才允许自动stop。post-hold端点结算也要求连续两份，首份后应主动再查。release仍应立即停止并仅保留一份150 ms安全副本；settled门禁只阻止tap，不阻止可行方向的新hold。
   - MT11固定慢速长按矩阵：420 ms成立后，从1x到设备物理上限的任意位置和方向，抓包应循环出现“方向start、约150 ms后一份有界同方向副本、约360 ms时两份连续stop、约240 ms pause”的节奏，整个按住期间不得发送逐档0x0F。方向副本只允许在drive-running状态发送；故意阻塞UI线程使150/360 ms Timer同时逾期时，pause之后不得有迟到方向穿透。内部pause期间 `continuousZoomActive`必须保持true，release/cancel/移出/后台或端点后不得再迟发任何方向；丢弃第一份方向或第一份内部stop时，副本应维持本轮启动/暂停。分别把step设为1.0和2.0，0x05时间线必须相同，连续采样0x18得到的真实倍率—时间斜率应基本一致；只有控制栏目标网格粗细不同。每100 ms查询权威0x18，UI只显示沿运动方向单调对齐的合法目标，迟到反馈不得让目标反向跳动。release/cancel立即stop并仅有一份150 ms安全副本，达到1x或0x16物理上限也自动stop。60秒watchdog按0x18进展重置；无进展、发送失败、模式切换及SDK离线必须可靠停止。由于SDK没有rate字段，这属于降低平均速度的软件节奏而非设备原生可调匀速；Qt Timer延迟还可能延长某次run，必须以真机画面确认启停体感是否可接受，若仍有明显顿挫则只能向厂商索取速度命令，不能把step绑定为速度。Manager Timer仍需fake SDK或真机验证。
   - MT11争议端点矩阵：在raw恰位于所按方向端点、但pending/settled保留目标表明仍可运动时启动hold。300 ms后连续注入两份旧端点0x18仍不得停止；只有本代次先收到一份非端点反馈后，后续连续两份同方向端点才可自动stop。另覆盖真实物理端点始终只回端点的场景：900 ms前不得误判本代次已运动，900 ms失效保护后连续两份端点必须可靠停止，不能保持到60秒watchdog。
   - MT11日志降噪矩阵：附件136条unmatched debug由0x05 125条和0x18 11条组成。正常长按不得再逐包打印该文本，但传输仍必须拒绝无匹配/过期ACK；非法CRC/长度/control/datagram和不同非法payload warning、网络发送失败及60秒无进展安全提示须保留。30秒长按的日志量应随手势而非固件反馈包数量增长。
...
选择MT11时视频有画面但缩放按钮仍灰色，应检查SDK在线、0x16/0x18新鲜度和模式pending；RTSP、A8的0x20及分辨率不能解锁MT11。settled等待只应锁定tap，物理方向可行的hold必须保持可用。>30x短点无动作是保护，应按住420 ms。正常长按抓包应看到方向start、150 ms同方向副本、360 ms双stop和240 ms pause的循环，不能出现逐档0x0F；若接管pending的0x0F，则在本轮start前还允许一次stop和150 ms handoff。step 1.0与2.0必须使用相同hold节奏，只改变目标网格。若长按没有运动，特别是上一轮释放后第一次按住无效，应抓包确认：待发安全停止是否先被同步兑现、只有settled反馈迟到时是否在420 ms阈值后立即发方向、首方向丢失时150 ms副本是否到达、单个旧端点0x18是否被300 ms起步保护和连续两份确认拒绝，以及release后是否还有迟发方向。控制栏只显示单个目标值；165.1端点且step 1.0时显示165.0，内部0x18仍可为165.1，重新按住减号必须能反向退出。日志不再逐包显示unmatched ACK，抓包才是方向、停止与时间线证据。
...
A8与MT11现在都只显示单个目标倍率。A8运动中raw不覆盖UI；MT11在tap期间显示已下发的合法绝对目标，hold期间根据0x18沿运动方向单调对齐合法step参考。MT11的非网格0x18实测仍保留在Manager内部用于确认、端点和诊断，但不再以Actual第二行显示。MT11 step不设置hold物理速度，也不改变固定360/240 ms节奏。
...
A8 hold仍按原有单次0x05与按压时长目标策略。MT11不使用A8的按压时长目标估算或80 ms时序，而以固定360 ms方向/240 ms停止节奏降低平均速度，并以100 ms的0x18实测更新单值目标；tap pending接管等待150 ms，上一轮hold若只剩settled反馈迟到则无需额外等待。release/cancel立即取消pace和待发方向、发送stop，并在150 ms后发送一份有界安全副本。
...
截至2026-08-24，既有独立结果仍为 `Mt11ProtocolTest` 11 passed、0 failed与 `SiyiProtocolTest` 42 passed、0 failed；本轮没有可用Qt 6构建目录，因此没有伪称重新运行这两套二进制。当前静态验证为：Gimbal JSON可解析、两份TS XML可解析、Manager头文件moc exit 0、`git diff --check` exit 0、两份lrelease exit 0；直接改动的 `MT11CameraControl.qml/GimbalZoomControl.qml` 用本机Qt 5.14 qmllint均exit 0，共享面板和选择器在本轮前后的Qt 5.14 lint均为同一exit 1，仍须由项目Qt 6.8 qmllint/完整构建确认。MT11控制栏已与A8一致只显示目标倍率，0x18实测继续在Manager内部确认；tap在1～30x按step发送0x0F、最后不足一步到精确30x，并要求连续两份目标0x18后才退休pending。hold使用与step无关的360 ms run/240 ms pause；每个run在150 ms有一份只允许drive-running的方向副本，每个内部stop立即双发，release/cancel清除全部未来start后立即stop并只保留一份150 ms结束安全副本。上一轮stop/settled窗口不再禁用新hold；起步端点有300 ms arm、连续两份确认、争议端点先见非端点及900 ms失效保护，单个乱序0x18不能吞掉第一次手势。右栏与选择器改为更浅、较低opacity的蓝灰背景，MT11双值关闭，整栏红色外圈删除，录像和SD/LOCAL局部失败态、重要warning仍保留。MT11三模式、顶部云台指向罗盘、RTSP原生Auto/GIO直连、Android硬解优先、双PIP与本地媒体的既有实现保持。两份custom TS均为20个context、156条message和147个唯一source；英文156条unfinished，中文156条finished、0条unfinished、0条空译文，context/source pair集合一致。上述结果仍不代表完整QGC Qt 6/Android构建、MT11真机节奏/顿挫体感/模式画面，或真实MAVLink云台的absolute/relative yaw、多云台切换、断链门禁与横竖屏布局已验收。
