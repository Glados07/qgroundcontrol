...
- **已有实测基础与最新日志边界**：Ubuntu 24.04 下 A8 Mini 云台控制及 H.265 RTSP 播放曾正常；Android 下同一云台使用 H.264 编码曾正常。MT11 `rtsp://192.168.144.24:8554/video1` 的SDP已确认为标准RTSP、H.265 Main、1920×1080、30 fps。2026-08-18 14:01最新附件来自Desktop Debug，物理上只连接MT11，但应用先后启动 `.24/video1`、误配置的 `.2/video1` 和未连接A8的 `.25/main.264`，并非单receiver基线。MT11 `.24/video1` 共22次start、20次在 `gst_rtspsrc_try_send` 收到EOF；Auto已始终保持effective Auto/`0x7`，显式TCP及其单向Auto回退也已按设计生效，所有source均记录 `teardownTimeoutMs 1000`。全程仍为0个source媒体buffer、0个decoder输出、0个sink首帧，因此最新附件只证明修正后的transport/teardown代码已运行，尚未证明MT11播放恢复或Android硬解可用。
...
- MT11实机SDP确认 `video1` 为H.265 Main、1920×1080、30 fps。14:01最新附件中Auto与显式TCP都在媒体建立前收到RTSP控制/读取EOF，因此Connection组为两路提供独立的“优先TCP”开关，两路缺省均为Auto。开关只影响RTP/RTCP transport首选及回退，不改URI、SDP、编码格式或解码策略。
...
- `GstVideoReceiver` 以成功启动代次记录stop completion：每个成功start最多完成一次stop；即使URI已清空也会释放旧pipeline，重复stop在没有活动pipeline和待完成代次时被忽略，从而避免同一次停止重复启动重连Timer。通用 `VideoManager` 为原生主/thermal receiver保存唯一生命周期状态，冷启动不再由初始化和render job各发一次start；异常停止后的RTSP重试改为1/2/4/8/15秒有上限退避。每个延迟回调携带generation，URI、transport、期望运行状态或新启动代次变化后，旧generation不得再次启动管线；只有首个解码/sink帧成立时才清零退避，单有source pad但无媒体buffer不会把每轮重试重置回1秒。Video 2继续使用自己的1～15秒有上限退避。
- 每次RTSP启动都记录requested/effective transport、requested/effective timeout、`rtspsrc.protocols` 回读值和 `teardownTimeoutMs 1000`。`rtspsrc` 的 `before-send` 回调记录每个实际外发请求的receiver、请求URI和method；bus error的critical除 `uri/source/domain/code/message/debug` 外还输出 `lastRtspMethod`，用于把EOF定位到OPTIONS、DESCRIBE、SETUP、PLAY等具体请求。停止进入NULL期间只抑制中间态PAUSE，TEARDOWN仍允许发送并继续受1秒 `teardown-timeout` 约束，不能把“抑制PAUSE”误写成“跳过RTSP正常关闭”。
- 14:01日志暴露并登记四类后续修正：消除同一主receiver冷启动时的第二个首次start；主路采用1/2/4/8/15秒退避并以generation淘汰陈旧Timer；Dual manager缓存primary active URI并在stop完成后至少保留1000 ms，防止主/副同URL交接并发；GStreamer记录before-send method且teardown只抑制PAUSE、允许TEARDOWN。上述均属于代码修正，仍须以新构建日志和实际画面验收，不能由静态diff推定真机成功。
...
- 仅保留 `src/CMakeLists.txt`、`src/Vehicle/VehicleSetup/VehicleSummary.qml` 两处feature必需例外，`VideoManager.h/.cc` 的通用串行生命周期与退避，`VideoReceiver.h`/`GstVideoReceiver.h/.cc`/`QtMultimediaReceiver.cc` 的通用启动URI快照、RTSP transport、teardown与解码诊断扩展，以及 `QGCLogging.cc` 的通用日志级别过滤修复；其余功能通过custom C++、QRC、独立custom QML模块、QML URL拦截和Android overlay接入。请求Auto时始终使用 `rtspsrc` 原生自动协商，不做Auto到TCP的自定义切换；仅请求TCP时，首帧前符合条件的资源读错误、timeout或EOS可使同URI后续重启单向回退Auto。core不引入相机型号、产品IP或custom设置依赖。
...
当前分支 `SecDev/ft/gimbal` 的二次开发业务仍位于 `custom`。`src` 差异登记为以下九个文件：前两个是custom PX4模块正常链接所需的受控例外，中间两个为所有原生VideoReceiver提供通用串行生命周期与RTSP退避，随后四个是所有VideoReceiver可复用的启动URI快照及GStreamer RTSP transport、teardown诊断与无媒体恢复扩展，最后一个修正通用日志级别过滤：
...
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.h` | 保存上次RTSP URI与用户请求transport、本轮requested/effective transport快照、显式TCP到Auto的单向fallback状态和是否收到source frame；不保留Auto到TCP状态。另以成功start代次保存待完成stop标志，使用原子状态记录“正在进入NULL的teardown”和最后一次允许发送的RTSP method，以互斥快照保存实际pipeline URI，并声明 `before-send`、decodebin深层元素与decoder src首buffer探针。`_makeSource()`只使用本轮transport快照，设置变化由管理器stop/restart后在下一轮start生效。 |
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.cc` | GUI调用start时先冻结URI、transport和low-latency并随任务传入私有worker，worker不再读取可能由设置线程并发修改的URI，也不会把旧attempt误记为新地址；实际pipeline URI以互斥副本供bus、watchdog、fallback和before-send线程读取。仅在创建标准 `rtsp://` 的 `rtspsrc` 时检查上述通用选项。requested Auto始终不设 `protocols`，完整保留 `rtspsrc` 原生协商；requested TCP首轮设置并回读 `protocols=GST_RTSP_LOWER_TRANS_TCP`。尚未收到媒体帧时，符合条件的资源读取错误、source timeout或EOS只会让显式TCP同一URI后续重启单向回退Auto，直到URI/设置变化，不会从Auto切到TCP。每轮记录requested/effective transport、protocols、timeout及 `teardownTimeoutMs 1000`；`before-send`按receiver记录外发request URI、method和是否抑制，bus error的critical记录含 `uri/source/domain/code/lastRtspMethod/message/debug`。停止进入NULL时只拦截rtspsrc中间态自动产生的PAUSE，TEARDOWN保持允许；1秒 `teardown-timeout` 用于等待正常关闭而不是跳过关闭。成功start代次最多发送一次stop completion，即使URI已清空也清理旧pipeline，无活动pipeline且无待完成代次的重复stop会被忽略，避免重复重连Timer。decodebin `deep-element-added`记录实际decoder，decoder src探针记录首个输出buffer及caps，显示sink仍记录自己的首帧。RTSP运行时将Auto/TCP的watchdog下限分别钳到8/5秒，不回写用户Fact。该hook不改URI、caps、延迟或decoder选择。 |
...
除上述九个已登记文件外，当前二次开发功能没有其他 `src` 差异。两路URL、transport Fact、默认策略、设置UI、receiver识别及第二路编排仍位于 `custom`；核心只提供与产品无关的原生receiver串行生命周期、退避、启动URI快照、transport及诊断，不知道A8/MT11或任何产品地址。顶部云台栏自动接管、底部航向罗盘条、Android H.264/H.265优先策略与 USB 飞控连接修复都完全位于 `custom`；根目录 `android/src` 保持原样，Android APK 通过构建目录 overlay 使用 custom Java 实现。
   ...
   -> VideoManager::init()
     ...
     -> GstVideoReceiver::_makeSource(rtsp://...)
        ...
        -> 显式TCP在首帧前符合条件的失败可单向回退Auto；Auto不自定义切到TCP
         -> before-send记录RTSP method；进入NULL时仅抑制PAUSE、允许TEARDOWN，teardown-timeout=1s
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - transport切换与停止生命周期验收：切换URL 2 Auto/TCP或在两个非空、非重复URL间修改时，上述“停止/重建”必须表现为stop/start同一 `VideoReceiver`并重建其GStreamer管线，不应触发 `videoObjectsAboutToBeReleased`或新建receiver。只有URL变空/重复、Video Source/全局stream关闭、cleanup或实际surface Item换代才进入receiver释放/新建路径。每个成功start代次最多只产生一次stop completion；清空URI时仍须释放旧pipeline并完成该代次，随后重复stop应记录无活动pipeline而不再次完成。冷启动时同一主URI在首个错误/stop前只能出现一次首次start，不得重现14:01日志的双start。主路连续无媒体失败应只有一条1/2/4/8/15秒有上限退避序列，Video 2仍只有Dual manager的一条1～15秒退避Timer；修改URI、transport或stream状态后，旧generation Timer即使到期也不得start。把活动主URI从 `.24/video1` 交给Video 2时，必须先看到active URI跟踪与stop后1000 ms hold；hold释放前Video 2不得start同一URI，新主同URI恢复则取消清除。不得再出现主/副在旧主管线释放窗口内同时打开 `.24/video1`。
   - RTSP运行包与日志验收：修改 `GstVideoReceiver.cc` 后删除旧桌面构建产物或至少强制重新configure并全量重编译，确认最终可执行文件时间戳来自本轮构建。默认Auto每次重启都应显示 `requestedTransport Auto effectiveTransport Auto effectiveProtocols 0x7`（具体允许位以目标GStreamer回读为准），且不得出现 `TCP (Auto fallback)` 或强制TCP日志。开启TCP优先时依次观察 `Starting ... transport TCP`、`Forcing RTSP-over-TCP ... by configuration`和 `Configured ... requestedTransport TCP effectiveTransport TCP effectiveProtocols 0x4`；若在首帧前报符合条件的资源读错误、source timeout或EOS，必须记录回退原因，下一次重启显示 `requestedTransport TCP effectiveTransport Auto`且不再强制protocols。回退后同URI/设置保持Auto，不得再切回TCP；修改URI或用户transport选择后才清除。每次实际外发RTSP请求必须有 `RTSP request ... method ... suppressed ...`，每条bus error critical必须同时含 `uri/source/domain/code/lastRtspMethod/message/debug`。正常停止进入NULL时允许看到PAUSE的 `suppressed true`，但TEARDOWN必须为 `suppressed false`；若MT11仍EOF，使用 `lastRtspMethod` 和相邻method序列确定失败在OPTIONS、DESCRIBE、SETUP还是PLAY，不再只引用内部 `gst_rtspsrc_try_send`。分别把持久化 `rtspTimeout` 设为0、1、5、20，Auto实际值不得低于8秒，TCP不得低于5秒，20秒不得被降低。`statusCode 0 success true` 后仍必须继续等到streaming、首个source media frame、实际decoder首输出及sink首帧，不能把接口受理、source pad或候选rank当成画面成功。
...
当前可确认的基线是：SDP能识别标准RTSP和H.265 Main 1080p30；用户已在官方QGC验证同一URL可播放。2026-08-18 14:01最新附件是Desktop Debug，不是Android硬解验收。虽然物理上只连接MT11，程序配置在采集过程中却先后运行主流 `.24/video1`、误输入的次流 `.2/video1`、次流 `.24/video1`和未连接A8的主流 `.25/main.264`，还出现主/副receiver短暂同时请求 `.24/video1`，因此它也不是“单一MT11 receiver不受配置变化干扰”的干净基线。必须以同一QGC构建内的精确URL、receiver身份、requested/effective transport、实际RTSP method、结构化bus error、source媒体帧、decoder输出和sink首帧为准。如需保留QGC桌面日志，使用：

14:01附件中MT11精确 `.24/video1` 共22次start、20次资源错误code 9，全部仍落在 `gstrtspsrc.c(6888): gst_rtspsrc_try_send()`，内容为 `Could not receive message. (Received end-of-file)`。其中21轮effective Auto/`0x7`（包含显式TCP失败后的两轮Auto fallback），只有用户后期开启TCP偏好的一轮为effective TCP/`0x4`；不存在旧版 `TCP (Auto fallback)`，且全部source配置都已打印 `teardownTimeoutMs 1000`，证明Auto与teardown设置修正进入了本次可执行文件。误配置 `.2/video1` 另有2次start/2次相同EOF；未连接A8的 `.25/main.264` 有13次start/12次 `gst_rtspsrc_retrieve_sdp` code 7 `Failed to connect`。所有URL合计仍为0个source媒体buffer、0个decoder输出、0个sink首帧。这把MT11黑屏继续限定在媒体pad之前的RTSP控制/session阶段；transport状态机生效不等于视频恢复，也不能据此判定H.265、MediaCodec、QML或双路同步故障。

较早附件缺少精确GStreamer错误，是因为旧 `QGCLogging::msgHandler` 对debug、info、warning、critical一律调用 `isDebugEnabled()`；当前代码已改为 `isEnabled(type)`。14:01附件已经取得effective protocols与资源读错误，但内部函数 `gst_rtspsrc_try_send` 仍不能说明失败的是OPTIONS、DESCRIBE、SETUP、PLAY还是停止请求。当前补丁因此增加 `before-send` method日志，并把bus error扩展为 `uri/source/domain/code/lastRtspMethod/message/debug`；停止进入NULL时只抑制rtspsrc自动PAUSE，仍允许TEARDOWN。14:01附件在38.272秒由主receiver启动 `.24/video1`，38.599秒次receiver又启动同一URI，39.600秒主流才切到 `.25/main.264`，证明仅比较配置/当前URI仍存在交接窗口。当前GUI调用start时冻结URI/transport/low-latency并由 `onStartAttempt` 将实际starting URI交给DualVideoManager；成功后转为active URI，stop完成后再保留至少1000 ms releasing guard，才允许Video 2接手同endpoint。MT11 SDK的ACK告警属于独立UDP控制链路，不能用于判断RTSP是否到达。
...
1. 设置层：复测前先退出全部RTSP客户端并断电重启MT11，等待服务启动后核对URL 2精确为 `rtsp://192.168.144.24:8554/video1`，不能是14:01附件中的 `.2/video1`；URL 1/2必须不同且两路先保持Auto。Video 1切换应记录自己的generation失效/新代次，Video 2切换应记录 `Secondary RTSP transport changed TCP/Auto`，随后 `Starting secondary video` 必须打印相同transport和requestedTimeout。若该值为0或过小，下一层还必须看到运行时提升后的effectiveTimeout。
2. source/transport层：每次RTSP启动都必须出现 `Starting RTSP receiver ... requestedTimeout ... effectiveTimeout ...` 和 `Configured RTSP source ... requestedTransport ... effectiveTransport ... effectiveProtocols ... teardownTimeoutMs 1000`。冷启动首个终止事件前同一主URI只能有一个首次start；连续失败后的主路间隔必须为1/2/4/8/15秒封顶，URL、transport、stream状态或新start代次变化后旧generation回调不能再启动。只有用户显式选择TCP的首轮才应出现 `Forcing RTSP-over-TCP transport for ... by configuration`；TCP失败回退Auto后不再出现该行。TCP下effectiveProtocols必须等于TCP-only标志 `0x4`；Auto每轮都应保留rtspsrc默认允许集合，且不得出现 `TCP (Auto fallback)` 或“did not retain”告警。逐条检查 `RTSP request` 的method和suppressed字段：进入NULL时PAUSE可被抑制，TEARDOWN必须继续允许；EOF必须同时给出 `lastRtspMethod`。用Wireshark过滤 `ip.addr == 192.168.144.24 && (rtsp || rtp || rtcp || tcp.port == 8554)` 证明Auto时是否有UDP RTP、TCP时是否在8554连接中持续出现interleaved媒体数据，并检查TEARDOWN/session释放与下一次SETUP之间的时序。
...
14:01最新QGC日志只能确认MT11在source媒体pad之前持续发生RTSP控制EOF；它同时证明Auto始终保持Auto和 `teardownTimeoutMs 1000` 已生效，但没有证明画面恢复。随后追加的唯一首次start、主路1/2/4/8/15秒generation退避、primary active URI交接保护、before-send method及仅抑制PAUSE/允许TEARDOWN尚无新真机日志。下一轮必须先重启MT11、确认URL 2精确为 `.24/video1`并冷启动干净构建，再分别验证URL 1单路MT11、URL 2单路MT11、A8+MT11双路、Auto/显式TCP回退、停止释放、PIP跨窗口和断流重连；不能用静态检查、接口 `success true` 或单独 `gst-launch` 成功宣称整机验收通过。
