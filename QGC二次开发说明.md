...
- **已有实测基础与最新日志边界**：Ubuntu 24.04 下 A8 Mini 云台控制及 H.265 RTSP 播放曾正常；Android 下同一云台使用 H.264 编码曾正常。MT11 `rtsp://192.168.144.24:8554/video1` 的SDP已由外部工具确认为标准RTSP、H.265 Main、1920×1080、30 fps，用户也已在官方QGC验证同一URL可播放。14:28 Desktop附件中MT11 16次start、16次均未到SETUP：两级OPTIONS兼容已进入DESCRIBE，但仍为EOF/timeout，全部请求 `realExtensionHeaders=0`；同一进程A8曾完整到source、decoder和sink首帧。随后0e44附件给出决定性网络证据：同机 `ffprobe` 直连相机 `192.168.144.25:8554`成功，而GStreamer 1.24.2明确报告正在连接代理 `192.168.163.1`，`strace`也只看到对该代理7897端口的AF_INET connect，没有连接相机8554。故本轮首要根因是GStreamer/GIO继承桌面系统代理，私网RTSP没有保证直连；先前OPTIONS/DESCRIBE EOF不能在没有socket目标证据时归因于相机。当前工作树已在 `gst_init_check()` 前默认把GStreamer的GIO resolver设为直连，并保留原有的basic-header→skip-OPTIONS有界兼容路径作为次级握手兜底；尚无该最终组合的真机成功日志，不能宣称MT11画面或Android双硬解已经恢复。
...
- MT11实机SDP确认 `video1` 为H.265 Main、1920×1080、30 fps。11:58附件先把此前笼统的RTSP控制/读取EOF定位到初始OPTIONS；14:28附件进一步证明跳过OPTIONS后能够真正外发DESCRIBE，但MT11仍在DESCRIBE响应前EOF或timeout，16轮均未到SETUP。Connection组仍为两路提供独立的“优先TCP”开关，两路缺省均为Auto；开关只影响取得SDP之后的RTP/RTCP transport首选及回退，不改URI、SDP、编码格式、RTSP请求兼容状态或解码策略。尚未发出method（`<none>`）、OPTIONS与DESCRIBE都发生在SETUP之前，当前实现不再用这些阶段的失败触发TCP到Auto回退。
...
- `GstVideoReceiver` 对RTSP的设置超时值和运行时watchdog值分开处理：设置Fact不被回写，实际TCP首选管线的超时下限为5秒，Auto及TCP回退到Auto后的超时下限为8秒。Auto始终不设置 `rtspsrc.protocols`，保留原生协商，不再自定义切换到TCP；显式TCP首轮限制为interleaved TCP。尚未收到媒体帧时，只有已经进入SETUP/PLAY阶段的符合条件资源读取错误、source EOS或source timeout，才可使同一URI和同一请求模式最多单向回退Auto一次；`<none>`/OPTIONS/DESCRIBE均在SETUP前，不能用于诊断RTP下层transport，故其失败不触发该回退。回退后不会切回TCP，URI或用户transport设置变化时才清除状态。
- `GstVideoReceiver` 以成功启动代次记录stop completion：每个成功start最多完成一次stop；即使URI已清空也会释放旧pipeline，重复stop在没有活动pipeline和待完成代次时被忽略，从而避免同一次停止重复启动重连Timer。通用 `VideoManager` 为原生主/thermal receiver保存唯一生命周期状态，冷启动不再由初始化和render job各发一次start；异常停止后的RTSP重试改为1/2/4/8/15秒有上限退避。每个延迟回调携带generation，URI、transport、期望运行状态或新启动代次变化后，旧generation不得再次启动管线；只有首个解码/sink帧成立时才清零退避，单有source pad但无媒体buffer不会把每轮重试重置回1秒。Video 2继续使用自己的1～15秒有上限退避；11:58日志发现被动的主receiver active/releasing通知会绕过该Timer，当前实现只有URI、启用状态、重复源判定或transport等真实用户配置变化才取消旧延迟，被动通知只保留并打印剩余时间。14:28附件共出现30条 `Keeping scheduled secondary video restart`，除用户在50.598秒真实切换Video 2 transport外，离线/重连A8的通知均未再提前绕过MT11 deadline，证明该门禁已进入实际二进制。
- 每次RTSP启动都记录requested/effective transport、requested/effective timeout、`rtspsrc.protocols`、`teardownTimeoutMs 1000`和OPTIONS兼容级别。OPTIONS级别0保持 `rtspsrc` 标准请求；精确的首帧前OPTIONS EOF使同一URI下一轮进入级别1，设置 `short-header=true`并仍然外发OPTIONS；同样EOF再进入级别2，由 `before-send` 抑制OPTIONS、关闭RTSP keepalive并继续正常DESCRIBE。持久级别与每轮active快照分离，compare-exchange保证同一attempt的重复bus error最多推进一次；状态跟随URI、跨Auto/TCP切换保留，URI变化或转为非RTSP时复位。
- 上述所有兼容级别均保留 `rtspsrc` 原生 `udp-reconnect=true`，避免回归成功UDP媒体会话后的RTSP控制连接恢复；级别1的一次失败attempt仍可能由GStreamer内部重发一次请求。`<none>`、OPTIONS与DESCRIBE均处于SETUP之前，因此这些阶段的失败均不触发TCP到Auto回退。GIO默认直连是本轮有系统调用证据支持的首要修复；basic-header与skip-OPTIONS只是直连后仍精确出现OPTIONS EOF时的有界次级兜底。14:28所有Real扩展计数为0，不能再称Real扩展、某个User-Agent值或某一具体header字节为已证明根因。
- `rtspsrc` 的 `before-send` 回调按receiver记录请求URI、method、User-Agent头数量、Real扩展头数量、OPTIONS兼容级别及是否抑制，不输出header值或认证信息。bus error的critical除 `uri/source/domain/code/message/debug` 外还输出 `lastRtspMethod`。停止进入NULL期间只抑制中间态PAUSE，TEARDOWN仍允许发送并受1秒 `teardown-timeout` 约束；teardown标志从请求NULL起一直保持到下一轮start，pipeline引用清除后的迟到PAUSE也必须被抑制。
- 14:28日志证明OPTIONS两级推进、skip-OPTIONS、Video 2 deadline门禁和PAUSE/TEARDOWN语义已经运行，但其应用日志没有记录实际TCP peer，不能排除OPTIONS/DESCRIBE经系统代理发送。GIO默认直连以及连接阶段`<none>`/OPTIONS/DESCRIBE不触发transport fallback均是在该附件之后加入的待真机修正；没有新日志/`strace`同时证明直连 `.24:8554`并得到MT11 SETUP、PLAY、source媒体帧、decoder首输出和sink首帧，不得由静态diff推定播放恢复。
...
- 仅保留 `src/CMakeLists.txt`、`src/Vehicle/VehicleSetup/VehicleSummary.qml` 两处feature必需例外，`VideoManager.h/.cc` 的通用串行生命周期与退避，`VideoReceiver.h`/`GstVideoReceiver.h/.cc`/`QtMultimediaReceiver.cc` 的通用启动URI快照、RTSP transport、OPTIONS EOF兼容、pre-SETUP失败分类、teardown与解码诊断扩展，以及 `QGCLogging.cc` 的通用日志级别过滤修复；GIO直连策略位于 `custom/src/CustomPlugin.cc`，其余功能通过custom C++、QRC、独立custom QML模块、QML URL拦截和Android overlay接入。请求Auto时始终使用 `rtspsrc` 原生自动协商，不做Auto到TCP的自定义切换；仅请求TCP时，首帧前符合条件且已经进入SETUP/PLAY阶段的资源读错误、timeout或EOS可使同URI后续重启单向回退Auto。请求兼容状态只依据通用RTSP错误、method与URI推进，不引入相机型号、产品IP或custom设置依赖。
...
- custom 翻译加载、简体中文目录和 `lupdate` 更新脚本已经接入；视频层文案已同步为通用 Video 1/Video 2、两路“优先RTSP-over-TCP”开关、仅显式TCP可回退Auto的说明，以及沿用旧Fact键但显示为Android H.264/H.265硬解优先的开关。本轮文案只替换source，未增减message；最终校验统计仍为：`custom.ts` 与 `custom_zh_CN.ts` 均为18个context、146条message，context/source完全一致；英文模板146条保持unfinished，中文146条全部finished，unfinished/空译文均为0，`lrelease` 验证通过。

## 2. 开发边界

1. 二次开发业务仍位于 `custom`。`src` 只允许已登记的受控例外：`src/CMakeLists.txt`、`src/Vehicle/VehicleSetup/VehicleSummary.qml`；为所有原生VideoReceiver提供唯一start/stop代次、generation取消和有上限RTSP退避的 `src/VideoManager/VideoManager.h`、`VideoManager.cc`；提供冻结启动URI的 `onStartAttempt`、`Auto/Tcp` 选项、RTSP安全超时、显式TCP到Auto单向fallback、同URI OPTIONS EOF两级兼容、stop代次去重、before-send方法日志及实际decoder输出诊断的 `src/VideoManager/VideoReceiver/VideoReceiver.h`、`GStreamer/GstVideoReceiver.h`、`GStreamer/GstVideoReceiver.cc`、`QtMultimedia/QtMultimediaReceiver.cc`；以及按实际 `QtMsgType` 保留info/warning/critical的 `src/Utilities/QGCLogging.cc`。这些改动保持通用，不识别A8/MT11或产品地址，Auto始终兼容原生协商；URL分路、产品默认值和设置UI仍全部在 `custom`。未登记的新 `src` 改动不允许并入。
...
| `custom/src/CustomPlugin.cc` | 上述中央入口的实现。`QGC_GST_STREAMING`构建下，构造函数在VideoManager/GStreamer初始化及GIO默认resolver创建之前调用匿名 `configureGStreamerNetworkPolicy()`：`QGC_GST_USE_SYSTEM_PROXY`经trim/lower后只有 `1/true/yes/on` 保留environment/system resolver，其余值均把 `GIO_USE_PROXY_RESOLVER`设为`dummy`，失败时输出critical。该进程级选择使后续所有GIO/GStreamer网络源（包括未来HTTP/HLS）默认直连；确需GIO代理时使用上述truthy opt-in，QtNetwork地图/下载代理不受影响。`init()`安装默认链路、翻译和各设置/Manager；创建独立A8、MT11控制器、`VideoCustomSettings`与DualVideoManager。`createVideoSink()`根据receiver父对象区分通用 Video 2 receiver与原生 Video 1 非thermal receiver；视频层不以设备型号命名。它把 `primaryRtspTcpOnly` 应用到Video 1 receiver、把 `secondaryRtspTcpOnly` 应用到Video 2 receiver，并把主receiver注入DualVideoManager，使重复源检测覆盖configured/current/starting/active/releasing各阶段URI；Video 1 transport变化只请求当前receiver停止，唯一start、generation和退避由通用 `VideoManager` 串行化，不在custom复制第二套主路Timer。当前产品本地媒体映射仍为 Video 1 -> A8 Manager、Video 2 -> MT11 Manager，两个本地录像状态机不会操作对方receiver。DualVideoManager释放前以DirectConnection通知MT11 Manager停止owned本地录像并清空Item/receiver，再销毁接收器和sink；应用退出同时收尾两套本地媒体并清理 Video 2。其他职责包括Android字号metadata、MAVLink视频消息过滤和QML URL拦截。 |
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | `GimbalControl` 设置组的元数据源，不执行云台或视频操作。定义11个Fact：A8、本地媒体、视频集成开关，以及 `mt11Enabled=true`、`mt11SdkHost=192.168.144.24`、`mt11SdkPort=37260`、`mt11ZoomStep=1.0x`。它不再定义RTSP URL；通用 Video 2 地址已迁入 `[Video]/secondaryRtspUrl`。 |
...
| `custom/src/Gimbal/Mt11Sdk.h` | 声明MT11独立 `QUdpSocket`传输对象，默认endpoint为 `192.168.144.24:37260`，提供协议九类命令的发送接口和状态信号。该endpoint只代表私有SDK控制链路，不决定RTSP URL 1/2，也不创建视频receiver。 |
...
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11的QML业务门面和运行状态，暴露启用/在线、1.0–30.0x缩放、相机录像、拍照、本地媒体、RGB/热成像模式及错误反馈；在当前产品映射下保存Video 2 Item和receiver弱引用，并声明receiver替换、录像启动结果、退出收尾接口。接口形状与共享相机面板兼容，但状态不与A8 Manager共用。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 持有 `Mt11Sdk`，每2秒轮询0x16/0x18/0x0A/0x10并以6秒最后响应watchdog判断在线；按MT11步长和设备上限处理tap/hold缩放，0x05连续变焦启动后另启60秒single-shot安全watchdog，若用户侧停止事件未到达则超时主动发送0停止并报告安全超时。拍照、录像与热成像均使用SDK命令和2–2.5秒pending保护。热成像切换发送0x11，直到0x10/0x11模式payload确认后才更新RGB/IR。在当前产品映射下，MT11本地拍照只有在Video 2 receiver已decoding且Item尺寸有效时才抓当前帧，拍照/录像只使用Video 2 Item/receiver，绝不操作Video 1/A8 receiver。录像命令是无ACK的toggle：命令pending期间修改MT11 Enabled/SDK Host/Port会把Fact原子恢复为已应用值并提示稍后重试；非pending设置变化会在切换endpoint前停止旧端点已确认的相机录像并结束对应本地会话。Video 2 receiver被替换/释放或应用退出时停止并收尾自身owned录像；关闭MT11 SDK不会自动移除通用Video 2。 |
...
| `custom/src/VideoManager/DualVideoManager.cc` | 监听通用 `[Video]/secondaryRtspUrl`、`secondaryRtspTcpOnly`以及原生Video Source/`rtspUrl`/`streamEnabled`，在 `FlightDisplayViewSecondaryVideo.qml` 把实际 `secondaryVideoContent` Item和window交给 `initVideoItem()` 后，由core plugin创建独立 `VideoReceiver`和sink；`findChild()`只作兼容回退。指定Item换代时会清空旧widget并有序重建receiver。创建对象后先经过 `QQuickWindow::BeforeSynchronizingStage`，再读取动态qgcvideosink Item的 `itemInitialized`；属性存在时只有其为true才启流，并监听 `itemInitializedChanged`和跨窗口变化，防止OpenGL上下文未完成时过早启动。新receiver在start前按第二路Fact设为Auto或TCP；传输开关变化时只重建Video 2管线。它独立维护启动、停止、1–15秒有上限退避重试、设置变化和主动pause；处理 `onStartDecodingComplete`失败，并对“streaming=true但迟迟没有decoding首帧”设置10–30秒有界watchdog后重建管线。只有secondary URI、启用状态、重复源判定或transport等真实配置变化才取消已有退避；主receiver的active/releasing被动通知即使触发 `_refreshSettings()`，也只能保留Timer并打印 `remainingMs`，不能提前start。URL为空，或经QUrl比较后与配置URL 1、主receiver当前URI、本轮 `onStartAttempt` 冻结的starting URI、成功start后的active URI、stop后的releasing URI任一相同时，不启动第二receiver。主stop完成或receiver销毁后，active URI由 `Qt::PreciseTimer` 至少保留1000 ms；同URI新start会取消清除，防止旧主管线释放前Video 2打开同一endpoint。所有回调校验当前QPointer receiver，替换/销毁时断开并清理连接；cleanup永久门控settings、render job和primary late signal，停止全部Timer后再释放receiver，退出阶段不得重建。停止/释放路径不受render-ready门闩限制。动态视频Item被销毁时先清除receiver中的裸widget指针并有序释放旧sink，后续新surface加载时重新创建receiver，避免沿用失效GL对象。释放前仍通知当前映射的MT11 Manager收尾owned本地录像，随后按receiver先销毁、sink后释放的顺序清理。相机SDK是否在线不参与通用RTSP启动判定。 |
...
本轮为Video 2补齐可直接核验的生命周期日志：`Starting secondary video` 同时打印transport和requestedTimeout；异步start/startDecoding完成回调打印数值 `statusCode` 与布尔 `success`。core `GstVideoReceiver` 还通过decodebin `deep-element-added`记录实际decoder创建，并在每个decoder src首个buffer记录receiver/name/URI/plugin/factory/instance/caps。`STATUS_MIN` 与 `STATUS_OK` 在原生枚举中同为数值0，旧日志显示 `STATUS_MIN` 只代表启动请求已被受理，不代表已经收到RTP、产生source pad、建立decoder、输出解码帧或显示首帧。
...
| `custom/src/Settings/VideoCustom.SettingsGroup.json` | 定义三个通用RTSP Fact：第二路URL `secondaryRtspUrl`（缺省 `rtsp://192.168.144.24:8554/video1`，空值禁用Video 2）、第一路 `primaryRtspTcpOnly=false`（Auto）和第二路 `secondaryRtspTcpOnly=false`（Auto）。两个transport Fact的展示文案为“优先TCP”；键名为持久化兼容保留，`true` 不再表示不可回退的TCP-only。元数据不含MT11型号语义，不创建receiver，也不把 `rtsp://` 改写为其他scheme。 |
| `custom/src/Settings/VideoCustomSettings.h` | 声明 `VideoCustomSettings : SettingsGroup` 及 `secondaryRtspUrl`、`primaryRtspTcpOnly`、`secondaryRtspTcpOnly` 三个 `Fact*` Q_PROPERTY，作为QML、QSettings、`CustomPlugin`与 `DualVideoManager` 之间的稳定通用视频设置接口。 |
| `custom/src/Settings/VideoCustomSettings.cc` | 使用 `DECLARE_SETTINGGROUP(VideoCustom, "Video")` 把三个Fact写入原生 `[Video]` 分组；两个transport Fact直接遵循JSON缺省及原生SettingsFact持久化语义。新 `[Video]/secondaryRtspUrl` 不存在时才读取旧 `[GimbalControl]/mt11RtspUrl`：旧值精确为历史出厂默认 `rtsp://192.168.144.25:8554/video1` 时转成 `rtsp://192.168.144.24:8554/video1`，其他自定义值和空字符串原样复制。已有新值绝不覆盖，旧键也不删除，保持升降级安全。 |
...
| `custom/src/UI/AppSettings/VideoSettings.qml` | Application Settings -> Video 的同路径覆盖页。保留原生Video Source、Connection、播放设置和Local Video Storage；当选择RTSP源时，在同一 `Connection` 组内将原生 `[Video]/rtspUrl` 标记为 `RTSP URL 1`，将 `[Video]/secondaryRtspUrl` 标记为 `RTSP URL 2`，并在各URL下方分别提供 `Prefer RTSP-over-TCP for URL 1/2` 开关。两路新安装均默认Auto；开启只表示先尝试TCP，TCP在首帧前无法建立媒体时QGC可回退到自动协商。设置页始终保留标准 `rtsp://` 地址。第二路为空时不显示Video 2；URL 2与配置中的URL 1或主receiver当前实际URI相同时显示警告并由Manager禁用重复接收器。Local Video Storage继续提供共享 `localMediaStorageEnabled`，只控制A8/MT11各自本地附加支路，不关闭相机SD动作；实际双路receiver、相机命令、录像、截图和解码策略由对应Manager执行。 |
...
| `custom/src/UI/AppSettings/GimbalControlSettingsGroup.qml` | Fly View设置页中的双相机私有UDP面板。A8组绑定 `enabled/sdkHost/sdkPort/zoomStep`；UniPod MT11组独立绑定 `mt11Enabled/mt11SdkHost/mt11SdkPort/mt11ZoomStep`，并仅提示视频地址统一在Video -> Connection的RTSP URL 1/2配置。根Loader透传隐式尺寸；本文件只编辑SDK Fact，不测试设备在线，也不把SDK地址自动同步成RTSP URL。 |
...
| `custom/translations/custom.ts` | `lupdate` 生成的英文源字符串模板，供各语言目录对齐context/source，CMake不把它编译为运行时 `.qm`。它已包含通用RTSP URL 1/2、两路优先RTSP-over-TCP开关与Auto回退说明、重复URL警告、双相机选择、MT11 SDK、RGB/热成像、本地媒体和连续变焦安全超时文本。此次只替换source，没有增减message；最终校验仍为18个context、146条message，英文模板146条保持unfinished，与简体中文目录的context/source完全一致。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录；CMake将其编译成 `custom_zh_CN.qm`放入 `:/i18n`，`CustomPlugin`匹配中文locale时安装translator。最终校验与英文模板保持相同的18个context、146条message，146条全部finished，unfinished和空译文均为0；`lrelease` 验证通过。 |
...
当前分支 `SecDev/ft/gimbal` 的二次开发业务仍位于 `custom`。`src` 差异登记为以下九个文件：前两个是custom PX4模块正常链接所需的受控例外，中间两个为所有原生VideoReceiver提供通用串行生命周期与RTSP退避，随后四个提供通用启动URI快照及RTSP transport、OPTIONS请求兼容、pre-SETUP错误分类、teardown诊断与无媒体恢复扩展，最后一个修正通用日志级别过滤。GStreamer/GIO默认直连策略已置于 `custom/src/CustomPlugin.cc`，不新增该项core受控例外：
...
| `src/VideoManager/VideoManager.h` | 为每个原生主/thermal `VideoReceiver` 保存通用生命周期状态：唯一restart Timer、generation、RTSP retry index、期望运行状态、start/stop pending、stop后是否重启、URI/transport快照及terminal cleanup门。该状态不读取custom设置、不识别Video 1/2产品映射或设备地址。 |
| `src/VideoManager/VideoManager.cc` | 初始化receiver时只登记状态，不再立即start；render初始化完成后通过GUI线程唯一入口启动，消除冷启动的第二个首次start。start/stop/restart串行化并防止重复pending；RTSP异常停止按1/2/4/8/15秒精确Timer退避，只有首个解码/sink帧后清零。每次取消/配置变化递增generation，回调还复核URI、transport和期望运行状态，旧Timer不能跨设置或新代次复活。空URI的pending start完成后会立即停止，cleanup后排队的render job/start/timer也不得复活receiver。非RTSP保持1秒失败重试，不包含相机型号或产品URL。 |
| `src/VideoManager/VideoReceiver/VideoReceiver.h` | 在通用receiver基类中增加 `RtspTransport::{Auto,Tcp}`、getter/setter和默认为 `Auto` 的状态，并增加携带本轮冻结URI的 `onStartAttempt(uri)` 信号。异步完成观察者不得再从可变 `uri` 属性推断旧管线实际endpoint。默认值不改变任何原生receiver；头文件不读QSettings、不识别Video 1/2、不包含相机型号或IP。 |
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.h` | 保存上次RTSP URI与用户请求transport、本轮requested/effective transport快照、显式TCP到Auto的单向fallback状态和是否收到source frame；不保留Auto到TCP状态。以两组原子整数保存同URI持久OPTIONS兼容状态与本attempt active快照：0为标准、1为基本头部、2为跳过OPTIONS。另以成功start代次保存待完成stop标志，使用原子状态记录“正在进入NULL的teardown”和最后一次允许发送的RTSP method，以互斥快照保存实际pipeline URI，并声明 `before-send`、OPTIONS兼容CAS推进、decodebin深层元素与decoder src首buffer探针。`_makeSource()`只使用本轮transport与兼容快照，用户transport切换不清除同URI已确认的OPTIONS兼容状态，URI变化或转为非RTSP则复位。 |
| `src/VideoManager/VideoReceiver/GStreamer/GstVideoReceiver.cc` | GUI调用start时先冻结URI、transport和low-latency并随任务传入私有worker，worker不再读取可能由设置线程并发修改的URI，也不会把旧attempt误记为新地址；实际pipeline URI以互斥副本供bus、watchdog、fallback和before-send线程读取。仅在创建标准 `rtsp://` 的 `rtspsrc` 时检查上述通用选项。requested Auto始终不设 `protocols`，完整保留 `rtspsrc` 原生协商；requested TCP首轮设置并回读 `protocols=GST_RTSP_LOWER_TRANS_TCP`。尚未收到媒体帧时，只有last method已进入SETUP/PLAY阶段的符合条件资源读取错误、source timeout或EOS才会让显式TCP同一URI后续重启单向回退Auto，直到URI/设置变化；`<none>`、OPTIONS与DESCRIBE均处于SETUP之前，不能据此推断RTP lower transport不兼容。首帧前精确 `RESOURCE_READ + OPTIONS + Received end-of-file` 以active快照和CAS推进OPTIONS兼容：级别1启用 `short-header`并仍发OPTIONS；再失败进入级别2，跳过OPTIONS并关闭keepalive后继续DESCRIBE。各级均保留 `rtspsrc` 原生 `udp-reconnect=true`。配置日志记录requested/effective transport、protocols、timeout、`teardownTimeoutMs 1000`、OPTIONS compatibility、shortHeader、udpReconnect和rtspKeepAlive；`before-send`记录method、请求URI、User-Agent/Real扩展头数量、OPTIONS级别和抑制状态，不记录header值或认证信息。bus error的critical记录含 `uri/source/domain/code/lastRtspMethod/message/debug`。停止进入NULL时只拦截rtspsrc中间态自动PAUSE，TEARDOWN保持允许；teardown标志保持到下一start，1秒 `teardown-timeout` 等待正常关闭。成功start代次最多一次stop completion；decodebin日志覆盖实际decoder与首输出，显示sink记录首帧；Auto/TCP watchdog下限为8/5秒。该通用hook不识别MT11/A8，不改URI、caps、延迟或decoder选择。 |
...
| `src/Utilities/QGCLogging.cc` | 自定义消息处理器按收到的 `QtMsgType` 调用 `QLoggingCategory::isEnabled(type)`。旧实现无论消息级别都检查 `isDebugEnabled()`，当 `qgc.*.debug=false` 时会错误吞掉同类别的info、warning和critical，导致GStreamer effective transport及bus error在Application Messages中消失；修复不改变各类别自身的过滤规则。`GstVideoReceiver` 还把原先只在debug中输出的GStreamer详情提升到同一critical结构化日志，关闭debug也能看到URI、消息源、GError domain/code、message和debug原因。 |

除上述九个已登记文件外，当前二次开发功能没有其他 `src` 差异。两路URL、transport Fact、默认策略、设置UI、GIO默认直连、receiver识别及第二路编排仍位于 `custom`；核心只提供与产品无关的原生receiver串行生命周期、退避、启动URI快照、transport、OPTIONS兼容、pre-SETUP失败分类及诊断，不知道A8/MT11或任何产品地址。顶部云台栏自动接管、底部航向罗盘条、Android H.264/H.265优先策略与 USB 飞控连接修复都完全位于 `custom`；根目录 `android/src` 保持原样，Android APK 通过构建目录 overlay 使用 custom Java 实现。
...
| `[Video]/primaryRtspTcpOnly` | bool / `false` | URL 1独立transport偏好。`false`=首轮使用 `rtspsrc` 原生Auto；`true`=首轮在标准 `rtsp://` 上优先interleaved TCP，首帧前读取错误、timeout或EOS可单向回退Auto。键名为兼容旧QSettings保留，不再表示永不回退的TCP-only。更改后由custom通知原生Video 1 receiver停止并按新偏好重启。 |
| `[Video]/secondaryRtspTcpOnly` | bool / `false` | URL 2独立transport偏好，新安装同样默认Auto。开启表示首轮优先TCP，首帧前无法建立媒体时保持同URI/同设置的Auto fallback。更改后只停止并重启Video 2 receiver，以重建其播放管线。 |
...
SDK控制endpoint与RTSP视频URL在程序职责上保持分离。两个URL只表示Video 1/Video 2，不表示A8/MT11型号；当前产品本地拍照/录像映射仍明确为 Video 1 receiver -> A8 Manager、Video 2 receiver -> MT11 Manager，右侧控制栏也仍依用户选择调用A8或MT11 SDK。这一产品映射不会将通用视频UI重新命名为设备型号。SDK在线不证明对应RTSP能解码，RTSP有画面也不证明缩放、拍照、录像或热成像命令可用。

升级兼容分为两条。MT11 SDK Host仍使用 `[GimbalControl]/mt11SdkHostDefaultMigrationVersion`：只有已存在的Host精确等于旧默认 `192.168.144.25` 时首次改为 `.24`。`VideoCustomSettings` 在 `[Video]/secondaryRtspUrl` 尚不存在时读取旧 `[GimbalControl]/mt11RtspUrl`：只把精确历史出厂默认 `.25/video1` 转成 `.24/video1`，其他用户自定义值和空字符串原样复制；新键已存在则不覆盖，旧键也不删除。新安装没有旧键时才使用 `.24/video1` 的新JSON缺省。

界面和控制流程：

1. Application Settings -> Video先选择RTSP源，再在同一Connection组配置 `RTSP URL 1` 与 `RTSP URL 2`。产品默认分别为 `rtsp://192.168.144.25:8554/main.264` 与 `rtsp://192.168.144.24:8554/video1`；URL 1严格对应Video 1，URL 2严格对应Video 2，两者都不使用相机型号作为设置名。每个URL下方的“优先RTSP-over-TCP”开关只影响该路transport首选和回退；两路新安装都缺省Auto（关）。URL仍填 `rtsp://...`，不填 `rtspt://...`。修改URL 2或它的transport时 `DualVideoManager`有序停止并重启该receiver，以按新值重建播放管线；URL 2与配置URL 1或主receiver实际生效URI相同时显示明确警告并禁用Video 2。Application Settings -> Fly View仍分别配置A8与MT11的Enabled/SDK Host/SDK Port/Zoom Step，但这些只控制右栏SDK。
...
QGCApplication
  ...
  -> CustomPlugin::init()
     ...
     -> VideoCustomSettings
        ...
        -> `[Video]/primaryRtspTcpOnly=false`：Video 1默认Auto
        -> `[Video]/secondaryRtspTcpOnly=false`：Video 2新安装默认Auto
        -> 两路都保留标准rtsp:// URI；开关表示优先TCP且允许首帧前单向fallback，不改写为rtspt://
        -> 新键缺失时读取旧 `[GimbalControl]/mt11RtspUrl`：精确历史 `.25/video1` 默认转为 `.24/video1`，其他值/空值原样复制，不删旧键
     -> DualVideoManager
        -> 读取通用secondaryRtspUrl/secondaryRtspTcpOnly；重复源同时比较主流configured/current/starting/active/releasing URI
        ...
        -> 创建receiver时应用自己的Auto/TCP transport；第二路开关变化stop/start同一receiver、只重建Video 2 GStreamer管线
   ...
   -> VideoManager::init()
      ...
      -> 创建 VideoReceiver / decodebin3
        ...
        -> CustomPlugin::createVideoSink()复用原生qgcvideosinkbin
           ...
           -> Video 1应用primaryRtspTcpOnly，Video 2应用secondaryRtspTcpOnly；两路设置不共享
     ...
     -> GstVideoReceiver::_makeSource(rtsp://...)
        -> RtspTransport::Auto：始终不写protocols，保留原生rtspsrc自动协商
        -> RtspTransport::Tcp：protocols=GST_RTSP_LOWER_TRANS_TCP，RTP/RTCP复用RTSP TCP连接
        -> 显式TCP在首帧前符合条件的失败可单向回退Auto；Auto不自定义切到TCP
         -> 同URI OPTIONS EOF按0标准、1基本头、2跳过OPTIONS逐级兼容
         -> 尚未发出method（<none>）、OPTIONS、DESCRIBE均属于SETUP前阶段，其失败不触发transport fallback
         -> 各级保留原生udp-reconnect，跳过OPTIONS级关闭RTSP keepalive；before-send记录method、头数量及抑制状态
...
Application Settings / Video
  ...
  -> 保留原生 Video Source / Connection / Settings / Local Video Storage
     -> Connection（选择RTSP源时）
        ...
        -> Prefer RTSP-over-TCP for URL 1 = `[Video]/primaryRtspTcpOnly`，默认false/Auto
        -> RTSP URL 2 = `[Video]/secondaryRtspUrl` -> Video 2，默认rtsp://192.168.144.24:8554/video1
        -> Prefer RTSP-over-TCP for URL 2 = `[Video]/secondaryRtspTcpOnly`，默认false/Auto
        -> 开启表示首轮优先interleaved TCP；首帧前无媒体可单向回退Auto，URL仍为rtsp://
...
2. Application Settings -> Video保留全部原生设置组；选择RTSP源后，Connection同组同时显示 `RTSP URL 1` 和 `RTSP URL 2`，分别对应Video 1与Video 2，每路下方都有独立 `Prefer RTSP-over-TCP` 开关。新安装默认必须分别为 URL 1 `rtsp://192.168.144.25:8554/main.264` + Auto（TCP优先关）、URL 2 `rtsp://192.168.144.24:8554/video1` + Auto（TCP优先关）。两个地址都必须保持标准 `rtsp://`，不得用 `rtspt://` 冒充transport设置。分别切换两路开关并确认只重启目标receiver，值持久化后重启应用仍保持；开启表示首轮优先TCP，首帧前失败可回退Auto；关闭后Auto始终保留原生协商，不能自定义转TCP。清空URL 2后Video 2移除；URL 2与URL 1设置值或主receiver实际URI相同时，页面必须告警且第二receiver不得启动。Local Video Storage保留本地照片/录像开关；Video Stream Integration在所有平台显示两个原有开关，兼容Fact键不改，但UI必须显示H.264/H.265硬解优先，缺省true且仅Android生效。
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - 双相机网络和视频设置迁移：确认A8既有SDK配置保持 `.25`不变，MT11 SDK实际发包目的为 `192.168.144.24:37260`。通用URL则按用户配置分别由Video 1/Video 2 receiver请求，SDK断开不得使通用视频框消失。新安装和清除设置后确认 `primaryRtspTcpOnly=false`、`secondaryRtspTcpOnly=false`；为两键分别预置true并重启，必须保留用户偏好，不得因URL或SDK Host迁移而被覆盖。对Video 2 URL迁移构造“新键缺失+旧键精确为历史 `.25/video1` 出厂默认”“新键缺失+旧键为其他用户值”“新键缺失+旧键为空”“新旧键都不存在”“新键已存在”：第一种必须写入 `.24/video1`，第二/三种必须原样复制，第四种使用新JSON缺省，第五种绝不覆盖，且所有路径都不删旧键。
   - MT11下一轮RTSP日志前置：先彻底退出QGC及其他RTSP客户端，再断电重启MT11并等待其视频服务完成启动；进入设置后逐字符核对URL 2必须精确为 `rtsp://192.168.144.24:8554/video1`，不得保留14:01日志中短暂出现的 `192.168.144.2`。URL 1必须与URL 2不同；产品双路基线保持URL 1 `.25/main.264`、URL 2 `.24/video1`且两路均Auto。保存设置后完整退出并只冷启动一次包含GIO直连和pre-SETUP transport分类修正的干净重编译程序，采集期间不要再编辑URL或transport；普通相机验收环境应清除 `QGC_GST_USE_SYSTEM_PROXY`，或至少不能把它设为 `1/true/yes/on`。14:28日志已经证明主路状态通知只保留Video 2计划deadline；下一轮仍须同时保留 `Keeping scheduled secondary video restart ... remainingMs ...` 和实际start时间，用于防止后续merge回归。
   ...
   - 双路并行解码、全屏和叠加层：URL 1/2配置为两个不同且可用的RTSP endpoint，先使用产品缺省transport（两路均Auto），保持兼容Fact `forceAndroidH265HardwareDecoder=true` 并冷启动。两路都必须依次出现各自source media frame、`Decoder element instantiated`、`Decoder produced its first output frame`和 `First decoded video frame reached the sink`；decoder日志中的receiver与instance必须各自不同，plugin/factory必须与该路SDP编码及目标Android候选相符。H.264直接兼容avc时不会出现adapter；H.265只有走转换路径时才出现独立 `qgcandroidh265hwdec`。这些日志与两个缩略框持续实时画面共同验收双实例，不能把rank或候选日志当成功。日志中Video 2必须先显示Loader传入的实际Item已准备（`Secondary video render item is ready true`）再出现Starting；随后依次得到start status OK、streaming true、source media frame、decoding-start status OK和decoding true。单独开启Video 2的TCP优先后，应只看到Video 2停止/重建；若TCP在首帧前发生符合条件的错误、timeout或EOS，后续应单向回退Auto而不中断Video 1。WAITING期间Loader/GL Item必须仍为active/visible。若只有streaming true而没有source frame或decoding true，10–30秒watchdog必须停止并以1–15秒退避重建Video 2管线，正常首帧必须取消watchdog；不能永久停在WAITING或每秒无限重建。若反复重建，先保留transport/RTP/GStreamer日志并做Auto/TCP A/B；只有source media frame已进入但decoder没有输出时，才进入编码/硬解A/B。Video 1、Video 2居中时分别双击进入/退出全屏；全屏期间工具栏、三视图PIP、WidgetLayer、罗盘条和母线告警均隐藏。Video 2继续验证fit/grid、Proximity Radar和Obstacle Distance；断流后退出对应全屏，不得影响另一路receiver。桌面独立PIP窗口开关和Fly View surface销毁/重建后必须重新看到render-ready与sink首帧，不能沿用旧sink或永久黑屏。
   - transport切换与停止生命周期验收：切换URL 2 Auto/TCP或在两个非空、非重复URL间修改时，上述“停止/重建”必须表现为stop/start同一 `VideoReceiver`并重建其GStreamer管线，不应触发 `videoObjectsAboutToBeReleased`或新建receiver。只有URL变空/重复、Video Source/全局stream关闭、cleanup或实际surface Item换代才进入receiver释放/新建路径。每个成功start代次最多只产生一次stop completion；清空URI时仍须释放旧pipeline并完成该代次，随后重复stop应记录无活动pipeline而不再次完成。冷启动时同一主URI在首个错误/stop前只能出现一次首次start，不得重现14:01日志的双start。主路连续无媒体失败应只有一条1/2/4/8/15秒有上限退避序列，Video 2仍只有Dual manager的一条1～15秒退避Timer；11:58附件中的8个副路计划延迟曾全部被提前绕过，14:28附件则出现30条保留deadline日志且未再被A8通知提前start。除真实URI、启用状态、重复源判定或transport变化外，任何主路active/releasing通知仍必须打印保留的 `remainingMs`，且下次start不得早于当前deadline。修改URI、transport或stream状态后，旧generation Timer即使到期也不得start。把活动主URI从 `.24/video1` 交给Video 2时，必须先看到active URI跟踪与stop后1000 ms hold；hold释放前Video 2不得start同一URI，新主同URI恢复则取消清除。不得再出现主/副在旧主管线释放窗口内同时打开 `.24/video1`。
   - RTSP运行包与日志验收：修改 `custom/src/CustomPlugin.cc`或 `GstVideoReceiver.cc` 后删除旧桌面构建产物或至少强制重新configure并全量重编译，确认最终可执行文件时间戳来自本轮构建。任何receiver start之前必须先看到 `GStreamer GIO proxy policy: direct resolver "dummy"`；用 `strace -f -e trace=network`复核AF_INET connect目标为 `.24:8554`/`.25:8554`而不是0e44附件中的 `192.168.163.1:7897`。只有专门的代理opt-in回归才把 `QGC_GST_USE_SYSTEM_PROXY`设为 `1`、`true`、`yes`或`on`并期待 `environment/system` 日志；该测试后清除变量。另验证地图瓦片和QtNetwork下载仍按原Qt代理策略工作。默认Auto每次重启都应显示 `requestedTransport Auto effectiveTransport Auto effectiveProtocols 0x7`（具体允许位以目标GStreamer回读为准）；开启TCP优先时首轮应显示effective TCP/`0x4`。只有last method已进入SETUP/PLAY阶段的符合条件错误、timeout或EOS才允许后续单向回退Auto；`lastRtspMethod <none>/OPTIONS/DESCRIBE`均不得产生transport fallback。每次配置必须输出 `optionsCompatibility/shortHeader/udpReconnect/rtspKeepAlive`：新URI首轮OPTIONS为0；直连后若仍精确OPTIONS EOF，下一轮为1、`shortHeader=true`、`udpReconnect=true`、`rtspKeepAlive=true`并仍外发OPTIONS；再次同类EOF后为2、keepalive=false，OPTIONS回调 `suppressed true`并紧接标准DESCRIBE。所有级别 `udpReconnect`均保持true；transport变化保留同URI的OPTIONS状态，URI变化则复位。每个请求日志应包含method、净化后的request URI、User-Agent头数量、Real扩展头数量、`optionsCompatibility`和`suppressed`。每条bus error critical仍须含 `uri/source/domain/code/lastRtspMethod/message/debug`。正常停止进入NULL时PAUSE为 `suppressed true`，迟到PAUSE仍须抑制，TEARDOWN须为false。分别把持久化 `rtspTimeout` 设为0、1、5、20，Auto实际值不得低于8秒，TCP不得低于5秒，20秒不得被降低。只有实际直连并出现DESCRIBE、SETUP、PLAY、streaming、source首帧、decoder首输出及sink首帧才算播放恢复；proxy policy、OPTIONS兼容日志、接口 `statusCode 0`或source pad都不算成功。
...
9. Ubuntu 24.04下对MT11做transport分层验收：先确认QGC缺省direct policy和系统调用均直连 `.24:8554`，再用SDP/discoverer确认 `rtsp://192.168.144.24:8554/video1` 仍是标准RTSP、H.265 Main、1920×1080、30 fps；将该URL单独放入URL 1并使用默认Auto，必须与官方QGC一样播放，所有重启都保留原生Auto。随后开启URL 1 TCP优先，确认 `effectiveProtocols=0x4`；只有进入SETUP/PLAY阶段后的失败才允许下一次重启显示requested TCP、effective Auto，之后不能反向切回TCP。同时保留A8回归：URL 1默认Auto下须独立成功。QGC本身不应依赖桌面代理忽略列表才能直连相机；单独执行系统 `gst-launch-1.0` 时仍需清理其代理环境或将 `.24/.25`加入忽略列表，因为外部进程不经过QGC `CustomPlugin`构造阶段的网络策略。
...
当前可确认的基线是：外部工具能从MT11 endpoint识别标准RTSP和H.265 Main 1080p30；用户已在官方QGC验证同一URL可播放。2026-08-19 14:28附件仍是Desktop Debug，不是Android硬解验收；它同时运行A8主receiver `.25/main.264`与MT11副receiver `.24/video1`，二者不请求同一endpoint。该日志已经覆盖OPTIONS两级兼容、副路deadline门禁和teardown修正，但尚未包含之后追加的GIO默认直连及OPTIONS/DESCRIBE pre-SETUP transport分类修正。0e44附件进一步证明同一主机的外部GStreamer 1.24.2对A8 URL实际使用了系统代理，而ffprobe直连成功；该证据高度支持“GStreamer/GIO代理路径是历史QGC握手异常的首要根因”，但抓取对象是A8的外部 `gst-launch`，不是历史QGC进程或MT11本身，因此最终仍须用新QGC的 `strace`闭环。必须以同一QGC构建内的精确URL、receiver身份、实际socket peer、requested/effective transport、OPTIONS兼容级别、实际RTSP method、结构化bus error、source媒体帧、decoder输出及sink首帧为准。如需保留QGC桌面日志，使用：
...
11:58附件首次给出完整 `before-send/lastRtspMethod` 证据。MT11副receiver共9次start，其中前8轮完整失败，第9轮在程序退出时中止；8轮完整失败全部是：第一次外发 `OPTIONS`，约0.4～0.5秒后第二次外发同一 `OPTIONS`，随后 `GST_RESOURCE_ERROR_READ code 9`、`lastRtspMethod "OPTIONS"`和 `Received end-of-file`。9轮合计17条OPTIONS，`DESCRIBE/SETUP/PLAY/PAUSE/TEARDOWN`均为0，source pad/媒体buffer、decoder实例/输出和sink首帧也均为0。首轮为effective TCP/`0x4`，其余为显式TCP失败后的effective Auto/`0x7`；两者在OPTIONS阶段完全相同，因为 `protocols` 只决定取得SDP后SETUP所允许的RTP/RTCP下层传输，不能改变RTSP控制连接的初始OPTIONS。每轮两条OPTIONS也不是两个receiver：GStreamer `gst_rtspsrc_try_send()` 源码在第一次响应读取为EEOF、尚未interleaved且 `udp-reconnect=true` 时会内部重连并重发同一请求一次，第二次仍EEOF才向bus报告。该日志只能确定当时实际连接到的peer在初始OPTIONS阶段关闭了控制连接；它没有记录socket目标，结合后来发现的GIO系统代理路径，不能再把该peer直接等同于MT11，也不能单凭此判断相机拒绝了method或某个请求头。
...
同一14:28进程中的A8提供了控制组：共15次start，前10次连接失败后，第11次在141.320秒依次外发OPTIONS、DESCRIBE、SETUP和PLAY，随后出现source pad、首个媒体buffer、`libav/avdec_h265`实例、1920×1080@25首输出和sink首帧；之后断流watchdog还证明PAUSE被抑制而TEARDOWN获准外发。MT11在A8成功播放前、播放期间和断流后都未越过DESCRIBE，因此不能把MT11黑屏归因于同一进程不具备RTSP能力、双路decoder资源竞争、H.265、MediaCodec、QML或render item。14:28被测版本在MT11首次DESCRIBE EOF后曾错误触发TCP到Auto fallback；当前实现把尚未发出method（`<none>`）、OPTIONS和DESCRIBE统一视为pre-SETUP阶段，不允许据此改变RTP transport。
...
较早附件缺少精确GStreamer错误，是因为旧 `QGCLogging::msgHandler` 对debug、info、warning、critical一律调用 `isDebugEnabled()`；当前代码已改为 `isEnabled(type)`。11:58附件证明before-send与结构化bus error进入实际二进制，14:28附件又证明两级OPTIONS兼容、deadline门禁及teardown语义已运行。停止进入NULL时仍只抑制rtspsrc自动PAUSE、允许TEARDOWN；teardown标志保持到下一次start，避免 `set_state(NULL)`异步完成、pipeline引用已清除后迟到PAUSE漏发。MT11 SDK的invalid ACK属于独立UDP控制链路，不能用于判断RTSP是否到达SDP。

11:58附件同时暴露Video 2退避绕过：8次失败分别计划2、4、8、15、15、15、15、15秒重启，但实际约0.548、2.322、0.171、1.452、2.119、0.835、6.120、2.621秒后就再次start，没有一次等到deadline。触发点均与离线A8主路的start/error或1000 ms releasing guard通知重合。当前修正使 `_applyDesiredState()` 在 `_restartTimer` 活动时只打印剩余时间并返回；只有secondary URI、启用状态、重复判定或transport等真实配置变化才显式取消Timer。该修正控制请求压力和生命周期确定性，但无法纠正GIO把RTSP送往代理的连接路径，因此不能把退避绕过当作黑屏根因。
...
不要只启用 `qgc.videomanager.videoreceiver.gstreamer.api`；source timeout、EOS、bus error以及上述transport日志属于 `qgc.videomanager.videoreceiver.gstreamer.gstvideoreceiver`。日志级别过滤修复后，该类别的warning/critical不再依赖debug开关，但完整协商细节仍建议使用上述精确规则或 `--logging:full`。修改本轮core文件后必须重新configure并干净重编译，不能只重建custom QML/C++对象，否则旧 `GstVideoReceiver.cc` 或 `QGCLogging.cc` 对象仍可能被链接进可执行文件。

以下顺序用于定位：

1. 设置层：复测前先退出全部RTSP客户端并断电重启MT11，等待服务启动后核对URL 2精确为 `rtsp://192.168.144.24:8554/video1`，不能是14:01附件中的 `.2/video1`；URL 1/2必须不同且两路先保持Auto。Video 1切换应记录自己的generation失效/新代次，Video 2切换应记录 `Secondary RTSP transport changed TCP/Auto`，随后 `Starting secondary video` 必须打印相同transport和requestedTimeout。若保留未连接A8主路，副路退避期间的主路状态变化只能出现 `Keeping scheduled secondary video restart ... remainingMs ...`，不能提前出现新的MT11 start。若该值为0或过小，下一层还必须看到运行时提升后的effectiveTimeout。
2. source/transport层：任何receiver start之前先确认GStreamer日志为 `proxy policy: direct resolver "dummy"`，并用 `strace`确认实际AF_INET peer为相机 `.24:8554`/`.25:8554`，不能仍是代理 `.163.1:7897`。每次RTSP启动都必须出现 `Starting RTSP receiver ... requestedTimeout ... effectiveTimeout ...` 和包含transport、protocols、optionsCompatibility、shortHeader、udpReconnect及rtspKeepAlive的 `Configured RTSP source`。冷启动首个终止事件前同一主URI只能有一个首次start；连续失败后的主路间隔必须为1/2/4/8/15秒封顶，URL、transport、stream状态或新start代次变化后旧generation回调不能再启动。只有用户显式选择TCP的首轮才应强制 `protocols=0x4`；Auto保留默认允许集合。逐条检查原有请求兼容：OPTIONS 0精确EOF后，下一轮级别1必须为基本头且仍外发OPTIONS；再次同类EOF后级别2须抑制OPTIONS、关闭keepalive并继续DESCRIBE。所有级别 `udpReconnect=true`，同URI切换Auto/TCP保留状态，换URI复位；连接阶段的`<none>`以及OPTIONS/DESCRIBE错误绝不能触发transport fallback。若直连后仍握手异常，再用Wireshark过滤 `ip.addr == 192.168.144.24 && (rtsp || rtp || rtcp || tcp.port == 8554)` 对比本程序各attempt与官方QGC工作会话的完整请求、响应/FIN时序；进入SETUP后再证明Auto是否有UDP RTP、TCP是否持续出现interleaved媒体，并检查TEARDOWN/session释放时序。
...
4. receiver/decode/render层：start/startDecoding回调中的 `statusCode 0 success true` 仅表示请求受理；`Secondary video streaming ... true` 只表示动态source pad已创建并链接，`First source media frame reached the receiver ...` 才证明首个媒体buffer进入receiver。之后必须看到该receiver的 `Decoder element instantiated ... plugin ... factory ... instance ...`、`Decoder produced its first output frame ... caps ...`，最后看到 `First decoded video frame reached the sink ...`。没有streaming时先查transport/source pad；有streaming但没有source frame时查RTP/depay/parser；已有source frame但没有decoder创建/输出时查codec、caps与实例资源；decoder已有输出但没有sink首帧时再查显示链、window与可见性。

14:28 QGC日志已证明MT11失败从OPTIONS推进到DESCRIBE、Video 2 deadline不再被主路通知绕过、PAUSE/TEARDOWN语义正确，同时以同进程A8完整播放排除通用解码/渲染故障；它也明确证明当时所有Real扩展计数为0，不能继续沿用旧触发推断。0e44随后以外部A8 `gst-launch`/`strace`证明了同一环境中的GStreamer代理误路由，但尚未直接捕获旧QGC或MT11 socket。下一轮必须重启MT11、确认URL 2精确为 `.24/video1`并冷启动干净构建，先检查direct resolver日志和实际peer `.24:8554`，再检查OPTIONS兼容按精确EOF单向推进、`<none>`/OPTIONS/DESCRIBE等所有pre-SETUP失败都不触发transport fallback，并以首次MT11 SETUP/PLAY/source首帧/decoder首输出/sink首帧作为恢复证据；随后再验证URL 1单路MT11、URL 2单路MT11、A8+MT11双路、真正SETUP后才允许的TCP到Auto回退、停止释放、PIP跨窗口和断流重连。不能用静态检查、proxy policy日志、接口 `success true` 或单独 `gst-launch` 成功宣称整机验收通过。
...
截至2026-08-18，本轮已独立编译并运行 `Mt11ProtocolTest`，QtTest报告9 passed、0 failed：7个业务slot加框架init/cleanup，覆盖SDK文档命令帧、RGB/热成像模式帧、严格CRC/长度/control解析、UDP合包原子性和业务payload。该结果仅属于纯 `Mt11Protocol`，不覆盖 `Mt11Sdk`真实UDP、Manager计时、QML、GStreamer、双receiver或真机固件。MT11 `rtsp://192.168.144.24:8554/video1` 已确认为H.265 Main 1920×1080@30 fps的标准RTSP endpoint。当前Video 1/Video 2分别使用 `[Video]/rtspUrl` 与 `[Video]/secondaryRtspUrl`，新安装两路都默认Auto并始终保留 `rtspsrc` 原生协商；只有兼容“优先TCP”键为true时，首帧前符合条件的失败才单向回退Auto。`CustomPlugin`构造阶段缺省设置 `GIO_USE_PROXY_RESOLVER=dummy`，防止RTSP被桌面代理重定向；`QGC_GST_USE_SYSTEM_PROXY`仅当值为1/true/yes/on时恢复environment/system策略，QtNetwork地图与下载不受此GIO选择影响。core为成功start代次去重stop completion、清空URI时仍清理旧pipeline，`before-send`记录method/OPTIONS兼容级别；同URI标准OPTIONS精确EOF后依次尝试基本头部、必要时跳过OPTIONS并关闭keepalive继续DESCRIBE。连接阶段`<none>`、OPTIONS与DESCRIBE均标为pre-SETUP，不再错误触发TCP到Auto。进入NULL后teardown flag保持到下一start，只抑制PAUSE、允许TEARDOWN，`teardown-timeout` 为1秒。通用 `VideoManager` 保证原生主路唯一首次start，并用generation约束1/2/4/8/15秒RTSP退避；custom第二receiver保持1～15秒退避，主路被动通知不得绕过已有deadline，且以primary active URI和stop后至少1000 ms hold保护同endpoint交接。`QGCLogging`按真实消息级别过滤，GStreamer日志同时给出proxy policy、transport、OPTIONS compatibility、`teardownTimeoutMs 1000`、`lastRtspMethod`、结构化错误、实际decoder创建/首输出和sink首帧。Android沿用旧Fact键但UI表示H.264/H.265硬解优先：H.264 direct avc与H.265 direct hvc1均为rank259，H.265 adapter为rank258，软件rank保留。`DualPipView`保留固定槽精确交换、原生PIP显隐、独立窗口与拖拽resize。两份TS均为18个context、146条message且context/source一致；英文模板146条保持unfinished，简体中文146条全部finished、unfinished/空译文均为0，`lrelease`验证通过。

11:58历史附件的确定结论是：它来自Desktop而非Android；物理上只连接MT11，但应用仍同时运行未连接A8的 `.25/main.264` 主receiver和MT11 `.24/video1` 副receiver，二者没有同URI并发。MT11共9次start、8次完整 `gst_rtspsrc_try_send` EOF和1次退出中止；17条请求回调全部为OPTIONS，8条bus error全部为 `lastRtspMethod OPTIONS`，DESCRIBE/SETUP/PLAY及source媒体buffer、decoder输出、sink首帧均为0。每轮第二条OPTIONS是GStreamer在首个EEOF后的内部 `udp-reconnect`重发，不是另一个receiver。首轮effective TCP/`0x4`与后续Auto/`0x7`结果相同，进一步排除RTP transport、H.265、MediaCodec、QML或双路同步作为当时失败层级。日志还证明8个Video 2计划退避全部被A8主路被动通知提前绕过；但它没有记录实际socket peer，不能把EOF直接归因于MT11。基本头部/跳过OPTIONS兼容、副路deadline门禁和teardown flag延长均是在该附件之后追加，尚无真机成功日志。

当前开发环境仍未完成这次GIO直连与通用双视频修正后的全工程Qt 6桌面/Android干净构建，因此不能把静态检查扩大为“MT11或QGC双路已验收”。下一轮先关闭全部客户端、断电重启MT11、确认URL 2精确为 `.24/video1`且两路Auto，再冷启动一次干净桌面程序/APK。日志必须先出现direct resolver策略，并由 `strace`证明AF_INET直接连接 `.24:8554`而不是系统代理；随后证明同URI唯一首次start、主路1/2/4/8/15秒退避不被旧generation复活、Video 2 deadline不被主路被动通知绕过、同URI交接hold至少1000 ms，以及标准OPTIONS失败时才按条件推进基本头部和skip-OPTIONS。连接阶段`<none>`、OPTIONS与DESCRIBE失败不得触发transport fallback；真正恢复必须依次出现DESCRIBE、SETUP、PLAY、source媒体帧、实际decoder首输出和sink首帧。完整矩阵仍包括MT11在URL 1 Auto单路、MT11在URL 2 Auto单路、显式TCP进入SETUP/PLAY阶段后的Auto fallback、A8 URL 1 Auto + MT11 URL 2 Auto同时显示、两路transport独立切换、stop代次与TEARDOWN释放、Video 2 Item/window重挂载、连续PIP交换/拖拽及断流重连。Android双路必须分别看到不同receiver/decoder instance的实际decoder首输出，再看到各自sink首帧；`vendor MediaCodec candidate`、rank或adapter注册都不能单独证明硬解或双实例成功。当前本地媒体仍映射Video 1 -> A8、Video 2 -> MT11；录像/媒体发布期间修改URL 2、URL 2 transport、Video Source或触发重复URL保护时，同步DirectConnection释放路径可能短时阻塞UI，该风险未改为异步状态机。A8+MT11真实ACK时序、RGB/热成像往返、双路长期解码/重连、三视图/全屏与叠加层、两套本地媒体receiver隔离、Android性能及媒体发布、SDK/RTSP各自故障和退出收尾仍须按本章矩阵在目标设备验收。
