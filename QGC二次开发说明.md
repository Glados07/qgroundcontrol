...
- MT11短按和长按使用不同命令域。短按按独立 `mt11ZoomStep` 精确计算0x0F目标，当前值和目标值都必须位于1.0～30.0x；默认步长1.0x，设置页与A8步长并排但不共享Fact。当前倍率大于30.0x时，放大和缩小短按都拒绝且提示“仅支持长按连续变倍”，避免0x0F把混合倍率突然拉回30x以内。长按420 ms成立后使用0x05原生连续运动，可在1.0x到 `min(设备0x16上限, 165.1x)` 之间双向运行；活动期间以350 ms查询0x18，到设备边界或60秒安全watchdog时停止。停止先立即发送0x05 `0`，150 ms后只发送一次安全重复并查询最终0x18，不发送A8式时间目标或释放后0x0F归整。
...
- 当前代码已完成扩展后的 `Mt11ProtocolTest`独立编译运行，QtTest为9 passed、0 failed（7个业务槽加init/cleanup），并独立编译 `Mt11Sdk.cc` 通过。测试除SDK文档命令帧、热成像帧、严格CRC/长度/control、UDP多帧原子性和相机/功能反馈外，还覆盖0x05 `7306`→165.1x、`ff09`→255.9x的 `uint16 LE/10`边界，以及0x16/0x18 `a501`→165.1x、`ff09`→255.9x的整数/小数格式边界。该结果只证明Protocol/Sdk编译与纯协议解析，不覆盖Manager时序或真机控制。295d中的 `Rejected invalid MT11 ACK 16 "a5 01"` 属于修正前旧构建：hex命令`16`是0x16最大倍率响应，当前代码会合法解析165.1x并由Manager接受为产品上限。MT11真实固件下的短按/长按运动、边界停止、RGB/热成像往返、双receiver长期播放、双机网络、本地媒体、Android性能和全工程Qt 6构建仍待目标设备验证。
...
| `custom/src/FlightDisplay/GimbalZoomControl.qml` | 合并栏顶部的A8/MT11共享手势壳，使用单列GridLayout从上到下显示加号、当前倍率和减号，并把完整列高传给外层布局。从当前Manager读取 `zoomControlsUnlocked/currentZoom/zoomStatusKnown/zoomInAvailable/zoomOutAvailable`；两侧MouseArea统一实现Idle/Pressed/Holding/Consumed状态，短按释放调用一次 `zoomIn/zoomOut`，长按420 ms成立后调用 `startZoomWithPressDuration(direction, totalMs)`，普通release调用 `stopZoom()`，取消、移出、隐藏、后台和销毁调用 `cancelZoom()`。具体倍率策略留在各Manager：A8仍使用卡录分辨率能力表和按时长显示目标；MT11短按使用1.0～30.0x的0x0F精确目标，长按使用0x05原生连续运动和0x18实测反馈，30.0x以上双向短按均拒绝。QML不把A8的 `qRound(totalMs / 600.0)` 目标、卡录分辨率上限或80 ms停止时序复制给MT11。 |
...
| `custom/src/Gimbal/GimbalControl.SettingsGroup.json` | `GimbalControl` 设置组的元数据源，不执行云台或视频操作。定义11个Fact：A8、本地媒体、视频集成开关，以及 `mt11Enabled=true`、`mt11SdkHost=192.168.144.24`、`mt11SdkPort=37260`、`mt11ZoomStep=1.0x`。MT11步长允许0.1～29.0x，只用于当前值和目标值都在1.0～30.0x时的0x0F短按；0x05长按连续运动不按该步长离散化。它不再定义RTSP URL；通用 Video 2 地址已迁入 `[Video]/secondaryRtspUrl`。 |
...
| `custom/src/Gimbal/Mt11ControlManager.h` | 声明MT11的QML业务门面和运行状态，暴露启用/在线、实测当前倍率、设备0x16最大倍率、独立短按步长、方向availability、缩放门禁、相机录像、拍照、本地媒体、RGB/热成像模式及错误反馈。常量明确区分0x0F绝对上限30.0x、当前产品混合倍率上限165.1x和协议反馈上限255.9x；接口形状与共享相机面板兼容，但状态不与A8 Manager共用。 |
| `custom/src/Gimbal/Mt11ControlManager.cc` | 持有 `Mt11Sdk`。短按按 `mt11ZoomStep`计算并量化到0.1x，以0x0F发送1.0～30.0x精确目标；当前实测倍率大于30.0x时双向短按都拒绝并提示仅长按，0x0F accepted只进入独立10秒确认期，中心倍率仍等待每250 ms查询的0x18实测值，已有反馈跨过6.5秒新鲜度不会提前取消该pending。长按用0x05原生连续变倍，可运行到 `min(0x16设备上限, 165.1x)`；活动时每350 ms查询0x18，到边界或60秒watchdog时停止，立即发0x05 `0`后150 ms只安全重复一次并查询最终0x18。常规每2秒查询0x16/0x18/0x0A/0x10；6秒无任意合法SDK包判离线，0x16能力和倍率位置分别超过6.5秒未刷新就锁定控制并停止活动运动。视频模式切换前停止连续变倍并立即使旧镜头倍率失效，pending期间不采纳旧镜头0x16/0x18；变化按原始 `mainStream` 枚举判断，因此Zoom/Wide/composite或RGB/IR切换都会重查新主镜头能力/位置。若外部控制器先改变mainStream，Manager会立即退休旧0x0F generation、清旧请求和状态；活动0x05只向旧镜头立即停止一次，并取消其尚未触发的150 ms副本，避免跨镜头补发。0x10/0x11确认后立即重查，2.5秒超时也清状态并重查。析构进入可能运行nested event loop的本地媒体收尾前，先停止全部SDK/control Timer、退休缩放/命令状态并断开SDK到Manager的回调；若此前仍有绝对/连续/待重试缩放，再同步发送两份0x05停止，然后才等待照片worker和本地媒体结束，避免半析构对象被迟到ACK或Timer重入。拍照、录像与当前Video 2本地媒体映射仍不操作Video 1/A8 receiver。 |
...
| `custom/test/Gimbal/Mt11ProtocolTest.cc` | MT11纯协议QtTest，共7个业务slot，覆盖SDK文档0x05/0x0A/0x0C/0x0F/0x16/0x18精确命令帧、0x10/0x11热成像帧、严格control/长度/CRC/sequence解析、UDP多帧原子性和状态payload。倍率边界明确覆盖0x05 `73 06`→165.1x、`ff 09`→255.9x，0x16/0x18 `a5 01`→165.1x、`ff 09`→255.9x，并拒绝0x0F 30.1x。当前独立运行结果为9 passed、0 failed（含init/cleanup），`Mt11Sdk.cc`也已独立编译通过；它不覆盖Manager时序、完整QGC Qt 6构建、真实UDP往返或真机镜头运动。 |
...
MT11完整缩放调用链为：任意合法SDK包维持6秒在线状态 -> 0x16设备能力与0x18/0x05实测位置分别维持6.5秒新鲜度 -> 两者已知且无模式切换pending时解锁 -> tap按独立 `mt11ZoomStep`生成1.0～30.0x的0x0F精确目标并以250 ms查询0x18确认 -> 大于30.0x时双向tap拒绝 -> hold在420 ms后用0x05连续运动，并以350 ms查询0x18直到1.0x或 `min(设备0x16, 165.1x)` -> release/cancel/边界/超时立即停止，150 ms后只安全重复一次 -> 模式原始 `mainStream` 变化或切换pending时清旧能力/位置并锁定，确认或超时后重查0x16/0x18。外部mainStream切换会退休旧绝对命令generation，活动连续运动只立即停止一次并取消旧镜头尚未触发的150 ms副本；退出则在nested媒体收尾前停Timer、退休状态、断SDK回调，并在需要时同步双发停止。该链不读取VideoReceiver尺寸、0x20卡录参数或 `A8MiniZoomPolicy`。
...
| `custom/src/UI/AppSettings/GimbalControlSettingsGroup.qml` | Fly View设置页中的双相机私有UDP面板。A8与UniPod MT11各自绑定Enabled/SDK Host/SDK Port；两套独立步长Fact在同一个 `Zoom Step` 分组中并排显示，窄屏自动上下堆叠。A8继续绑定 `zoomStep`；MT11绑定默认1.0x的 `mt11ZoomStep`，并明确它只影响1.0～30.0x的0x0F短按，长按连续变倍不使用离散步长。页面仅提示视频地址统一在Video -> Connection的RTSP URL 1/2配置；本文件不测试设备在线，也不把SDK地址自动同步成RTSP URL。 |
...
| `custom/translations/custom.ts` | `lupdate` 生成的英文源字符串模板，供各语言目录对齐context/source，CMake不把它编译为运行时 `.qm`。它已包含通用RTSP URL 1/2、第二路独立receiver/留空禁用提示、重复URL警告、双相机选择、并列Zoom Step中的A8 Mini/MT11标签、MT11 SDK、30x以上仅长按提示、RGB/热成像、本地媒体和连续变焦安全超时文本；两路RTSP-over-TCP开关及相关回退说明的6条message已经删除。当前实测为18个context、146条message，英文模板146条保持unfinished，与简体中文目录的context/source完全一致。 |
| `custom/translations/custom_zh_CN.ts` | 简体中文locale目录；CMake将其编译成 `custom_zh_CN.qm`放入 `:/i18n`，`CustomPlugin`匹配中文locale时安装translator。当前实测与英文模板保持相同的18个context、146条message，146条全部finished，unfinished和空译文均为0；本轮只完成TS结构/译文统计，未把完整QGC构建或真机验收写成已通过。 |
...
| `mt11ZoomStep` | 0.1–29.0 / `1.0x` | MT11短按0x0F精确倍率步长；设置页与A8步长并排显示，但Fact和算法互不共享。只有当前实测倍率及计算目标都在1.0～30.0x时允许短按；当前大于30.0x时放大、缩小均拒绝并提示仅长按。hold改用0x05原生连续变倍，可覆盖1.0x到设备0x16上限且产品策略最多165.1x，不按该步长离散化。 |
  ...
  -> CustomPlugin::init()
     ...
     -> GimbalVideoStreamSupport 安装 A8 Mini 默认值
      -> Mt11ControlManager
         ...
         -> 每2秒常规轮询0x16/0x18/0x0A/0x10；6秒无合法包判离线，0x16能力/倍率位置各自6.5秒失效
         -> 短按：mt11ZoomStep默认1.0，0x0F精确目标仅1～30x；30x以上双向拒绝并提示仅长按
         -> 长按：0x05连续运动至min(设备0x16, 165.1x)，活动时350 ms轮询0x18，停止后150 ms只安全重复一次
         -> 0x0F确认期间250 ms轮询0x18、最长10秒；显示使用实测反馈，不套A8的卡录分辨率/时间目标
...
1. Application Settings -> Fly View -> Instrument Panel显示 `Show Heading Compass Bar`，页面同时保留Viewer3D、`SIYI A8 Mini Gimbal Camera`和 `UniPod MT11 Gimbal Camera`；两套SDK host/port各自保存，A8与MT11独立Zoom Step在同一组内桌面并排、窄屏堆叠。MT11默认1.0x，修改后只影响1～30x短按0x0F，不能改变长按0x05上限；MT11组只提示两路视频URL统一位于Video -> Connection，不再显示MT11专用URL名称。
...
7. 验证合并控制栏的输入、安全、相机状态和布局：
   ...
   - MT11协议倍率边界：抓包核对0x05/0x0A/0x0B/0x0C/0x0F/0x10/0x11/0x16/0x18、production sequence 0和小端CRC。0x05 ACK `73 06`必须解析为165.1x、`ff 09`为255.9x；0x16/0x18 `a5 01`必须解析为165.1x、`ff 09`为255.9x。Protocol/Sdk接受至255.9x只代表线格式能力，Manager必须把当前产品操作上限钳制到 `min(设备0x16, 165.1x)`。0x0F只允许1.0～30.0x，并拒绝30.1x；不得使用A8的卡录分辨率上限或旧版小端兼容回退解释MT11 0x16/0x18。当前纯协议测试9 passed、0 failed和 `Mt11Sdk.cc`独立编译已通过，但此项的Manager/真机运动仍未验收。
   - MT11短按矩阵：默认 `mt11ZoomStep=1.0`，另覆盖0.1与29.0边界；1～30x内每次短按只发送一个量化到0.1x的精确0x0F目标，目标受1.0/30.0钳制。当前倍率严格大于30.0x时，`+`和`-`都不得发送0x0F，二者都提示仅支持长按；恰好30.0x时加号提示长按，减号仍可按步长回到30x以下。accepted ACK不能立即伪造中心倍率，Manager须每250 ms查询0x18并等待实测值，最多10秒；该独立确认窗口不能被已有0x18恰好跨过6.5秒新鲜度时提前取消。
   - MT11长按与新鲜度矩阵：420 ms成立后只以0x05开始原生连续运动，活动期间每350 ms查询0x18；向上到 `min(设备0x16, 165.1x)`、向下到1.0x时主动停止。release、cancel、隐藏、后台、能力/状态失效、离线和60秒watchdog都必须立即发送0x05 `0`，150 ms后恰好再发一次安全停止并请求最终0x18，不得采用A8的80 ms、`qRound(total/600)`目标或释放后0x0F归整。0x16能力与0x18位置分别超过6.5秒未刷新时须锁定缩放并停止活动运动；任意合法SDK包超过6秒未到则判离线，不能由RTSP画面代替。
...
选择MT11时视频有画面但缩放按钮仍灰色，应独立检查私有SDK是否在线、0x16能力与0x18位置是否都在6.5秒新鲜度内，以及0x10/0x11模式切换是否仍pending；RTSP画面、A8的0x20和卡录分辨率都不能解锁MT11。若日志仍出现 `Rejected invalid MT11 ACK 16 "a5 01"`，说明运行的是旧构建；当前代码必须把该0x16 payload解析为165.1x。当前倍率大于30x时短按两方向提示仅长按是预期保护，不是按钮故障；应按住420 ms启动0x05，再通过每350 ms的0x18反馈确认运动。若长按仍无动作，抓包核对0x05开始/停止、0x16设备上限、0x18反馈、6秒在线watchdog和6.5秒能力/状态新鲜度，不要用A8分辨率表推断MT11上限。
...
A8目标2.0期间出现raw 1.6，表示设备报告了尚未到达目标的物理运动位置，不是2.0被误解码。由于A8 UI显示的是当前目标，0x0f成功后应继续显示2.0；后续raw 1.8或其他中间值只更新内部actual，不能把中心数字退回1.0或改成任意小数。目标3.0、4.0、5.0或5.5时遵循同一规则；发送失败才保持原目标，取消后的旧hold目标不得稍后重放。MT11中心倍率使用实测反馈，不能套用这套A8乐观目标显示。
...
截至2026-08-20，本轮已独立编译并运行 `Mt11ProtocolTest`，QtTest报告9 passed、0 failed：7个业务slot加框架init/cleanup；`Mt11Sdk.cc`也已独立编译通过。协议层已区分0x0F的1～30x绝对命令范围与0x05/0x16/0x18最高255.9x的线反馈范围，`a5 01`现按0x16/0x18格式合法解析为165.1x；Manager以165.1x封顶当前产品，短按只在1～30x使用独立默认1.0步长，长按以0x05连续运动、350 ms反馈轮询和150 ms单次安全停止覆盖设备范围，且用6.5秒能力/状态新鲜度及模式切换重查锁定陈旧状态。外部mainStream变化会退休旧缩放generation并取消跨镜头的150 ms副本；析构也会在nested媒体收尾前停止控制Timer、退休状态、断开SDK回调并按需同步双发停止。上述结果仍不代表Manager时序、完整QGC Qt 6构建或MT11真机控制通过。当前Video 1/Video 2分别使用 `[Video]/rtspUrl` 与 `[Video]/secondaryRtspUrl`，两路固定不设置 `rtspsrc.protocols`、使用GStreamer原生Auto；Auto仍可自行协商TCP。`CustomPlugin`在GStreamer初始化前缺省设置 `GIO_USE_PROXY_RESOLVER=dummy`，truthy `QGC_GST_USE_SYSTEM_PROXY`才恢复environment/system，QtNetwork不受影响。core保留stop代次、generation退避、OPTIONS 0/1/2兼容和teardown语义，已删除应用RTSP传输枚举、强制TCP、TCP到Auto回退和5秒TCP专用超时；常规日志不再逐attempt打印source属性、OPTIONS属性、method或 `teardownTimeoutMs`，只保留proxy策略、结构化错误、source首帧、实际decoder实例/首输出和sink首帧，深度协议属性由GST_DEBUG/strace/pcap验证。RTSP resource error在签名首次出现或变化时warning，紧接着相同签名重复时debug；其他GStreamer error仍critical。Android H.264 direct avc与H.265 direct hvc1候选rank为259，H.265 adapter为258，软件rank保留。`DualPipView`保留固定槽交换、原生PIP显隐、独立窗口与拖拽resize。`SimulatedCameraControl`已连接真实 `hasVideoChanged`信号，295d中的三条signal-not-found应消失。`qgc_json_zh_CN.ts`已用ASCII逗号修复ChibiOS/NuttX和五项 `apmVehicleType`，消除enum mismatch。两份custom TS均为18个context、146条message，英文146条unfinished、中文146条finished且空译文为0。
